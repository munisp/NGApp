...
class TestKYCVerificationWorkflow:
    ...
    async def test_successful_business_kyc(self):
        """Test successful business KYC verification with Ballerine"""
        async with await WorkflowEnvironment.start_time_skipping() as env:
            async with Worker(
                env.client,
                task_queue="test-kyc-queue",
                ...
                activities=[
                    ...
                    kyc_activities.verify_identity_ocr,
                    kyc_activities.check_sanctions,
                    kyc_activities.verify_with_ballerine,
                    kyc_activities.approve_kyc,
                    kyc_activities.send_kyc_notification,
                ],
            ):
                ...
                assert result["status"] == "approved"
                assert "ballerine_kyb" in result["steps_completed"]
...
class TestKYCActivities:
    ...
    @pytest.mark.asyncio
    async def test_verify_with_ballerine_success(self):
        """Test successful Ballerine KYB verification"""
        ballerine_data = {
            "user_id": "BUSINESS-001",
            "business_info": {
                "name": "Acme Corp",
        ...
        result = await kyc_activities.verify_with_ballerine(ballerine_data)
        assert result["verified"] is True
        assert result["risk_score"] < 0.5
        assert result["platform"] == "Ballerine"
