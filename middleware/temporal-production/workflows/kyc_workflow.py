...
Orchestrates identity verification process using Ballerine and OCR technologies
"""
...
class KYCVerificationWorkflow:
    ...
    async def run(self, kyc_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 4: Ballerine KYB Verification (for business accounts)
            if kyc_data.get("kyc_type") == "business":
                workflow.logger.info("Step 4: Performing Ballerine KYB verification")
                ballerine_result = await workflow.execute_activity(
                    verify_with_ballerine,
                    {
                        "user_id": kyc_data.get("user_id"),
                ...
                if not ballerine_result["verified"]:
                    workflow.logger.warning(f"Ballerine KYB failed: {ballerine_result['reason']}")
                    workflow_result["status"] = "kyb_failed"
                    workflow_result["errors"].append(ballerine_result["reason"])
                    workflow_result["verification_details"]["ballerine"] = ballerine_result
                    ...
                    await workflow.execute_activity(
                        ...
                        {
                            "user_id": kyc_data.get("user_id"),
                            "reason": f"Business verification failed: {ballerine_result['reason']}"
                        },
                        start_to_close_timeout=timedelta(seconds=30),
                    )
                    ...
                    await workflow.execute_activity(
                        send_kyc_notification,
                        {
                            "user_id": kyc_data.get("user_id"),
                            "type": "kyc_rejected",
                            "message": f"Business verification failed: {ballerine_result['reason']}"
                        },
                        start_to_close_timeout=timedelta(seconds=15),
                    )
                ...
                workflow_result["steps_completed"].append("ballerine_kyb")
                workflow_result["verification_details"]["ballerine"] = {
                    "verified": True,
                    "risk_score": ballerine_result.get("risk_score"),
                    "business_verified": True
                }
                workflow.logger.info(f"Ballerine KYB passed with risk score: {ballerine_result.get('risk_score')}")
