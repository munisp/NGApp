...
const EXPECTED_CHARTS = [
  "kafka", "redis", "temporal", "keycloak", "opensearch",
  "apisix", "mojaloop", "permify", "dapr", "fluvio",
  "lakehouse", "tigerbeetle", "postgresql",
];
...
describe("Kubernetes Helm Charts", () => {
  describe("Chart Directory Structure", () => {
    ...
    it("should have Chart.yaml for each component", () => {
      for (const chart of EXPECTED_CHARTS) {
        ...
        expect(fs.existsSync(chartYaml), `Missing Chart.yaml: ${chart}`).toBe(true);
      }
    });
    ...
    it("should have values.yaml for each component", () => {
      for (const chart of EXPECTED_CHARTS) {
        ...
        expect(fs.existsSync(valuesYaml), `Missing values.yaml: ${chart}`).toBe(true);
      }
    });
    ...
    it("should have templates directory for each component", () => {
      for (const chart of EXPECTED_CHARTS) {
        ...
        expect(fs.existsSync(templatesDir), `Missing templates/: ${chart}`).toBe(true);
      }
    });
  });
  ...
  describe("Chart.yaml Validation", () => {
    for (const chart of EXPECTED_CHARTS) {
      it(`${chart}: Chart.yaml should have apiVersion v2`, () => {
        const content = fs.readFileSync(path.join(CHARTS_DIR, chart, "Chart.yaml"), "utf-8");
  ...
  describe("Template Validation", () => {
    for (const chart of EXPECTED_CHARTS) {
      ...
      it(`${chart}: should have service.yaml template`, () => {
        const service = path.join(CHARTS_DIR, chart, "templates", "service.yaml");
        expect(fs.existsSync(service), `Missing service.yaml: ${chart}`).toBe(true);
      });

      it(`${chart}: should have a workload template (deployment or statefulset)`, () => {
        const templatesDir = path.join(CHARTS_DIR, chart, "templates");
        const files = fs.readdirSync(templatesDir);
        const hasWorkload = files.some(f => f.includes("deployment") || f.includes("statefulset"));
  ...
  describe("Values.yaml Validation", () => {
    for (const chart of EXPECTED_CHARTS) {
      it(`${chart}: values.yaml should define resource requests`, () => {
        const content = fs.readFileSync(path.join(CHARTS_DIR, chart, "values.yaml"), "utf-8");
  ...
  describe("Umbrella Chart", () => {
    it("should have Chart.yaml with all 13 dependencies", () => {
      const content = fs.readFileSync(
        path.join(CHARTS_DIR, "pos-54link-umbrella", "Chart.yaml"), "utf-8"
      );
      for (const chart of EXPECTED_CHARTS) {
    ...
    it("should have values.yaml with all component sections", () => {
      const content = fs.readFileSync(
        path.join(CHARTS_DIR, "pos-54link-umbrella", "values.yaml"), "utf-8"
      );
      for (const chart of EXPECTED_CHARTS) {
    ...
    it("should have values-production.yaml with production overrides", () => {
      const prodValues = path.join(CHARTS_DIR, "pos-54link-umbrella", "values-production.yaml");
    ...
    it("should have namespace template", () => {
      const ns = path.join(CHARTS_DIR, "pos-54link-umbrella", "templates", "namespace.yaml");
      expect(fs.existsSync(ns)).toBe(true);
    });

    it("should have network policy template", () => {
      const np = path.join(CHARTS_DIR, "pos-54link-umbrella", "templates", "networkpolicy.yaml");
  ...
  describe("Template Content Quality", () => {
    for (const chart of EXPECTED_CHARTS) {
      it(`${chart}: templates should use Helm templating`, () => {
        ...
        expect(hasHelmSyntax, `No Helm templating found in ${chart}`).toBe(true);
      });
    }
  });
  ...
  describe("Total Chart Count", () => {
    it("should have exactly 14 chart directories (13 + umbrella)", () => {
      const dirs = fs.readdirSync(CHARTS_DIR).filter(d =>
        fs.statSync(path.join(CHARTS_DIR, d)).isDirectory()
      );
