...
export function createLogger(service: string) {
  function log(level: LogLevel, message: string, extra?: Partial<StructuredLog>) {
  ...
  return {
    debug: (msg: string, extra?: Partial<StructuredLog>) => log("debug", msg, extra),
    info: (msg: string, extra?: Partial<StructuredLog>) => log("info", msg, extra),
    warn: (msg: string, extra?: Partial<StructuredLog>) => log("warn", msg, extra),
    error: (msg: string, extra?: Partial<StructuredLog>) => log("error", msg, extra),
    fatal: (msg: string, extra?: Partial<StructuredLog>) => log("fatal", msg, extra),
  };
}
...
export function apiVersionMiddleware(req: Request, res: Response, next: NextFunction) {
  const requestedVersion = (req.headers[API_VERSION_HEADER.toLowerCase()] as string) ?? DEFAULT_VERSION;
  const version = SUPPORTED_VERSIONS.includes(requestedVersion) ? requestedVersion : DEFAULT_VERSION;
