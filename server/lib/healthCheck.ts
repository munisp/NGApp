...
export async function checkDatabase(dbPool: any): Promise<DependencyCheck> {
  ...
  try {
    ...
    return { name: "postgresql", status: "down", latencyMs: Date.now() - start, message: err.message };
  }
}
...
export async function checkRedis(redisClient: any): Promise<DependencyCheck> {
  ...
  try {
    ...
    return { name: "redis", status: "down", latencyMs: Date.now() - start, message: err.message };
  }
}

export async function checkTigerBeetle(tbClient: any): Promise<DependencyCheck> {
  const start = Date.now();
  try {
    ...
    return { name: "tigerbeetle", status: "down", latencyMs: Date.now() - start, message: err.message };
  }
}

export async function getHealthStatus(deps: { db?: any; redis?: any; tb?: any }): Promise<HealthStatus> {
  ...
  const status: HealthStatus["status"] = 
    downCount === 0 ? "healthy" : 
    downCount < checks.length ? "degraded" : "unhealthy";
...
export function createCircuitBreaker(name: string, config: Partial<CircuitBreakerConfig> = {}) {
  ...
  function getState(): CircuitBreakerState {
    ...
    if (cbState.state === "open" && Date.now() - cbState.openedAt >= cfg.resetTimeoutMs) {
  ...
  async function execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    if (current.state === "half_open" && cbState.halfOpenAttempts >= cfg.halfOpenMaxAttempts) {
      cbState.state = "open";
      cbState.openedAt = Date.now();
      throw new Error(`Circuit breaker [${name}] re-opened after half_open max attempts.`);
    }
  ...
  function onFailure() {
    ...
    if (cbState.state === "closed" && cbState.failures >= cfg.failureThreshold) {
      cbState.state = "open";
      cbState.openedAt = Date.now();
      console.warn(`[CircuitBreaker:${name}] OPENED after ${cbState.failures} failures`);
    }
  }
...
export const circuitBreakers = {
  stripe: createCircuitBreaker("stripe", { failureThreshold: 3, resetTimeoutMs: 60_000 }),
  sms: createCircuitBreaker("sms-termii", { failureThreshold: 5, resetTimeoutMs: 30_000 }),
  erp: createCircuitBreaker("erp-sync", { failureThreshold: 5, resetTimeoutMs: 45_000 }),
  kafka: createCircuitBreaker("kafka", { failureThreshold: 3, resetTimeoutMs: 20_000 }),
  tigerbeetle: createCircuitBreaker("tigerbeetle", { failureThreshold: 3, resetTimeoutMs: 15_000 }),
};
...
export function validateEnvironment(): EnvValidationResult {
  ...
  for (const key of REQUIRED_ENVS) {
    const val = process.env[key] || process.env[key.replace("DATABASE_URL", "POSTGRES_URL")];
    if (!val) {
  ...
  for (const key of RECOMMENDED_ENVS) {
    if (!process.env[key]) {
      warnings.push(`Missing recommended env: ${key} (some features may be disabled)`);
    }
  }
  ...
  if (jwt && jwt.length < 32) {
    warnings.push("JWT_SECRET is shorter than 32 characters — consider using a stronger secret");
  }
