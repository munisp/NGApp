...
export type TransactionState = 
  ...
  | "settled" | "reconciled" | "failed" | "reversed" | "disputed";
...
export function getValidNextStates(current: TransactionState): TransactionState[] {
  return VALID_TRANSITIONS[current] ?? [];
}

export function validateTransition(from: TransactionState, to: TransactionState): { valid: boolean; error?: string } {
  if (!VALID_TRANSITIONS[from]) {
    return { valid: false, error: `Unknown state: ${from}` };
  }
  if (!canTransition(from, to)) {
    return { valid: false, error: `Invalid transition: ${from} → ${to}. Valid: ${VALID_TRANSITIONS[from].join(", ")}` };
  }
  return { valid: true };
}
...
export type AgentOnboardingState = 
  | "applied" | "kyc_pending" | "kyc_approved" | "training" 
  | "approval_pending" | "active" | "suspended" | "terminated";
...
export function canAgentTransition(from: AgentOnboardingState, to: AgentOnboardingState): boolean {
  return AGENT_TRANSITIONS[from]?.includes(to) ?? false;
}

export function getAgentNextStates(current: AgentOnboardingState): AgentOnboardingState[] {
...
export type DisputeState = 
  | "filed" | "investigating" | "evidence_requested" | "escalated"
  | "resolved_customer" | "resolved_merchant" | "appealed" | "closed";

const DISPUTE_TRANSITIONS: Record<DisputeState, DisputeState[]> = {
  filed: ["investigating", "closed"],
  investigating: ["evidence_requested", "escalated", "resolved_customer", "resolved_merchant"],
...
export type SettlementState = "pending" | "processing" | "completed" | "failed" | "reconciled";
...
export function canSettlementTransition(from: SettlementState, to: SettlementState): boolean {
...
const COMMISSION_RATES: CommissionTierRate[] = [
  { tier: "basic",      cashInRate: 0.005, cashOutRate: 0.008, transferRate: 0.003, airtimeRate: 0.025, billsRate: 0.010 },
  { tier: "standard",   cashInRate: 0.007, cashOutRate: 0.010, transferRate: 0.005, airtimeRate: 0.030, billsRate: 0.015 },
  { tier: "premium",    cashInRate: 0.010, cashOutRate: 0.015, transferRate: 0.008, airtimeRate: 0.040, billsRate: 0.020 },
  { tier: "enterprise", cashInRate: 0.012, cashOutRate: 0.018, transferRate: 0.010, airtimeRate: 0.050, billsRate: 0.025 },
];

export function getCommissionRate(tier: string, txType: string): number {
  const tierRates = COMMISSION_RATES.find(r => r.tier === tier) ?? COMMISSION_RATES[0];
...
export function calculateCommission(tier: string, txType: string, amount: number): number {
...
): { totalDelta: number; adjustments: { type: string; amount: number; oldCommission: number; newCommission: number; delta: number }[] } {
