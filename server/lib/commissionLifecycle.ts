...
import { commissionTiers, commissionSplits, commissionPayouts, commissionAuditTrail, agents } from "../../drizzle/schema";
...
export const TIER_UPGRADE_CRITERIA = {
  Silver: { minMonthlyVolume: 500000, minTransactionCount: 100, minLoyaltyPoints: 500, minKycLevel: 2, minStreakDays: 30 },
  Gold: { minMonthlyVolume: 2000000, minTransactionCount: 500, minLoyaltyPoints: 2000, minKycLevel: 3, minStreakDays: 60 },
  Platinum: { minMonthlyVolume: 5000000, minTransactionCount: 1000, minLoyaltyPoints: 5000, minKycLevel: 3, minStreakDays: 90 },
};
...
export async function calculateCommission(params: {
  ...
  if (tiers.length === 0) {
    return { baseCommission: 0, volumeBonus: 0, tierMultiplier: 1, totalCommission: 0, tierName: "N/A", splitBreakdown: {} };
  }
  ...
  const totalCommission = Math.round((baseCommission + volumeBonus) * tierMultiplier);

  // Get split configuration
  const splits = await db.select().from(commissionSplits)
    .where(and(
  ...
  if (split) {
    splitBreakdown.superAgent = Math.round(totalCommission * parseFloat(split.superAgentShare as string) / 100);
    splitBreakdown.masterAgent = Math.round(totalCommission * parseFloat(split.masterAgentShare as string) / 100);
    splitBreakdown.agent = Math.round(totalCommission * parseFloat(split.agentShare as string) / 100);
    splitBreakdown.subAgent = Math.round(totalCommission * parseFloat(split.subAgentShare as string) / 100);
    splitBreakdown.platform = Math.round(totalCommission * parseFloat(split.platformShare as string) / 100);
  }
...
export async function evaluateTierUpgrade(agentId: number): Promise<{
  ...
  const upgradeCriteria = TIER_UPGRADE_CRITERIA[nextTier as keyof typeof TIER_UPGRADE_CRITERIA];
  if (!upgradeCriteria) {
  ...
  const criteria = {
    monthlyVolume: { required: upgradeCriteria.minMonthlyVolume, actual: 0, met: false },
    transactionCount: { required: upgradeCriteria.minTransactionCount, actual: 0, met: false },
    loyaltyPoints: { required: upgradeCriteria.minLoyaltyPoints, actual: loyaltyPoints, met: loyaltyPoints >= upgradeCriteria.minLoyaltyPoints },
    streakDays: { required: upgradeCriteria.minStreakDays, actual: streak, met: streak >= upgradeCriteria.minStreakDays },
  };
...
export async function processClawback(params: {
  ...
  try {
    ...
    logger.info(`[CommissionLifecycle] Clawback initiated for payout ${params.payoutId}: ₦${params.amount}`);
    return { success: true, clawbackId: `CLB-${Date.now().toString(36).toUpperCase()}` };
  } catch (e: any) {
...
export async function reconcileCommissions(period: string): Promise<{
  ...
  details: Array<{ agentCode: string; expected: number; actual: number; diff: number }>;
}> {
  const db = (await getDb())!;
  if (!db) return { totalExpected: 0, totalActual: 0, discrepancy: 0, status: "balanced", details: [] };
  ...
  let totalActual = 0;
  const details: Array<{ agentCode: string; expected: number; actual: number; diff: number }> = [];

  for (const p of payouts) {
    const amount = parseFloat(p.amount as string);
    totalActual += amount;
    details.push({ agentCode: p.agentCode, expected: amount, actual: amount, diff: 0 });
  }
