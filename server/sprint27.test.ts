...
describe("Sprint 27: i18n Multi-Language Support", () => {
  it("should have i18n module with 6+ languages", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/lib/i18n.ts"), "utf-8");
  ...
  it("should have LanguageSelector component", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/components/LanguageSelector.tsx"), "utf-8");
...
describe("Sprint 27: Email Delivery Service", () => {
  it("should have email delivery module with templates", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/lib/emailDelivery.ts"), "utf-8");
...
describe("Sprint 27: Webhook Signature Verification", () => {
  it("should have webhook signature module with HMAC", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/lib/webhookSignature.ts"), "utf-8");
...
describe("Sprint 27: Rate Limit Configuration", () => {
  it("should have rate limit config with per-endpoint limits", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/lib/rateLimitConfig.ts"), "utf-8");
...
describe("Sprint 27: API Versioning", () => {
  it("should have API versioning middleware", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/lib/apiVersioning.ts"), "utf-8");
...
describe("Sprint 27: Audit Trail", () => {
  it("should have audit trail middleware", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/lib/auditTrail.ts"), "utf-8");
    expect(content).toContain("action");
    expect(content).toContain("severity");
  });

  it("should have AuditTrailPage UI", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/pages/AuditTrailPage.tsx"), "utf-8");
...
describe("Sprint 27: Accessibility", () => {
  it("should have AccessibilityProvider component", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/components/AccessibilityProvider.tsx"), "utf-8");
...
describe("Sprint 27: Request Tracing", () => {
  it("should have request tracing middleware with correlation IDs", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/lib/requestTracing.ts"), "utf-8");
...
describe("Sprint 27: Data Export Router", () => {
  it("should have data export router", () => {
    const content = fs.readFileSync(path.join(ROOT, "server/routers/dataExportRouter.ts"), "utf-8");
...
describe("Sprint 27: Print Stylesheet", () => {
  it("should have print stylesheet", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/styles/print.css"), "utf-8");
    expect(content).toContain("@media print");
    expect(content).toContain("display: none");
  });

  it("should be imported in index.css", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/index.css"), "utf-8");
...
describe("Sprint 27: OpenAPI Specification", () => {
  it("should have OpenAPI 3.0 spec", () => {
    const content = fs.readFileSync(path.join(ROOT, "docs/openapi.json"), "utf-8");
...
describe("Sprint 27: Grafana Dashboards", () => {
  it("should have 4 Grafana dashboards", () => {
    ...
    expect(fs.existsSync(path.join(dashDir, "fraud-detection.json"))).toBe(true);
    expect(fs.existsSync(path.join(dashDir, "agent-performance.json"))).toBe(true);
...
describe("Sprint 27: Prometheus Alerts", () => {
  it("should have Prometheus alerting rules", () => {
    const content = fs.readFileSync(path.join(ROOT, "infra/monitoring/prometheus-alerts.yml"), "utf-8");
...
describe("Sprint 27: API Docs & System Status Pages", () => {
  it("should have ApiDocs page", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/pages/ApiDocs.tsx"), "utf-8");
    expect(content).toContain("ApiDocs");
    expect(content).toContain("endpoint");
  });

  it("should have SystemStatus page", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/pages/SystemStatus.tsx"), "utf-8");
...
describe("Sprint 27: Nav Items", () => {
  it("should have Audit Trail in DashboardLayout nav", () => {
    const content = fs.readFileSync(path.join(ROOT, "client/src/components/DashboardLayout.tsx"), "utf-8");
    expect(content).toContain("Audit Trail");
    expect(content).toContain("/audit-trail");
  });
});
