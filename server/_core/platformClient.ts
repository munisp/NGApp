...
async function platformFetch<T>(
  ...
  let res: Response;
  try {
    res = await fetch(url, { ...fetchOptions, ...agentOption, headers } as RequestInit);
  } catch (err) {
    throw new PlatformError(service, 503, `Service unreachable: ${String(err)}`);
  }
...
export const kycPlatform = {
  ...
  listCases: (params: { skip?: number; limit?: number; status?: string }, token: string) => {
    const qs = new URLSearchParams(
      Object.entries(params)
        .filter(([, v]) => v !== undefined)
        .map(([k, v]) => [k, String(v)])
    ).toString();
    return platformFetch("kyc", PLATFORM_URLS.kyc, `/cases${qs ? `?${qs}` : ""}`, { token });
  },
};
...
export const videoKycPlatform = {
  ...
  createChallenge: (payload: { session_id: string; challenge_type?: string }, token: string) =>
  ...
  respondChallenge: (
    ...
    platformFetch("video-kyc", PLATFORM_URLS.videoKyc, `/challenge/${challengeId}/respond`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  challengeStatus: (challengeId: string, token: string) =>
    platformFetch("video-kyc", PLATFORM_URLS.videoKyc, `/challenge/${challengeId}/status`, {
      token,
    }),
...
export const fraudPlatform = {
  ...
  /** List fraud reports */
  listReports: (params: { skip?: number; limit?: number; tenant_id?: number }, token: string) => {
    const qs = new URLSearchParams(
      Object.entries(params)
        .filter(([, v]) => v !== undefined)
        .map(([k, v]) => [k, String(v)])
    ).toString();
    return platformFetch("fraud", PLATFORM_URLS.fraud, `/api/v1/fraud-reports${qs ? `?${qs}` : ""}`, { token });
  },

  /** Update a fraud report status */
  updateReport: (reportId: number, payload: Record<string, unknown>, token: string) =>
    platformFetch("fraud", PLATFORM_URLS.fraud, `/api/v1/fraud-reports/${reportId}`, {
      method: "PUT",
      body: JSON.stringify(payload),
      token,
    }),
...
export const settlementPlatform = {
  ...
  processSettlement: (payload: { settlement_date?: string; agent_ids?: string[] }, token: string) =>
    platformFetch("settlement", PLATFORM_URLS.settlement, "/settlements/process", {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  getOutstanding: (token: string) =>
    platformFetch("settlement", PLATFORM_URLS.settlement, "/settlements/outstanding", { token }),
  ...
  getHistory: (params: { limit?: number; offset?: number }, token: string) => {
    ...
    return platformFetch("settlement", PLATFORM_URLS.settlement, `/settlements/history${qs ? `?${qs}` : ""}`, { token });
  },
};
...
export const geofencingPlatform = {
  ...
  reportLocation: (terminalId: string, payload: { lat: number; lng: number; accuracy?: number }, token: string) =>
    platformFetch("geofencing", PLATFORM_URLS.geofencing, `/terminals/${terminalId}/location`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  checkZone: (terminalId: string, token: string) =>
    platformFetch("geofencing", PLATFORM_URLS.geofencing, `/terminals/${terminalId}/check`, { token }),
  ...
  updateZone: (zoneId: string, payload: Record<string, unknown>, token: string) =>
...
export const loyaltyPlatform = {
  ...
  getProfile: (agentId: string, token: string) =>
    platformFetch("loyalty", PLATFORM_URLS.loyalty, `/loyalty/agents/${agentId}/profile`, { token }),

  /** Award loyalty points */
  awardPoints: (agentId: string, payload: { points: number; reason: string; txRef?: string }, token: string) =>
    platformFetch("loyalty", PLATFORM_URLS.loyalty, `/loyalty/agents/${agentId}/points`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  redeemPoints: (agentId: string, payload: { points: number; reward_id: string }, token: string) =>
    platformFetch("loyalty", PLATFORM_URLS.loyalty, `/loyalty/agents/${agentId}/redeem`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  listRewards: (token: string) =>
    platformFetch("loyalty", PLATFORM_URLS.loyalty, "/loyalty/rewards", { token }),
};
...
export const floatPlatform = {
  ...
  getBalance: (agentId: string, token: string) =>
    platformFetch("float", PLATFORM_URLS.float, `/api/v1/float/agents/${agentId}/balance`, { token }),
  ...
  getTransactions: (agentId: string, limit = 100, token: string) =>
    platformFetch("float", PLATFORM_URLS.float, `/api/v1/float/agents/${agentId}/transactions?limit=${limit}`, { token }),
};
...
export const disputePlatform = {
  ...
  list: (params: { agent_id?: string; status?: string; limit?: number }, token: string) => {
    const qs = new URLSearchParams(
      Object.entries(params)
        .filter(([, v]) => v !== undefined)
        .map(([k, v]) => [k, String(v)])
    ).toString();
    return platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes${qs ? `?${qs}` : ""}`, { token });
  },

  /** Get a specific dispute */
  get: (disputeId: string, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/${disputeId}`, { token }),

  /** Add a message/evidence to a dispute */
  addMessage: (disputeId: string, payload: { message: string; attachments?: string[] }, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/${disputeId}/messages`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  /** Resolve a dispute */
  resolve: (disputeId: string, payload: { resolution: string; outcome: string }, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/${disputeId}/resolve`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  getUserDisputes: (userId: string, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/user/${userId}`, { token }),

  /** Issue provisional credit for a dispute */
  issueProvisionalCredit: (disputeId: string, payload: { amount: number; reason: string; issued_by: string }, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/${disputeId}/provisional-credit`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  initiateChargeback: (disputeId: string, payload: { amount: number; reason: string; chargeback_code?: string; initiated_by: string }, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/${disputeId}/chargeback`, {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  completeChargeback: (disputeId: string, completedBy: string, success: boolean, notes: string, token: string) =>
    platformFetch("dispute", PLATFORM_URLS.dispute, `/disputes/${disputeId}/chargeback/complete`, {
      method: "POST",
...
export const analyticsPlatform = {
  ...
  transactionSummary: (params: { start_date?: string; end_date?: string; agent_id?: string }, token: string) => {
    const qs = new URLSearchParams(
      Object.entries(params)
        .filter(([, v]) => v !== undefined)
        .map(([k, v]) => [k, String(v)])
    ).toString();
    return platformFetch("analytics", PLATFORM_URLS.analytics, `/analytics/reports/transactions${qs ? `?${qs}` : ""}`, { token });
  },

  /** Get revenue analysis */
  revenueAnalysis: (params: { start_date?: string; end_date?: string }, token: string) => {
    const qs = new URLSearchParams(
      Object.entries(params)
        .filter(([, v]) => v !== undefined)
        .map(([k, v]) => [k, String(v)])
    ).toString();
    return platformFetch("analytics", PLATFORM_URLS.analytics, `/analytics/reports/revenue${qs ? `?${qs}` : ""}`, { token });
  },

  /** Get risk analytics */
  riskAnalytics: (token: string) =>
    platformFetch("analytics", PLATFORM_URLS.analytics, "/analytics/reports/risk", { token }),
};
...
export const notificationPlatform = {
  ...
  send: (payload: {
    ...
    platformFetch("notification", PLATFORM_URLS.notification, "/notifications/send", {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
  ...
  sendBulk: (payload: { notifications: unknown[] }, token: string) =>
    platformFetch("notification", PLATFORM_URLS.notification, "/notifications/bulk", {
      method: "POST",
      body: JSON.stringify(payload),
      token,
    }),
