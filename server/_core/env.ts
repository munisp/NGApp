...
export const ENV = {
  ...
  platformServiceToken: process.env.PLATFORM_SERVICE_TOKEN ?? "54link-service-token-dev",
  ...
  keycloakClientSecret: process.env.KEYCLOAK_CLIENT_SECRET ?? "54link-keycloak-dev-secret",
  ...
  vaultSecretPath: process.env.VAULT_SECRET_PATH ?? "secret/data/pos-shell-demo",
  ...
  // ── MDM microservices ────────────────────────────────────────────────────────
  mdmComplianceEngineUrl: process.env.MDM_COMPLIANCE_ENGINE_URL ?? "http://mdm-compliance-engine:8091",
  mdmGeofenceServiceUrl:  process.env.MDM_GEOFENCE_SERVICE_URL  ?? "http://mdm-geofence-service:8092",

  // ── Resilience / offline sub-services ──────────────────────────────────────
  resilienceAgentUrl:  process.env.RESILIENCE_AGENT_URL   ?? "https://resilience.54link.io",
  offlineQueueUrl:     process.env.OFFLINE_QUEUE_URL      ?? "https://queue.54link.io",
  analyticsServiceUrl: process.env.ANALYTICS_SERVICE_URL  ?? "https://analytics.54link.io",
  ...
  vapidPublicKey:  process.env.VAPID_PUBLIC_KEY  ?? "BE4Tbbh5r0IGPRlQ_0ePL0AEJfiWJynWxxM0UDmffgbenp87U4upzpn0aNysgCVQdT8IUfNSG3Dx6_k2Wn6lRgA",
  vapidPrivateKey: process.env.VAPID_PRIVATE_KEY ?? "vBqalBipE6mu4a592N8c1wucdpun-RaKemy8gZDa99M",
  ...
  PLATFORM_VIDEO_KYC_URL:    process.env.PLATFORM_VIDEO_KYC_URL    ?? "http://video-kyc-service:8071",
  PLATFORM_FRAUD_URL:        process.env.PLATFORM_FRAUD_URL        ?? "http://fraud-engine:8072",
  PLATFORM_SETTLEMENT_URL:   process.env.PLATFORM_SETTLEMENT_URL   ?? "http://settlement-service:8073",
  PLATFORM_GEOFENCING_URL:   process.env.PLATFORM_GEOFENCING_URL   ?? "http://mdm-geofence-service:8092",
  PLATFORM_LOYALTY_URL:      process.env.PLATFORM_LOYALTY_URL      ?? "http://loyalty-service:8074",
  PLATFORM_FLOAT_URL:        process.env.PLATFORM_FLOAT_URL        ?? "http://float-manager:8075",
  PLATFORM_DISPUTE_URL:      process.env.PLATFORM_DISPUTE_URL      ?? "http://dispute-service:8076",
  PLATFORM_ANALYTICS_URL:    process.env.PLATFORM_ANALYTICS_URL    ?? "http://analytics-service:8077",
  PLATFORM_NOTIFICATION_URL: process.env.PLATFORM_NOTIFICATION_URL ?? "http://notification-service:8078",
  ...
  s3PresignExpiry: parseInt(process.env.S3_PRESIGN_EXPIRY_SECONDS ?? "3600", 10),
  ...
  cronSecret:     process.env.CRON_SECRET       ?? "54link-cron-dev-secret-change-in-prod",
  internalApiKey: process.env.INTERNAL_API_KEY  ?? "54link-internal-dev-key-change-in-prod",
};
