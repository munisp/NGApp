...
export async function runKycExpiryCheck() {
  ...
  if (!db) { console.warn("[Cron] No DB — skipping KYC expiry check"); return { flagged: 0 }; }
  ...
  try {
    ...
    for (const agent of expiringAgents) {
      ...
      if (isExpired) {
        ...
        expired++;
        console.log(`[KYC] Agent ${agent.agentCode} KYC expired on ${expiryDate.toISOString()}`);
      } else {
        // Send warning notification
        flagged++;
        const daysUntilExpiry = Math.ceil((expiryDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        console.log(`[KYC] Agent ${agent.agentCode} KYC expires in ${daysUntilExpiry} days`);
      }
    }
    
    console.log(`[Cron] KYC expiry check complete: ${expired} expired, ${flagged} expiring soon`);
