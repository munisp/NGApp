...
export async function runDisputeAutoEscalation() {
  ...
  if (!db) { console.warn("[Cron] No DB — skipping dispute escalation"); return { escalated: 0 }; }
  ...
  try {
    ...
    for (const dispute of overdueDisputes) {
      const hoursOpen = (now.getTime() - new Date(dispute.createdAt).getTime()) / (1000 * 60 * 60);
      ...
      if (escalationResult.shouldEscalate) {
        ...
        escalated++;
        console.log(`[Cron] Escalated dispute #${dispute.id}: ${escalationResult.reasons.join(", ")}`);
      }
    }
    
    console.log(`[Cron] Dispute auto-escalation complete: ${escalated}/${overdueDisputes.length} escalated`);
