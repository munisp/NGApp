...
const listSurveys = protectedProcedure
  ...
  .query(async ({ input }) => {
    ...
    const [{ total }] = await db.select({ total: count() }).from(customer_journey_events);
...
const submitSurvey = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    const db = (await getDb())!;
    if (input.id) {
      const [existing] = await db.select().from(customer_journey_events).where(eq(customer_journey_events.id, input.id));
