...
const listSurveys = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const rows = await db.select().from(customer_journey_events).orderBy(desc(customer_journey_events.id)).limit(lim).offset(offset);
      const [{ total }] = await db.select({ total: count() }).from(customer_journey_events).limit(100);
...
const getSurveyStats = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [{ total }] = await db.select({ total: count() }).from(customer_journey_events).limit(100);
      const recent = await db.select().from(customer_journey_events).orderBy(desc(customer_journey_events.id)).limit(5);
...
const submitSurvey = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(customer_journey_events).where(eq(customer_journey_events.id, input.id)).limit(100);
