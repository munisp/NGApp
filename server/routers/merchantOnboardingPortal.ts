...
export const merchantOnboardingPortalRouter = router({
  ...
  getApplication: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      ...
      const docs = await db.select().from(merchantKycDocs).where(eq(merchantKycDocs.merchantId, input.id)).limit(100);
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    const [pending] = await db.select({ value: count() }).from(merchants).where(eq(merchants.status, "pending")).limit(100);
