...
export const notificationInboxRouter = router({
  getStats: protectedProcedure.input(z.object({ userId: z.string() })).query(async ({ input }) => {
    try {
      ...
      const [total] = await db.select({ value: count() }).from(notification_logs).where(eq(notification_logs.recipientId, input.userId)).limit(100);
      const [unread] = await db.select({ value: count() }).from(notification_logs).where(and(eq(notification_logs.recipientId, input.userId), eq(notification_logs.status, "pending"))).limit(100);
