...
const dashboard = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const recent = await db.select().from(tenants).orderBy(desc(tenants.id)).limit(5);
...
const getConfigs = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [row] = await db.select().from(tenants).where(eq(tenants.id, input.id)).limit(100);
        if (!row) throw new TRPCError({ code: "NOT_FOUND", message: "getConfigs: record not found" });
        return row;
      }
      const rows = await db.select().from(tenants).orderBy(desc(tenants.id)).limit(input.limit ?? 10).offset(((input.page ?? 1) - 1) * (input.limit ?? 10));
      const [{ total }] = await db.select({ total: count() }).from(tenants).limit(100);
...
const updateConfig = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(tenants).where(eq(tenants.id, input.id)).limit(100);
        if (!existing) throw new TRPCError({ code: "NOT_FOUND", message: "updateConfig: record not found" });
        return { success: true, id: input.id, message: "updateConfig completed", timestamp: new Date().toISOString() };
      }
      return { success: true, message: "updateConfig completed", timestamp: new Date().toISOString() };
    } catch (error) {
      if (error instanceof TRPCError) throw error;
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
    }
