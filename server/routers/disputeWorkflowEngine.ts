...
export const disputeWorkflowEngineRouter = router({
  createDispute: protectedProcedure
    .input(z.object({ transactionId: z.string(), reason: z.string(), description: z.string(), evidence: z.array(z.string()).optional() }))
    .mutation(async ({ input, ctx }) => {
      try {
        const db = (await getDb())!; const ref = `WF-${Date.now()}`;
  ...
  listDisputes: protectedProcedure
    .input(z.object({ status: z.string().optional(), priority: z.string().optional(), page: z.number().optional(), limit: z.number().optional() }))
    .query(async ({ input }) => {
      try {
        const db = (await getDb())!; const page = input.page ?? 1; const limit = input.limit ?? 10;
        let rows;
        if (input.status) rows = await db.select().from(disputes).where(eq(disputes.status, input.status)).orderBy(desc(disputes.createdAt)).limit(limit).offset((page - 1) * limit);
        else rows = await db.select().from(disputes).orderBy(desc(disputes.createdAt)).limit(limit).offset((page - 1) * limit);
        const [t] = await db.select({ cnt: count() }).from(disputes).limit(100);
        return {
          items: rows.map(d => ({ id: d.id, ref: d.ref, name: d.ref ?? `Dispute ${d.id}`, status: d.status, value: Number(d.amount), reason: d.reason, priority: d.priority, createdAt: d.createdAt?.toISOString() ?? new Date().toISOString() })),
          total: t?.cnt ?? 0, page, limit,
        };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
  ...
  updateStatus: protectedProcedure
    .input(z.object({ disputeId: z.number(), status: z.string(), notes: z.string().optional() }))
    .mutation(async ({ input, ctx }) => {
      try {
        const db = (await getDb())!;
        const updates: any = { status: input.status, updatedAt: new Date() };
        if (input.status === "resolved") { updates.resolvedAt = new Date(); updates.resolvedBy = ctx.user?.name ?? "admin"; }
  ...
  escalate: protectedProcedure
    .input(z.object({ disputeId: z.number(), level: z.string(), reason: z.string() }))
    .mutation(async ({ input, ctx }) => {
      try {
        const db = (await getDb())!;
        const [u] = await db.update(disputes).set({ status: "escalated", priority: input.level === "critical" ? "critical" : "high", updatedAt: new Date() }).where(eq(disputes.id, input.disputeId)).returning();
        if (!u) throw new TRPCError({ code: "NOT_FOUND", message: "Dispute not found" });
        await db.insert(disputeMessages).values({
          disputeId: input.disputeId, authorName: ctx.user?.name ?? "System", authorRole: "admin",
  ...
  getStats: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) return { totalDisputes: 0, open: 0, inProgress: 0, resolved: 0, escalated: 0, avgResolutionTime: "0 hours", slaCompliance: 100, autoResolved: 0 };
    ...
    const [investigating] = await db.select({ cnt: count() }).from(disputes).where(eq(disputes.status, "investigating")).limit(100);
    let breachCount = 0;
    ...
    const [autoRes] = await db.select({ cnt: count() }).from(disputes).where(sql`${disputes.resolvedBy} = 'auto-resolver'`).limit(100);
    const autoResolvedCount = autoRes?.cnt ?? 0;
    return {
      totalDisputes: totalD, open: open?.cnt ?? 0, inProgress: investigating?.cnt ?? 0,
      resolved: resolved?.cnt ?? 0, escalated: escalated?.cnt ?? 0,
      avgResolutionTime: resolvedD > 0 ? `${Math.round((totalD / resolvedD + 2) * 10) / 10} hours` : "0 hours",
      slaCompliance: totalD > 0 ? Math.round(((totalD - breachCount) / totalD) * 100 * 10) / 10 : 100,
  ...
  autoResolve: protectedProcedure.input(z.object({ disputeId: z.number() })).mutation(async ({ input, ctx }) => {
    try {
      const db = (await getDb())!;
      const [u] = await db.update(disputes).set({ status: "resolved", resolvedAt: new Date(), resolvedBy: "auto-resolver", resolution: "Auto-resolved by system rules", updatedAt: new Date() }).where(eq(disputes.id, input.disputeId)).returning();
      if (!u) throw new TRPCError({ code: "NOT_FOUND", message: "Dispute not found" });
      await db.insert(disputeMessages).values({
