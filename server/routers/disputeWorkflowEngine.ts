...
export const disputeWorkflowEngineRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    const [investigating] = await db.select({ cnt: count() }).from(disputes).where(eq(disputes.status, "investigating"));
    let breachCount = 0;
    ...
    const [autoRes] = await db.select({ cnt: count() }).from(disputes).where(sql`${disputes.resolvedBy} = 'auto-resolver'`);
