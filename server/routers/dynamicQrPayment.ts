...
export const dynamicQrPaymentRouter = router({
  ...
  generate: protectedProcedure.input(z.object({ agentId: z.number(), amount: z.number().positive(), description: z.string().optional() })).mutation(async ({ input }) => {
    try {
      ...
      const [qr] = await db.insert(qrCodes).values({ code, agentId: input.agentId, type: "payment", status: "active", amount: String(input.amount), currency: "NGN", description: input.description ?? "Dynamic QR Payment" }).returning();
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    const [active] = await db.select({ value: count() }).from(qrCodes).where(eq(qrCodes.status, "active")).limit(100);
