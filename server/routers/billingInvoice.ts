...
interface InvoiceLineItem {
  ...
  unitPrice: number;
  total: number;
  category: "transaction_fee" | "subscription" | "implementation" | "overage" | "credit" | "adjustment";
}
  ...
  createStripeInvoice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const existingCustomers = await stripe.customers.list({ email: input.customerEmail, limit: 1 });
        if (existingCustomers.data.length > 0) {
          ...
          customer = await stripe.customers.create({
            ...
            metadata: { tenant_id: String(input.tenantId), client_id: input.clientId, platform: "54link" },
          });
        }
        const invoice = await stripe.invoices.create({
          customer: customer.id,
          collection_method: input.autoCollect ? "charge_automatically" : "send_invoice",
          days_until_due: input.autoCollect ? undefined : input.dueInDays,
          currency: input.currency,
          metadata: { tenant_id: String(input.tenantId), client_id: input.clientId, period_start: input.periodStart, period_end: input.periodEnd, user_id: String(ctx.user.id) },
          description: `54Link billing for period ${input.periodStart} to ${input.periodEnd}`,
        });
        for (const item of input.lineItems) {
          await stripe.invoiceItems.create({ customer: customer.id, invoice: invoice.id, amount: item.amount, currency: input.currency, description: item.description, quantity: item.quantity });
        }
        const finalizedInvoice = await stripe.invoices.finalizeInvoice(invoice.id);
        return { success: true, stripeInvoiceId: finalizedInvoice.id, stripeInvoiceUrl: finalizedInvoice.hosted_invoice_url, stripeInvoicePdf: finalizedInvoice.invoice_pdf, status: finalizedInvoice.status, amountDue: finalizedInvoice.amount_due, currency: finalizedInvoice.currency, customerId: customer.id };
      } catch (err: any) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `Stripe invoice creation failed: ${err.message}` });
      }
    }),
  ...
  collectPayment: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        return { success: true, status: invoice.status, amountPaid: invoice.amount_paid, paidAt: invoice.status_transitions?.paid_at ? new Date(invoice.status_transitions.paid_at * 1000).toISOString() : null };
      } catch (err: any) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `Payment collection failed: ${err.message}` });
      }
    }),
  ...
  getStripeInvoiceStatus: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        return { id: invoice.id, status: invoice.status, amountDue: invoice.amount_due, amountPaid: invoice.amount_paid, amountRemaining: invoice.amount_remaining, currency: invoice.currency, hostedUrl: invoice.hosted_invoice_url, pdfUrl: invoice.invoice_pdf, customerEmail: invoice.customer_email, dueDate: invoice.due_date ? new Date(invoice.due_date * 1000).toISOString() : null, paidAt: invoice.status_transitions?.paid_at ? new Date(invoice.status_transitions.paid_at * 1000).toISOString() : null };
      } catch (err: any) {
        throw new TRPCError({ code: "NOT_FOUND", message: `Invoice not found: ${err.message}` });
      }
    }),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        const session = await stripe.checkout.sessions.create({
          ...
          line_items: [{ price_data: { currency: input.currency, product_data: { name: `54Link Invoice: ${input.invoiceId}`, description: input.description }, unit_amount: input.amount }, quantity: 1 }],
          ...
          metadata: { tenant_id: String(input.tenantId), invoice_id: input.invoiceId, user_id: String(ctx.user.id), customer_email: input.customerEmail },
        });
        return { checkoutUrl: session.url, sessionId: session.id };
      } catch (err: any) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `Checkout session creation failed: ${err.message}` });
      }
    }),
});
