...
export const dataThresholdAlertsRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(observabilityAlerts).limit(100);
    const [firing] = await db.select({ value: count() }).from(observabilityAlerts).where(eq(observabilityAlerts.status, "firing")).limit(100);
