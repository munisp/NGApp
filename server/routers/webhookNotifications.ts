...
export const webhookNotificationsRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [totalEndpoints] = await db.select({ value: count() }).from(webhookEndpoints).limit(100);
    const [totalDeliveries] = await db.select({ value: count() }).from(webhookDeliveries).limit(100);
