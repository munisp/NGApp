...
export const customerJourneyAnalyticsRouter = router({
  listSteps: protectedProcedure
    .input(z.object({ page: z.number().default(1), limit: z.number().default(50), customerId: z.number().optional(), journeyType: z.string().optional() }))
    .query(async ({ input }) => {
      try {
        ...
        const [{ total }] = await db.select({ total: count() }).from(customerJourneySteps).where(where).limit(100);
        return { items, total };
      } catch (error) {
  ...
  recordStep: protectedProcedure
    .input(z.object({
      customerId: z.number(), journeyType: z.string(), stepName: z.string(), stepOrder: z.number(),
      channel: z.string(), metadata: z.any().optional(),
    }))
  ...
  journeyTypes: protectedProcedure.query(() => ["onboarding", "first_transaction", "kyc_verification", "loan_application", "dispute_resolution", "churn_prevention"]),
});
