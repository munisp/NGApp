...
export const customer_journey_eventsRouter = router({
  list: protectedProcedure.input(z.object({ customerId: z.number().optional(), stage: z.string().optional(), limit: z.number().default(50), offset: z.number().default(0) })).query(async ({ input }) => {
    try {
      ...
      if (input.stage) conditions.push(eq(customerJourneySteps.stage, input.stage));
      ...
      const [{ total }] = await db.select({ total: count() }).from(customerJourneySteps).where(conditions.length ? and(...conditions) : undefined).limit(100);
  ...
  getById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [row] = await db.select().from(customerJourneySteps).where(eq(customerJourneySteps.id, input.id)).limit(100);
  ...
  trackEvent: protectedProcedure.input(z.object({ customerId: z.number(), stage: z.enum(["awareness", "consideration", "onboarding", "first_transaction", "active", "loyal", "churned"]), eventType: z.string(), metadata: z.record(z.string(), z.any()).optional() })).mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [row] = await db.insert(customerJourneySteps).values({ ...input, createdAt: new Date() }).returning();
      return row;
    } catch (error) {
      if (error instanceof TRPCError) throw error;
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
  ...
  getFunnelAnalysis: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const stageCounts = await db.select({ stage: customerJourneySteps.stage, count: count() }).from(customerJourneySteps).groupBy(customerJourneySteps.stage).limit(100);
    const funnel = JOURNEY_STAGES.map((stage, i) => {
      const found = stageCounts.find((s: Record<string, unknown>) => s.stage === stage);
      const stageCount = found ? Number(found.count) : 0;
      const prevCount = i > 0 ? (stageCounts.find((s: Record<string, unknown>) => s.stage === JOURNEY_STAGES[i - 1]))?.count || 0 : stageCount;
