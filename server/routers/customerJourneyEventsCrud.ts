...
export const customer_journey_eventsRouter = router({
  ...
  getFunnelAnalysis: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const stageCounts = await db.select({ stage: customerJourneySteps.stage as any, count: count() }).from(customerJourneySteps).groupBy(customerJourneySteps.stage as any).limit(100);
