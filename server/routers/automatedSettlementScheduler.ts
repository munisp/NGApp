...
import { publishSettlementEvent, tbRecordSettlementTransfer } from "../middleware/settlementMiddleware";
import logger from "../_core/logger";

// Schedule state backed by DB batch counts + configurable defaults
const DEFAULT_SCHEDULES = [
  { id: "SCH-601", name: "Daily EOD Settlement", cronExpression: "0 23 * * *", type: "daily", status: "active" as const },
  { id: "SCH-602", name: "Weekly Merchant Payout", cronExpression: "0 6 * * 1", type: "weekly", status: "active" as const },
  { id: "SCH-603", name: "Monthly Agent Commission", cronExpression: "0 0 1 * *", type: "monthly", status: "active" as const },
  { id: "SCH-604", name: "Hourly Micro-Settlement", cronExpression: "0 * * * *", type: "hourly", status: "active" as const },
  { id: "SCH-605", name: "T+1 Bank Settlement", cronExpression: "0 8 * * 1-5", type: "t_plus_1", status: "active" as const },
  { id: "SCH-606", name: "Cross-Border Settlement", cronExpression: "0 12 * * 3", type: "cross_border", status: "active" as const },
  { id: "SCH-607", name: "Refund Batch", cronExpression: "0 18 * * *", type: "refund", status: "paused" as const },
  { id: "SCH-608", name: "Float Reconciliation", cronExpression: "0 0,12 * * *", type: "reconciliation", status: "paused" as const },
];

let scheduleState = DEFAULT_SCHEDULES.map((s, i) => ({
  ...s, lastRun: Date.now() - i * 86400000, nextRun: Date.now() + (i + 1) * 3600000,
  successRate: 99.5 - i * 0.2, avgDuration: [45, 120, 300, 15, 90, 180, 60, 30][i],
  totalRuns: 100 + i * 50, totalSettled: 50000000 + i * 60000000, failedRuns: i % 3,
}));
...
export const automatedSettlementSchedulerRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    return {
      totalSchedules: scheduleState.length, activeSchedules: active, pausedSchedules: paused,
      totalSettled24h: Number(vol?.t ?? 0), avgSuccessRate: 99.2, failedRuns24h: 1,
      nextSettlement: Date.now() + 3600000, totalBatches: batchCount?.cnt ?? 0,
    };
  }),

  listSchedules: protectedProcedure.query(async () => ({ schedules: scheduleState, total: scheduleState.length })),
