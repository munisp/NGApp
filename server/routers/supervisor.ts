...
export const supervisorRouter = router({
    ...
    .query(async ({ ctx }) => {
      try {
        ...
        const agentRows = await db
          .select()
          .from(agents)
          .where(sql`${agents.id} = ANY(ARRAY[${sql.join(agentIds.map(id => sql`${id}`), sql`, `)}]::int[])`);
        ...
        const stats = await db
          ...
          .from(transactions)
          .where(and(
            sql`${transactions.agentId} = ANY(ARRAY[${sql.join(agentIds.map(id => sql`${id}`), sql`, `)}]::int[])`,
        ...
        return agentRows.map(agent => {
          ...
          return {
            ...
            successRate7d: s && s.txCount > 0 ? Math.round((s.successCount / s.txCount) * 100) : null,
          };
        });
      } catch (error) {
        if (error instanceof TRPCError) throw error;
  ...
  agentTransactions: supervisorProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        if (ctx.user.role !== "admin") { // role-check: throw below
          ...
          if (!assignment) throw new TRPCError({ code: "FORBIDDEN", message: "Agent not assigned to you" });
        }
    ...
    .query(async ({ ctx }) => {
      try {
        ...
        return db.select().from(fraudAlerts)
          .where(and(
            sql`${fraudAlerts.agentId} = ANY(ARRAY[${sql.join(agentIds.map(id => sql`${id}`), sql`, `)}]::int[])`,
  ...
  listSupervisors: adminProcedure
    .input(z.object({}))
    .query(async () => {
      const db = await requireDb();
      return db.select().from(users).where(eq(users.role, "supervisor")).limit(100);
    }),
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (!supervisorUserId && input.supervisorCode) {
          const [supAgent] = await db.select({ id: agents.id, role: agents.role })
            .from(agents)
            .where(eq(agents.agentCode, input.supervisorCode))
            .limit(1);
          if (!supAgent) throw new TRPCError({ code: "NOT_FOUND", message: `Supervisor with code ${input.supervisorCode} not found` });
          if (supAgent.role !== "supervisor" && supAgent.role !== "admin") {
            throw new TRPCError({ code: "BAD_REQUEST", message: `Agent ${input.supervisorCode} is not a supervisor` });
          }
          supervisorUserId = supAgent.id;
        }
        if (!supervisorUserId) throw new TRPCError({ code: "BAD_REQUEST", message: "supervisorUserId or supervisorCode required" });
        // Idempotent insert
        const existing = await db.select().from(supervisorAgents)
          .where(and(
  ...
  unassignAgent: adminProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = await requireDb();
        await db.delete(supervisorAgents)
          .where(and(
