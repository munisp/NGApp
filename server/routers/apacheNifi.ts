...
export const apacheNifiRouter = router({
  listPipelines: protectedProcedure.input(z.object({ limit: z.number().min(1).max(100).default(50) }).optional()).query(async ({ input }) => {
    try {
      ...
      const [registry] = await db.select().from(systemConfig).where(eq(systemConfig.key, "apache_nifi_registry")).limit(1);
  ...
  getConfig: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [config] = await db.select().from(systemConfig).where(eq(systemConfig.key, "apache_nifi_config")).limit(1);
  ...
  trigger: protectedProcedure.input(z.object({ pipelineId: z.string().min(1).max(128).optional(), force: z.boolean().default(false) })).mutation(async ({ input }) => {
    try {
      ...
      await db.insert(auditLog).values({ action: "apache_nifi_triggered", resource: "apache_nifi", resourceId: runId, status: "success", metadata: { pipelineId: input.pipelineId, force: input.force } });
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(auditLog).where(eq(auditLog.action, "apache_nifi_triggered")).limit(100);
