...
export const superAdminRouter = router({
  ...
  tenants: router({
    ...
    create: superAdminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const [existing] = await db.select({ id: tenants.id }).from(tenants).where(eq(tenants.slug, input.slug));
  ...
  analytics: router({
    overview: superAdminProcedure.query(async () => {
      ...
      const [fraudCount] = await db.select({ c: count() }).from(fraudAlerts).where(eq(fraudAlerts.status, "open"));
      return {
