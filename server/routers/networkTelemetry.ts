...
export const networkTelemetryRouter = router({
  ...
  getTopEndpoints: protectedProcedure.input(z.object({ limit: z.number().min(1).max(50).default(10) }).optional()).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const rows = await db.select({ component: platform_health_checks.serviceName, checkCount: count(), avgLatency: avg(platform_health_checks.responseTime) }).from(platform_health_checks).groupBy(platform_health_checks.serviceName).orderBy(desc(count())).limit(input?.limit ?? 10);
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    const [avgLat] = await db.select({ value: avg(platform_health_checks.responseTime) }).from(platform_health_checks).limit(100);
