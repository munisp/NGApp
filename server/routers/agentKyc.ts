...
export const agentKycRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    const [total] = await db.select({ value: count() }).from(kycSessions);
    const statusCounts = await db.select({ status: kycSessions.status, cnt: count() }).from(kycSessions).groupBy(kycSessions.status);
