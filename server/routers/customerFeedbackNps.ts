...
const getNpsScore = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(tenantFeeOverrides).limit(100);
...
const submitFeedback = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(tenantFeeOverrides).where(eq(tenantFeeOverrides.id, input.id)).limit(100);
