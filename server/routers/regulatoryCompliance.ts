...
const runCheck = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    ...
    if (input.id) {
      ...
      if (!existing) throw new TRPCError({ code: "NOT_FOUND", message: "runCheck: record not found" });
      return { success: true, id: input.id, message: "runCheck completed", timestamp: new Date().toISOString() };
    }
    return { success: true, message: "runCheck completed", timestamp: new Date().toISOString() };
  });
