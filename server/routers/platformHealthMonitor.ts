...
const getOverview = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(platformSettings).limit(100);
...
const createIncident = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(platformSettings).where(eq(platformSettings.id, input.id)).limit(100);
