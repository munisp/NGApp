...
const getCurrentTps = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const rows = await db.select().from(velocityLimits).orderBy(desc(velocityLimits.id)).limit(lim).offset(offset);
      const [{ total }] = await db.select({ total: count() }).from(velocityLimits).limit(100);
