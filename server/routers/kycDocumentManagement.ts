...
const list = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(kycDocuments).limit(100);
...
const approve = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(kycDocuments).where(eq(kycDocuments.id, input.id)).limit(100);
