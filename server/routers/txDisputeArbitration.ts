...
export const txDisputeArbitrationRouter = router({
  listDisputes: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const disputeList = rows.map(r => ({
        ...
        slaDeadlineAt: r.slaDeadlineAt ? new Date(r.slaDeadlineAt).toISOString() : null,
  ...
  getDispute: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [r] = await db.select().from(disputes).where(eq(disputes.id, numId)).limit(1);
      if (!r) return null;

      // Get messages/timeline for this dispute
      const messages = await db.select().from(disputeMessages)
  ...
  resolveDispute: protectedProcedure
    .input(z.object({
      disputeId: z.string(),
      outcome: z.enum(["claimant_favor", "respondent_favor", "split", "dismissed"]),
    ...
    .mutation(async ({ input }) => {
      ...
      try { await cacheSet(`txDisputeArbitration:resolved:${numId}`, JSON.stringify({ outcome: input.outcome, ts: Date.now() }), 600); } catch {}
      try { await tbCreateTransfer({ debitAccountId: "1", creditAccountId: "2", amount: input.refundAmount ?? 0 }); } catch {}
      try { await fluvioProduce("pos.txdisputearbitration", { value: JSON.stringify({ event: "dispute.resolved", disputeId: numId, outcome: input.outcome, ts: Date.now() }) }); } catch {}
      try { await permifyCheck({ subjectType: "user", subjectId: "system", entityType: "txDisputeArbitration", entityId: String(numId), permission: "resolve" }); } catch {}

      logger.info(`[TxDisputeArbitration] Dispute ${numId} resolved: ${input.outcome}`);
  ...
  escalateDispute: protectedProcedure
    .input(z.object({
      disputeId: z.string(),
      reason: z.string(),
      escalateTo: z.enum(["senior_investigator", "dispute_committee", "legal_team", "cbn"]),
    }))
    .mutation(async ({ input }) => {
      const db = (await getDb())!;
      const numId = parseInt(input.disputeId.replace(/\D/g, "")) || 0;

      const escalationMap: Record<string, number> = { senior_investigator: 1, dispute_committee: 2, legal_team: 3, cbn: 4 };
      ...
      const slaExtension = { senior_investigator: 5, dispute_committee: 10, legal_team: 15, cbn: 30 };
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    }).from(disputes).where(eq(disputes.status, "resolved"));
    const avgResolutionDays = Math.round(Number(avgRes?.avg ?? 0) * 10) / 10;

    // SLA compliance — disputes resolved within SLA deadline
    const [withinSla] = await db.select({ cnt: count() }).from(disputes).where(
      sql`${disputes.status} = 'resolved' AND (${disputes.slaDeadlineAt} IS NULL OR ${disputes.updatedAt} <= ${disputes.slaDeadlineAt})`
    );
    const slaComplianceRate = resolvedCount > 0 ? Math.round((Number(withinSla?.cnt ?? 0) / resolvedCount) * 1000) / 10 : 0;

    // Resolution outcome breakdown
    const outcomes = await db.select({
      resolution: disputes.resolution,
      cnt: count(),
    }).from(disputes).where(eq(disputes.status, "resolved")).groupBy(disputes.resolution);

    const outcomeMap: Record<string, number> = {};
    for (const o of outcomes) { outcomeMap[o.resolution ?? "unknown"] = o.cnt; }
    ...
    const [refundAgg] = await db.select({
      ...
      sql`${disputes.resolution} IN ('claimant_favor', 'split')`
    );
    ...
    const byType = await db.select({
      type: disputes.type,
      cnt: count(),
    }).from(disputes).groupBy(disputes.type);
    ...
    return {
      ...
      avgResolutionDays,
      slaComplianceRate,
      claimantFavorRate: Math.round(((outcomeMap["claimant_favor"] ?? 0) / resolvedTotal) * 1000) / 10,
      respondentFavorRate: Math.round(((outcomeMap["respondent_favor"] ?? 0) / resolvedTotal) * 1000) / 10,
      splitRate: Math.round(((outcomeMap["split"] ?? 0) / resolvedTotal) * 1000) / 10,
      dismissedRate: Math.round(((outcomeMap["dismissed"] ?? 0) / resolvedTotal) * 1000) / 10,
