...
const CBN_SERVICE_URL = process.env.CBN_REPORTING_SERVICE_URL ?? "http://localhost:8010";

async function callCbnService(path: string, method: "GET" | "POST" = "GET", body?: unknown) {
  try {
    const res = await fetch(`${CBN_SERVICE_URL}${path}`, {
      method,
      headers: { "Content-Type": "application/json", "X-Internal-Key": process.env.INTERNAL_API_KEY ?? "internal-key-54link" },
...
async function generateMonthlyReportFromDb(year: number, month: number, institutionCode: string) {
...
async function generateQuarterlyFraudReportFromDb(year: number, quarter: number, institutionCode: string) {
  const db = (await getDb())!;
  if (!db) return null;
...
export const cbnReportingRouter = router({
  ...
  health: protectedProcedure.query(async () => {
    ...
    return { serviceAvailable: !!svc, serviceUrl: CBN_SERVICE_URL, timestamp: new Date().toISOString() };
  }),
