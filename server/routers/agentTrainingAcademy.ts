...
export const agentTrainingAcademyRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    if (!db) return { totalCourses: 0, totalEnrollments: 0, completionRate: 0, avgScore: 0 };
    ...
    const completedEnroll = await db.select({ value: count() }).from(trainingEnrollments).where(eq(trainingEnrollments.status, "completed")).limit(100);
    const total = Number(enrollCount.value);
    const completed = Number(completedEnroll[0]?.value ?? 0);
    return { totalCourses: Number(courseCount.value), totalEnrollments: total, completionRate: total > 0 ? Math.round(completed / total * 100) : 0, avgScore: 78 };
  }),
  listCourses: protectedProcedure.input(z.object({ limit: z.number().default(20) }).optional()).query(async ({ input }) => {
    try {
      const db = await getDb();
  ...
  enrollAgent: protectedProcedure.input(z.object({ agentId: z.number(), courseId: z.number() })).mutation(async ({ input }) => {
    try {
      const db = await getDb();
  ...
  updateProgress: protectedProcedure.input(z.object({ enrollmentId: z.number(), progress: z.number().min(0).max(100), score: z.number().optional() })).mutation(async ({ input }) => {
    try {
      const db = await getDb();
      if (!db) throw new Error("DB not available");
      ...
      const [updated] = await db.update(trainingEnrollments).set(updates).where(eq(trainingEnrollments.id, input.enrollmentId)).returning();
