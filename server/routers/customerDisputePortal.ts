...
export const customerDisputePortalRouter = router({
  ...
  getDispute: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      ...
      const messages = await db.select().from(disputeMessages).where(eq(disputeMessages.disputeId, input.id)).orderBy(disputeMessages.createdAt).limit(100);
  ...
  getStats: protectedProcedure.input(z.object({ customerId: z.number() })).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [total] = await db.select({ value: count() }).from(disputes).where(eq(disputes.agentId, input.customerId)).limit(100);
      const [open] = await db.select({ value: count() }).from(disputes).where(and(eq(disputes.agentId, input.customerId), eq(disputes.status, "open"))).limit(100);
