...
export const distributedTracingDashRouter = router({
  list: protectedProcedure.input(z.object({ limit: z.number().min(1).max(200).default(50) }).optional()).query(async ({ input }) => {
    try {
      ...
      const rows = await db.select().from(platform_health_checks).where(eq(platform_health_checks.serviceName, "distributed_tracing")).orderBy(desc(platform_health_checks.checkedAt)).limit(input?.limit ?? 50);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(platform_health_checks).where(eq(platform_health_checks.serviceName, "distributed_tracing")).limit(100);
    const [healthy] = await db.select({ value: count() }).from(platform_health_checks).where(and(eq(platform_health_checks.serviceName, "distributed_tracing"), eq(platform_health_checks.status, "healthy"))).limit(100);
    const [avgLat] = await db.select({ value: avg(platform_health_checks.responseTime) }).from(platform_health_checks).where(eq(platform_health_checks.serviceName, "distributed_tracing")).limit(100);
