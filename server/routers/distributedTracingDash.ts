...
export const distributedTracingDashRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(platform_health_checks).where(eq(platform_health_checks.serviceName, "distributed_tracing"));
    const [healthy] = await db.select({ value: count() }).from(platform_health_checks).where(and(eq(platform_health_checks.serviceName, "distributed_tracing"), eq(platform_health_checks.status, "healthy")));
    const [avgLat] = await db.select({ value: avg(platform_health_checks.responseTime) }).from(platform_health_checks).where(eq(platform_health_checks.serviceName, "distributed_tracing"));
