...
export const workflowEngineRouter = router({
  listDefinitions: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const [{ total }] = await db.select({ total: count() }).from(workflowDefinitions).where(where).limit(100);
        return { items, total };
      } catch (error) {
  ...
  startInstance: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
        const [def] = await db.select().from(workflowDefinitions).where(eq(workflowDefinitions.id, input.definitionId)).limit(100);
    ...
    .query(async ({ input }) => {
      try {
        ...
        const items = await db.select().from(workflowInstances).where(where).orderBy(desc(workflowInstances.createdAt))
          .limit(input.limit).offset((input.page - 1) * input.limit);
        const [{ total }] = await db.select({ total: count() }).from(workflowInstances).where(where).limit(100);
        return { items, total };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
  ...
  advanceStep: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
        const [instance] = await db.select().from(workflowInstances).where(eq(workflowInstances.id, input.instanceId)).limit(100);
        if (!instance) throw new TRPCError({ code: "NOT_FOUND", message: "Instance not found" });
        const [def] = await db.select().from(workflowDefinitions).where(eq(workflowDefinitions.id, instance.definitionId)).limit(100);
  ...
  summary: protectedProcedure.query(async () => {
    ...
    const [defs] = await db.select({ total: count() }).from(workflowDefinitions).where(eq(workflowDefinitions.isActive, true)).limit(100);
    const [active] = await db.select({ total: count() }).from(workflowInstances).where(eq(workflowInstances.status, "active")).limit(100);
    const [completed] = await db.select({ total: count() }).from(workflowInstances).where(eq(workflowInstances.status, "completed")).limit(100);
