...
const list = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(geoFences).limit(100);
...
const getById = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [row] = await db.select().from(geoFences).where(eq(geoFences.id, input.id)).limit(100);
...
const create = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(geoFences).where(eq(geoFences.id, input.id)).limit(100);
