...
export const broadcastAnnouncementsRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    const [total] = await db.select({ value: count() }).from(notification_logs).where(eq(notification_logs.recipientType, "broadcast"));
    const [sent] = await db.select({ value: count() }).from(notification_logs).where(and(eq(notification_logs.recipientType, "broadcast"), eq(notification_logs.status, "sent")));
