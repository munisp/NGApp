...
const KYC_DOC_TYPES = ["cac_certificate", "tin_certificate", "utility_bill", "bank_statement", "id_card", "passport", "bvn_verification", "memart"];
const KYC_STAGES = ["document_collection", "verification", "compliance_review", "approval", "activation"];
...
export const merchantKycOnboardingRouter = router({
  listDocs: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const [{ total }] = await db.select({ total: count() }).from(merchantKycDocs).where(where).limit(100);
        return { items, total };
      } catch (error) {
  ...
  uploadDoc: protectedProcedure
    .input(z.object({
      merchantId: z.number(), docType: z.string(), docUrl: z.string(), docNumber: z.string().optional(), expiryDate: z.string().optional(),
    }))
  ...
  kycProgress: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) return { required: KYC_DOC_TYPES, submitted: [], approved: [], rejected: [], progress: 0, stage: KYC_STAGES[0] };
        const docs = await db.select().from(merchantKycDocs).where(eq(merchantKycDocs.merchantId, input.merchantId)).limit(100);
        const submitted = docs.map(d => d.docType);
        const approved = docs.filter(d => d.status === "approved").map(d => d.docType);
        const rejected = docs.filter(d => d.status === "rejected").map(d => d.docType);
        const progress = Math.round((approved.length / KYC_DOC_TYPES.length) * 100);
        ...
        return { required: KYC_DOC_TYPES, submitted, approved, rejected, progress, stage };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
