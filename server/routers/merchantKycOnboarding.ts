...
export const merchantKycOnboardingRouter = router({
  listDocs: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [{ total }] = await db.select({ total: count() }).from(merchantKycDocs).where(where);
      return { items, total };
    }),
  ...
  kycProgress: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const docs = await db.select().from(merchantKycDocs).where(eq(merchantKycDocs.merchantId, input.merchantId));
