...
export const mdmRouter = router({
  ...
  listDevices: adminProcedure
    ...
    .input(z.object({
      status: z.enum(["online", "offline", "updating", "error", "all"]).default("all"),
  ...
  issueCommand: adminProcedure
    .input(z.object({
      deviceId: z.number(),
      command: z.enum(["UPDATE", "RECONFIG", "RESTART", "WIPE", "PING", "SCREENSHOT"]),
      payload: z.record(z.string(), z.unknown()).optional(),
    }))
    .mutation(async ({ input, ctx }) => {
      try {
        const db = await requireDb();
        const [device] = await db.select().from(devices).where(eq(devices.id, input.deviceId)).limit(100);
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (input.deviceIds && input.deviceIds.length > 0) {
          targetDevices = await db.select().from(devices)
            .where(sql`${devices.id} = ANY(ARRAY[${sql.join(input.deviceIds.map(id => sql`${id}`), sql`, `)}]::int[])`);
        } else {
          targetDevices = await db.select().from(devices)
            .where(eq(devices.status, "online"));
        }

        if (targetDevices.length === 0) return { devicesTargeted: 0, commandsIssued: 0 };

        const commands = await db.insert(deviceCommands)
          .values(targetDevices.map(d => ({
            deviceId: d.id,
            command: "UPDATE" as const,
            payload: { appVersion: input.appVersion, downloadUrl: input.downloadUrl },
        ...
        return { devicesTargeted: targetDevices.length, commandsIssued: commands.length };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
  ...
  stats: adminProcedure
    ...
    .query(async () => {
      ...
      const summary: Record<string, number> = { online: 0, offline: 0, updating: 0, error: 0 };
      for (const row of rows) {
        summary[row.status] = row.count;
      }
      summary.total = Object.values(summary).reduce((a: any, b: any) => a + b, 0);
      return summary;
    }),
  ...
  heartbeat: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        const db = await requireDb();
        let [device] = await db.select().from(devices)
        ...
        if (input.latE6 !== undefined && input.lonE6 !== undefined && device) {
          ...
          for (const zone of zones) {
            ...
            if (zone.type === "polygon" && zone.polygonJson) {
              ...
              if (Array.isArray(polygon) && polygon.length >= 3) {
                ...
                if (isOutside) {
                  ...
                  for (const vertex of polygon) {
                    ...
                    const dLon1 = (ptLon - vertex[1]) * 111320 * Math.cos(ptLat * Math.PI / 180);
              ...
              if (!zone.centerLat || !(zone.centerLng ?? zone.longitude) || !zone.radiusMeters) continue;
              ...
              distM = Math.sqrt(
                ...
                Math.pow(dLon * 111320 * Math.cos(Number(zone.centerLat) * Math.PI / 180), 2)
              );
              isOutside = distM > Number(zone.radiusMeters);
            }
            if (isOutside) {
              // Insert geofence violation (idempotent: skip if open violation exists)
              const existing = await db.select().from(mdmGeofenceViolations)
                .where(and(
        ...
        // ── Compliance check ────────────────────────────────────────────────────
        if (device) {
          const policies = await db.select().from(deviceCompliancePolicies)
            .where(and(
              eq(deviceCompliancePolicies.enabled, true),
            )).limit(20);

          let overallStatus: "compliant" | "non_compliant" | "unknown" = "compliant";
          for (const policy of policies) {
            const rules = policy.rules as Record<string, unknown>;
            const violations: { type: string; details: Record<string, unknown> }[] = [];

            if (rules.minBatteryLevel && input.batteryLevel !== undefined &&
                input.batteryLevel < (rules.minBatteryLevel as number)) {
              violations.push({ type: "low_battery", details: { actual: input.batteryLevel, threshold: rules.minBatteryLevel } });
            }
            if (rules.minAppVersion && input.appVersion) {
              const [maj, min] = (input.appVersion ?? "0.0.0").split(".").map(Number);
              const [rmaj, rmin] = ((rules.minAppVersion as string) ?? "0.0.0").split(".").map(Number);
              if (maj < rmaj || (maj === rmaj && min < rmin)) {
                violations.push({ type: "outdated_app", details: { actual: input.appVersion, required: rules.minAppVersion } });
              }
            }
            if (rules.allowedNetworkTypes && input.networkType &&
                !(rules.allowedNetworkTypes as string[]).includes(input.networkType)) {
              violations.push({ type: "disallowed_network", details: { actual: input.networkType, allowed: rules.allowedNetworkTypes } });
            }

            for (const v of violations) {
              overallStatus = "non_compliant";
              const existing = await db.select().from(deviceComplianceViolations)
                .where(and(
        ...
        const pendingCommands = await db.select().from(deviceCommands)
          .where(and(
        ...
        return {
          ...
          pendingCommands: pendingCommands.map((c: typeof deviceCommands.$inferSelect) => ({
            id: c.id,
            command: c.command,
            payload: c.payload,
          })),
  ...
  generateEnrollmentToken: adminProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = await requireDb();
        const [agent] = await db.select().from(agents).where(eq(agents.agentCode, input.agentCode)).limit(100);
        if (!agent) throw new TRPCError({ code: "NOT_FOUND", message: "Agent not found" });
        ...
        const serial = input.serialNumber ?? `PENDING-${agent.agentCode}-${Date.now()}`;
        const existing = await db.select().from(devices)
          .where(eq(devices.serialNumber, serial));

        if (existing.length > 0) {
          await db.update(devices)
            .set({ enrollmentToken: token, enrollmentExpiresAt: expiresAt, updatedAt: new Date() })
            .where(eq(devices.serialNumber, serial));
        } else {
        ...
        return { token, expiresAt, qrPayload, agentCode: input.agentCode, serial };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const device = allDevices.find(
          d => d.enrollmentToken === input.token &&
        ...
        if (!agent || agent.id !== device.agentId) {
          throw new TRPCError({ code: "FORBIDDEN", message: "Agent code mismatch" });
        }
        ...
        return { deviceId: device.id, enrolled: true, agentCode: input.agentCode, deviceToken: persistentToken };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
  ...
  ackCommand: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        await db.update(deviceCommands).set({
          status: input.status,
          acknowledgedAt: input.status === "acknowledged" ? new Date() : undefined,
  ...
  disableTerminal: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const io = getIO();
        if (io) {
          io.of("/terminal").to(`agent:${input.agentCode}`).emit("terminal:kill-switch", {
            reason: input.reason,
  ...
  listPolicies: adminProcedure
    ...
    .query(async ({ input }) => {
      try {
        const db = await requireDb();
        const rows = await db.select().from(deviceCompliancePolicies)
  ...
  upsertPolicy: adminProcedure
    .input(z.object({
      ...
      rules: z.object({
        ...
        allowedNetworkTypes: z.array(z.enum(["wifi", "4g", "3g", "2g", "offline"])).optional(),
        maxInactiveHours: z.number().min(1).optional(),
      }),
      severity: z.enum(["low", "medium", "high", "critical"]).default("medium"),
      enabled: z.boolean().default(true),
      enforcementAction: z.enum(["notify", "restrict", "wipe"]).default("notify"),
    }))
    .mutation(async ({ input, ctx }) => {
      try {
        const db = await requireDb();
        if (input.id) {
          await db.update(deviceCompliancePolicies).set({
            name: input.name,
          ...
          const [row] = await db.insert(deviceCompliancePolicies).values({
            name: input.name,
            description: input.description,
  ...
  listViolations: adminProcedure
    .input(z.object({
      ...
      status: z.enum(["open", "acknowledged", "resolved", "suppressed", "all"]).default("open"),
      limit: z.number().min(1).max(200).default(50),
      offset: z.number().min(0).default(0),
    }))
    .query(async ({ input }) => {
      try {
        const db = await requireDb();
        const conditions = [];
        if (input.deviceId) conditions.push(eq(deviceComplianceViolations.deviceId, input.deviceId));
        if (input.status !== "all") conditions.push(eq(deviceComplianceViolations.status, input.status));
        const rows = await db.select().from(deviceComplianceViolations)
  ...
  acknowledgeViolation: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await db.update(deviceComplianceViolations).set({
  ...
  listGeofenceViolations: adminProcedure
    ...
    .query(async ({ input }) => {
      try {
        const db = await requireDb();
        const conditions = [];
        if (input.deviceId) conditions.push(eq(mdmGeofenceViolations.deviceId, input.deviceId));
        if (input.status !== "all") conditions.push(eq(mdmGeofenceViolations.status, input.status));
        const rows = await db.select().from(mdmGeofenceViolations)
          .where(conditions.length > 0 ? and(...conditions) : undefined)
    ...
    .query(async ({ input }) => {
      try {
        ...
        const [items, [{ total }]] = await Promise.all([
          db.select().from(otaReleases).orderBy(desc(otaReleases.createdAt)).limit(input.limit).offset(input.offset),
  ...
  createOtaRelease: adminProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = await requireDb();
        const [row] = await db.insert(otaReleases).values({ ...input, status: "draft" }).returning();
        return row;
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
  ...
  publishOtaRelease: adminProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = await requireDb();
        const [row] = await db.update(otaReleases)
  ...
  listOtaUpdateLog: adminProcedure
    .input(z.object({ deviceId: z.number().optional(), releaseId: z.number().optional(), limit: z.number().default(50), offset: z.number().default(0) }))
    .query(async ({ input }) => {
      try {
        const db = await requireDb();
        const conditions = [];
        if (input.deviceId) conditions.push(eq(otaUpdateLog.deviceId, input.deviceId));
        if (input.releaseId) conditions.push(eq(otaUpdateLog.releaseId, input.releaseId));
        const where = conditions.length > 0 ? and(...conditions) : undefined;
        return db.select().from(otaUpdateLog).where(where).orderBy(desc(otaUpdateLog.startedAt)).limit(input.limit).offset(input.offset);
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
  ...
  recordOtaUpdate: protectedProcedure
    .input(z.object({
      ...
      status: z.enum(["pending", "downloading", "installing", "success", "failed"]),
      errorMessage: z.string().optional(),
    }))
    .mutation(async ({ input }) => {
      try {
        const db = await requireDb();
        const existing = await db.select({ id: otaUpdateLog.id }).from(otaUpdateLog)
          .where(and(eq(otaUpdateLog.deviceId, input.deviceId), eq(otaUpdateLog.releaseId, input.releaseId)))
          .limit(1);
        if (existing.length > 0) {
          const completedAt = ["success", "failed"].includes(input.status) ? new Date() : undefined;
          const [row] = await db.update(otaUpdateLog)
            .set({ status: input.status, errorMessage: input.errorMessage, completedAt })
            .where(eq(otaUpdateLog.id, existing[0].id))
            .returning();
          return row;
        }
        const [row] = await db.insert(otaUpdateLog).values({
          deviceId: input.deviceId,
  ...
  enableTerminal: adminProcedure
    .input(z.object({
      ...
      const io = getIO();
      if (io) {
        io.of("/terminal").to(`agent:${input.agentCode}`).emit("terminal:kill-switch-lift", {
