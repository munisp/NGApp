...
import { commissionTiers, commissionPayouts, commissionRules, commissionAuditTrail } from "../../drizzle/schema";
import { eq, desc, count, sql } from "drizzle-orm";
import { TRPCError } from "@trpc/server";
import { publishCommissionEvent, tbRecordCommissionCredit } from "../middleware/commissionMiddleware";
...
export const agentCommissionCalcRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    return {
      ...
      activeAgents: totalPay?.cnt ?? 0, tiers: tierCount?.cnt ?? 0,
      splitRules: ruleCount?.cnt ?? 0, lastCalculation: Date.now() - 3600000,
    };
  }),
  ...
  calculateCommission: protectedProcedure.input(z.object({
    ...
    return {
      ...
      splits: [
        { recipient: "Agent", amount: Math.round(commission * 0.65 * 100) / 100 },
        { recipient: "Master Agent", amount: Math.round(commission * 0.15 * 100) / 100 },
        { recipient: "Super Agent", amount: Math.round(commission * 0.10 * 100) / 100 },
        { recipient: "Platform", amount: Math.round(commission * 0.10 * 100) / 100 },
      ],
    };
  }),
