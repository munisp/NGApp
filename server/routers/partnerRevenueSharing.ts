...
export const partnerRevenueSharingRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [totalRules] = await db.select({ value: count() }).from(commissionRules).limit(100);
    const [totalPayouts] = await db.select({ value: count() }).from(commissionPayouts).limit(100);
    const [totalPaid] = await db.select({ value: sum(commissionPayouts.amount) }).from(commissionPayouts).where(eq(commissionPayouts.status, "paid")).limit(100);
