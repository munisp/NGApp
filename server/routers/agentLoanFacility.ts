...
export const agentLoanFacilityRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [{ total }] = await db.select({ total: count() }).from(agentLoans).where(where);
      return { items, total };
    }),
  ...
  disburse: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      const db = (await getDb())!;
      if (!db) throw new Error("Database unavailable");
      const [loan] = await db.select().from(agentLoans).where(eq(agentLoans.id, input.loanId));
  ...
  portfolioSummary: protectedProcedure.query(async () => {
    ...
    const [defaulted] = await db.select({ count: count() }).from(agentLoans).where(eq(agentLoans.status, "defaulted"));
