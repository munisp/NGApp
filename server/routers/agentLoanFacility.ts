...
// Business rules
const INTEREST_RATES = { float_advance: 2.5, working_capital: 5.0, emergency: 8.0 }; // monthly %
const MAX_LOAN_MULTIPLIER = 3; // max loan = 3x average monthly volume
const MIN_CREDIT_SCORE = 500;
const CREDIT_SCORE_WEIGHTS = {
  txVolume: 0.30, repaymentHistory: 0.25, accountAge: 0.20, floatUtilization: 0.15, fraudHistory: 0.10,
};
...
export const agentLoanFacilityRouter = router({
  ...
  list: protectedProcedure
    .input(z.object({
      ...
      status: z.enum(["pending", "approved", "disbursed", "repaying", "completed", "defaulted", "rejected"]).optional(),
    ...
    .query(async ({ input }) => {
      try {
        ...
        const items = await db.select().from(agentLoans).where(where)
          .orderBy(desc(agentLoans.createdAt))
          .limit(input.limit).offset((input.page - 1) * input.limit);
        const [{ total }] = await db.select({ total: count() }).from(agentLoans).where(where).limit(100);
        return { items, total };
      } catch (error) {
    ...
    .mutation(async ({  input  }) => {
      try {
        ...
        if (creditScore < MIN_CREDIT_SCORE) {
          throw new Error(`Credit score ${creditScore} below minimum ${MIN_CREDIT_SCORE}`);
        }
        // Calculate interest
        ...
        const [loan] = await db.insert(agentLoans).values({
  ...
  approve: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        const db = (await getDb())!;
        if (!db) throw new Error("Database unavailable");
        await db.update(agentLoans).set({
          status: "approved",
  ...
  disburse: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) throw new Error("Database unavailable");
        const [loan] = await db.select().from(agentLoans).where(eq(agentLoans.id, input.loanId)).limit(100);
  ...
  recordRepayment: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const [loan] = await db.select().from(agentLoans).where(eq(agentLoans.id, input.loanId)).limit(100);
        if (!loan) throw new Error("Loan not found");
        const newRepaid = parseFloat(String(loan.amountRepaid || "0")) + input.amount;
        ...
        return { success: true, amountRepaid: newRepaid, remaining: totalRepayable - newRepaid, fullyRepaid: isFullyRepaid };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
  ...
  creditScore: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) return { score: 0, breakdown: {}, eligible: false, maxLoanAmount: 0 };
  ...
  portfolioSummary: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    if (!db) return { totalLoans: 0, totalDisbursed: "0", totalRepaid: "0", defaultRate: 0, activeLoans: 0 };
    const [stats] = await db.select({
      totalLoans: count(),
      totalDisbursed: sum(agentLoans.principalAmount),
      totalRepaid: sum(agentLoans.amountRepaid),
    }).from(agentLoans);
    const [defaulted] = await db.select({ count: count() }).from(agentLoans).where(eq(agentLoans.status, "defaulted")).limit(100);
    const [active] = await db.select({ count: count() }).from(agentLoans)
    ...
    return {
      ...
      defaultRate: stats.totalLoans ? ((defaulted.count || 0) / stats.totalLoans) * 100 : 0,
...
async function calculateCreditScore(db: any, agentId: number): Promise<number> {
  ...
  const [txStats] = await db.select({ total: sum(transactions.amount), count: count() })
    .from(transactions).where(and(eq(transactions.agentId, agentId), gte(transactions.createdAt, new Date(Date.now() - 90 * 86400000))));
  const volumeScore = Math.min(((txStats.count || 0) / 100) * 300, 300);
  // Repayment history score (0-250)
  const [loanStats] = await db.select({ total: count() }).from(agentLoans)
    .where(and(eq(agentLoans.agentId, agentId), eq(agentLoans.status, "completed")));
  const repaymentScore = Math.min((loanStats.total || 0) * 50, 250);
  // Account age score (0-200)
  const [agent] = await db.select().from(agents).where(eq(agents.id, agentId)).limit(100);
  const ageMonths = agent ? (Date.now() - new Date(agent.createdAt).getTime()) / (30 * 86400000) : 0;
  const ageScore = Math.min(ageMonths * 15, 200);
  // Float utilization (0-150)
  const floatScore = agent ? Math.min((parseFloat(String(agent.floatBalance || "0")) / parseFloat(String(agent.floatLimit || "1000000"))) * 150, 150) : 0;
