...
async function ensureDefaults() {
  ...
  if (!db) return;
  try {
    const existing = await db.select({ c: count() }).from(commissionTiers).limit(100);
    ...
    const existingSplits = await db.select({ c: count() }).from(commissionSplits).limit(100);
...
export const commissionEngineRouter = router({
  ...
  tiers: protectedProcedure.query(async () => {
    ...
    const rows = await db.select().from(commissionTiers).orderBy(commissionTiers.id).limit(100);
  ...
  createTier: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const [maxRow] = await db.select({ c: count() }).from(commissionTiers).limit(100);
  ...
  splits: protectedProcedure.query(async () => {
    ...
    const rows = await db.select().from(commissionSplits).orderBy(commissionSplits.id).limit(100);
  ...
  createSplit: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const [maxRow] = await db.select({ c: count() }).from(commissionSplits).limit(100);
        const nextNum = Number(maxRow.c) + 1;
        const splitId = `CS-${String(nextNum).padStart(3, "0")}`;
  ...
  simulate: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        if (db) {
          const tierRows = await db.select().from(commissionTiers).where(eq(commissionTiers.isActive, true)).limit(100);
          tiers = tierRows.map(formatTier);
          const splitRows = await db.select().from(commissionSplits).where(eq(commissionSplits.isActive, true)).limit(100);
  ...
  analytics: protectedProcedure.query(async () => {
    ...
    const [tierCount] = await db.select({ c: count() }).from(commissionTiers).where(eq(commissionTiers.isActive, true)).limit(100);
    const [splitCount] = await db.select({ c: count() }).from(commissionSplits).where(eq(commissionSplits.isActive, true)).limit(100);
    const [payoutCount] = await db.select({ c: count() }).from(commissionPayouts).limit(100);
