...
const listTables = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(deviceLocations).limit(100);
...
const runHealthCheck = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(deviceLocations).where(eq(deviceLocations.id, input.id)).limit(100);
