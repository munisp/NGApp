...
const renewContract = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(billingRevenuePeriods).limit(100);
...
const suspendBilling = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(billingRevenuePeriods).where(eq(billingRevenuePeriods.id, input.id)).limit(100);
