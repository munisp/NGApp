...
import { transactions, txMonitoringAlerts, agents, fraudAlerts } from "../../drizzle/schema";
...
export const realtimeTxMonitorRouter = router({
  ...
  dashboardKpis: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    if (!db) return { totalTxToday: 0, volumeToday: "0", activeAlerts: 0, avgTps: 0, failureRate: 0 };
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const [txStats] = await db.select({
      totalTx: count(),
      totalVolume: sum(transactions.amount),
    }).from(transactions).where(gte(transactions.createdAt, today));
    const [failedStats] = await db.select({ failedCount: count() }).from(transactions)
      .where(and(gte(transactions.createdAt, today), eq(transactions.status, "failed")));
    const [alertStats] = await db.select({ activeAlerts: count() }).from(txMonitoringAlerts)
      .where(eq(txMonitoringAlerts.resolved, false));
    const totalTx = txStats.totalTx || 0;
    const failureRate = totalTx > 0 ? ((failedStats.failedCount || 0) / totalTx) * 100 : 0;
    return {
