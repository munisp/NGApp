...
export const fraudRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const items = filtered.slice(offset, offset + input.limit).map((a: any) => ({
          ...a,
  ...
  updateStatus: protectedProcedure
    .input(
      z.object({
        id: z.number(),
        status: z.enum(["open", "investigating", "escalated", "dismissed", "resolved"]),
      })
    )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        import("../lib/fluvioClient.js").then(({ publishFraudAlertEvent }) =>
          publishFraudAlertEvent({
            id: alert.id,
            type: input.type,
            severity: input.severity,
            agentId: input.agentId ?? 0,
            transactionRef: input.transactionId ? `TXN-${input.transactionId}` : undefined,
          })
        ).catch((e: unknown) => console.error("[Fluvio] Fraud alert event failed:", e));
  ...
  listRules: protectedProcedure.query(async () => {
    ...
    if (!db) return [];
    try {
      const rows = await db.select().from(fraudRules).orderBy(fraudRules.createdAt).limit(100);
  ...
  createRule: protectedProcedure
    .input(z.object({
      ...
      category: z.enum(["velocity", "geofence", "device_fingerprint", "amount_anomaly", "time_of_day", "blacklist", "custom"]),
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const [rule] = await db.insert(fraudRules).values({
          name: input.name,
  ...
  updateRule: protectedProcedure
    .input(z.object({
      ...
      category: z.enum(["velocity", "geofence", "device_fingerprint", "amount_anomaly", "time_of_day", "blacklist", "custom"]).optional(),
      description: z.string().optional(),
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const [rule] = await db.update(fraudRules).set(updates).where(eq(fraudRules.id, id)).returning();
  ...
  toggleRule: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
        const [rule] = await db.update(fraudRules)
  ...
  seedDefaultRules: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      ...
      const existing = await db.select({ id: fraudRules.id }).from(fraudRules).limit(1);
      if (existing.length > 0) return { seeded: 0, message: "Rules already exist — no changes made" };
      const DEFAULT_RULES = [
        {
          name: "Velocity: Max 5 Transactions per 10 Minutes",
          category: "velocity" as const,
          description: "Flags agents who process more than 5 transactions within any 10-minute window. Prevents rapid-fire fraud.",
          threshold: "0.8000",
          windowSeconds: 600,
          maxCount: 5,
          enabled: true,
        ...
        },
        {
          ...
          category: "velocity" as const,
          description: "Flags agents exceeding 20 transactions in a rolling 60-minute window.",
          threshold: "0.7500",
          windowSeconds: 3600,
          maxCount: 20,
          enabled: true,
        ...
        },
        {
          ...
          description: "Flags transactions where the amount exceeds 3× the agent's 30-day daily average. Catches unusual large-value fraud.",
          threshold: "0.7000",
          windowSeconds: 86400,
          maxCount: 1,
          enabled: true,
        ...
        },
        {
          ...
          category: "amount_anomaly" as const,
          description: "Flags any single transaction exceeding ₦500,000 for mandatory manual review.",
          threshold: "0.9000",
          windowSeconds: null as any,
          maxCount: null as any,
          enabled: true,
          createdBy: "system",
        },
        {
          ...
          description: "Flags transactions processed outside the agent's registered geofence zone. Detects terminal relocation fraud.",
          threshold: "0.8500",
          windowSeconds: null as any,
          maxCount: null as any,
          enabled: true,
        ...
        },
        {
          ...
          description: "Flags transactions from a device ID not registered to the agent. Detects SIM-swap and device cloning.",
          threshold: "0.9000",
          windowSeconds: null as any,
          maxCount: null as any,
          enabled: true,
        ...
        },
        {
          ...
          category: "device_fingerprint" as const,
          description: "Flags agents who transact from 3 or more distinct device IDs within 24 hours.",
          threshold: "0.8000",
          windowSeconds: 86400,
          maxCount: 3,
          enabled: true,
        ...
        },
        {
          ...
          description: "Flags transactions processed between 23:00 and 05:00 WAT. Agency banking fraud peaks in late-night hours.",
          threshold: "0.6500",
          windowSeconds: null as any,
          maxCount: null as any,
          enabled: true,
        ...
        },
        {
          ...
          description: "Blocks transactions where the customer phone number appears on the CBN/NFIU fraud blacklist.",
          threshold: "1.0000",
          windowSeconds: null as any,
          maxCount: null as any,
          enabled: true,
        ...
        },
        {
          ...
          description: "Flags 3 or more transactions of the exact same amount within 30 minutes. Detects structured/smurfing fraud.",
          threshold: "0.7500",
          windowSeconds: 1800,
          maxCount: 3,
          enabled: true,
      ...
      const inserted = await db.insert(fraudRules).values(DEFAULT_RULES).returning({ id: fraudRules.id });
      await writeAuditLog({
        agentId: agent.id,
        agentCode: agent.agentCode,
      ...
      return { seeded: inserted.length, message: `${inserted.length} default fraud rules created successfully` };
    } catch (error) {
      if (error instanceof TRPCError) throw error;
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
    }
