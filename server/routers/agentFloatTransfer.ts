...
export const agentFloatTransferRouter = router({
  transfer: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [sender] = await db.select({ floatBalance: agents.floatBalance }).from(agents)
      ...
      const [recipient] = await db.select({ id: agents.id, agentCode: agents.agentCode }).from(agents)
        .where(eq(agents.agentCode, input.recipientAgentCode)).limit(1);
      if (!recipient) throw new TRPCError({ code: "NOT_FOUND", message: "Recipient agent not found" });
      ...
      await writeAuditLog({
        ...
        action: "AGENT_FLOAT_TRANSFERRED", resource: "agent_float_transfer", resourceId: ref, status: "success",
        metadata: { recipientCode: input.recipientAgentCode, amount: input.amount, narration: input.narration },
      });

      return { ref, amount: input.amount, recipientCode: input.recipientAgentCode, status: "completed", timestamp: new Date().toISOString() };
    }),
