...
export const networkTrendsRouter = router({
  dashboard: protectedProcedure.query(async () => {
    ...
    const regions = await db.select({ location: agents.location }).from(agents).where(eq(agents.isActive, true)).groupBy(agents.location);
  ...
  getAgentGrowth: protectedProcedure.input(z.object({ days: z.number().default(30) }).optional()).query(async ({ input }) => {
    ...
    const [newAgents] = await db.select({ value: count() }).from(agents).where(gte(agents.createdAt, since));
