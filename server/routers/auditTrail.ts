...
export const auditTrailRouter = router({
  list: protectedProcedure.input(z.object({ limit: z.number().default(50), offset: z.number().default(0), action: z.string().optional(), resource: z.string().optional(), status: z.string().optional() })).query(async ({ input }) => {
    try {
      ...
      if (input.status) conditions.push(eq(auditLog.tenantId, input.status as any));
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(auditLog).limit(100);
    const [success] = await db.select({ value: count() }).from(auditLog).where(eq(auditLog.status, "success")).limit(100);
    const [failure] = await db.select({ value: count() }).from(auditLog).where(eq(auditLog.status, "failure")).limit(100);
