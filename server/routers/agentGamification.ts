...
const BADGE_DEFINITIONS = [
  { id: "first_tx", name: "First Transaction", description: "Complete your first transaction", xp: 10, icon: "trophy", tier: "bronze" },
  { id: "tx_100", name: "Century Club", description: "Complete 100 transactions", xp: 100, icon: "star", tier: "silver" },
  { id: "tx_1000", name: "Transaction Master", description: "Complete 1,000 transactions", xp: 500, icon: "trophy", tier: "gold" },
  { id: "volume_1m", name: "Millionaire Agent", description: "Process ₦1M in volume", xp: 200, icon: "star", tier: "gold" },
  { id: "volume_10m", name: "Volume Champion", description: "Process ₦10M in volume", xp: 1000, icon: "star", tier: "platinum" },
  { id: "zero_fraud", name: "Perfect Record", description: "30 days with zero fraud alerts", xp: 300, icon: "shield", tier: "diamond" },
  { id: "top_performer", name: "Top Performer", description: "Rank #1 in weekly leaderboard", xp: 500, icon: "heart", tier: "diamond" },
  { id: "early_bird", name: "Early Bird", description: "Complete 10 transactions before 8 AM", xp: 50, icon: "sunrise", tier: "silver" },
  { id: "kyc_complete", name: "Fully Verified", description: "Complete all KYC requirements", xp: 150, icon: "shield", tier: "gold" },
  { id: "referral_5", name: "Recruiter", description: "Refer 5 new agents", xp: 250, icon: "heart", tier: "gold" },
];
...
export const agentGamificationRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    const [stats] = await db.select({ activePlayers: count(), topScore: sum(agentAchievements.xpEarned) }).from(agentAchievements);
  ...
  getLeaderboard: protectedProcedure
    .input(z.object({ period: z.enum(["daily", "weekly", "monthly", "all_time"]).default("monthly"), limit: z.number().default(20) }).optional())
    .query(async ({ input }) => {
      const db = (await getDb())!;
      if (!db) return { leaderboard: [], period: input?.period || "monthly", updatedAt: new Date().toISOString() };
      ...
      const leaderboard = data.map((d, i) => ({
        ...
        badges: [],
        tier: i < 2 ? "diamond" : i < 5 ? "platinum" : i < 10 ? "gold" : "silver",
        streak: Math.max(1, 30 - i),
      }));
      return { leaderboard, period: input?.period || "monthly", updatedAt: new Date().toISOString() };
    }),
  ...
  getAgentProfile: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [xpStats] = await db.select({ totalXp: sum(agentAchievements.xpEarned) }).from(agentAchievements).where(eq(agentAchievements.agentId, agentIdNum));
      const badges = await db.select().from(agentBadges).where(eq(agentBadges.agentId, agentIdNum));
  ...
  awardAchievement: protectedProcedure
    .input(z.object({ agentId: z.number(), achievementType: z.string(), description: z.string(), xp: z.number().default(10) }))
  ...
  awardBadge: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [existing] = await db.select().from(agentBadges).where(and(eq(agentBadges.agentId, input.agentId), eq(agentBadges.badgeId, input.badgeId)));
