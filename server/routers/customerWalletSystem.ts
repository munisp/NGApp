...
export const customerWalletSystemRouter = router({
  getBalance: protectedProcedure.input(z.object({ customerId: z.number() })).query(async ({ input }) => {
    try {
      ...
      const [credits] = await db.select({ total: sum(transactions.amount) }).from(transactions).where(and(eq(transactions.agentId, input.customerId), eq(transactions.type, "Cash In"))).limit(100);
      const [debits] = await db.select({ total: sum(transactions.amount) }).from(transactions).where(and(eq(transactions.agentId, input.customerId), eq(transactions.type, "Cash Out"))).limit(100);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [totalCustomers] = await db.select({ value: count() }).from(customers).limit(100);
