...
export const slaMonitoringRouter = router({
  listDefinitions: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [{ total }] = await db.select({ total: count() }).from(sla_definitions).where(where);
      return { items, total };
    }),
  ...
  listBreaches: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const items = await db.select().from(sla_breaches).where(where).orderBy(desc(sla_breaches.createdAt))
        .limit(input.limit).offset((input.page - 1) * input.limit);
      const [{ total }] = await db.select({ total: count() }).from(sla_breaches).where(where);
      return { items, total };
    }),
  ...
  summary: protectedProcedure.query(async () => {
    ...
    const [slas] = await db.select({ total: count() }).from(sla_definitions);
    const [active] = await db.select({ total: count() }).from(sla_definitions).where(eq(sla_definitions.isActive, true));
    const [breaches] = await db.select({ total: count() }).from(sla_breaches);
    const [open] = await db.select({ total: count() }).from(sla_breaches).where(sql`${sla_breaches.resolvedAt} IS NULL`);
