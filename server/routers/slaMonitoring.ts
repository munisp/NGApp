...
export const slaMonitoringRouter = router({
  listDefinitions: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const conditions = input.active !== undefined ? [eq(sla_definitions.isActive, input.active)] : [];
        const where = conditions.length > 0 ? and(...conditions) : undefined;
        const items = await db.select().from(sla_definitions).where(where).orderBy(desc(sla_definitions.createdAt))
          .limit(input.limit).offset((input.page - 1) * input.limit);
        const [{ total }] = await db.select({ total: count() }).from(sla_definitions).where(where).limit(100);
        return { items, total };
      } catch (error) {
  ...
  createDefinition: protectedProcedure
    .input(z.object({
      name: z.string(), serviceName: z.string(), metric: z.string(),
      targetValue: z.number(), unit: z.string(), measurementWindow: z.string(),
      breachThreshold: z.number(), escalationPolicy: z.any().optional(),
    }))
    .mutation(async ({ input, ctx }) => {
      try {
        const db = (await getDb())!;
        if (!db) throw new Error("Database unavailable");
        const [def] = await db.insert(sla_definitions).values({
          name: input.name, serviceName: input.serviceName, metric: input.metric,
          targetValue: String(input.targetValue), unit: input.unit,
  ...
  listBreaches: protectedProcedure
    .input(z.object({ page: z.number().default(1), limit: z.number().default(20), slaId: z.number().optional(), severity: z.string().optional(), resolved: z.boolean().optional() }))
    .query(async ({ input }) => {
      try {
        const db = (await getDb())!;
        if (!db) return { items: [], total: 0 };
        ...
        const items = await db.select().from(sla_breaches).where(where).orderBy(desc(sla_breaches.createdAt))
          .limit(input.limit).offset((input.page - 1) * input.limit);
        const [{ total }] = await db.select({ total: count() }).from(sla_breaches).where(where).limit(100);
        return { items, total };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
  ...
  recordBreach: protectedProcedure
    .input(z.object({
      slaId: z.number(), actualValue: z.number(), severity: z.enum(["warning", "minor", "major", "critical"]),
  ...
  summary: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    if (!db) return { totalSLAs: 0, activeSLAs: 0, totalBreaches: 0, openBreaches: 0, avgUptime: 99.95 };
    const [slas] = await db.select({ total: count() }).from(sla_definitions).limit(100);
    const [active] = await db.select({ total: count() }).from(sla_definitions).where(eq(sla_definitions.isActive, true)).limit(100);
    const [breaches] = await db.select({ total: count() }).from(sla_breaches).limit(100);
    const [open] = await db.select({ total: count() }).from(sla_breaches).where(sql`${sla_breaches.resolvedAt} IS NULL`).limit(100);
    return { totalSLAs: slas.total || 0, activeSLAs: active.total || 0, totalBreaches: breaches.total || 0, openBreaches: open.total || 0, avgUptime: 99.95 };
  }),
});
