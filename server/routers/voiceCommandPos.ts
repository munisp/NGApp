...
const INTENT_MAP: Record<string, { type: string; description: string }> = {
  send_money: { type: "Transfer", description: "Send money to another account" },
...
export const voiceCommandPosRouter = router({
  processCommand: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        // Intent detection
        if (lower.includes("send") || lower.includes("transfer")) detectedIntent = "send_money";
        else if (lower.includes("deposit") || lower.includes("cash in")) detectedIntent = "cash_in";
        else if (lower.includes("withdraw") || lower.includes("cash out")) detectedIntent = "cash_out";
        else if (lower.includes("airtime") || lower.includes("recharge")) detectedIntent = "buy_airtime";
        else if (lower.includes("bill") || lower.includes("pay")) detectedIntent = "pay_bill";
        else if (lower.includes("balance") || lower.includes("check")) detectedIntent = "check_balance";

        // Amount extraction
        const amountMatch = lower.match(/(\d[\d,]*(?:\.\d{2})?)\s*(?:naira|ngn|#)?/);
        ...
        await writeAuditLog({
          ...
          action: "VOICE_COMMAND_PROCESSED", resource: "voice_command", status: "success",
          metadata: { transcript: input.transcript, language: input.language, intent: detectedIntent, amount, phone },
        });
  ...
  confirmAndExecute: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (!intentInfo) throw new TRPCError({ code: "BAD_REQUEST", message: "Unknown intent" });
        ...
        const [tx] = await db.insert(transactions).values({
          ref, agentId: session.id, type: intentInfo.type,
          amount: String(input.amount), commission: String(commission),
        ...
        await writeAuditLog({
          agentId: session.id, agentCode: session.agentCode,
          action: "VOICE_TX_EXECUTED", resource: "voice_transaction", resourceId: ref, status: "success",
          metadata: { intent: input.intent, amount: input.amount, type: intentInfo.type },
        });

        return { ref, type: intentInfo.type, amount: input.amount, commission, status: "success", transactionId: tx.id };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const items = await db.select().from(transactions)
          .where(and(eq(transactions.agentId, session.id), sql`${transactions.metadata}->>'voiceInitiated' = 'true'`))
  ...
  getStats: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      return { totalCommands: 0, totalVoiceTxns: stats.total, totalAmount: stats.totalAmount };
    } catch (error) {
      if (error instanceof TRPCError) throw error;
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
    }
