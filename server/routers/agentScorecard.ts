...
export const agentScorecardRouter = router({
  getScorecard: protectedProcedure.input(z.object({ agentId: z.number() })).query(async ({ input }) => {
    try {
      ...
      const [txStats] = await db.select({ txCount: count(), volume: sum(transactions.amount) }).from(transactions).where(eq(transactions.agentId, input.agentId)).limit(100);
      const [successTx] = await db.select({ cnt: count() }).from(transactions).where(and(eq(transactions.agentId, input.agentId), eq(transactions.status, "success"))).limit(100);
      const [disputeCount] = await db.select({ cnt: count() }).from(disputes).where(eq(disputes.agentId, input.agentId)).limit(100);
      ...
      return { agentId: input.agentId, name: agent.name, tier: agent.tier, location: agent.location, metrics: { txCount: Number(txStats.txCount), volume: Number(txStats.volume ?? 0), successRate, disputeRate, disputeCount: Number(disputeCount.cnt) }, overallScore, rating: overallScore >= 90 ? "Excellent" : overallScore >= 70 ? "Good" : overallScore >= 50 ? "Average" : "Needs Improvement" };
    } catch (error) {
  ...
  listScorecards: protectedProcedure.input(z.object({ limit: z.number().default(50), minScore: z.number().optional() })).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const rows = await db.select().from(agentPerformanceScores).orderBy(desc(agentPerformanceScores.overallScore)).limit(input.limit);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [avgScore] = await db.select({ value: avg(agentPerformanceScores.overallScore) }).from(agentPerformanceScores).limit(100);
    const [total] = await db.select({ value: count() }).from(agentPerformanceScores).limit(100);
