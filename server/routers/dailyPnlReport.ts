...
export const dailyPnlReportRouter = router({
  getReport: protectedProcedure.input(z.object({ date: z.string().optional(), agentId: z.number().optional() }).optional()).query(async ({ input }) => {
    ...
    const [revenue] = await db.select({ totalAmount: sum(transactions.amount), totalFee: sum(transactions.fee), totalCommission: sum(transactions.commission), txCount: count() }).from(transactions).where(and(...conditions, eq(transactions.status, "success")));
    const [failed] = await db.select({ value: count() }).from(transactions).where(and(...conditions, eq(transactions.status, "failed")));
    const [billing] = await db.select({ totalBilled: sum(platformBillingLedger.platformRevenue) }).from(platformBillingLedger).where(and(gte(platformBillingLedger.createdAt, startOfDay), lte(platformBillingLedger.createdAt, endOfDay)));
    return {
