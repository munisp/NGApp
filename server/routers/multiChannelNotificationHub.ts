...
export const multiChannelNotificationHubRouter = router({
  dashboard: protectedProcedure.query(async () => {
    ...
    const [total] = await db.select({ value: count() }).from(notification_logs).limit(100);
    const [channels] = await db.select({ value: count() }).from(notification_channels).limit(100);
    const [pending] = await db.select({ value: count() }).from(notification_logs).where(eq(notification_logs.status, "pending")).limit(100);
  ...
  listChannels: protectedProcedure.query(async () => {
    ...
    const rows = await db.select().from(notification_channels).limit(100);
