...
export const commissionCalculatorRouter = router({
  dashboard: protectedProcedure.query(async () => {
    ...
    const [ruleCount] = await db.select({ value: count() }).from(commissionRules);
    const [tierCount] = await db.select({ value: count() }).from(commissionTiers);
    const [payoutCount] = await db.select({ value: count() }).from(commissionPayouts);
