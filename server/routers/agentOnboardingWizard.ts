...
export const agentOnboardingWizardRouter = router({
  getProgress: protectedProcedure.input(z.object({ agentId: z.number() })).query(async ({ input }) => {
    try {
      ...
      const [kyc] = await db.select({ cnt: count() }).from(kycSessions).where(and(eq(kycSessions.agentId, input.agentId), eq(kycSessions.status, "completed"))).limit(100);
      const [floatReq] = await db.select({ cnt: count() }).from(floatTopUpRequests).where(eq(floatTopUpRequests.agentId, input.agentId)).limit(100);
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    const [pending] = await db.select({ value: count() }).from(agents).where(eq(agents.isActive, false)).limit(100);
