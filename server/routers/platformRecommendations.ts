...
export const platformRecommendationsRouter = router({
  dashboard: protectedProcedure.query(async () => {
    ...
    const rows = await db.select().from(systemConfig).where(sql`${systemConfig.key} LIKE 'recommendations_%'`).limit(100);
  ...
  list: protectedProcedure.input(z.object({ limit: z.number().default(20) }).optional()).query(async ({ input }) => {
    ...
    const rows = await db.select().from(systemConfig).where(sql`${systemConfig.key} LIKE 'recommendations_%'`).limit(input?.limit ?? 20);
    return { items: rows.map(r => ({ id: r.key.replace("recommendations_", ""), ...JSON.parse(String(r.value ?? "{}")) })), total: rows.length };
  }),
  ...
  delete: protectedProcedure.input(z.object({ itemId: z.string() })).mutation(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new Error("DB not available");
    await db.delete(systemConfig).where(eq(systemConfig.key, "recommendations_" + input.itemId));
    return { success: true };
  }),
});
