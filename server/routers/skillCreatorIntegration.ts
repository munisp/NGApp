...
const getSkillInfo = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(workflowInstances).limit(100);
...
const generateRouter = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(workflowInstances).where(eq(workflowInstances.id, input.id)).limit(100);
