...
export const autoReconciliationEngineRouter = router({
  reconcile: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const txns = await db.select({ count: sql<number>`COUNT(*)`, total: sql<number>`COALESCE(SUM(${transactions.amount}), 0)` }).from(transactions).where(between(transactions.createdAt, start, end)).limit(100);
        const floats = await db.select({ count: sql<number>`COUNT(*)`, total: sql<number>`COALESCE(SUM(${transactions.amount}), 0)` }).from(transactions).limit(100);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [{ count }] = await db.select({ count: sql<number>`COUNT(*)` }).from(transactions).limit(100);
