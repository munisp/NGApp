...
export const recurringPaymentsRouter = router({
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const schedule = {
          id: scheduleId, agentId: session.id, ...input,
          status: "active", createdAt: new Date().toISOString(), nextRun: input.startDate,
        ...
        await db.insert(platformSettings)
          ...
          .onConflictDoUpdate({ target: platformSettings.key, set: { value: JSON.stringify(schedule) } });

        await writeAuditLog({
          agentId: session.id, agentCode: session.agentCode,
          action: "RECURRING_PAYMENT_CREATED", resource: "recurring_payment", resourceId: scheduleId, status: "success",
          metadata: { type: input.type, amount: input.amount, frequency: input.frequency },
        });
  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const schedules = (rows.rows ?? []).map((r: Record<string, unknown>) => {
  ...
  cancel: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await writeAuditLog({
          agentId: session.id, agentCode: session.agentCode,
          action: "RECURRING_PAYMENT_CANCELLED", resource: "recurring_payment", resourceId: input.scheduleId, status: "success",
        });
