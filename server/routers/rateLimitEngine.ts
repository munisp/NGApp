...
export const rateLimitEngineRouter = router({
  listRules: protectedProcedure
    .input(z.object({ page: z.number().default(1), limit: z.number().default(50), active: z.boolean().optional() }))
    .query(async ({ input }) => {
      try {
        ...
        const [{ total }] = await db.select({ total: count() }).from(rateLimitRules).where(where).limit(100);
        return { items, total };
      } catch (error) {
  ...
  createRule: protectedProcedure
    .input(z.object({
      name: z.string(), endpoint: z.string(), method: z.string().default("*"),
      maxRequests: z.number().min(1), windowSeconds: z.number().min(1), burstAllowance: z.number().default(0),
      scope: z.enum(["global", "per_ip", "per_agent", "per_user"]).default("global"),
    }))
  ...
  updateRule: protectedProcedure
    .input(z.object({ ruleId: z.number(), maxRequests: z.number().optional(), windowSeconds: z.number().optional(), burstAllowance: z.number().optional(), active: z.boolean().optional() }))
