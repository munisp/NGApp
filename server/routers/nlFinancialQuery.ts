...
export const nlFinancialQueryRouter = router({
  query: protectedProcedure.input(z.object({ query: z.string(), limit: z.number().default(10) })).mutation(async ({ input }) => {
    ...
    await db.insert(auditLog).values({ action: "nl_query_executed", resource: "nl_financial_query", resourceId: "query-" + crypto.randomUUID(), status: "success", metadata: { query: input.query } });
