...
export const agentTrainingRouter = router({
  ...
  getCourse: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      ...
      const [enrollCount] = await db.select({ value: count() }).from(trainingEnrollments).where(eq(trainingEnrollments.courseId, input.id)).limit(100);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [totalCourses] = await db.select({ value: count() }).from(trainingCourses).limit(100);
    const [totalEnrollments] = await db.select({ value: count() }).from(trainingEnrollments).limit(100);
