...
export const transactionReceiptGeneratorRouter = router({
  dashboard: protectedProcedure.query(async () => {
    ...
    if (!db) return { totalTemplates: 0, totalGenerated: 0, thermalReceipts: 0, emailReceipts: 0 };
    const rows = await db.select().from(systemConfig).where(sql`\${systemConfig.key} LIKE 'receipt_template_%'`).limit(100);
    return { totalTemplates: rows.length, totalGenerated: 0, thermalReceipts: 0, emailReceipts: 0 };
  }),
  listTemplates: protectedProcedure.input(z.object({ limit: z.number().default(20) }).optional()).query(async ({ input }) => {
    try {
      const db = await getDb();
      if (!db) return { templates: [], total: 0 };
      const rows = await db.select().from(systemConfig).where(sql`\${systemConfig.key} LIKE 'receipt_template_%'`).limit(input?.limit ?? 20);
      return { templates: rows.map(r => ({ id: r.key.replace("receipt_template_", ""), ...JSON.parse(String(r.value ?? "{}")) })), total: rows.length };
    } catch (error) {
  ...
  generateReceipt: protectedProcedure.input(z.object({ transactionId: z.number(), templateId: z.string().optional() })).mutation(async ({ input }) => {
    try {
      const db = await getDb();
      if (!db) throw new Error("DB not available");
