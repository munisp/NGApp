...
const listDevices = protectedProcedure
  ...
  .query(async ({ input }) => {
    ...
    const [{ total }] = await db.select({ total: count() }).from(mdmGeofenceViolations);
...
const updateFirmware = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    const db = (await getDb())!;
    const [existing] = await db.select().from(mdmGeofenceViolations).where(eq(mdmGeofenceViolations.id, input.id));
