...
const listDevices = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const rows = await db.select().from(mdmGeofenceViolations).orderBy(desc(mdmGeofenceViolations.id)).limit(lim).offset(offset);
      const [{ total }] = await db.select({ total: count() }).from(mdmGeofenceViolations).limit(100);
...
const getFleetStats = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [{ total }] = await db.select({ total: count() }).from(mdmGeofenceViolations).limit(100);
      const recent = await db.select().from(mdmGeofenceViolations).orderBy(desc(mdmGeofenceViolations.id)).limit(5);
...
const updateFirmware = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [existing] = await db.select().from(mdmGeofenceViolations).where(eq(mdmGeofenceViolations.id, input.id)).limit(100);
      if (!existing) throw new TRPCError({ code: "NOT_FOUND", message: "updateFirmware: record not found" });
      if (input.data) {
        const [updated] = await db.update(mdmGeofenceViolations).set(input.data as any).where(eq(mdmGeofenceViolations.id, input.id)).returning();
