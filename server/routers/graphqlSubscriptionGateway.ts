...
export const graphqlSubscriptionGatewayRouter = router({
  listSubscriptions: protectedProcedure.input(z.object({ limit: z.number().min(1).max(200).default(50), status: z.enum(["active", "paused", "error"]).optional() }).optional()).query(async ({ input }) => {
    ...
    const rows = await db.select().from(platform_health_checks).where(eq(platform_health_checks.component, "graphql_subscription")).orderBy(desc(platform_health_checks.checkedAt)).limit(input?.limit ?? 50);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(platform_health_checks).where(eq(platform_health_checks.component, "graphql_subscription"));
    const [healthy] = await db.select({ value: count() }).from(platform_health_checks).where(and(eq(platform_health_checks.component, "graphql_subscription"), eq(platform_health_checks.status, "healthy")));
