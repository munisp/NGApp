...
const ErpConfigInputSchema = z.object({
  erpType: z.enum(["odoo", "sap", "netsuite", "quickbooks", "sage", "dynamics365", "custom"]),
...
async function getOrCreateConfig(db: Awaited<ReturnType<typeof getDb>>) {
  ...
  const [created] = await db.insert(erpConfig).values({
    erpType: "odoo",
    name: "Default ERP",
    baseUrl: "",
  }).returning();
...
function buildHeaders(cfg: { erpType: string; apiKey?: string | null; username?: string | null }) {
...
async function pushTransactionToErp(
  ...
  if (!cfg.baseUrl) return { success: false, error: "ERP base URL not configured" };
  try {
    const endpoint = cfg.erpType === "odoo"
...
export const erpRouter = router({
  ...
  getConfig: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      return {
        ...cfg,
        apiKey: cfg.apiKey ? `${cfg.apiKey.slice(0, 4)}${"*".repeat(Math.max(0, (cfg.apiKey?.length ?? 0) - 8))}${cfg.apiKey.slice(-4)}` : "",
      };
    } catch (error) {
  ...
  testWebhook: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      ...
      const cfg = await getOrCreateConfig(db);
      if (!cfg.baseUrl) {
        return { success: false, latencyMs: null, message: "ERP base URL is not configured. Save a configuration first." };
      }
      const start = Date.now();
      try {
        const pingUrl = cfg.erpType === "odoo"
        ...
        return { success: true, latencyMs, message: `Connected to ${cfg.name} (${cfg.erpType}) in ${latencyMs}ms` };
      } catch (e: unknown) {
        const latencyMs = Date.now() - start;
        return {
          success: false,
  ...
  syncNow: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      ...
      const cfg = await getOrCreateConfig(db);
      if (!cfg.syncEnabled) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "ERP sync is disabled. Enable it in configuration first." });
      }
      if (!cfg.baseUrl) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "ERP base URL is not configured." });
      }
      // Find transactions not yet synced to ERP
      const pendingLogs = await db.select({ entityId: erpSyncLog.entityId })
        .from(erpSyncLog)
        .where(and(eq(erpSyncLog.entityType, "transaction"), eq(erpSyncLog.status, "synced")));
      const syncedIds = new Set(pendingLogs.map(r => r.entityId));
      const allTx = await db.select().from(transactions).orderBy(desc(transactions.createdAt)).limit(100);
      const toSync = allTx.filter(tx => !syncedIds.has(String(tx.id)));
      let synced = 0;
      let failed = 0;
      for (const tx of toSync) {
        const result = await pushTransactionToErp(cfg, tx as unknown as Record<string, unknown>);
      ...
      await db.update(erpConfig).set({
        lastSyncAt: new Date(),
        lastSyncStatus: failed === 0 ? "success" : synced > 0 ? "partial" : "failed",
        lastSyncCount: synced,
        lastSyncError: failed > 0 ? `${failed} transaction(s) failed to sync` : null,
        updatedAt: new Date(),
  ...
  getSyncLog: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const rows = await db.select().from(erpSyncLog)
  ...
  retrySync: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
        const cfg = await getOrCreateConfig(db);
        const [logEntry] = await db.select().from(erpSyncLog).where(eq(erpSyncLog.id, input.logId)).limit(1);
        if (!logEntry) throw new TRPCError({ code: "NOT_FOUND", message: "Sync log entry not found" });
        if (logEntry.entityType !== "transaction") {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Only transaction sync entries can be retried" });
        }
        const result = await pushTransactionToErp(cfg, (logEntry.payload ?? {}) as Record<string, unknown>);
        await db.update(erpSyncLog).set({
          status: result.success ? "synced" : "failed",
          errorMessage: result.error ?? null,
