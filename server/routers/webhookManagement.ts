...
export const webhookManagementRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    const [subs] = await db.select({ total: count() }).from(webhookEndpoints);
    const [activeSubs] = await db.select({ total: count() }).from(webhookEndpoints).where(eq(webhookEndpoints.isActive, true));
    const [deliveries] = await db.select({ total: count() }).from(webhookDeliveries);
    const [failed] = await db.select({ total: count() }).from(webhookDeliveries).where(eq(webhookDeliveries.status, "failed"));
    return {
  ...
  dashboard: protectedProcedure.query(async () => {
    ...
    const [subs] = await db.select({ total: count() }).from(webhookEndpoints);
    const [active] = await db.select({ total: count() }).from(webhookEndpoints).where(eq(webhookEndpoints.isActive, true));
    const since24h = new Date(Date.now() - 86400000);
    const [del24h] = await db.select({ total: count() }).from(webhookDeliveries).where(gte(webhookDeliveries.createdAt, since24h));
  ...
  retryFailed: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const db = (await getDb())!;
      if (db && id) {
        const [log] = await db.select().from(webhookDeliveries).where(eq(webhookDeliveries.id, id));
        if (log) {
