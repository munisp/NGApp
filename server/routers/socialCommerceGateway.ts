...
export const socialCommerceGatewayRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [stats] = await db.select({ totalOrders: count(), totalVolume: sum(sql`CAST(amount AS numeric)`) }).from(transactions).where(eq(transactions.channel, "App")).limit(100);
