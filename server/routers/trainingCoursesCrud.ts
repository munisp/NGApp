...
export const trainingCoursesRouter = router({
  list: protectedProcedure.input(z.object({ category: z.string().optional(), isMandatory: z.boolean().optional(), limit: z.number().default(20), offset: z.number().default(0) })).query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(trainingCourses).where(conditions.length ? and(...conditions) : undefined).limit(100);
  ...
  getById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [row] = await db.select().from(trainingCourses).where(eq(trainingCourses.id, input.id)).limit(100);
