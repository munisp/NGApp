...
const uploadBatch = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(merchantPayouts).limit(100);
...
const processBatch = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(merchantPayouts).where(eq(merchantPayouts.id, input.id)).limit(100);
