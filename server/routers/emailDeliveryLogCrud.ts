...
export const emailDeliveryLogRouter = router({
  ...
  getById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [row] = await db.select().from(emailDeliveryLog).where(eq(emailDeliveryLog.id, input.id)).limit(100);
  ...
  getDeliverabilityScore: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [stats] = await db.select({ total: count(), delivered: sql<number>`COUNT(*) FILTER (WHERE status = 'delivered')`, bounced: sql<number>`COUNT(*) FILTER (WHERE status = 'bounced')`, failed: sql<number>`COUNT(*) FILTER (WHERE status = 'failed')` }).from(emailDeliveryLog).limit(100);
