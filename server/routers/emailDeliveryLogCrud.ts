...
export const emailDeliveryLogRouter = router({
  list: protectedProcedure.input(z.object({ status: z.string().optional(), limit: z.number().default(20), offset: z.number().default(0) })).query(async ({ input }) => {
    ...
    const [{ total }] = await db.select({ total: count() }).from(emailDeliveryLog).where(conditions.length ? and(...conditions) : undefined);
    return { items: rows, total };
  }),
  getById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db = (await getDb())!;
    const [row] = await db.select().from(emailDeliveryLog).where(eq(emailDeliveryLog.id, input.id));
    if (!row) throw new TRPCError({ code: "NOT_FOUND", message: "Email delivery log not found" });
    return row;
  }),
  getDeliverabilityScore: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [stats] = await db.select({ total: count(), delivered: sql<number>`COUNT(*) FILTER (WHERE status = 'delivered')`, bounced: sql<number>`COUNT(*) FILTER (WHERE status = 'bounced')`, failed: sql<number>`COUNT(*) FILTER (WHERE status = 'failed')` }).from(emailDeliveryLog);
    ...
    return { deliveryRate: Math.round(deliveryRate * 100) / 100, bounceRate: Math.round(bounceRate * 100) / 100, total: stats.total, delivered: stats.delivered, bounced: stats.bounced, failed: stats.failed, health: deliveryRate > 95 ? "excellent" : deliveryRate > 85 ? "good" : deliveryRate > 70 ? "needs_attention" : "critical" };
  }),
  retryFailed: protectedProcedure.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = (await getDb())!;
    const [record] = await db.select().from(emailDeliveryLog).where(eq(emailDeliveryLog.id, input.id));
