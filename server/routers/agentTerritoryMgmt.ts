...
export const agentTerritoryMgmtRouter = router({
  ...
  getTerritory: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      ...
      if (!zone) return null;
      const assignments = await db.select().from(agentGeofenceZones).where(eq(agentGeofenceZones.zoneId, input.id)).limit(100);
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [totalZones] = await db.select({ value: count() }).from(geofenceZones).limit(100);
    const [totalAssignments] = await db.select({ value: count() }).from(agentGeofenceZones).limit(100);
