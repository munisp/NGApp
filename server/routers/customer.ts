...
import {
  customers, transactions, disputes,
  qrCodes, kycSessions, shareableLinks,
  fido2Credentials, fido2Challenges,
  creditScoreHistory, creditApplications,
} from "../../drizzle/schema";
...
async function resolveCustomer(userId: number | string) {
  ...
  const [customer] = await db.select().from(customers).where(eq(customers.keycloakSub, String(userId))).limit(100);
...
export const customerRouter = router({
  ...
  account: router({
    ...
    update: customerProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          const [updated] = await db.update(customers).set({ ...input, updatedAt: new Date() }).where(eq(customers.id, customer.id)).returning();
    ...
    balance: customerProcedure.query(async ({ ctx }) => {
      try {
        const { customer } = await resolveCustomer(ctx.user.id);
        return { walletBalance: customer.walletBalance, dailyLimit: customer.dailyLimit, monthlyLimit: customer.monthlyLimit };
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
    ...
    register: protectedProcedure
      .input(z.object({ firstName: z.string(), lastName: z.string(), phone: z.string(), email: z.string().email().optional(), bvn: z.string().length(11).optional() }))
      .mutation(async ({ input }) => {
        try {
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
          const [existing] = await db.select({ id: customers.id }).from(customers).where(eq(customers.phone, input.phone)).limit(100);
    ...
    receipt: protectedProcedure
      ...
      .query(async ({ input }) => {
        try {
          ...
          const [tx] = await db.select().from(transactions).where(eq(transactions.ref, input.ref)).limit(100);
    ...
    resolve: protectedProcedure
      ...
      .query(async ({ input }) => {
        try {
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "NOT_FOUND" });
          const [qr] = await db.select().from(qrCodes).where(eq(qrCodes.code, input.code)).limit(100);
    ...
    resolveLink: protectedProcedure
      ...
      .query(async ({ input }) => {
        try {
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "NOT_FOUND" });
          const [link] = await db.select().from(shareableLinks).where(eq(shareableLinks.slug, input.slug)).limit(100);
          if (!link) throw new TRPCError({ code: "NOT_FOUND", message: "Payment link not found" });
          if (link.expiresAt && link.expiresAt < new Date()) throw new TRPCError({ code: "BAD_REQUEST", message: "Payment link has expired" });
          return link;
        } catch (error) {
          if (error instanceof TRPCError) throw error;
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
    ...
    list: customerProcedure
      ...
      .query(async ({ ctx, input }) => {
        try {
          ...
          const [items, [{ total }]] = await Promise.all([
            db.select().from(disputes).where(eq(disputes.agentId, customer.preferredAgentId ?? 0)).orderBy(desc(disputes.createdAt)).limit(input.limit).offset(offset),
            db.select({ total: count() }).from(disputes).where(eq(disputes.agentId, customer.preferredAgentId ?? 0)),
          ]);
          return { items, total };
        } catch (error) {
          if (error instanceof TRPCError) throw error;
  ...
  kyc: router({
    status: customerProcedure.query(async ({ ctx }) => {
      try {
        const { db, customer } = await resolveCustomer(ctx.user.id);
        const [session] = await db.select().from(kycSessions).where(eq(kycSessions.agentId, customer.preferredAgentId ?? 0)).orderBy(desc(kycSessions.createdAt)).limit(1);
        return session ?? null;
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
    ...
    initiate: customerProcedure
      .input(z.object({ docType: z.enum(["NIN", "BVN_CARD", "PASSPORT", "DRIVERS_LICENCE", "VOTER_CARD"]) }))
      .mutation(async ({ ctx, input }) => {
        try {
          const { db, customer } = await resolveCustomer(ctx.user.id);
  ...
  fido2: router({
    listCredentials: customerProcedure.query(async ({ ctx }) => {
      try {
        ...
        }).from(fido2Credentials).where(eq(fido2Credentials.userId, Number(ctx.user.id))).limit(100);
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
      }
    ...
    registerCredential: customerProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
          const [row] = await db.insert(fido2Credentials).values({
    ...
    createChallenge: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          const [row] = await db.insert(fido2Challenges).values({
            challenge,
            userId: input.userId,
            type: input.type,
            expiresAt,
    ...
    verifyChallenge: protectedProcedure
      ...
      .query(async ({ input }) => {
        try {
          ...
          if (!db) return null;
          const now = new Date();
          const [row] = await db.select().from(fido2Challenges)
            .where(eq(fido2Challenges.challenge, input.challenge))
            .limit(1);
          if (!row || row.expiresAt < now) return null;
          await db.update(fido2Challenges).set({ usedAt: now }).where(eq(fido2Challenges.id, row.id));
  ...
  credit: router({
    scoreHistory: customerProcedure
      ...
      .query(async ({ input }) => {
        try {
          const db = (await getDb())!;
          if (!db) return [];
          return db.select().from(creditScoreHistory)
    ...
    addScore: protectedProcedure
      .input(z.object({ agentId: z.number(), score: z.number().min(0).max(1000), factors: z.record(z.string(), z.number()).optional() }))
      .mutation(async ({ input, ctx }) => {
        try {
          if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
          const rating = input.score >= 800 ? "AAA" : input.score >= 700 ? "AA" : input.score >= 600 ? "A" : input.score >= 500 ? "BBB" : input.score >= 400 ? "BB" : input.score >= 300 ? "B" : input.score >= 200 ? "CCC" : "D";
          const [row] = await db.insert(creditScoreHistory).values({
    ...
    listApplications: protectedProcedure
      .input(z.object({ agentId: z.number().optional(), status: z.string().optional(), limit: z.number().default(20), offset: z.number().default(0) }))
      .query(async ({ input }) => {
        try {
          const db = (await getDb())!;
          if (!db) return [];
          const conditions: ReturnType<typeof eq>[] = [];
          if (input.agentId) conditions.push(eq(creditApplications.agentId, input.agentId));
          if (input.status) conditions.push(sql`${creditApplications.status} = ${input.status}`);
          return db.select().from(creditApplications)
    ...
    applyForCredit: customerProcedure
      .input(z.object({ agentId: z.number(), requestedAmount: z.string(), termDays: z.number().default(30) }))
      .mutation(async ({ input }) => {
        try {
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
          const [row] = await db.insert(creditApplications).values({
            agentId: input.agentId,
    ...
    reviewApplication: protectedProcedure
      .input(z.object({ id: z.number(), status: z.enum(["approved", "rejected"]), approvedAmount: z.string().optional(), reviewNote: z.string().optional() }))
      .mutation(async ({ input, ctx }) => {
        try {
          if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
          const db = (await getDb())!;
          if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
          const [row] = await db.update(creditApplications).set({
            status: input.status,
            ...
            reviewedAt: new Date(),
            updatedAt: new Date(),
          }).where(eq(creditApplications.id, input.id)).returning();
          return row;
        } catch (error) {
          if (error instanceof TRPCError) throw error;
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "Internal server error" });
  ...
  profile: router({
    ...
    update: customerProcedure
      .input(z.object({ firstName: z.string().optional(), lastName: z.string().optional(), email: z.string().email().optional(), address: z.string().optional() }))
      .mutation(async ({ ctx, input }) => {
        try {
          const { db, customer } = await resolveCustomer(ctx.user.id);
          const [updated] = await db.update(customers).set({ ...input, updatedAt: new Date() }).where(eq(customers.id, customer.id)).returning();
