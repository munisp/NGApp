...
async function resolveCustomer(userId: number | string) {
  ...
  const [customer] = await db.select().from(customers).where(eq(customers.keycloakSub, String(userId)));
...
export const customerRouter = router({
  ...
  account: router({
    ...
    register: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const [existing] = await db.select({ id: customers.id }).from(customers).where(eq(customers.phone, input.phone));
  ...
  transactions: router({
    ...
    receipt: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [tx] = await db.select().from(transactions).where(eq(transactions.ref, input.ref));
  ...
  qr: router({
    resolve: protectedProcedure
      ...
      .query(async ({ input }) => {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "NOT_FOUND" });
        const [qr] = await db.select().from(qrCodes).where(eq(qrCodes.code, input.code));
    ...
    resolveLink: protectedProcedure
      ...
      .query(async ({ input }) => {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "NOT_FOUND" });
        const [link] = await db.select().from(shareableLinks).where(eq(shareableLinks.slug, input.slug));
  ...
  fido2: router({
    listCredentials: customerProcedure.query(async ({ ctx }) => {
      ...
      }).from(fido2Credentials).where(eq(fido2Credentials.userId, Number(ctx.user.id)));
    }),
