...
export const loanDisbursementRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(creditApplications);
    const [disbursed] = await db.select({ value: count() }).from(creditApplications).where(eq(creditApplications.status, "disbursed"));
    const [totalAmount] = await db.select({ value: sum(creditApplications.approvedAmount) }).from(creditApplications).where(eq(creditApplications.status, "disbursed"));
