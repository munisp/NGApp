...
const getOverallScore = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      const rows = await db.select().from(platform_health_checks).orderBy(desc(platform_health_checks.id)).limit(lim).offset(offset);
      const [{ total }] = await db.select({ total: count() }).from(platform_health_checks).limit(100);
...
const acknowledgeAlert = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      if (input.id) {
        const [existing] = await db.select().from(platform_health_checks).where(eq(platform_health_checks.id, input.id)).limit(100);
      ...
      const [row] = await db.insert(platform_health_checks).values(input.data as any || {}).returning();
