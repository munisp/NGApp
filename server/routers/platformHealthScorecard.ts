...
const getOverallScore = protectedProcedure
  ...
  .query(async ({ input }) => {
    ...
    const [{ total }] = await db.select({ total: count() }).from(platform_health_checks);
...
const acknowledgeAlert = protectedProcedure
  ...
  .mutation(async ({ input }) => {
    const db = (await getDb())!;
    if (input.id) {
      const [existing] = await db.select().from(platform_health_checks).where(eq(platform_health_checks.id, input.id));
