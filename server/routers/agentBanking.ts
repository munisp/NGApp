  ...
  transactions: router({
    ...
    get: agentProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [tx] = await db.select().from(transactions).where(and(eq(transactions.id, input.id), eq(transactions.agentId, input.agentId)));
  ...
  float: router({
    balance: agentProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [agent] = await db.select({ floatBalance: agents.floatBalance, commissionBalance: agents.commissionBalance }).from(agents).where(eq(agents.id, input.agentId));
  ...
  loyalty: router({
    profile: agentProcedure
      ...
      .query(async ({ input }) => {
        const db = (await getDb())!;
        if (!db) throw new TRPCError({ code: "NOT_FOUND" });
        const [agent] = await db.select({ loyaltyPoints: agents.loyaltyPoints, tier: agents.tier }).from(agents).where(eq(agents.id, input.agentId));
        if (!agent) throw new TRPCError({ code: "NOT_FOUND" });
        return agent;
      }),
    history: agentProcedure
