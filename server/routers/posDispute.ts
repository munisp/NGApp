...
export const posDisputeRouter = router({
  fileDispute: protectedProcedure
    .input(z.object({
      ...
      reason: z.enum(["wrong_amount", "failed_but_debited", "duplicate_charge", "unauthorized", "service_not_received", "other"]),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [tx] = await db.select().from(transactions)
        .where(and(eq(transactions.ref, input.transactionRef), eq(transactions.agentId, session.id))).limit(1);
      if (!tx) throw new TRPCError({ code: "NOT_FOUND", message: "Transaction not found or not yours" });

      const [dispute] = await db.insert(disputes).values({
        ref: `DSP-${crypto.randomUUID().slice(0, 8).toUpperCase()}`,
        agentId: session.id, transactionId: tx.id, transactionRef: input.transactionRef,
        reason: input.reason, description: input.description, status: "open",
        evidence: JSON.stringify({ expectedAmount: input.expectedAmount, customerPhone: input.customerPhone, filedFromPOS: true }),
      }).returning();

      await writeAuditLog({
        agentId: session.id, agentCode: session.agentCode,
        action: "POS_DISPUTE_FILED", resource: "dispute", resourceId: String(dispute.id), status: "success",
        metadata: { transactionRef: input.transactionRef, reason: input.reason },
      });

      return { disputeId: dispute.id, transactionRef: input.transactionRef, status: "open", createdAt: new Date().toISOString() };
    }),
  ...
  listMyDisputes: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const items = await db.select().from(disputes)
        .where(and(...conditions)).orderBy(desc(disputes.createdAt)).limit(input.limit);
  ...
  getDisputeStatus: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [dispute] = await db.select().from(disputes)
        .where(and(eq(disputes.id, input.disputeId), eq(disputes.agentId, session.id))).limit(1);
