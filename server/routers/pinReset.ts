...
export const pinResetRouter = router({
  ...
  requestOtp: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (agentRows.length === 0) {
        ...
        return { success: true, message: "If the details match, an OTP has been sent." };
      }
      ...
      await db
        .delete(otpTokens)
        .where(eq(otpTokens.agentId, agent.id));
      ...
      if (!smsResult.success) {
        ...
        const maskedPhone = input.phone.slice(0, 4) + "****" + input.phone.slice(-3);
        console.error(`[pinReset] SMS delivery failed for ${maskedPhone}: ${smsResult.error}`);
      } else {
        const maskedPhone = input.phone.slice(0, 4) + "****" + input.phone.slice(-3);
        console.info(`[pinReset] OTP SMS sent to ${maskedPhone} — messageId: ${smsResult.messageId}`);
      }
  ...
  resetPin: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (agentRows.length === 0) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Invalid OTP or agent code" });
      }
      ...
      return { success: true, message: "PIN updated successfully. Please log in with your new PIN." };
    }),
});
