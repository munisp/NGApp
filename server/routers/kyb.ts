...
const KYB_RISK_ENGINE_URL = process.env.KYB_RISK_ENGINE_URL || "http://localhost:8131";
const KYB_ANALYTICS_URL = process.env.KYB_ANALYTICS_URL || "http://localhost:8132";
...
const businessTypeSchema = z.enum([
  ...
  "sole_proprietorship", "non_profit", "trust",
]);
...
export const kybRouter = router({
  ...
  uploadDocument: protectedProcedure
    .input(z.object({
      ...
      doc_type: z.enum([
        ...
        "bank_statement", "memart", "board_resolution",
        "id_card", "passport", "bvn_verification",
        "scuml_certificate", "cbn_license",
      ]),
  ...
  complianceReport: adminProcedure
    .input(z.object({
      report_type: z.enum(["monthly", "quarterly", "annual"]).default("monthly"),
  ...
  analyticsDashboard: adminProcedure
    .query(async () => {
      const result = await serviceCall(
  ...
  lakehouseETL: adminProcedure
    .input(z.object({
      data_type: z.enum(["kyb_verifications", "compliance_reports", "anomaly_detections"]),
  ...
  healthCheck: adminProcedure
    .query(async () => {
      ...
      return {
        kyb_engine: goHealth ? { status: "healthy", ...goHealth } : { status: "unavailable" },
        kyb_risk_engine: rustHealth ? { status: "healthy", ...rustHealth } : { status: "unavailable" },
        kyb_analytics: pyHealth ? { status: "healthy", ...pyHealth } : { status: "unavailable" },
      };
    }),
});
