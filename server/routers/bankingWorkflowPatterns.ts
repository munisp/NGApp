...
export const bankingWorkflowPatternsRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [totalDefs] = await db.select({ value: count() }).from(workflowDefinitions).limit(100);
    const [totalInstances] = await db.select({ value: count() }).from(workflowInstances).limit(100);
