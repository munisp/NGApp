...
export const gl_journal_entriesRouter = router({
  list: protectedProcedure.input(z.object({ limit: z.number().default(20), offset: z.number().default(0) })).query(async ({ input }) => {
    ...
    const rows = await db.select().from(gl_journal_entries).orderBy(desc(gl_journal_entries.id)).limit(input.limit).offset(input.offset);
    const [{ total }] = await db.select({ total: count() }).from(gl_journal_entries);
    return { items: rows, total };
  }),
  getById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db = (await getDb())!;
    const [row] = await db.select().from(gl_journal_entries).where(eq(gl_journal_entries.id, input.id));
    if (!row) throw new TRPCError({ code: "NOT_FOUND", message: "Journal entry not found" });
    return row;
  }),
  create: protectedProcedure.input(z.object({ debitAccountId: z.number(), creditAccountId: z.number(), amount: z.string(), description: z.string().min(5), reference: z.string().optional() })).mutation(async ({ input }) => {
    const db = (await getDb())!;
  ...
  reverse: protectedProcedure.input(z.object({ id: z.number(), reason: z.string().min(5) })).mutation(async ({ input }) => {
    const db = (await getDb())!;
    const [original] = await db.select().from(gl_journal_entries).where(eq(gl_journal_entries.id, input.id));
