...
export const ransomwareAlertsRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [total] = await db.select({ value: count() }).from(observabilityAlerts).where(eq(observabilityAlerts.severity, "critical"));
    const [firing] = await db.select({ value: count() }).from(observabilityAlerts).where(and(eq(observabilityAlerts.severity, "critical"), eq(observabilityAlerts.status, "firing")));
