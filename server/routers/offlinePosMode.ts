...
export const offlinePosModeRouter = router({
  ...
  getStats: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    if (!db) return { totalSessions: 0, activeSessions: 0, totalOfflineTxns: 0 };
    ...
    const totalStarted = Number((startedRows.rows?.[0] as Record<string, unknown>)?.total ?? 0);
    const totalEnded = Number((endedRows.rows?.[0] as Record<string, unknown>)?.total ?? 0);

    return { totalSessions: totalStarted, activeSessions: Math.max(0, totalStarted - totalEnded), totalOfflineTxns: 0 };
  }),
});
