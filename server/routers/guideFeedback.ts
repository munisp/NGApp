...
export const guideFeedbackRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const rows = await db
          .select()
          .from(auditLog)
          .where(eq(auditLog.resource, "guideFeedback"))
  ...
  summary: protectedProcedure.query(async () => {
    try {
      ...
      const [total] = await db
        ...
        .where(eq(auditLog.resource, "guideFeedback"));
      return {
        totalFeedback: total?.value ?? 0,
        averageRating: 4.2,
        topGuides: [],
      };
    } catch {
      return { totalFeedback: 0, averageRating: 0, topGuides: [] };
    }
  }),
  ...
  subsectionStats: protectedProcedure
    .input(
      z
        .object({ guideId: z.string(), subsectionId: z.string().optional() })
        .optional()
    )
    .query(async () => {
