...
export const observabilityAlertsRouter = router({
  list: protectedProcedure.input(z.object({ status: z.string().optional(), severity: z.string().optional(), service: z.string().optional(), limit: z.number().default(20), offset: z.number().default(0) })).query(async ({ input }) => {
    try {
      ...
      const [{ total }] = await db.select({ total: count() }).from(observabilityAlerts).where(conditions.length ? and(...conditions) : undefined).limit(100);
  ...
  getById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [row] = await db.select().from(observabilityAlerts).where(eq(observabilityAlerts.id, input.id)).limit(100);
  ...
  acknowledge: protectedProcedure.input(z.object({ id: z.number(), acknowledgedBy: z.number() })).mutation(async ({ input }) => {
    try {
      const db = (await getDb())!;
      const [alert] = await db.select().from(observabilityAlerts).where(eq(observabilityAlerts.id, input.id)).limit(100);
  ...
  getSummary: protectedProcedure.query(async () => {
    const db = (await getDb())!;
    const [stats] = await db.select({ total: count(), firing: sql<number>`COUNT(*) FILTER (WHERE status = 'firing')`, acknowledged: sql<number>`COUNT(*) FILTER (WHERE status = 'acknowledged')`, resolved: sql<number>`COUNT(*) FILTER (WHERE status = 'resolved')` }).from(observabilityAlerts).limit(100);
