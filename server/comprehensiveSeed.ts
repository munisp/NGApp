...
export async function seedComprehensiveData(pool: Pool): Promise<{ seeded: Record<string, number> }> {
  ...
  try {
    ...
    // ── 2. Sectors ──────────────────────────────────────────────────────────
    const sectorInsert = `
      INSERT INTO sectors (id, name, regulator, description, risk_level, org_count, avg_compliance_score, created_at, updated_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,NOW(),NOW())
      ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, updated_at=NOW()`;
    const sectorData = [
      [1, "Financial Services", "CBN", "Banking, microfinance, and financial institutions regulated by the Central Bank of Nigeria", "high", 45, 82.5],
      [2, "Telecommunications", "NCC", "Mobile network operators, ISPs, and VAS providers regulated by NCC", "high", 28, 68.0],
      [3, "Healthcare", "NHIA", "Hospitals, HMOs, and health data processors under NHIA oversight", "critical", 120, 55.0],
      [4, "Insurance", "NAICOM", "Insurance companies and brokers regulated by NAICOM", "medium", 62, 71.0],
      [5, "Energy & Utilities", "NERC", "Power generation, distribution, and oil & gas companies", "high", 35, 73.0],
      [6, "Fintech", "CBN/SEC", "Payment processors, lending platforms, and digital banks", "high", 85, 65.0],
      [7, "Education", "NUC/UBEC", "Universities, schools, and EdTech platforms", "medium", 200, 48.0],
      [8, "E-Commerce", "FCCPC", "Online marketplaces and digital commerce platforms", "medium", 150, 52.0],
    ];
