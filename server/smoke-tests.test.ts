...
describe("Dispute Resolution", () => {
  ...
  it("should enforce 48-hour SLA for dispute resolution", () => {
    ...
    const slaDeadline = new Date(disputeCreatedAt.getTime() + SLA_HOURS * 60 * 60 * 1000);
    expect(slaDeadline.toISOString()).toBe("2026-04-22T10:00:00.000Z");
  });

  it("should support all dispute statuses", () => {
    const validStatuses = ["open", "under_review", "escalated", "resolved", "closed", "rejected"];
...
describe("Commission Engine", () => {
  ...
  it("should define 9 commission tiers", () => {
    const TIERS = [
      { name: "Cash-In Basic", type: "cash_in", rate: 0.50, minVolume: 0, maxVolume: 100000 },
      { name: "Cash-In Silver", type: "cash_in", rate: 0.60, minVolume: 100001, maxVolume: 500000 },
      { name: "Cash-In Gold", type: "cash_in", rate: 0.75, minVolume: 500001, maxVolume: 2000000 },
      { name: "Cash-In Platinum", type: "cash_in", rate: 0.90, minVolume: 2000001, maxVolume: 999999999 },
      { name: "Cash-Out Basic", type: "cash_out", rate: 0.80, minVolume: 0, maxVolume: 100000 },
      { name: "Cash-Out Premium", type: "cash_out", rate: 1.00, minVolume: 100001, maxVolume: 999999999 },
      { name: "Transfer Basic", type: "transfer", rate: 0.30, minVolume: 0, maxVolume: 999999999 },
      { name: "Bill Payment", type: "bill_payment", rate: 0.20, minVolume: 0, maxVolume: 999999999 },
      { name: "Airtime", type: "airtime", rate: 3.00, minVolume: 0, maxVolume: 999999999 },
    ];
    expect(TIERS.length).toBe(9);
    expect(TIERS[0].name).toBe("Cash-In Basic");
    expect(TIERS[8].name).toBe("Airtime");
    expect(TIERS[8].rate).toBe(3.00);
  });
  ...
  it("should calculate cascade split amounts", () => {
    ...
    const split = { superAgent: 7, masterAgent: 12, agent: 63, subAgent: 13, platform: 5 };

    const superAgentAmount = Math.round(totalCommission * split.superAgent / 100);
    const masterAgentAmount = Math.round(totalCommission * split.masterAgent / 100);
    ...
    expect(superAgentAmount + masterAgentAmount + agentAmount + subAgentAmount + platformAmount).toBe(10000);
  });

  it("should define tier upgrade criteria", () => {
    const criteria = {
      Silver: { minMonthlyVolume: 500000, minTransactionCount: 100, minLoyaltyPoints: 500, minStreakDays: 30 },
      Gold: { minMonthlyVolume: 2000000, minTransactionCount: 500, minLoyaltyPoints: 2000, minStreakDays: 60 },
      Platinum: { minMonthlyVolume: 5000000, minTransactionCount: 1000, minLoyaltyPoints: 5000, minStreakDays: 90 },
    };
