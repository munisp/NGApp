...
describe('PostgreSQL Performance Configuration', () => {
  describe('Production Configuration', () => {
    ...
    it('enables pg_stat_statements and auto_explain', () => {
      const content = fs.readFileSync(confPath, 'utf-8');
      expect(content).toMatch(/shared_preload_libraries\s*=\s*'pg_stat_statements,auto_explain'/);
    });
  ...
  describe('Performance Indexes', () => {
    ...
    it('uses CONCURRENTLY for all index creation', () => {
      const content = fs.readFileSync(indexPath, 'utf-8');
      const createIndexLines = content.split('\n').filter(l => l.includes('CREATE INDEX'));
    ...
    it('covers all high-volume tables with indexes', () => {
      ...
      const criticalTables = [
        'transactions', 'fraud_alerts', 'audit_log', 'agents',
        'loyalty_history', 'chat_sessions', 'chat_messages',
        'devices', 'kyc_sessions', 'settlement_reconciliation',
        'commission_payouts', 'webhook_deliveries', 'referrals',
        'disputes', 'reversal_requests', 'float_topup_requests',
        'email_queue', 'erp_sync_log', 'dlq_messages',
        'api_key_usage', 'device_locations', 'customers',
        'merchants', 'analytics_metrics', 'connectivity_log',
      ];
  ...
  describe('Maintenance Scripts', () => {
    ...
    it('includes materialized view refresh commands', () => {
      ...
      expect(content).toMatch(/REFRESH MATERIALIZED VIEW CONCURRENTLY mv_daily_agent_summary/);
      expect(content).toMatch(/REFRESH MATERIALIZED VIEW CONCURRENTLY mv_hourly_platform_kpis/);
      expect(content).toMatch(/REFRESH MATERIALIZED VIEW CONCURRENTLY mv_agent_leaderboard/);
    });
  ...
  describe('Schema Index Coverage', () => {
    ...
    it('all tenant-isolated tables have tenantId index', () => {
      const tenantTables = ['users', 'agents', 'transactions', 'fraud_alerts', 'disputes', 'kyc_sessions', 'pos_terminals', 'merchants', 'customers'];
      for (const table of tenantTables) {
        expect(content).toMatch(new RegExp(`${table.replace(/_/g, '.')}.*tenantId.*idx`, 's'));
      }
    });
  });
});
