...
export function getErrorStats() {
  const now = Date.now();
  const last5min = recentErrors.filter(e => now - new Date(e.timestamp).getTime() < 5 * 60 * 1000);
  const last1hr = recentErrors.filter(e => now - new Date(e.timestamp).getTime() < 60 * 60 * 1000);
  return {
    total: recentErrors.length,
    last5Minutes: last5min.length,
    lastHour: last1hr.length,
    topPaths: Object.entries(
      last1hr.reduce((acc, e) => { acc[e.path] = (acc[e.path] || 0) + 1; return acc; }, {} as Record<string, number>)
