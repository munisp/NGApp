...
export async function publishSettlementEvent(params: {
  eventType: "settlement.batch.started" | "settlement.batch.completed" | "settlement.batch.failed"
    | "settlement.agent.credited" | "settlement.reconciliation.started" | "settlement.reconciliation.completed"
    | "settlement.netting.calculated" | "settlement.disbursement.initiated";
  batchId?: string;
  agentId?: number;
  agentCode?: string;
  amount?: number;
  ...
  try {
    ...
    logger.warn(`[Kafka] Settlement event failed (fail-open): ${(e as Error).message}`);
  }
}
...
export async function acquireSettlementLock(batchId: string): Promise<boolean> {
  try {
    const result = await cacheIncr(`settlement:lock:${batchId}`, SETTLEMENT_LOCK_TTL);
    return result === 1;
  } catch { return true; }
}

export async function releaseSettlementLock(batchId: string): Promise<void> {
  try { await cacheDel(`settlement:lock:${batchId}`); } catch { /* ignore */ }
}

export async function cacheSettlementBatchStatus(batchId: string, status: Record<string, unknown>): Promise<void> {
  try { await cacheSet(`settlement:batch:${batchId}`, JSON.stringify(status), 3600); } catch { /* ignore */ }
}

export async function getCachedSettlementBatchStatus(batchId: string): Promise<Record<string, unknown> | null> {
  try {
...
export async function tbRecordSettlementTransfer(params: {
  ...
  try {
    ...
    logger.warn(`[TB-Settlement] Transfer failed (fail-open): ${(e as Error).message}`);
    return null;
  }
}

// ── Temporal: Settlement Workflow Status ──────────────────────────────────
export async function getSettlementWorkflowStatus(batchId: string): Promise<{ status: string; agentsProcessed: number; totalAmount: number } | null> {
  try {
...
export async function canTriggerSettlement(agentCode: string, agentRole: string): Promise<boolean> {
  try {
...
export async function canApproveSettlement(agentCode: string, agentRole: string): Promise<boolean> {
  try {
    return await permifyCheck({
      subjectType: "agent",
      subjectId: agentCode,
...
export async function streamSettlementEvent(params: {
  ...
  try {
    ...
    logger.debug(`[Fluvio] Settlement stream failed (fail-open): ${(e as Error).message}`);
  }
}
...
export async function triggerSettlementSnapshot(date?: string): Promise<boolean> {
  try {
...
export async function daprPublishSettlementNotification(params: {
  ...
  try {
    const res = await fetch(`http://localhost:${DAPR_HTTP_PORT}/v1.0/publish/settlement-pubsub/settlement-notifications`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...params,
...
export async function validateKeycloakTokenForSettlement(token: string): Promise<boolean> {
  try {
...
export function getSettlementRateLimitConfig() {
  return {
    ...
    plugins: {
      "limit-count": {
        count: 50, time_window: 60, key_type: "var", key: "remote_addr",
        rejected_code: 429, rejected_msg: "Settlement API rate limit exceeded",
        policy: "redis",
...
export async function initiateIlpSettlementTransfer(params: {
  ...
  try {
    const res = await fetch(`${MOJALOOP_SIDECAR_URL}/mojaloop/settlement-transfer`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
export async function getSettlementMiddlewareHealth(): Promise<Record<string, { status: string; latencyMs: number }>> {
  ...
  const check = async (name: string, fn: () => Promise<boolean>) => {
    ...
    } catch { results[name] = { status: "unavailable", latencyMs: Date.now() - start }; }
  };
