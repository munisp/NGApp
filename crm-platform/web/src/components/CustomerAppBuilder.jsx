...
import { Layout, BarChart3, Users, TrendingUp, Clock, Search, Filter, CheckCircle, AlertTriangle, Settings, ArrowRight } from 'lucide-react'
...
export default function CustomerAppBuilder() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Layout className="w-7 h-7 text-white p-1 rounded-lg bg-fuchsia-600" /> Let customers build custom mini-apps inside the CRM</h1></div>
      <div className="grid grid-cols-4 gap-3">{[{ l: "Active Apps", v: "1,240" }, { l: "Adoption", v: "90.8%" }, { l: "D30 Retention", v: "89%" }, { l: "Avg Engagement", v: "12 min/day" }, ].flat().map(s => (<div key={s.l} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3"><p className="text-xs text-gray-500">{s.l}</p><p className="text-xl font-bold text-gray-900 dark:text-white">{s.v}</p></div>))}</div>
