...
import { Users, BarChart3, TrendingUp, Clock, Search, Filter, CheckCircle, AlertTriangle, Settings, ArrowRight } from 'lucide-react'
...
export default function CohortStudio() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Users className="w-7 h-7 text-white p-1 rounded-lg bg-blue-600" /> Dynamic customer segmentation and cohort analysis</h1></div>
      <div className="grid grid-cols-4 gap-3">{[{ l: "Active Segments", v: "24" }, { l: "Largest Cohort", v: "12,400" }, { l: "Retention Rate", v: "78%" }, { l: "LTV Growth", v: "+12%" }, ].flat().map(s => (<div key={s.l} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3"><p className="text-xs text-gray-500">{s.l}</p><p className="text-xl font-bold text-gray-900 dark:text-white">{s.v}</p></div>))}</div>
