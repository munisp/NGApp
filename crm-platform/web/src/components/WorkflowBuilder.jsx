...
import { Workflow, BarChart3, Users, TrendingUp, Clock, Search, Filter, CheckCircle, AlertTriangle, Settings, ArrowRight } from 'lucide-react'
...
export default function WorkflowBuilder() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Workflow className="w-7 h-7 text-white p-1 rounded-lg bg-cyan-600" /> Visual drag-and-drop workflow automation builder</h1></div>
      <div className="grid grid-cols-4 gap-3">{[{ l: "Active Workflows", v: "142" }, { l: "Executions/Day", v: "3,840" }, { l: "Avg Time Saved", v: "4.2 hrs" }, { l: "Success Rate", v: "97.8%" }, ].flat().map(s => (<div key={s.l} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3"><p className="text-xs text-gray-500">{s.l}</p><p className="text-xl font-bold text-gray-900 dark:text-white">{s.v}</p></div>))}</div>
