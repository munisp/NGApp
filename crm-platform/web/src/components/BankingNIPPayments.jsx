...
const BankingNIPPayments = () => {
  const { tenant } = useTenant()
  const { t } = useTranslation()

  return (
    <div role="region" aria-label="BankingNIPPayments">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">NIP 3.0 Payments</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">NIBSS instant payment initiation and tracking</p>
        </div>
        <FallbackBadge />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-gray-100">NIP 3.0 Payments</p>
        </motion.div>
      </div>
...
export default BankingNIPPayments
