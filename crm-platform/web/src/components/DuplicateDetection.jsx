...
import { Copy, BarChart3, Users, TrendingUp, Clock, Search, Filter, CheckCircle, AlertTriangle, Settings, ArrowRight } from 'lucide-react'
...
export default function DuplicateDetection() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Copy className="w-7 h-7 text-white p-1 rounded-lg bg-rose-600" /> ML-powered duplicate detection and merge</h1></div>
      <div className="grid grid-cols-4 gap-3">{[{ l: "Duplicates Found", v: "2,840" }, { l: "Auto-Merged", v: "1,920" }, { l: "Confidence", v: "96.8%" }, { l: "Records Saved", v: "₦2.1M" }, ].flat().map(s => (<div key={s.l} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3"><p className="text-xs text-gray-500">{s.l}</p><p className="text-xl font-bold text-gray-900 dark:text-white">{s.v}</p></div>))}</div>
