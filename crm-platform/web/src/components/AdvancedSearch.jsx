    ...
    <div role="region" aria-label="AdvancedSearch"  className="p-6 max-w-full">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Multi-field search across customers, transactions, and agents</p>
        </div>
      </div>
      ...
      {activeTab === 'search' && (
        <>
          ...
          <div className="flex gap-3 mb-4">
            ...
            <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-1 px-4 py-2.5 border rounded-lg text-sm hover:bg-gray-50">
          ...
          {showFilters && (
            <div className="bg-white rounded-xl border p-4 mb-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-medium text-gray-700">Filter Fields</h3>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {SEARCH_FIELDS.map(field => (
                  <button key={field.id} onClick={() => addFilter(field)}
                    className={`px-2 py-1 rounded text-xs border transition ${activeFilters.find(f => f.id === field.id) ? 'bg-indigo-100 border-indigo-300 text-indigo-700' : 'hover:bg-gray-50'}`}>
                    {field.label}
                  </button>
                ))}
              </div>
                ...
                <div className="space-y-2 pt-3 border-t">
                  {activeFilters.map(filter => (
                    <div key={filter.id} className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 w-28">{filter.label}</span>
                      ...
                      <button onClick={() => removeFilter(filter.id)} className="p-1 hover:bg-gray-100 rounded"><X className="w-3 h-3 text-gray-400" /></button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs text-gray-500">KYC</th>
                    <th className="px-4 py-2 text-left text-xs text-gray-500">Products</th>
                    <th className="px-4 py-2 text-left text-xs text-gray-500">Risk</th>
                    <th className="px-4 py-2 text-left text-xs text-gray-500">Region</th>
                  </tr>
                </thead>
                <tbody>
                  {results.map(r => (
                    <tr key={r.id} className="border-b hover:bg-gray-50 cursor-pointer">
                      ...
                      <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs ${r.status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}>{r.status}</span></td>
      ...
      {activeTab === 'saved' && (
        <div className="bg-white rounded-xl border">
          {SAVED_SEARCHES.map(search => (
            <div key={search.id} className="flex items-center justify-between px-4 py-3 border-b last:border-0 hover:bg-gray-50 cursor-pointer">
              ...
              <div className="text-right">
                <p className="text-sm text-gray-600">{search.results.toLocaleString()} results</p>
