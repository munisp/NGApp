...
export const SUPPORTED_CURRENCIES      = ["NGN", "USD", "GBP", "EUR", "GHS", "KES", "ZAR", "XOF"] as const;
...
export const BILL_CATEGORIES = ["electricity", "water", "cable_tv", "internet", "insurance", "government"] as const;
export type BillCategory = typeof BILL_CATEGORIES[number];

export const ELECTRICITY_DISCOS = [
  "AEDC", "BEDC", "EEDC", "EKEDC", "IBEDC", "IKEDC", "JED", "KAEDCO", "PHED"
] as const;
...
// ── Terminal hardware ─────────────────────────────────────────────────────────
export const SUPPORTED_TERMINAL_MODELS = ["PAX A920", "PAX A920 Pro", "Aisino A75", "Verifone P400", "Ingenico Move 5000"] as const;
...
export const NG_PHONE_PREFIXES: Record<string, string[]> = {
  ...
  Airtel:    ["0701","0708","0802","0808","0812","0902","0907","0901","0904","0912"],
...
export function validateBvn(bvn: string): { valid: boolean; reason?: string } {
  ...
  if (!bvn.startsWith("2"))   return { valid: false, reason: "BVN must start with digit 2" };
  return { valid: true };
}
...
export function validateNgPhone(phone: string): { valid: boolean; reason?: string; carrier?: string } {
  const normalised = phone.replace(/^\+234/, "0").replace(/\s/g, "");
  if (!/^0[789][01]\d{8}$/.test(normalised)) {
    return { valid: false, reason: "Invalid Nigerian phone number (e.g. 08012345678)" };
  }
...
export function validateNuban(account: string): { valid: boolean; reason?: string } {
  if (!/^\d{10}$/.test(account)) return { valid: false, reason: "Account number must be exactly 10 digits (NUBAN)" };
  return { valid: true };
}
...
export function validateCacNumber(cac: string): { valid: boolean; type?: string; reason?: string } {
  if (REGEX_CAC_RC.test(cac)) return { valid: true, type: "Registered Company" };
  if (REGEX_CAC_BN.test(cac)) return { valid: true, type: "Business Name" };
  if (REGEX_CAC_IT.test(cac)) return { valid: true, type: "Incorporated Trustee" };
  return { valid: false, reason: "Invalid CAC number (e.g. RC123456, BN1234567, IT/12345)" };
}
...
export function validateTin(tin: string): { valid: boolean; reason?: string } {
  if (!REGEX_TIN.test(tin)) return { valid: false, reason: "TIN must be 8 or 10 digits" };
  return { valid: true };
}
