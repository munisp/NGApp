...
const (
	ConsumerGroupID = "claims-service-group"
)
...
func main() {
	...
	log := logrus.WithField("component", "ClaimsConsumer")
	log.Infof("Starting Claims Service Kafka Consumer. Temporal Host: %s", cfg.TemporalHostPort)

	// Initialize Metrics
	metrics := metrics.NewMetrics("claims_consumer")
	metrics.StartMetricsServer(":9092", log.WithField("service", "metrics"))
	...
	topics := []string{
		cfg.KafkaTopicPrefix + string(events.ReserveAdjustment),
	...
	log.Info("Claims Service Kafka Consumer stopped.")
}
