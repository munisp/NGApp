...
const (
	ConsumerGroupID = "policy-service-group"
)
...
func main() {
	...
	log := logrus.WithField("component", "PolicyConsumer")
	log.Infof("Starting Policy Service Kafka Consumer. Temporal Host: %s", cfg.TemporalHostPort)

	// Initialize Metrics
	metrics := metrics.NewMetrics("policy_consumer")
	metrics.StartMetricsServer(":9091", log.WithField("service", "metrics"))
	...
	topics := []string{
		cfg.KafkaTopicPrefix + string(events.PremiumAdjustment),
	...
	log.Info("Policy Service Kafka Consumer stopped.")
}
