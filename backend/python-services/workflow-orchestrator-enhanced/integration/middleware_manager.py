...
if __name__ == "__main__":
    ...
    config = MiddlewareConfig(
        ...
        permify=PermifyConfig(grpc_addr="localhost:3476", tenant_id="default"),
        redis=RedisConfig(),
        tigerbeetle=TigerBeetleConfig(cluster_id=1, addresses=["localhost:3000"]),
        lakehouse=LakehouseConfig(api_url="http://localhost:8000", s3_bucket="workflows", api_key="key"),
        apisix=APISIXConfig(admin_url="http://localhost:9180", gateway_url="http://localhost:9080", api_key="key"),
        postgres=PostgreSQLConfig(host=os.getenv('DB_HOST', 'localhost'), port=5432, database="workflows", user=os.getenv('DB_USER', 'postgres'), password=os.getenv('DB_PASSWORD', '')),
    )
