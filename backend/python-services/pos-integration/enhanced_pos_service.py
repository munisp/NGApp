    ...
    async def _fetch_exchange_rates(self):
        ...
        if base_rates is None:
            base_rates = {
                ...
                "INR": 74.5, "BRL": 5.2, "NGN": 770.0, "KES": 130.0,
                "GHS": 12.5, "ZAR": 18.5, "XOF": 600.0,
            }
    ...
    async def _calculate_location_score(self, transaction: POSTransaction) -> float:
        ...
        try:
            ...
            terminal_location = db.execute(
                "SELECT latitude, longitude, business_radius FROM pos_terminals WHERE terminal_id = :tid",
                {"tid": transaction.terminal_id}
            ).fetchone()

            transaction_location = db.execute(
                "SELECT latitude, longitude FROM pos_transactions WHERE transaction_id = :txid",
            ...
            term_lat, term_lon, biz_radius = terminal_location
            ...
            allowed_radius = biz_radius if biz_radius and biz_radius > 0 else 500.0
