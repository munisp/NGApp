...
export const agentTierEnum = pgEnum("agent_tier", ["Bronze", "Silver", "Gold", "Platinum"]);
export const txTypeEnum = pgEnum("tx_type", [
  "Cash In", "Cash Out", "Transfer", "Card Payment",
  "QR Payment", "NFC Payment", "Airtime", "Bill Payment",
  "Reversal", "Nano Loan", "Insurance",
]);
export const txChannelEnum = pgEnum("tx_channel", ["Cash", "Card", "USSD", "QR", "NFC", "App"]);
export const txStatusEnum = pgEnum("tx_status", ["success", "pending", "failed", "reversed", "pending_reversal_approval"]);
export const fraudSeverityEnum = pgEnum("fraud_severity", ["critical", "high", "medium", "low"]);
export const fraudStatusEnum = pgEnum("fraud_status", ["open", "investigating", "escalated", "dismissed", "resolved"]);
export const loyaltyTypeEnum = pgEnum("loyalty_type", ["earned", "redeemed", "bonus", "penalty", "challenge"]);
export const chatStatusEnum = pgEnum("chat_status", ["open", "assigned", "resolved", "escalated"]);
export const senderTypeEnum = pgEnum("sender_type", ["agent", "support", "system"]);
...
export const commissionRuleTypeEnum = pgEnum("commission_rule_type", ["percentage", "flat", "tiered"]);
export const qrCodeTypeEnum = pgEnum("qr_code_type", ["payment", "profile", "collection", "agent_id", "product", "event", "loyalty"]);  // expanded for router compatibility
export const qrCodeStatusEnum = pgEnum("qr_code_status", ["active", "used", "expired", "revoked"]);
export const inventoryStatusEnum = pgEnum("inventory_status", ["in_stock", "low_stock", "out_of_stock", "discontinued"]);
export const simStatusEnum = pgEnum("sim_status", ["active", "inactive", "suspended", "standby", "failed", "disabled"]);  // expanded
export const reversalStatusEnum = pgEnum("reversal_status", ["pending", "approved", "rejected", "processed", "completed", "failed"]);  // expanded
export const linkTypeEnum = pgEnum("link_type", ["payment", "collection", "profile", "invoice", "subscription", "donation"]);  // expanded
export const linkStatusEnum = pgEnum("link_status", ["active", "expired", "paused", "deleted", "used", "revoked"]);  // expanded
export const customerStatusEnum = pgEnum("customer_status", ["pending_kyc", "active", "suspended", "blacklisted"]);
export const tenantStatusEnum = pgEnum("tenant_status", ["trial", "active", "suspended", "churned"]);
export const erpSyncStatusEnum = pgEnum("erp_sync_status", ["pending", "synced", "failed", "skipped"]);
export const adStatusEnum = pgEnum("ad_status", ["draft", "active", "paused", "completed", "expired", "rejected"]);  // expanded
export const vatRateTypeEnum = pgEnum("vat_rate_type", ["standard", "zero", "exempt"]);
...
export const merchantCategoryEnum = pgEnum("merchant_category", [
  "retail", "food_beverage", "health", "education", "transport", "utilities", "government", "other"
]);
// P3-C: Developer API
...
// P3-B: Credit scoring
export const creditRatingEnum = pgEnum("credit_rating", ["AAA", "AA", "A", "BBB", "BB", "B", "CCC", "D", "N/A"]);
export const creditApplicationStatusEnum = pgEnum("credit_application_status", ["pending", "approved", "rejected", "disbursed", "repaid", "defaulted"]);
...
export const agents = pgTable("agents", {
  ...
  terminalModel: varchar("terminalModel", { length: 64 }).default("PAX A920 MAX"),
  ...
  floatBalance: numeric("floatBalance", { precision: 15, scale: 2 }).default("0.00").notNull(),
  floatLimit: numeric("floatLimit", { precision: 15, scale: 2 }).default("1000000.00").notNull(),
  commissionBalance: numeric("commissionBalance", { precision: 15, scale: 2 }).default("0.00").notNull(),
  ...
  creditLimit: numeric("creditLimit", { precision: 15, scale: 2 }).default("0.00"),
  ...
  commissionSplitOverride: numeric("commissionSplitOverride", { precision: 5, scale: 2 }),  // override default split %
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
}, (t) => ({
...
export const transactions = pgTable("transactions", {
  ...
  fraudScore: numeric("fraudScore", { precision: 5, scale: 2 }).default("0.00"),
  ...
  agentIdCreatedAtIdx: index("tx_agentId_createdAt_idx").on(t.agentId, t.createdAt),
  statusCreatedAtIdx: index("tx_status_createdAt_idx").on(t.status, t.createdAt),
  refIdx: uniqueIndex("tx_ref_idx").on(t.ref),
  idempotencyKeyIdx: uniqueIndex("tx_idempotencyKey_idx").on(t.idempotencyKey),
...
export const fraudAlerts = pgTable("fraud_alerts", {
  ...
  statusCreatedAtIdx: index("fraud_status_createdAt_idx").on(t.status, t.createdAt),
...
export const auditLog = pgTable("audit_log", {
  ...
  agentIdCreatedAtIdx: index("audit_agentId_createdAt_idx").on(t.agentId, t.createdAt),
...
export const floatTopUpRequests = pgTable("float_topup_requests", {
  id: serial("id").primaryKey(),
  agentId: integer("agentId").notNull(),
  requestedAmount: numeric("requestedAmount", { precision: 15, scale: 2 }).notNull(),
  status: topupStatusEnum("status").default("pending").notNull(),
  approvedBy: varchar("approvedBy", { length: 64 }),
  notes: text("notes"),
  supervisorApprovalRequired: boolean("supervisorApprovalRequired").default(false).notNull(),
...
export const devices = pgTable("devices", {
  ...
  // ── Compliance ─────────────────────────────────────────────────────────────────────
  complianceStatus: varchar("complianceStatus", { length: 32 }).default("unknown"),
...
export const deviceCommands = pgTable("device_commands", {
  ...
  deviceIdStatusIdx: index("cmd_deviceId_status_idx").on(t.deviceId, t.status),
}));
...
export const disputes = pgTable("disputes", {
  ...
  agentIdStatusIdx: index("dispute_agentId_status_idx").on(t.agentId, t.status),
...
export const refunds = pgTable("refunds", {
  ...
  status: varchar("status", { length: 32 }).default("pending").notNull(),
  method: varchar("method", { length: 32 }).default("original_method").notNull(),
...
export const velocityLimits = pgTable("velocity_limits", {
  ...
  maxSingleTxAmount: numeric("maxSingleTxAmount", { precision: 15, scale: 2 }).default("50000.00").notNull(),
  maxDailyVolume: numeric("maxDailyVolume", { precision: 15, scale: 2 }).default("500000.00").notNull(),
  // Canonical names
  dailyTxLimit: numeric("dailyTxLimit", { precision: 15, scale: 2 }).default("500000.00").notNull(),
  singleTxLimit: numeric("singleTxLimit", { precision: 15, scale: 2 }).default("100000.00").notNull(),
...
export const complianceReports = pgTable("compliance_reports", {
  ...
  tenantIdPeriodIdx: index("compliance_tenantId_period_idx").on(t.tenantId, t.period),
}));
...
export const deviceLocations = pgTable("device_locations", {
  ...
  deviceIdCreatedAtIdx: index("dloc_deviceId_createdAt_idx").on(t.deviceId, t.createdAt),
}));
...
export const kycSessions = pgTable("kyc_sessions", {
  ...
  agentId: integer("agentId"),
  customerId: integer("customerId"),
  sessionRef: varchar("sessionRef", { length: 64 }).notNull().unique().default(sql`gen_random_uuid()`),
...
export const serviceRecords = pgTable("service_records", {
  id: serial("id").primaryKey(),
  terminalId: integer("terminalId").references(() => posTerminals.id).notNull(),
...
export const commissionRules = pgTable("commission_rules", {
  ...
  ruleType: commissionRuleTypeEnum("ruleType").default("percentage").notNull(),
...
export const multiSimProfiles = pgTable("multi_sim_profiles", {
  ...
  dataUsageMb: numeric("dataUsageMb", { precision: 12, scale: 2 }).default("0"),
...
export const reversalRequests = pgTable("reversal_requests", {
  ...
  agentId: integer("agentId").references(() => agents.id).notNull(),
  reason: text("reason").notNull(),
  amount: numeric("amount", { precision: 15, scale: 2 }).notNull(),
  currency: varchar("currency", { length: 3 }).default("NGN").notNull(),
  ...
  agentIdStatusIdx: index("reversal_agentId_status_idx").on(t.agentId, t.status),
}));
...
export const customers = pgTable("customers", {
  ...
  walletBalance: numeric("walletBalance", { precision: 15, scale: 2 }).default("0.00").notNull(),
  dailyLimit: numeric("dailyLimit", { precision: 15, scale: 2 }).default("50000.00").notNull(),
  monthlyLimit: numeric("monthlyLimit", { precision: 15, scale: 2 }).default("300000.00").notNull(),
...
export const tenants = pgTable("tenants", {
  ...
  monthlyVolume: numeric("monthlyVolume", { precision: 20, scale: 2 }).default("0.00").notNull(),
...
export const erpSyncLog = pgTable("erp_sync_log", {
  ...
  statusNextRetryIdx: index("erp_status_nextRetry_idx").on(t.status, t.nextRetryAt),
...
export const storefrontAds = pgTable("storefront_ads", {
  ...
  spent: numeric("spent", { precision: 12, scale: 2 }).default("0.00").notNull(),
...
export const vatRecords = pgTable("vat_records", {
  ...
  vatRate: numeric("vatRate", { precision: 5, scale: 4 }).default("0.075").notNull(),
...
export const mqttBridgeConfig = pgTable("mqtt_bridge_config", {
  ...
  brokerUrl: text("brokerUrl").notNull().default("mqtt://broker.54link.io:1883"),
  port: integer("port").default(1883).notNull(),
  useTls: boolean("useTls").default(false).notNull(),
  username: varchar("username", { length: 128 }).default(""),
  password: text("password").default(""),
  clientId: varchar("clientId", { length: 128 }).default("54link-fluvio-bridge"),
  topicMappings: json("topicMappings").$type<Array<{
    mqttTopic: string;
    fluvioTopic: string;
    transform?: string;
  }>>().default([]),
...
export const analyticsMetrics = pgTable("analytics_metrics", {
  ...
  metricNameBucketIdx: index("analytics_metricName_bucket_idx").on(t.metricName, t.bucketMinute),
}));
...
export const emailQueue = pgTable("email_queue", {
  ...
  templateData: json("templateData").$type<Record<string, unknown>>().default({}),
  ...
  statusCreatedAtIdx: index("email_status_createdAt_idx").on(t.status, t.createdAt),
}));
...
export const merchants = pgTable("merchants", {
  ...
  walletBalance: numeric("walletBalance", { precision: 15, scale: 2 }).default("0.00").notNull(),
  totalVolume: numeric("totalVolume", { precision: 20, scale: 2 }).default("0.00").notNull(),
  ...
  merchantCodeIdx: uniqueIndex("merchants_merchantCode_idx").on(t.merchantCode),
...
export const merchantSettlements = pgTable("merchant_settlements", {
  id: serial("id").primaryKey(),
  merchantId: integer("merchantId").references(() => merchants.id).notNull(),
  period: varchar("period", { length: 10 }).notNull(), // YYYY-MM-DD
  grossAmount: numeric("grossAmount", { precision: 15, scale: 2 }).notNull(),
  feeAmount: numeric("feeAmount", { precision: 15, scale: 2 }).default("0.00").notNull(),
  ...
  merchantIdPeriodIdx: index("ms_merchantId_period_idx").on(t.merchantId, t.period),
}));
...
export const apiKeyUsage = pgTable("api_key_usage", {
  ...
  apiKeyIdCreatedAtIdx: index("apiusage_apiKeyId_createdAt_idx").on(t.apiKeyId, t.createdAt),
}));
...
export const creditScoreHistory = pgTable("credit_score_history", {
  ...
  agentIdComputedAtIdx: index("credit_agentId_computedAt_idx").on(t.agentId, t.computedAt),
}));
...
export const creditApplications = pgTable("credit_applications", {
  ...
  interestRate: numeric("interestRate", { precision: 5, scale: 4 }).default("0.05"),
  ...
  agentIdStatusIdx: index("credit_app_agentId_status_idx").on(t.agentId, t.status),
}));
...
export const otaUpdateLog = pgTable("ota_update_log", {
  id: serial("id").primaryKey(),
  deviceId: integer("deviceId").references(() => devices.id).notNull(),
  releaseId: integer("releaseId").references(() => otaReleases.id).notNull(),
...
export const dataRightsRequests = pgTable("data_rights_requests", {
  ...
  statusCreatedAtIdx: index("ddr_status_createdAt_idx").on(t.status, t.createdAt),
}));
...
export const fraudRules = pgTable("fraud_rules", {
  ...
  description: text("description"),
  threshold: numeric("threshold", { precision: 5, scale: 4 }).default("0.7000").notNull(),
  ...
  categoryEnabledIdx: index("fraud_rules_category_enabled_idx").on(t.category, t.enabled),
}));
...
export const agentPushSubscriptions = pgTable("agent_push_subscriptions", {
  ...
  agentCodeIdx: index("agent_push_subscriptions_agent_code_idx").on(t.agentCode),
}));
export type AgentPushSubscription = typeof agentPushSubscriptions.$inferSelect;
export type InsertAgentPushSubscription = typeof agentPushSubscriptions.$inferInsert;
...
export const connectivityLog = pgTable("connectivity_log", {
  ...
  agentRecordedIdx: index("connectivity_log_agent_recorded_idx").on(t.agentCode, t.recordedAt),
}));
...
export const simProbeLog = pgTable("sim_probe_log", {
  ...
  agentProbedIdx:   index("sim_probe_log_agent_probed_idx").on(t.agentCode, t.probedAt),
  slotProbedIdx:    index("sim_probe_log_slot_probed_idx").on(t.slot, t.probedAt),
}));
...
export const simOrchestratorConfig = pgTable("sim_orchestrator_config", {
  ...
  relayEndpoint:   varchar("relayEndpoint", { length: 256 }).notNull()
  ...
  terminalIdx: uniqueIndex("sim_orchestrator_config_terminal_idx").on(t.terminalId),
}));
export type SimOrchestratorConfig = typeof simOrchestratorConfig.$inferSelect;
export type InsertSimOrchestratorConfig = typeof simOrchestratorConfig.$inferInsert;
...
export const simFailoverLog = pgTable("sim_failover_log", {
  ...
  terminalSwitchedIdx: index("sim_failover_log_terminal_switched_idx").on(t.terminalId, t.switchedAt),
  agentSwitchedIdx:    index("sim_failover_log_agent_switched_idx").on(t.agentCode, t.switchedAt),
}));
...
export const deviceCompliancePolicies = pgTable("device_compliance_policies", {
  ...
  enabled: boolean("enabled").default(true).notNull(),
  enforcementAction: varchar("enforcementAction", { length: 32 }).default("notify"), // notify|restrict|wipe
  createdBy: varchar("createdBy", { length: 64 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
}, (t) => ({
...
export const dlqMessages = pgTable("dlq_messages", {
  ...
  status:       varchar("status", { length: 32 }).notNull().default("pending_retry"),
  resolvedAt:   timestamp("resolvedAt"),
  createdAt:    timestamp("createdAt").defaultNow().notNull(),
}, (t) => ({
...
export const referrals = pgTable("referrals", {
  id: serial("id").primaryKey(),
  referrerAgentId: integer("referrer_agent_id").notNull().references(() => agents.id),
  ...
  bonusCash: numeric("bonus_cash", { precision: 10, scale: 2 }).default("0").notNull(),
...
export const onboardingStepEnum = pgEnum("onboarding_step", [
  "profile", "kyc", "float", "terminal", "training", "activated",
]);
...
export const agentOnboardingProgress = pgTable("agent_onboarding_progress", {
  ...
  agentCode: varchar("agent_code", { length: 32 }).notNull(),
  currentStep: onboardingStepEnum("current_step").default("profile").notNull(),
...
export const settlementReconciliation = pgTable("settlement_reconciliation", {
  ...
  discrepancy: numeric("discrepancy", { precision: 18, scale: 2 }).default("0").notNull(),
...
// ═══════════════════════════════════════════════════════════════════════════════
export const rateAlertDirectionEnum = pgEnum("rate_alert_direction", ["above", "below"]);
export const rateAlertStatusEnum = pgEnum("rate_alert_status", ["active", "paused", "triggered", "expired"]);
...
export const rateAlerts = pgTable("rate_alerts", {
  ...
  agentStatusIdx: index("rate_alert_agent_status_idx").on(t.agentId, t.status),
...
// ═══════════════════════════════════════════════════════════════════════════════
export const emailProviderEnum = pgEnum("email_provider", ["sendgrid", "ses", "smtp", "console"]);
...
export const emailDeliveryLog = pgTable("email_delivery_log", {
  ...
  providerIdx: index("email_delivery_provider_idx").on(t.provider, t.createdAt),
...
export const inviteCodeTypeEnum = pgEnum("invite_code_type", ["one_time", "multi_use"]);
export const inviteCodeStatusEnum = pgEnum("invite_code_status", ["active", "used", "expired", "revoked"]);
...
export const tenantBranding = pgTable("tenant_branding", {
  ...
  primaryColor: varchar("primaryColor", { length: 9 }).default("#2563EB").notNull(),
  secondaryColor: varchar("secondaryColor", { length: 9 }).default("#1E40AF").notNull(),
  accentColor: varchar("accentColor", { length: 9 }).default("#F59E0B").notNull(),
  backgroundColor: varchar("backgroundColor", { length: 9 }).default("#0F172A").notNull(),
  textColor: varchar("textColor", { length: 9 }).default("#F8FAFC").notNull(),
  fontFamily: varchar("fontFamily", { length: 64 }).default("Inter").notNull(),
...
export const corridorStatusEnum = pgEnum("corridor_status", ["active", "paused", "disabled"]);
...
export const tenantCorridors = pgTable("tenant_corridors", {
  ...
  destinationCurrency: varchar("destinationCurrency", { length: 3 }).notNull(),
  status: corridorStatusEnum("status").default("active").notNull(),
  minAmount: numeric("minAmount", { precision: 20, scale: 2 }).default("10.00").notNull(),
  maxAmount: numeric("maxAmount", { precision: 20, scale: 2 }).default("1000000.00").notNull(),
  dailyLimit: numeric("dailyLimit", { precision: 20, scale: 2 }).default("5000000.00").notNull(),
  estimatedDeliveryMinutes: integer("estimatedDeliveryMinutes").default(30).notNull(),
  paymentMethods: json("paymentMethods").$type<string[]>().default(["bank_transfer", "mobile_money"]),
  deliveryMethods: json("deliveryMethods").$type<string[]>().default(["bank_deposit", "mobile_wallet"]),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
}, (t) => ({
  tenantIdIdx: index("tenant_corridors_tenantId_idx").on(t.tenantId),
  routeIdx: index("tenant_corridors_route_idx").on(t.sourceCountry, t.destinationCountry),
}));
...
export const tenantFeeOverrides = pgTable("tenant_fee_overrides", {
  ...
  feeValue: numeric("feeValue", { precision: 10, scale: 4 }).default("1.5000").notNull(),
  minFee: numeric("minFee", { precision: 20, scale: 2 }).default("100.00").notNull(),
  maxFee: numeric("maxFee", { precision: 20, scale: 2 }).default("50000.00").notNull(),
  tieredRules: json("tieredRules").$type<Array<{ minAmount: number; maxAmount: number; fee: number }>>(),
  description: text("description"),
  isActive: boolean("isActive").default(true).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
...
// ─── Tenant Sub-Users ───────────────────────────────────────────────────────
export const tenantUserRoleEnum = pgEnum("tenant_user_role", ["tenant_admin", "tenant_operator", "tenant_viewer"]);
...
export const commissionCascadeHistory = pgTable("commission_cascade_history", {
  ...
  transactionAmount: numeric("transactionAmount", { precision: 15, scale: 2 }).notNull(),
  totalCommission: numeric("totalCommission", { precision: 15, scale: 2 }).notNull(),
  ...
  recipientHierarchyRole: varchar("recipientHierarchyRole", { length: 32 }).notNull(),
  recipientHierarchyLevel: integer("recipientHierarchyLevel").notNull(),
  // Commission split details
  splitPercentage: numeric("splitPercentage", { precision: 5, scale: 2 }).notNull(),
...
export const reconciliationItems = pgTable("reconciliation_items", {
  ...
  externalAmount: numeric("external_amount", { precision: 15, scale: 2 }).notNull(),
...
export const realtime_tx_alerts = pgTable("realtime_tx_alerts", {
  id: serial("id").primaryKey(),
...
export const commissionTiers = pgTable("commission_tiers", {
  ...
  minVolume: numeric("min_volume", { precision: 15, scale: 2 }).default("0").notNull(),
  maxVolume: numeric("max_volume", { precision: 15, scale: 2 }).default("999999999").notNull(),
  rate: numeric("rate", { precision: 8, scale: 4 }).notNull(),
  flatFee: numeric("flat_fee", { precision: 10, scale: 2 }).default("0").notNull(),
  bonusRate: numeric("bonus_rate", { precision: 8, scale: 4 }).default("0").notNull(),
...
export const commissionSplits = pgTable("commission_splits", {
  ...
  transactionType: varchar("transaction_type", { length: 32 }).notNull(),
  superAgentShare: numeric("super_agent_share", { precision: 5, scale: 2 }).notNull(),
  masterAgentShare: numeric("master_agent_share", { precision: 5, scale: 2 }).notNull(),
  agentShare: numeric("agent_share", { precision: 5, scale: 2 }).notNull(),
  subAgentShare: numeric("sub_agent_share", { precision: 5, scale: 2 }).notNull(),
...
export const loadTestRunStatusEnum = pgEnum("load_test_run_status", ["running", "completed", "failed", "cancelled"]);
...
export const loadTestRuns = pgTable("load_test_runs", {
  ...
  results: json("results").$type<{
    ...
    p99LatencyMs: number;
    maxLatencyMs: number;
    zipfDistribution: Array<{ merchantId: number; requestCount: number; percentage: number }>;
    latencyHistogram: Array<{ bucket: string; count: number }>;
    timeline: Array<{ second: number; rps: number; avgLatencyMs: number; errorRate: number }>;
  }>(),
  errorMessage: text("error_message"),
}, (t) => ({
...
export const billingModelTypeEnum = pgEnum("billing_model_type", ["revenue_share", "subscription", "hybrid"]);
...
export const platformBillingLedger = pgTable("platform_billing_ledger", {
  ...
  agentCommission: numeric("agent_commission", { precision: 12, scale: 2 }).notNull(),
  switchFee: numeric("switch_fee", { precision: 12, scale: 2 }).notNull(),
  aggregatorFee: numeric("aggregator_fee", { precision: 12, scale: 2 }).notNull(),
  platformNetFee: numeric("platform_net_fee", { precision: 12, scale: 2 }).notNull(),
  billingModel: billingModelTypeEnum("billing_model").notNull().default("revenue_share"),
  clientRevenue: numeric("client_revenue", { precision: 12, scale: 2 }).notNull(),
  platformRevenue: numeric("platform_revenue", { precision: 12, scale: 2 }).notNull(),
...
export type PlatformBillingLedgerEntry = typeof platformBillingLedger.$inferSelect;
export type NewPlatformBillingLedgerEntry = typeof platformBillingLedger.$inferInsert;
...
export const billingRevenuePeriods = pgTable("billing_revenue_periods", {
  ...
  grossVolume: numeric("gross_volume", { precision: 18, scale: 2 }).notNull().default("0.00"),
  totalFees: numeric("total_fees", { precision: 15, scale: 2 }).notNull().default("0.00"),
  totalClientRevenue: numeric("total_client_revenue", { precision: 15, scale: 2 }).notNull().default("0.00"),
  totalPlatformRevenue: numeric("total_platform_revenue", { precision: 15, scale: 2 }).notNull().default("0.00"),
  totalAgentCommissions: numeric("total_agent_commissions", { precision: 15, scale: 2 }).notNull().default("0.00"),
  totalSwitchFees: numeric("total_switch_fees", { precision: 15, scale: 2 }).notNull().default("0.00"),
  totalAggregatorFees: numeric("total_aggregator_fees", { precision: 15, scale: 2 }).notNull().default("0.00"),
  ...
  avgTxPerAgent: numeric("avg_tx_per_agent", { precision: 8, scale: 2 }).default("0.00"),
  periodOpexEstimate: numeric("period_opex_estimate", { precision: 15, scale: 2 }).default("0.00"),
  netPlatformProfit: numeric("net_platform_profit", { precision: 15, scale: 2 }).default("0.00"),
  billingModel: billingModelTypeEnum("billing_model").notNull().default("revenue_share"),
  currency: varchar("currency", { length: 3 }).notNull().default("NGN"),
  ...
  compositeIdx: index("brp_composite_idx").on(t.periodType, t.periodStart, t.billingModel),
}));
...
export const billingReconciliationReports = pgTable("billing_reconciliation_reports", {
  id: serial("id").primaryKey(),
  ...
  projectedGrossVolume: numeric("projected_gross_volume", { precision: 18, scale: 2 }),
  projectedPlatformRevenue: numeric("projected_platform_revenue", { precision: 15, scale: 2 }),
  projectedClientRevenue: numeric("projected_client_revenue", { precision: 15, scale: 2 }),
  projectedAgents: integer("projected_agents"),
  projectedTxPerAgent: numeric("projected_tx_per_agent", { precision: 8, scale: 2 }),
  actualTransactions: integer("actual_transactions"),
  actualGrossVolume: numeric("actual_gross_volume", { precision: 18, scale: 2 }),
  actualPlatformRevenue: numeric("actual_platform_revenue", { precision: 15, scale: 2 }),
  actualClientRevenue: numeric("actual_client_revenue", { precision: 15, scale: 2 }),
  actualAgents: integer("actual_agents"),
  actualTxPerAgent: numeric("actual_tx_per_agent", { precision: 8, scale: 2 }),
  revenueVariancePct: numeric("revenue_variance_pct", { precision: 8, scale: 2 }),
  volumeVariancePct: numeric("volume_variance_pct", { precision: 8, scale: 2 }),
  agentVariancePct: numeric("agent_variance_pct", { precision: 8, scale: 2 }),
  insights: json("insights"),
  generatedBy: varchar("generated_by", { length: 64 }).default("billing-reconciliation-engine"),
...
export type NewBillingReconciliationReport = typeof billingReconciliationReports.$inferInsert;
...
export type NewBillingRoleAssignment = typeof billingRoleAssignments.$inferInsert;
...
export type BillingProvisioningHistoryEntry = typeof billingProvisioningHistory.$inferSelect;
export type NewBillingProvisioningHistoryEntry = typeof billingProvisioningHistory.$inferInsert;
...
export const faceEnrollments = pgTable("face_enrollments", {
  ...
  enrollmentType: varchar("enrollmentType", { length: 32 }).notNull().default("kyc"), // kyc | login | payment
  embeddingVector: text("embeddingVector").notNull(), // JSON-serialized 512-d float array
  embeddingVersion: varchar("embeddingVersion", { length: 32 }).notNull().default("arcface_w600k_r50"),
...
export const biometricAuditEvents = pgTable("biometric_audit_events", {
  ...
  deviceInfo: json("deviceInfo").$type<{ userAgent?: string; platform?: string; screen?: string }>(),
  ipAddress: varchar("ipAddress", { length: 64 }),
  geoLocation: json("geoLocation").$type<{ lat?: number; lng?: number; country?: string }>(),
