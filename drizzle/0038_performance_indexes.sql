...
-- Webhook logs: high-volume table, needs composite indexes
...
-- Audit log: frequently queried with filters
CREATE INDEX IF NOT EXISTS idx_audit_log_user_id ON audit_log(user_id);
CREATE INDEX IF NOT EXISTS idx_audit_log_action ON audit_log(action);
CREATE INDEX IF NOT EXISTS idx_audit_log_created_at ON audit_log(created_at);
CREATE INDEX IF NOT EXISTS idx_audit_log_status ON audit_log(status);

-- Participants: lookup by status and tier
...
CREATE INDEX IF NOT EXISTS idx_outbound_transfers_created_at ON outbound_transfers(created_at);
