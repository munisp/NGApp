...
KYC_PROVIDER = os.getenv("KYC_PROVIDER", "mock")  # mock, nibss, smile_id, onfido
...
def get_bvn_provider() -> BVNProvider:
    ...
    elif provider == "mock" or ENVIRONMENT == "development":
        return MockBVNProvider()
    else:
        logger.warning(f"Unknown BVN provider: {provider}, using mock")
        return MockBVNProvider()


def get_liveness_provider() -> LivenessProvider:
    """Get configured liveness provider"""
    provider = os.getenv("LIVENESS_PROVIDER", KYC_PROVIDER)
    ...
    if provider == "smile_id":
        ...
        logger.warning(f"Unknown liveness provider: {provider}, using mock")
        return MockLivenessProvider()


def get_document_provider() -> DocumentVerificationProvider:
    """Get configured document verification provider"""
    provider = os.getenv("DOCUMENT_PROVIDER", KYC_PROVIDER)
    ...
    if provider == "smile_id":
        ...
        logger.warning(f"Unknown document provider: {provider}, using mock")
        return MockDocumentVerificationProvider()
