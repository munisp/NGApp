...
const ExchangeRates: React.FC = () => {
  ...
  const rates: ExchangeRate[] = [
    ...
    { from: 'NGN', to: 'XOF', rate: 0.386, change: 0.0, flag: '\u{1F1F8}\u{1F1F3}', name: 'West African CFA' },
    { from: 'NGN', to: 'XAF', rate: 0.386, change: 0.0, flag: '\u{1F1E8}\u{1F1F2}', name: 'Central African CFA' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-xs text-slate-400 bg-slate-50 px-3 py-1 rounded-full">Updated 2 mins ago</span>
        </div>
            ...
            <tbody>
              {rates.map((rate) => (
                <tr key={rate.to} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-right font-mono text-sm font-semibold text-slate-900">{(1 / rate.rate).toFixed(2)}</td>
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <button className="text-sm font-medium text-indigo-600 hover:text-indigo-500">Exchange</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <select className={`${selectClass} md:w-32`}>
            <option>USD/NGN</option><option>GBP/NGN</option><option>EUR/NGN</option>
          </select>
          <input type="number" placeholder="Target rate" className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:outline-none" />
          <button className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200">Set Alert</button>
        </div>
      </div>
    </div>
  );
