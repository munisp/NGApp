      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-100 p-5 space-y-6">
        ...
        <div>
          ...
          <input type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)}
            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:outline-none transition-all" placeholder="08012345678" required />
        </div>
          ...
          <div>
            ...
            <div className="flex">
              ...
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)}
                className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-r-xl text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:outline-none transition-all" placeholder="Enter amount" required />
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center justify-between p-3.5 rounded-xl hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-yellow-400 to-yellow-500 rounded-xl flex items-center justify-center"><span className="text-xs font-bold text-yellow-900">M</span></div>
                <div><p className="text-sm font-semibold text-slate-900">MTN {i % 2 === 0 ? 'Data' : 'Airtime'}</p><p className="text-xs text-slate-400">08012345678</p></div>
              </div>
              <p className="text-sm font-bold text-slate-900 tabular-nums">\u20a6{(1000 * i).toLocaleString()}</p>
            </div>
          ))}
        </div>
      </div>
