    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {selectedMethod === 'qr' && (
        <div className="bg-white rounded-2xl border border-slate-100 p-6 text-center">
          ...
          <p className="mt-4 text-sm text-slate-500">Show this QR code to receive payment instantly</p>
          <div className="mt-6 max-w-xs mx-auto">
            ...
            <div className="flex">
              ...
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-r-xl text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:outline-none" placeholder="0.00" />
            </div>
          </div>
          <button className="mt-4 px-6 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200">Download QR Code</button>
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" className={inputClass} placeholder="What is this payment for?" />
            </div>
            <button className="w-full py-3.5 px-6 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200">Generate Payment Link</button>
      ...
      {selectedMethod === 'account' && (
        <div className="bg-white rounded-2xl border border-slate-100 p-6">
            ...
            {[
              ...
              <div key={item.label} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {item.copyable && (
                    <button onClick={() => handleCopy(item.value)} className="text-xs text-indigo-600 hover:text-indigo-500 font-medium">{copied ? 'Copied!' : 'Copy'}</button>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="mt-4 p-4 bg-amber-50 border border-amber-100 rounded-xl">
            <p className="text-sm text-amber-800"><strong>Note:</strong> This is a dedicated virtual account. Funds sent here will be credited to your NGN wallet within minutes.</p>
          </div>
          <button className="w-full mt-4 py-3.5 px-6 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200">Share Account Details</button>
        </div>
      )}
    </div>
  );
