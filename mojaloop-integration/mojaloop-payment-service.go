...
import (
	...
	"mojaloop-integration/mojaloop"
)

type MojaloopPaymentService struct {
	db              *gorm.DB
	mojaloopClient  *mojaloop.MojaloopClient
...
func NewMojaloopPaymentService(
	...
	mojaloopClient := mojaloop.NewMojaloopClient(mojaloopBaseURL, fspiID, apiKey)
...
func (s *MojaloopPaymentService) lookupParties(ctx context.Context, payment *Payment) error {
	...
	payerReq := mojaloop.PartyLookupRequest{
		PartyIdType:     "MSISDN",
	...
	payeeReq := mojaloop.PartyLookupRequest{
		PartyIdType:     "MSISDN",
...
func (s *MojaloopPaymentService) requestQuote(ctx context.Context, payment *Payment) error {
	...
	quoteReq := mojaloop.QuoteRequest{
		QuoteID:       quoteID,
		...
		Payee: mojaloop.Party{
			PartyIdType:     "MSISDN",
			PartyIdentifier: payment.PayeePartyID,
		},
		AmountType: "SEND",
		Amount: mojaloop.Money{
...
func (s *MojaloopPaymentService) prepareTransfer(ctx context.Context, payment *Payment) error {
	...
	transferReq := mojaloop.TransferRequest{
...
func (s *MojaloopPaymentService) fulfillTransfer(ctx context.Context, payment *Payment) error {
	...
	fulfilment := mojaloop.GenerateFulfilment(payment.Condition)
