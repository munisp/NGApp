...
test.describe("Partner Onboarding Wizard", () => {
  ...
  test("should display the onboarding wizard with invite code step", async ({ page }) => {
    ...
    const inviteInput = page.locator('input[placeholder*="invite"], input[placeholder*="code"], input[placeholder*="INVITE"]').first();
  ...
  test("should reject invalid invite code", async ({ page }) => {
    ...
    const validateBtn = page.locator("button", { hasText: /validate|verify|next|continue/i }).first();
  ...
  test("should validate invite code and proceed to company details", async ({ page }) => {
  ...
  test("should complete company details step", async ({ page }) => {
    ...
    const inviteInput = page.locator('input[placeholder*="invite"], input[placeholder*="code"], input[placeholder*="INVITE"]').first();
    await inviteInput.fill("PARTNER-2026-DEMO");
    await page.locator("button", { hasText: /validate|verify|next|continue/i }).first().click();
    
    await page.waitForTimeout(1_000);
    
    // Step 2: Fill company details
    const companyName = page.locator('input[placeholder*="company"], input[placeholder*="Company"], input[name*="company"]').first();
    if (await companyName.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await companyName.fill("TestPartner Ltd");
    }
    
    const regNumber = page.locator('input[placeholder*="registration"], input[placeholder*="RC"], input[name*="reg"]').first();
    if (await regNumber.isVisible({ timeout: 2_000 }).catch(() => false)) {
      await regNumber.fill("RC123456");
    }
    
    const contactEmail = page.locator('input[type="email"], input[placeholder*="email"]').first();
    if (await contactEmail.isVisible({ timeout: 2_000 }).catch(() => false)) {
      await contactEmail.fill("admin@testpartner.com");
    }
    
    const contactPhone = page.locator('input[placeholder*="phone"], input[type="tel"]').first();
    if (await contactPhone.isVisible({ timeout: 2_000 }).catch(() => false)) {
      await contactPhone.fill("+2348012345678");
    }
    
    // Click next
    const nextBtn = page.locator("button", { hasText: /next|continue|proceed/i }).first();
  ...
  test("should show branding preview with custom colors", async ({ page }) => {
    ...
    const colorInput = page.locator('input[type="color"], input[placeholder*="color"], input[placeholder*="#"]').first();
