...
test.describe("Float Top-Up Approval Workflow", () => {
  test("admin can approve a float top-up request", async ({ page }) => {
    ...
    await expect(page.locator("text=/admin|overview/i").first()).toBeVisible({ timeout: 10_000 });
    ...
    if (await floatTab.isVisible()) {
      ...
      await expect(page.locator("text=/pending|request/i").first()).toBeVisible({ timeout: 5_000 });
      ...
      if (await approveBtn.isVisible()) {
        ...
        const confirmBtn = page.locator("button", { hasText: /confirm|yes/i }).first();
        if (await confirmBtn.isVisible()) {
          await confirmBtn.click();
        }
        await expect(page.locator("text=/approved|success/i").first()).toBeVisible({ timeout: 5_000 });
      }
    }
  });
});
