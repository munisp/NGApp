...
test.describe("Admin Dashboard Navigation", () => {
  test("should load admin dashboard with sidebar navigation", async ({ page }) => {
...
test.describe("Agent Logout Flow", () => {
  test("should logout agent and return to login screen", async ({ page }) => {
    ...
    const logoutBtn = page.locator("button", { hasText: /logout|sign.?out|exit/i }).first();
    if (await logoutBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await logoutBtn.click();
      
      // Should return to login screen
      await expect(
        page.locator("text=54Link POS").first()
      ).toBeVisible({ timeout: 10_000 });
      
      // Agent code input should be visible again
      await expect(
        page.locator('input[placeholder*="AGT"]')
