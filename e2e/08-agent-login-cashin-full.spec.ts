...
test.describe("Agent Login & Cash-In — Full Flow", () => {
  ...
  test("should display the POS login screen with agent code input", async ({ page }) => {
  ...
  test("should navigate to PIN entry after valid agent code", async ({ page }) => {
    const agentInput = page.locator('input[placeholder*="AGT"]');
    ...
    await expect(
      page.locator('[data-digit], input[type="password"], text=/enter.*pin/i').first()
    ).toBeVisible({ timeout: 10_000 });
  });

  test("should complete full login with agent code and PIN", async ({ page }) => {
  ...
  test("should perform cash-in transaction after login", async ({ page }) => {
    ...
    // Fill amount
    const amountInput = page.locator('input[placeholder*="amount"], input[placeholder*="Amount"], input[type="number"]').first();
    if (await amountInput.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await amountInput.fill("5000");
    }
    
    // Fill customer phone if visible
    const phoneInput = page.locator('input[placeholder*="phone"], input[placeholder*="customer"], input[placeholder*="Phone"]').first();
    if (await phoneInput.isVisible({ timeout: 2_000 }).catch(() => false)) {
      await phoneInput.fill("08012345678");
    }
    
    // Submit
    const submitBtn = page.locator("button", { hasText: /confirm|proceed|submit|send/i }).first();
    ...
    await expect(
      page.locator("text=/receipt|success|completed|₦5,000|transaction/i").first()
    ).toBeVisible({ timeout: 15_000 });
  });
});
