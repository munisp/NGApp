...
test.describe("Cash-Out Transaction Flow", () => {
  ...
  test("should perform cash-out with customer phone and amount", async ({ page }) => {
    await loginAsAgent(page);
    ...
    // Submit
    const submitBtn = page.locator("button", { hasText: /confirm|proceed|submit|withdraw/i }).first();
    ...
    // Verify success
    await expect(
      page.locator("text=/receipt|success|completed|₦3,000|transaction|approved/i").first()
  ...
  test("should reject cash-out exceeding daily limit", async ({ page }) => {
    ...
    const phoneInput = page.locator('input[placeholder*="phone"], input[placeholder*="customer"]').first();
    ...
    await expect(
      page.locator("text=/limit|exceed|insufficient|error|maximum|invalid/i").first()
