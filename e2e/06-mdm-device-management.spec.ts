...
test.describe("MDM Device Management", () => {
  ...
  test("Admin can view MDM fleet dashboard", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="fleet-total-devices"], h2:has-text("Fleet"), h1:has-text("MDM")')).toBeVisible({ timeout: 10000 });

    // Should show device count metrics
    const deviceCount = page.locator('[data-testid="total-devices-count"], [data-testid="fleet-stats"]');
    await expect(deviceCount).toBeVisible({ timeout: 5000 });

    // Online/offline breakdown should be present
    await expect(page.locator('text=/online|offline/i').first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test("Admin can view device compliance violations list", async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Compliance"), [data-testid="compliance-violations"]')).toBeVisible({ timeout: 10000 });
  ...
  test("Admin can view device details and compliance status", async ({ page }) => {
    await page.goto(`${BASE_URL}/admin/mdm`);
    await page.waitForLoadState("networkidle");

    // Click on first device in the list
    const firstDevice = page.locator('[data-testid="device-row"], tr[data-device-id]').first();
    if (await firstDevice.isVisible()) {
      await firstDevice.click();
      await page.waitForLoadState("networkidle");

      // Device detail page should show compliance info
      await expect(page.locator('[data-testid="device-detail"], [data-testid="compliance-status"]')).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test("Admin can view OTA update status", async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("OTA"), h1:has-text("Firmware"), [data-testid="ota-updates"]')).toBeVisible({ timeout: 10000 });

    // Should show firmware versions
    await expect(page.locator('[data-testid="firmware-version"], text=/v\d+\.\d+/').first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test("Admin can view geofence violations", async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Geofence"), [data-testid="geofence-violations"]')).toBeVisible({ timeout: 10000 });
  });

  test("MDM API returns device list with compliance status", async ({ page }) => {
    // Test the tRPC API directly
    const response = await page.request.post(`${BASE_URL}/api/trpc/mdm.listDevices`, {
    ...
    expect(Array.isArray(data?.devices) || typeof data?.total === "number").toBeTruthy();
  });

  test("MDM API returns compliance violations", async ({ page }) => {
    const response = await page.request.post(`${BASE_URL}/api/trpc/mdm.getComplianceViolations`, {
  ...
  test("OTA service health endpoint is reachable", async ({ page }) => {
    ...
    const response = await page.request.get(`${otaUrl}/api/v1/ota/health`).catch(() => null);
...
test.describe("MDM Remote Actions", () => {
  ...
  test("Admin can initiate device lock from device detail page", async ({ page }) => {
    await page.goto(`${BASE_URL}/admin/mdm`);
    await page.waitForLoadState("networkidle");
    ...
    const lockBtn = page.locator('[data-testid="remote-lock-btn"], button:has-text("Lock Device")');
  ...
  test("Admin can push OTA update to device", async ({ page }) => {
    ...
    const pushBtn = page.locator('[data-testid="push-update-btn"], button:has-text("Push Update")').first();
