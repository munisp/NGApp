...
export default function ReferralProgram() {
  ...
  const statusColor = (s: string) =>
    s === "activated" ? "bg-blue-100 text-blue-800" :
    s === "rewarded" ? "bg-green-100 text-green-800" :
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Track agent referrals, activations, and reward disbursements</p>
        </div>
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: "Activated", value: stats.activated, icon: TrendingUp, color: "text-blue-600" },
              { label: "Rewarded", value: stats.rewarded, icon: Gift, color: "text-green-600" },
              { label: "Total Rewards (₦)", value: `₦${Number(stats.totalRewardAmount ?? 0).toLocaleString()}`, icon: Gift, color: "text-green-600" },
            ].map((s: any) => (
              <Card key={s.label}>
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-48">
            ...
            <Input className="pl-9" placeholder="Search referrer or referee..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {!isLoading && data?.items.map((r: any) => (
                    <tr key={r.id} className="border-b hover:bg-muted/30">
                      <td className="py-2 px-3">
                        <p className="font-medium text-sm">{r.referrerCode}</p>
                      </td>
                      <td className="py-2 px-3">
                        <p className="text-sm">{r.refereeCode ?? "—"}</p>
                        <p className="text-xs text-muted-foreground font-mono">{r.referralCode}</p>
                      </td>
                      <td className="py-2 px-3 text-right font-mono">
                        {r.bonusCash ? `₦${Number(r.bonusCash).toLocaleString()}` : "—"}
                      </td>
                      <td className="py-2 px-3">
                      ...
                      <td className="py-2 px-3">
                        {r.status === "activated" && (
                          <Button
                            ...
                            onClick={() => rewardMutation.mutate({ id: r.id })}
                  ...
                  {!isLoading && data?.items.length === 0 && (
                    <tr><td colSpan={6} className="text-center py-8 text-muted-foreground">No referral records found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
