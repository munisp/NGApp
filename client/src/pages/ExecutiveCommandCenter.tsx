...
export default function ExecutiveCommandCenter() {
  const { data: stats, isLoading, refetch } = trpc.executiveCommandCenter.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">C-suite dashboard with KPI drill-downs, briefings, and strategic insights</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalKPIs ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">On Target</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.kpisOnTarget ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Below Target</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.kpisBelowTarget ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Health</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.overallHealth ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Revenue Growth %</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.revenueGrowth ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Efficiency %</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.operationalEfficiency ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Risk Score</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.riskScore ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Last Updated</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.lastUpdated ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for executive command center.</p>
