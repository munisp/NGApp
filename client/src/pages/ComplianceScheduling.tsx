...
import { Clock, Shield, Calendar, Search, Plus, Edit, Save, X } from "lucide-react";
...
export default function ComplianceScheduling() {
  ...
  const saveEdit = (id: string) => {
    setSchedules(prev => prev.map(s => s.id === id ? { ...s, startTime: editStart, endTime: editEnd } : s));
    setEditId(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Clock className="w-6 h-6 text-blue-400" /> Compliance Scheduling</h1>
          <p className="text-slate-400 text-sm mt-1">Configure time-based enforcement windows for MDM compliance policies</p>
        </div>
        <div className="flex gap-2">
          <Badge variant="outline" className="text-xs border-green-600 text-green-400">{policies.data?.length ?? 0} DB Policies</Badge>
          <Badge variant="outline" className="text-xs border-blue-600 text-blue-400">{schedules.filter(s => s.enforced).length} Active Schedules</Badge>
        </div>
      </div>
      ...
      <div className="relative max-w-md">
        ...
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search policies..." className="pl-9 bg-slate-800 border-slate-700 text-white" />
      </div>

      {/* Schedule Table */}
      <Card className="bg-slate-900/50 border-slate-700">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm text-white flex items-center gap-2"><Calendar className="w-4 h-4" /> Enforcement Windows</CardTitle>
        </CardHeader>
              ...
              {filtered.map(s => (
                <tr key={s.id} className="hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 text-center">
                    <Badge variant="outline" className={`text-[10px] ${severityColor[s.severity]}`}>{s.severity}</Badge>
                  </td>
                  <td className="px-4 py-3 text-center">
                    {editId === s.id ? (
                      <div className="flex items-center gap-1 justify-center">
                        <Input value={editStart} onChange={e => setEditStart(e.target.value)} className="w-16 h-6 text-[10px] bg-slate-800 border-slate-600 text-white" />
                        <span className="text-slate-500">-</span>
                        <Input value={editEnd} onChange={e => setEditEnd(e.target.value)} className="w-16 h-6 text-[10px] bg-slate-800 border-slate-600 text-white" />
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex gap-0.5 justify-center flex-wrap">
                      {DAYS.map(d => (
                        <span key={d} className={`px-1 py-0.5 rounded text-[9px] ${s.daysOfWeek.includes(d) ? "bg-blue-600/30 text-blue-300" : "bg-slate-800 text-slate-600"}`}>{d[0]}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <Switch checked={s.enforced} onCheckedChange={() => toggleEnforced(s.id)} />
                  </td>
                  <td className="px-4 py-3 text-center">
                    {editId === s.id ? (
                      <div className="flex gap-1 justify-center">
                        <Button size="icon" variant="ghost" className="h-6 w-6 text-green-400" onClick={() => saveEdit(s.id)}><Save className="w-3 h-3" /></Button>
                        <Button size="icon" variant="ghost" className="h-6 w-6 text-red-400" onClick={() => setEditId(null)}><X className="w-3 h-3" /></Button>
                      </div>
                    ) : (
                      <Button size="icon" variant="ghost" className="h-6 w-6 text-slate-400" onClick={() => startEdit(s)}><Edit className="w-3 h-3" /></Button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Total Policies", value: schedules.length, color: "text-blue-400" },
          { label: "Active", value: schedules.filter(s => s.enforced).length, color: "text-green-400" },
          { label: "Critical", value: schedules.filter(s => s.severity === "critical").length, color: "text-red-400" },
          { label: "24/7 Enforced", value: schedules.filter(s => s.startTime === "00:00" && s.endTime === "23:59").length, color: "text-purple-400" },
        ].map(stat => (
