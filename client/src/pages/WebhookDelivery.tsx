    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Webhook className="w-6 h-6 text-indigo-400" /> Webhook Delivery</h1>
            <p className="text-slate-400 text-sm mt-1">Configure real-time event delivery to external systems</p>
          </div>
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            ...
            <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-lg">
              ...
              <div className="space-y-4">
                <div><Label>Endpoint URL</Label><Input className="bg-slate-700 border-slate-600" value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} placeholder="https://your-system.com/webhook" /></div>
                <div><Label>Description</Label><Input className="bg-slate-700 border-slate-600" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Optional description" /></div>
                <div>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader><CardTitle className="text-white">Registered Endpoints</CardTitle></CardHeader>
          <CardContent>
            {(endpoints as any[]).length === 0 ? (
              ...
              <div className="space-y-3">
                {(endpoints as any[]).map((ep: any) => (
                  <div key={ep.endpoint_id} className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <code className="text-xs text-slate-400">{ep.endpoint_id}</code>
                        </div>
                        <p className="text-white font-medium text-sm truncate">{ep.url}</p>
                        <p className="text-slate-400 text-xs mt-1">{ep.description || "No description"}</p>
                      ...
                      <div className="flex gap-2 ml-4">
                        <Button size="sm" variant="outline" className="border-slate-600 text-slate-300 hover:text-white" onClick={() => deliverMut.mutate({ endpointId: String(ep.endpoint_id), eventType: "test.ping", payload: { message: "NDSEP test ping", timestamp: new Date().toISOString() } })}>
