...
export default function AgentLoanOrigination() {
  const { data: stats, isLoading, refetch } = trpc.agentLoanOrigination.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Micro-loan application, credit scoring, and approval workflow for agents</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalLoans ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Active</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.activeLoans ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Disbursed</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.totalDisbursed ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Outstanding</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.totalOutstanding ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Avg Rate %</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.avgInterestRate ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Default Rate %</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.defaultRate ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Pending</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.pendingApproval ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Avg Credit Score</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.avgCreditScore ?? "—")}</p>
            </div>
            </div>
