...
export default function ReportSchedulerPage() {
  const { data, isLoading } = trpc.reportScheduler.listSchedules.useQuery({ page: 1, limit: 50 });
  const schedules = trpc.reportScheduler.listSchedules.useQuery({ page: 1, limit: 50 });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Active</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{data?.items?.filter((s: any) => s.status === "active").length ?? 0}</div></CardContent></Card>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b"><th className="text-left p-2">Report</th><th className="text-left p-2">Frequency</th><th className="text-left p-2">Format</th><th className="text-left p-2">Recipients</th><th className="text-left p-2">Status</th></tr></thead>
              <tbody>
