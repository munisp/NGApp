  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          key={i}
          className={`w-3 h-3 ${i <= Math.round(value) ? "fill-amber-400 text-amber-400" : "text-slate-300"}`}
        />
      ))}
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center gap-6">
        ...
        <span className="text-xs text-slate-500 flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3 text-violet-500" />Client Improvement — % of clients improving compliance score after audit</span>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">DPCO Name</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">Licence</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">State</th>
                  ...
                  <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500">Active Engagements</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500">Total CARs</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500">Overall Score</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500">Rating</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((dpco, idx) => {
                  ...
                  const isTop3 = idx < 3;
                  return (
                    <tr key={dpco.id} className={`border-b border-slate-50 hover:bg-slate-50 transition-colors ${isTop3 ? "bg-amber-50/30" : ""}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-xs font-medium text-slate-900">{dpco.name}</p>
                            <p className="text-xs text-slate-400 capitalize">{dpco.organisation_type?.replace("_", " ")}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-xs font-mono text-slate-600">{dpco.licence_number ?? "—"}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-xs text-slate-600">{dpco.state ?? "—"}</span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="text-xs font-medium text-slate-700">{m.activeEngagements}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="text-xs font-medium text-slate-700">{m.totalCars}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
