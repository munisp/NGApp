...
export default function RealtimePnlDashboard() {
  const { data: stats, isLoading, refetch } = trpc.realtimePnlDashboard.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Live profit and loss tracking per agent, region, and product line</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalRevenue ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Total Costs</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.totalCosts ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Net Profit</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.totalProfit ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Avg Margin %</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.avgMargin ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Transactions</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.totalTransactions ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Regions</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.regions ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Top Performer</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.topPerformer ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Growth %</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.profitGrowth ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for real-time p&l dashboard.</p>
