    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Award className="w-6 h-6 text-amber-400" /> Compliance Certificates</h1>
            <p className="text-slate-400 text-sm mt-1">Issue, verify, and manage NDPA compliance certificates</p>
          </div>
              ...
              <div className="space-y-4">
                <div><Label>Certificate Type</Label>
                  <Select value={form.certType} onValueChange={v => setForm(f => ({ ...f, certType: v }))}>
                    ...
                    <SelectContent className="bg-slate-700 border-slate-600">{CERT_TYPES.map(t => <SelectItem key={t} value={t}>{t.replace(/_/g, " ").toUpperCase()}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div><Label>Notes</Label><Input className="bg-slate-700 border-slate-600" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Optional notes..." /></div>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader><CardTitle className="text-white flex items-center gap-2"><Search className="w-5 h-5 text-amber-400" /> Certificate Verification</CardTitle></CardHeader>
          <CardContent>
            <div className="flex gap-3">
              <Input className="bg-slate-700 border-slate-600 text-white flex-1" placeholder="Enter certificate number (e.g. NDSEP-NDPA-...)" value={verifyNum} onChange={e => setVerifyNum(e.target.value)} />
            </div>
              ...
              <div className={`mt-4 p-4 rounded-lg border ${verifyQuery.data.valid ? "bg-green-500/10 border-green-500/30" : "bg-red-500/10 border-red-500/30"}`}>
                ...
                {verifyQuery.data.valid && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div><span className="text-slate-400">Organization:</span> <span className="text-white">{String(verifyQuery.data.orgName ?? "—")}</span></div>
                    <div><span className="text-slate-400">Sector:</span> <span className="text-white">{String(verifyQuery.data.sector ?? "—")}</span></div>
                    <div><span className="text-slate-400">Issued:</span> <span className="text-white">{verifyQuery.data.issuedAt ? new Date(String(verifyQuery.data.issuedAt)).toLocaleDateString("en-NG") : "—"}</span></div>
                    <div><span className="text-slate-400">Expires:</span> <span className="text-white">{verifyQuery.data.expiresAt ? new Date(String(verifyQuery.data.expiresAt)).toLocaleDateString("en-NG") : "—"}</span></div>
                    <div className="col-span-2"><span className="text-slate-400">Issuer:</span> <span className="text-white">{String(verifyQuery.data.issuer ?? "—")}</span></div>
                  </div>
                )}
              </div>
            )}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(certs as any[]).length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-8 text-slate-400">No certificates issued yet</td></tr>
                  ) : (certs as any[]).map((c: any) => {
                    const isExpired = c.expires_at && new Date(String(c.expires_at)) < new Date();
                    return (
                      <tr key={c.cert_number} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                        <td className="py-2 px-3"><code className="text-xs text-amber-400">{String(c.cert_number ?? "").slice(0, 30)}...</code></td>
                        <td className="py-2 px-3 text-white">{c.org_name ?? "—"}</td>
                        <td className="py-2 px-3"><Badge variant="outline" className="text-xs border-slate-600 text-slate-300">{String(c.cert_type ?? "").replace(/_/g, " ").toUpperCase()}</Badge></td>
                        <td className="py-2 px-3 text-slate-400">{c.issued_at ? new Date(String(c.issued_at)).toLocaleDateString("en-NG") : "—"}</td>
                        <td className="py-2 px-3 text-slate-400">{c.expires_at ? new Date(String(c.expires_at)).toLocaleDateString("en-NG") : "Never"}</td>
