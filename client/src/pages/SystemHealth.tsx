...
const SERVICES: ServiceRow[] = [
  ...
  { name: "Keycloak", key: "keycloak", description: "Identity & access management" },
  { name: "TigerBeetle", key: "tbSidecar", description: "Double-entry ledger sidecar" },
  { name: "Temporal", key: "temporal", description: "Workflow orchestration engine" },
...
const statusColor = (val: string | undefined): "default" | "secondary" | "destructive" | "outline" => {
  if (!val || val === "not configured" || val === "offline" || val === "error") return "destructive";
  if (val === "connected" || val === "running" || val === "configured" || val === "ok") return "default";
  return "secondary";
};
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400 mt-1">54Link Agency Banking Platform — Infrastructure Status</p>
          </div>
        ...
        <Card className={`mb-6 border-0 ${
          ...
          <CardContent className="py-4 px-6 flex items-center gap-4">
            <div className={`w-4 h-4 rounded-full ${
              overallStatus === "ok" ? "bg-emerald-400 shadow-[0_0_8px_#34d399]" : "bg-red-400 shadow-[0_0_8px_#f87171]"
            } animate-pulse`} />
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {SERVICES.map((svc: any) => {
            const val = health ? String(health[svc.key] ?? "unknown") : undefined;
            const color = statusColor(val);
            return (
              <Card key={svc.key} className="bg-slate-800/50 border-slate-700">
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm font-semibold text-white">{svc.name}</CardTitle>
                    {loading ? (
        ...
        {health && (
          <Card className="bg-slate-900/50 border-slate-700">
            <CardHeader>
              <CardTitle className="text-sm font-semibold text-slate-300">Raw Health Response</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="mt-6 flex gap-3">
          <a href="/" className="text-sm text-orange-400 hover:text-orange-300 underline">
            ← Back to POS Shell
          </a>
          <a href="/admin" className="text-sm text-orange-400 hover:text-orange-300 underline">
            Admin Panel →
          </a>
        </div>
      </div>
