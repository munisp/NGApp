    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><MessageSquare className="w-6 h-6 text-green-400" /> SMS Enforcement Alerts</h1>
            <p className="text-slate-400 text-sm mt-1">Termii-powered SMS notifications for breach alerts, penalty notices, and regulatory deadlines</p>
          </div>
          <Dialog open={sendOpen} onOpenChange={setSendOpen}>
            ...
            <DialogContent className="bg-slate-800 border-slate-700 text-white">
              ...
              <div className="space-y-4">
                ...
                <div><Label>Recipient Phone (+234...)</Label><Input className="bg-slate-700 border-slate-600" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+2348012345678" /></div>
                <div><Label>Custom Message (optional)</Label><Input className="bg-slate-700 border-slate-600" value={form.customMessage} onChange={e => setForm(f => ({ ...f, customMessage: e.target.value }))} placeholder="Override default message..." /></div>
                <div className="bg-slate-700/50 rounded p-3 text-xs text-slate-400">
                  <p className="font-medium text-slate-300 mb-1">Default message preview:</p>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader><CardTitle className="text-white flex items-center gap-2"><Bell className="w-5 h-5 text-green-400" /> Alert History</CardTitle></CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(history as any[]).length === 0 ? (
                    <tr><td colSpan={5} className="text-center py-8 text-slate-400">No SMS alerts sent yet</td></tr>
                  ) : (history as any[]).map((h: any, i: number) => (
                    <tr key={i} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                      <td className="py-2 px-3"><Badge variant="outline" className="text-xs border-slate-600 text-slate-300">{String(h.alert_type ?? h.type ?? "alert").replace(/_/g, " ").toUpperCase()}</Badge></td>
                      <td className="py-2 px-3 text-slate-300">{h.phone ?? h.recipient ?? "—"}</td>
                      <td className="py-2 px-3 text-slate-400 max-w-xs truncate">{h.message ?? "—"}</td>
                      <td className="py-2 px-3 text-slate-400">{h.sent_at ? new Date(String(h.sent_at)).toLocaleDateString("en-NG") : "—"}</td>
