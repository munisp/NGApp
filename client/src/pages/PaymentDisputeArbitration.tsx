...
export default function PaymentDisputeArbitration() {
  const { data: stats, isLoading, refetch } = trpc.paymentDisputeArbitration.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Multi-party payment dispute resolution and arbitration workflow</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalDisputes ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Open</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.openDisputes ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Resolved</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.resolvedDisputes ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Avg Resolution</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.avgResolutionTime ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Disputed Amount</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.totalDisputedAmount ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Escalated</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.escalatedCount ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">SLA Compliance %</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.slaCompliance ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Refund Rate %</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.refundRate ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for dispute arbitration.</p>
