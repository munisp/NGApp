...
export default function PlatformSlaMonitor() {
  const { data: stats, isLoading, refetch } = trpc.platformSlaMonitor.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Service level agreement compliance tracking with breach alerts and penalties</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalSLAs ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Met</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.metSLAs ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Breached</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.breachedSLAs ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">At Risk</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.atRiskSLAs ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Penalties</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.totalPenalties ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Compliance %</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.overallCompliance ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Avg Performance</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.avgPerformance ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Frequency</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.monitoringFrequency ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for sla monitor.</p>
