      ...
      <div className="flex gap-2">
        {[1, 2, 3, 4, 5].map((v) => (
          <button
            ...
            type="button"
            onClick={() => onChange(v)}
            className={`w-10 h-10 rounded-lg border text-sm font-semibold transition-all ${value === v ? "bg-blue-600 border-blue-500 text-white" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-blue-500"}`}
          >
            {v}
          </button>
        ))}
    ...
    <DashboardLayout>
      <div className="p-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">DPIA Wizard</h1>
            <p className="text-sm text-gray-400">Data Protection Impact Assessment — NDPA 2023 Article 28</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mb-8">
          {steps.map((s, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${i < step ? "bg-green-600 text-white" : i === step ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-500"}`}>
        ...
        {step === 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
            ...
            <div>
              <Label className="text-gray-300 mb-1.5 block">Assessment Title *</Label>
              <Input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} className="bg-gray-800 border-gray-700 text-white" placeholder="e.g. Customer Analytics Platform DPIA" />
            </div>
            <div>
            ...
            <div>
              <Label className="text-gray-300 mb-1.5 block">Processing Purpose *</Label>
              <Textarea value={form.processingPurpose} onChange={e => setForm(p => ({ ...p, processingPurpose: e.target.value }))} className="bg-gray-800 border-gray-700 text-white" rows={3} placeholder="Describe the purpose of the data processing activity..." />
            </div>
        ...
        {step === 1 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-6">
            <h2 className="font-semibold text-white text-lg">Data Scope</h2>
            <div>
              <Label className="text-gray-300 mb-3 block">Data Categories (select all that apply)</Label>
              <div className="flex flex-wrap gap-2">
                {DATA_CATEGORIES.map(cat => (
                  <button key={cat} type="button" onClick={() => toggleCategory(cat)}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${form.dataCategories.includes(cat) ? "bg-blue-600 border-blue-500 text-white" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-blue-500"}`}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            <div>
              <Label className="text-gray-300 mb-3 block">Data Subjects (select all that apply)</Label>
              <div className="flex flex-wrap gap-2">
                {DATA_SUBJECTS.map(sub => (
                  <button key={sub} type="button" onClick={() => toggleSubject(sub)}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${form.dataSubjects.includes(sub) ? "bg-purple-600 border-purple-500 text-white" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-purple-500"}`}>
                    {sub}
                  </button>
                ))}
              </div>
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setStep(0)} className="border-gray-600 text-gray-300"><ChevronLeft className="w-4 h-4 mr-1" /> Back</Button>
        ...
        {step === 2 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
            <h2 className="font-semibold text-white text-lg flex items-center gap-2"><Sparkles className="w-5 h-5 text-yellow-400" /> AI Risk Analysis</h2>
            <p className="text-gray-400 text-sm">Let the AI analyse your DPIA for NDPA 2023 compliance risks and suggest mitigations.</p>
            <div className="bg-gray-800 rounded-lg p-4 text-sm space-y-2">
              <div><span className="text-gray-400">Title:</span> <span className="text-white">{form.title}</span></div>
              <div><span className="text-gray-400">Purpose:</span> <span className="text-white">{form.processingPurpose}</span></div>
              <div><span className="text-gray-400">Data categories:</span> <span className="text-white">{form.dataCategories.join(", ") || "None selected"}</span></div>
              <div><span className="text-gray-400">Data subjects:</span> <span className="text-white">{form.dataSubjects.join(", ") || "None selected"}</span></div>
            </div>
            {aiResult && (
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 space-y-3">
                ...
                <div>
                  <div className="text-xs text-gray-400 mb-1">Key Risk Factors ({aiResult.riskFactors?.length ?? 0})</div>
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setStep(1)} className="border-gray-600 text-gray-300"><ChevronLeft className="w-4 h-4 mr-1" /> Back</Button>
        ...
        {step === 3 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-6">
            <h2 className="font-semibold text-white text-lg">Risk Assessment & Mitigations</h2>
            ...
            <div>
              <Label className="text-gray-300 mb-2 block">Risk Factors</Label>
              <div className="space-y-2 mb-2">
                {form.riskFactors.map((rf: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2 text-sm">
                    ...
                    <span className="text-gray-300 flex-1">{rf.factor ?? rf}</span>
              ...
              <div className="flex gap-2">
                <Input value={newRisk} onChange={e => setNewRisk(e.target.value)} className="bg-gray-800 border-gray-700 text-white text-sm" placeholder="Add a risk factor..." onKeyDown={e => { if (e.key === "Enter") { if (newRisk.trim()) { setForm(p => ({ ...p, riskFactors: [...p.riskFactors, { factor: newRisk.trim(), severity: "medium", ndpaArticle: "Art. 28" }] })); setNewRisk(""); } } }} />
                <Button type="button" size="sm" onClick={() => { if (newRisk.trim()) { setForm(p => ({ ...p, riskFactors: [...p.riskFactors, { factor: newRisk.trim(), severity: "medium", ndpaArticle: "Art. 28" }] })); setNewRisk(""); } }} className="bg-gray-700 hover:bg-gray-600"><Plus className="w-4 h-4" /></Button>
              </div>
            </div>
            <div>
              <Label className="text-gray-300 mb-2 block">Mitigations</Label>
              <div className="space-y-2 mb-2">
                {form.mitigations.map((m: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2 text-sm">
                    <CheckCircle2 className="w-3.5 h-3.5 text-green-400 flex-shrink-0" />
                    <span className="text-gray-300 flex-1">{m.action ?? m}</span>
              ...
              <div className="flex gap-2">
                <Input value={newMitigation} onChange={e => setNewMitigation(e.target.value)} className="bg-gray-800 border-gray-700 text-white text-sm" placeholder="Add a mitigation action..." onKeyDown={e => { if (e.key === "Enter") { if (newMitigation.trim()) { setForm(p => ({ ...p, mitigations: [...p.mitigations, { action: newMitigation.trim(), priority: "medium" }] })); setNewMitigation(""); } } }} />
                <Button type="button" size="sm" onClick={() => { if (newMitigation.trim()) { setForm(p => ({ ...p, mitigations: [...p.mitigations, { action: newMitigation.trim(), priority: "medium" }] })); setNewMitigation(""); } }} className="bg-gray-700 hover:bg-gray-600"><Plus className="w-4 h-4" /></Button>
              </div>
            </div>
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setStep(2)} className="border-gray-600 text-gray-300"><ChevronLeft className="w-4 h-4 mr-1" /> Back</Button>
        ...
        {step === 4 && (
          <div className="text-center py-10">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">DPIA Saved</h2>
            <p className="text-gray-400 mb-6">Your Data Protection Impact Assessment has been recorded (ID: {savedId}).</p>
        ...
        {(assessments as any[]).length > 0 && step === 0 && (
          <div className="mt-8">
            <h3 className="text-white font-semibold mb-3">Recent Assessments</h3>
              ...
              {(assessments as any[]).slice(0, 5).map((a: any) => (
                <div key={a.id} className="bg-gray-900 border border-gray-800 rounded-lg px-4 py-3 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-gray-400 text-xs">{a.created_by_name} · {new Date(a.created_at).toLocaleDateString()}</div>
                  </div>
