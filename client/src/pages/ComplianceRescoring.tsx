    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><BarChart3 className="w-6 h-6 text-teal-400" /> Compliance Rescoring</h1>
            <p className="text-slate-400 text-sm mt-1">Track compliance score history and trigger batch rescoring runs</p>
          </div>
        ...
        <div className="flex items-center gap-4">
          <Select value={String(orgId)} onValueChange={v => setOrgId(Number(v))}>
            <SelectTrigger className="w-64 bg-slate-700 border-slate-600 text-white"><SelectValue placeholder="Select Organization" /></SelectTrigger>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">6-Month Trend</p>
            </CardContent>
          </Card>
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-white">{(history as any[]).length}</p>
              <p className="text-xs text-slate-400 mt-1">Score Records</p>
            </CardContent>
          </Card>
        </div>

        <Card className="bg-slate-800 border-slate-700">
          <CardHeader><CardTitle className="text-white flex items-center gap-2"><TrendingUp className="w-5 h-5 text-teal-400" /> Score History</CardTitle></CardHeader>
          <CardContent>
                ...
                {(history as any[]).map((h: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4 p-3 bg-slate-700/50 rounded-lg">
                      <div className="text-slate-400 text-xs w-24">{h.scored_at ? new Date(String(h.scored_at)).toLocaleDateString("en-NG") : "—"}</div>
                      ...
                      <div className="text-xs text-slate-400 w-16">{h.scored_by ?? "system"}</div>
                    </div>
                  );
                })}
              </div>
