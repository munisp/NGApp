...
import {
  Shield, Users, Search, UserCog, Loader2
} from "lucide-react";
          ...
          <div className="flex gap-1">
            {(["all", "admin", "user"] as const).map(f => (
              <Button
                key={f}
                size="sm"
                variant={(!roleFilter && f === "all") || roleFilter === f ? "default" : "outline"}
                ...
                <tr key={u.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    {u.id !== user?.id && (
                      ...
                      >
                        {updateRole.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : u.role === "admin" ? "Demote" : "Promote"}
                      </Button>
                    )}
                  </td>
                </tr>
