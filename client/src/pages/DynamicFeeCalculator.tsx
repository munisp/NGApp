...
export default function DynamicFeeCalculator() {
  const { data: stats, isLoading, refetch } = trpc.dynamicFeeCalculator.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Real-time fee computation with tiered pricing and CBN-compliant structures</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalFeeStructures ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Active</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.activeFeeStructures ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Fee Revenue</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.totalFeeRevenue ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Avg Rate</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.avgFeeRate ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Fee Types</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.feeTypes ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Last Updated</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.lastUpdated ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Pending Changes</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.pendingChanges ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Compliance</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.complianceStatus ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for fee calculator.</p>
