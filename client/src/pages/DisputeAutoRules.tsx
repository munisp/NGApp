...
import { Scale, Plus, Gavel, RefreshCw, AlertTriangle, FileSearch, ArrowUpRight } from "lucide-react";

const actionLabels: Record<string, { label: string; color: string; icon: any }> = {
  auto_refund: { label: "Auto Refund", color: "text-green-400", icon: RefreshCw },
  auto_reject: { label: "Auto Reject", color: "text-red-400", icon: AlertTriangle },
  escalate_to_supervisor: { label: "Escalate", color: "text-yellow-400", icon: ArrowUpRight },
  request_evidence: { label: "Request Evidence", color: "text-blue-400", icon: FileSearch },
};
...
export default function DisputeAutoRules() {
  ...
  const [newOperator, setNewOperator] = useState<"eq" | "gt" | "lt" | "contains">("lt");
  const [newValue, setNewValue] = useState("");
  const [newAction, setNewAction] = useState<"auto_refund" | "auto_reject" | "escalate_to_supervisor" | "request_evidence">("auto_refund");
  ...
  const testResult = trpc.sprint23.disputeAutoRules.evaluate.useQuery(
    { amount: parseFloat(testAmount) || 0, reason: testReason, category: "transaction" },
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader><DialogTitle>Create Auto-Resolution Rule</DialogTitle></DialogHeader>
              <div className="space-y-4 pt-2">
                ...
                <Select value={newAction} onValueChange={(v) => setNewAction(v as any)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="escalate_to_supervisor">Escalate to Supervisor</SelectItem>
                    <SelectItem value="request_evidence">Request Evidence</SelectItem>
                  </SelectContent>
                </Select>
                <Input type="number" placeholder="Max amount for auto-resolve" value={newMaxAmount} onChange={(e) => setNewMaxAmount(e.target.value)} />
                <Button
                  className="w-full"
                  onClick={() => createMutation.mutate({
                    name: newName,
                    condition: { field: newField, operator: newOperator, value: newField === "amount" ? parseFloat(newValue) : newValue },
                    action: newAction,
                    ...
                    return (
                      <tr key={rule.id} className="border-b border-border/50 hover:bg-muted/30">
                        ...
                        <td className="text-center py-2 px-3">
                          <div className={`flex items-center justify-center gap-1 ${actionInfo?.color}`}>
                            {Icon && <Icon className="w-3 h-3" />}
                            <span className="text-xs">{actionInfo?.label}</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-3">
                          {rule.maxAmount === Infinity ? "Unlimited" : `₦${rule.maxAmount.toLocaleString()}`}
                        </td>
                        <td className="text-center py-2 px-3">
                          <Badge variant="outline">{rule.resolutionCount}</Badge>
                        </td>
                        <td className="text-center py-2 px-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Input type="number" placeholder="Dispute amount" value={testAmount} onChange={(e) => setTestAmount(e.target.value)} />
              <Input placeholder="Dispute reason" value={testReason} onChange={(e) => setTestReason(e.target.value)} />
              <div className="p-2 rounded-lg bg-muted/30 text-sm">
                {testResult.data ? (
                  <div className={actionLabels[testResult.data.action]?.color}>
                    <strong>{actionLabels[testResult.data.action]?.label}</strong>
                    <span className="text-muted-foreground ml-2">via: {testResult.data.rule.name}</span>
                  </div>
                ) : (
                  <span className="text-muted-foreground">No matching rule — manual review required</span>
                )}
              </div>
            </div>
          </CardContent>
