...
export default function CarrierSlaDashboard() {
  ...
  // @ts-ignore — Sprint 85: pre-existing type mismatch from router/page interface
  const compliance = trpc.carrierSla.getComplianceReport.useQuery({ period: "weekly" });
      ...
      <div className="container py-6 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Uptime, latency, and packet loss tracking per carrier</p>
          </div>
        </div>
        ...
        {targets.data && (
          <Card>
            <CardHeader><CardTitle>SLA Targets</CardTitle></CardHeader>
            <CardContent>
                  ...
                  <tbody>
                    ...
                    {targets.data.map((t: any) => (
                      <tr key={t.carrier} className="border-b hover:bg-muted/30">
                        ...
                        <td className="p-2 text-right">{t.maxPacketLossPct}%</td>
                        <td className="p-2 text-right font-bold" style={{ color: t.actualUptime >= t.uptimeTarget ? "#22c55e" : "#ef4444" }}>
                          {t.actualUptime}%
                        </td>
                        <td className="p-2 text-center">
                          {t.compliant ? <CheckCircle className="h-4 w-4 text-green-500 mx-auto" /> : <XCircle className="h-4 w-4 text-red-500 mx-auto" />}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {violations.data && violations.data.length > 0 && (
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><XCircle className="h-5 w-5 text-red-500" /> Recent Violations (24h)</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-2">
                {violations.data.map((v: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-red-500/10">
                    <div>
                      <p className="font-medium">{v.carrier} — {v.region}</p>
                      <p className="text-sm text-muted-foreground">{v.violation}</p>
                    </div>
                    <div className="text-right">
                      <Badge variant="destructive">{v.severity}</Badge>
                      <p className="text-xs text-muted-foreground mt-1">{v.timestamp}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {compliance.data && (
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Clock className="h-5 w-5" /> Weekly Compliance Report</CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div className="text-center p-4 rounded-lg bg-muted/50">
                  <p className="text-3xl font-bold text-green-500">{compliance.data.compliantCarriers}</p>
                  <p className="text-sm text-muted-foreground">Compliant Carriers</p>
                </div>
                <div className="text-center p-4 rounded-lg bg-muted/50">
                  <p className="text-3xl font-bold text-red-500">{compliance.data.nonCompliantCarriers}</p>
                  <p className="text-sm text-muted-foreground">Non-Compliant</p>
                </div>
                <div className="text-center p-4 rounded-lg bg-muted/50">
                  <p className="text-3xl font-bold">{compliance.data.overallScore}%</p>
                  <p className="text-sm text-muted-foreground">Overall SLA Score</p>
                </div>
              </div>
              <div className="space-y-2">
                {compliance.data.details.map((d: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-2 rounded bg-muted/30">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant={d.compliant ? "default" : "destructive"}>{d.compliant ? "Pass" : "Fail"}</Badge>
                    </div>
                  </div>
                ))}
              </div>
