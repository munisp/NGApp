...
export default function PlatformMigrationToolkit() {
  const { data: stats, isLoading, refetch } = trpc.platformMigrationToolkit.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Data migration, system cutover, and rollback management tools</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalMigrations ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Completed</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.completedMigrations ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">In Progress</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.inProgressMigrations ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Records Migrated</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.totalRecordsMigrated ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Failed Records</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.totalFailedRecords ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Success %</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.overallSuccessRate ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Scheduled</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.scheduledMigrations ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Integrity %</p>
              <p className="text-xl font-bold text-orange-400">{String(stats?.dataIntegrityScore ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for migration toolkit.</p>
