      ...
      <div className="container py-6 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Connection quality, protocol selection, bandwidth monitoring</p>
          </div>
        </div>
        ...
        {metrics.data && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <Card><CardContent className="pt-6 text-center">
              ...
              <p className="text-2xl font-bold mt-2">{metrics.data.totalConnections}</p>
              <p className="text-xs text-muted-foreground">Total Connections</p>
            </CardContent></Card>
            <Card><CardContent className="pt-6 text-center">
              <Zap className="h-6 w-6 text-green-500 mx-auto" />
              <p className="text-2xl font-bold mt-2">{metrics.data.activeWebSocket}</p>
              <p className="text-xs text-muted-foreground">WebSocket</p>
            </CardContent></Card>
            <Card><CardContent className="pt-6 text-center">
              <p className="text-2xl font-bold text-yellow-500">{metrics.data.activeSSE}</p>
              <p className="text-xs text-muted-foreground">SSE</p>
            </CardContent></Card>
            <Card><CardContent className="pt-6 text-center">
              <p className="text-2xl font-bold text-orange-500">{metrics.data.activeLongPoll}</p>
              <p className="text-xs text-muted-foreground">Long-Poll</p>
            </CardContent></Card>
            <Card><CardContent className="pt-6 text-center">
              <p className="text-2xl font-bold text-red-500">{metrics.data.offlineAgents}</p>
        ...
        {resilience.data && (
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Globe className="h-5 w-5" /> Resilience Status</CardTitle></CardHeader>
            <CardContent>
                ...
                {resilience.data.regions.map((r: any) => (
                  <div key={r.region} className="p-4 rounded-lg bg-muted/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant={r.status === "healthy" ? "default" : r.status === "degraded" ? "secondary" : "destructive"}>
                        {r.status}
                      </Badge>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div><span className="text-muted-foreground">Latency:</span> {r.avgLatencyMs}ms</div>
                      <div><span className="text-muted-foreground">Loss:</span> {r.packetLossPct}%</div>
                      <div><span className="text-muted-foreground">Bandwidth:</span> {r.bandwidthKbps} kbps</div>
                    </div>
                    <div className="mt-2 text-xs text-muted-foreground">
                      Protocol: {r.recommendedProtocol} | Agents: {r.activeAgents}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Diagnostic Tool */}
        <Card>
          <CardHeader><CardTitle>Run Diagnostic</CardTitle></CardHeader>
          <CardContent>
            ...
            <div className="p-4 rounded-lg bg-muted/50 font-mono text-sm min-h-[200px]">
              <p className="text-muted-foreground">Select a diagnostic tool above to begin testing...</p>
