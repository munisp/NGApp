...
export default function AgentLogin() {
  ...
  const handleCodeSubmit = () => {
    if (agentCode.trim().length < 3) { toast.error("Enter a valid agent code"); return; }
    setStep("pin");
  };
  ...
  const handlePinKey = (key: string) => {
    ...
    if (newPin.length === 4) {
      setLoading(true);
      loginMutation.mutate({ agentCode: agentCode.trim().toUpperCase(), pin: newPin });
    }
  };
  ...
  const requestOtpMutation = trpc.pinReset.requestOtp.useMutation({
    onSuccess: () => {
      setOtpSent(true);
      toast.success("If your details match, an OTP has been sent to your phone");
  ...
  const handleRequestOtp = () => {
    if (resetAgentCode.trim().length < 3) { toast.error("Enter your agent code"); return; }
    if (resetPhone.trim().length < 10)    { toast.error("Enter your registered phone number"); return; }
    setLoading(true);
  ...
  const resetPinMutation = trpc.pinReset.resetPin.useMutation({
    onSuccess: () => {
      ...
      setResetPhone(""); setResetOtp(""); setResetNewPin("");
      setResetNewPinConfirm(""); setResetAgentCode(""); setOtpSent(false);
      setLoading(false);
    },
    onError: (err: { message: string }) => {
      toast.error(err.message);
  ...
  const handleResetPin = () => {
    if (resetOtp.trim().length !== 6)          { toast.error("Enter the 6-digit OTP"); return; }
    if (resetNewPin.length !== 4)              { toast.error("PIN must be 4 digits"); return; }
    if (resetNewPin !== resetNewPinConfirm)    { toast.error("PINs do not match"); return; }
    setLoading(true);
  ...
  const handleNewPinKey = (key: string, target: "new" | "confirm") => {
    ...
    if (key === "DEL") { setter((p) => p.slice(0, -1)); return; }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6"
      ...
      <div className="mb-8 text-center">
        ...
        <div className="text-2xl font-bold text-white" style={{ fontFamily: DISP }}>54Link POS</div>
        <div className="text-xs text-gray-500 mt-1">Agency Banking Terminal</div>
      </div>
      ...
      {step === "code" && (
        <div className="w-full max-w-sm">
          ...
          <input type="text" value={agentCode}
            ...
            style={{ background: CARD, border: `1px solid ${BORDER}`, fontFamily: MONO, letterSpacing: "0.15em" }}
            autoFocus />
          ...
          <button onClick={() => { setResetAgentCode(agentCode); setStep("forgot_phone"); }}
          ...
          <button
            onClick={async () => {
              try {
                const probe = await fetch('/api/auth/login', { method: 'GET', redirect: 'manual' });
                if (probe.status === 503) {
                  const body = await probe.json().catch(() => ({}));
                  toast.error(body.message ?? 'Keycloak SSO is not configured on this server.');
                  return;
                }
              } catch { /* network error — proceed with redirect */ }
            ...
            style={{ background: CARD, border: `1px solid ${BORDER}`, color: "#a3b3cc", fontFamily: DISP }}>
          ...
          <div className="text-center mt-2 text-xs text-gray-700">Powered by Keycloak OIDC</div>
        </div>
      )}
      ...
      {step === "pin" && (
        <div className="w-full max-w-xs">
          <div className="text-center mb-6">
            ...
            <div className="text-base font-bold text-white" style={{ fontFamily: MONO }}>{agentCode}</div>
          </div>
          <div className="flex justify-center gap-4 mb-8">
            {[0,1,2,3].map((i: any) => (
              <div key={i} className="w-4 h-4 rounded-full transition-all"
                style={{ background: i < pin.length ? BLUE : BORDER,
          ...
          <div className="grid grid-cols-3 gap-3">
            {PAD_KEYS.map((key, idx) => (
              <button key={idx} onClick={() => key && handlePinKey(key)}
                ...
                style={{ background: key === "DEL" ? BORDER : CARD, color: key === "DEL" ? RED : "white",
                  border: `1px solid ${BORDER}`, fontFamily: MONO, cursor: key ? "pointer" : "default" }}>
                {key === "DEL" ? "⌫" : key}
              </button>
            ))}
          </div>
          {loading && <div className="text-center mt-6 text-sm text-gray-400">Authenticating…</div>}
          <button onClick={() => { setStep("code"); setPin(""); }}
      ...
      {step === "forgot_phone" && (
        <div className="w-full max-w-sm">
          <div className="text-center mb-6">
            ...
            <div className="text-xs text-gray-500">Enter your agent code and registered phone to receive an OTP</div>
          </div>
          <div className="text-sm text-gray-400 mb-2">Agent Code</div>
          <input type="text" value={resetAgentCode}
            ...
            style={{ background: CARD, border: `1px solid ${BORDER}`, fontFamily: MONO, letterSpacing: "0.1em" }} />
          <div className="text-sm text-gray-400 mb-2">Registered Phone Number</div>
          <input type="tel" value={resetPhone}
      ...
      {step === "forgot_newpin" && (
        <div className="w-full max-w-sm">
          ...
          <input type="text" value={resetOtp}
            ...
            style={{ background: CARD, border: `1px solid ${resetOtp.length === 6 ? GREEN : BORDER}`, fontFamily: MONO }} />

          {/* New PIN */}
          <div className="text-sm text-gray-400 mb-2 text-center">New 4-Digit PIN</div>
          <div className="flex justify-center gap-4 mb-4">
            {[0,1,2,3].map((i: any) => (
              <div key={i} className="w-4 h-4 rounded-full transition-all"
                style={{ background: i < resetNewPin.length ? GREEN : BORDER,
                  transform: i < resetNewPin.length ? "scale(1.2)" : "scale(1)" }} />
            ))}
          </div>

          {/* Confirm PIN */}
          <div className="text-sm text-gray-400 mb-2 text-center">Confirm PIN</div>
          <div className="flex justify-center gap-4 mb-5">
            {[0,1,2,3].map((i: any) => (
              <div key={i} className="w-4 h-4 rounded-full transition-all"
                style={{
                  background: i < resetNewPinConfirm.length
                    ? (resetNewPin.startsWith(resetNewPinConfirm) ? GREEN : RED)
                    : BORDER,
                  transform: i < resetNewPinConfirm.length ? "scale(1.2)" : "scale(1)",
                }} />
            ))}
          </div>
          ...
          <button onClick={handleResetPin}
            disabled={loading || resetOtp.length !== 6 || resetNewPin.length !== 4 || resetNewPinConfirm.length !== 4}
            className="w-full py-4 rounded-2xl font-bold text-base transition-all active:scale-95 disabled:opacity-50"
            style={{
              background: resetNewPin === resetNewPinConfirm && resetNewPin.length === 4 ? GREEN : "#374151",
              color: resetNewPin === resetNewPinConfirm && resetNewPin.length === 4 ? "#000" : "#9ca3af",
              fontFamily: DISP,
            }}>
            {loading ? "Resetting…" : resetNewPin !== resetNewPinConfirm && resetNewPinConfirm.length === 4
              ? "PINs don't match" : "✓ Reset PIN"}
          </button>

          <button onClick={() => setStep("forgot_phone")}
