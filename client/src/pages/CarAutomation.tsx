    ...
    <DashboardLayout>
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">CAR Automation</h1>
            <p className="text-sm text-gray-400">Compliance Audit Return — Automated generation and submission (NDPA 2023 Section 43)</p>
          </div>
        </div>

        {/* Generation panel */}
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
          <h2 className="font-semibold text-white mb-4 flex items-center gap-2"><Sparkles className="w-5 h-5 text-yellow-400" /> Generate New CAR</h2>
          <div className="flex items-end gap-4 flex-wrap">
            <div>
              <label className="text-gray-400 text-xs mb-1.5 block">Organisation</label>
              <select value={selectedOrgId} onChange={e => setSelectedOrgId(Number(e.target.value))} className="bg-gray-800 border border-gray-700 text-white rounded-md px-3 py-2 text-sm min-w-[200px]">
                {(orgs as any[]).map((o: any) => <option key={o.id} value={o.id}>{o.name}</option>)}
              </select>
            </div>
            <div>
              <label className="text-gray-400 text-xs mb-1.5 block">Reporting Year</label>
              <select value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))} className="bg-gray-800 border border-gray-700 text-white rounded-md px-3 py-2 text-sm">
        ...
        {generatedReport && (
          <div className="bg-gray-900 border border-cyan-800 rounded-xl p-6 mb-6">
            ...
            {generatedReport.sections?.length > 0 && (
              <div>
                <div className="text-xs text-gray-400 mb-2 font-medium uppercase tracking-wide">Report Sections</div>
                  ...
                  {generatedReport.sections.map((s: any, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-sm">
                      ...
                      <div>
                        ...
                        {s.summary && <span className="text-gray-400 ml-2">{s.summary}</span>}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* Historical CARs */}
        <div>
          <h2 className="font-semibold text-white mb-3">Historical CARs</h2>
          {(cars as any[]).length === 0 ? (
            <div className="text-center py-10 text-gray-500 bg-gray-900 border border-gray-800 rounded-xl">
            ...
            <div className="space-y-3">
              {(cars as any[]).map((car: any) => (
                <div key={car.id} className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4 flex items-center justify-between">
                  <div>
                    <div className="font-medium text-white">{car.title ?? `CAR ${car.reporting_year}`}</div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[car.status] ?? "text-gray-400 bg-gray-800"}`}>
