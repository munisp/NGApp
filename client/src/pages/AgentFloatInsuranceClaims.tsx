...
export default function AgentFloatInsuranceClaims() {
  const { data: stats, isLoading, refetch } = trpc.agentFloatInsuranceClaims.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Agent float insurance claim lifecycle management and processing</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalClaims ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Approved</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.approvedClaims ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Pending</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.pendingClaims ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Rejected</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.rejectedClaims ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Claimed Amount</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.totalClaimedAmount ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Paid Amount</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.totalPaidAmount ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-orange-400">{String(stats?.claimApprovalRate ?? "—")}</p>
            </div>
            </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6">
              ...
              <div className="text-gray-400 text-sm">
                <p>This module provides comprehensive management capabilities for insurance claims.</p>
