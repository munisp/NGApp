...
export default function AgentKycDocVault() {
  const { data: stats, isLoading, refetch } = trpc.agentKycDocVault.getStats.useQuery();
      ...
      <div className="p-6 space-y-6 min-h-screen bg-gray-950 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Secure document storage with OCR verification and compliance tracking</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-blue-400">{String(stats?.totalDocuments ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Verified</p>
              <p className="text-xl font-bold text-emerald-400">{String(stats?.verified ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Pending</p>
              <p className="text-xl font-bold text-amber-400">{String(stats?.pending ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Rejected</p>
              <p className="text-xl font-bold text-rose-400">{String(stats?.rejected ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">OCR Confidence</p>
              <p className="text-xl font-bold text-purple-400">{String(stats?.avgOcrConfidence ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Doc Types</p>
              <p className="text-xl font-bold text-cyan-400">{String(stats?.documentTypes ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs text-gray-400 mb-1">Compliance %</p>
              <p className="text-xl font-bold text-indigo-400">{String(stats?.complianceRate ?? "—")}</p>
            </div>
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
