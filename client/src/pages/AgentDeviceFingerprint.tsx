      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Device fingerprinting for fraud prevention and agent verification</p>
          </div>
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(list.data?.devices ?? []).filter((item: any) =>
                    !search || JSON.stringify(item).toLowerCase().includes(search.toLowerCase())
                  ).map((item: any) => (
                    <tr key={item.id || item.agentId} className="border-b hover:bg-muted/50">
                      <td className="py-3 px-2"><div className="font-medium">{item.serial}</div><div className="text-xs text-muted-foreground">{item.id}</div></td>
                      <td className="py-3 px-2">{item.agentId}</td>
                      <td className="py-3 px-2">{item.model}</td>
                      <td className="py-3 px-2">{item.trustScore}</td>
                      <td className="py-3 px-2"><Badge variant={item.status === "verified" ? "outline" : "secondary"}>{item.status || "—"}</Badge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
