...
export default function AgentPerformanceScoring() {
  ...
  const selected = useMemo(() => mockAgents.find((a: any) => a.agentId === selectedAgent), [selectedAgent]);
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(["platinum", "gold", "silver", "bronze"] as const).map((tier: any) => (
            <Card key={tier} className={tierBg[tier]}>
              <CardContent className="pt-4 text-center">
                <Medal className={`w-6 h-6 mx-auto mb-1 ${tierColors[tier]}`} />
                <p className="text-xl font-bold">{mockAgents.filter((a: any) => a.tier === tier).length}</p>
