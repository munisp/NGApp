...
export default function RateAlertAnalytics() {
  ...
  const expiredAlerts = alerts?.filter(a => a.status === 'expired').length || 0;

  // Calculate average time to trigger based on history data
  const avgTimeToTrigger = history?.length ? history.reduce((sum, h) => {
    if (h.triggeredAt) {
      const triggeredTime = new Date(h.triggeredAt).getTime();
      const createdTime = h.createdAt ? new Date(h.createdAt).getTime() : triggeredTime - (24 * 60 * 60 * 1000);
      return sum + (triggeredTime - createdTime);
    }
    return sum;
  }, 0) / (history.filter(h => h.triggeredAt).length || 1) : 0;
  ...
  const currencyPairs = alerts?.reduce((acc, alert) => {
  ...
  // Condition breakdown
  const conditions = alerts?.reduce((acc, alert) => {
    acc[alert.condition] = (acc[alert.condition] || 0) + 1;
    return acc;
  }, {} as Record<string, number>) || {};

  // Most popular target rates
  const targetRates = alerts?.map(a => ({
