...
import { Search, Share2, GitFork, Eye, LayoutGrid, Copy, ExternalLink, Lock, Unlock, Users } from "lucide-react";
...
export default function SharedLayoutGallery() {
  ...
  const [sortBy, setSortBy] = useState<"popular" | "recent" | "forks">("popular");
  const [selectedTag, setSelectedTag] = useState<string | undefined>();
  const [importDialog, setImportDialog] = useState<{ open: boolean; token: string }>({ open: false, token: "" });
  const [shareTokenInput, setShareTokenInput] = useState("");

  const { data, isLoading } = trpc.sharedLayouts.gallery.useQuery(
    { search: search || undefined, tag: selectedTag, sortBy }
  );
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <Button variant="outline" onClick={() => setImportDialog({ open: true, token: "" })}>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {layouts.map((layout: any) => {
              ...
              return (
                <Card key={layout.id} className="hover:border-primary/30 transition-colors">
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-base">{layout.name}</CardTitle>
                        <CardDescription className="text-xs mt-1">{layout.description}</CardDescription>
                      </div>
                  ...
                  <CardContent className="pb-2">
                    ...
                    <div className="bg-muted/30 rounded-md p-2 h-20 grid grid-cols-12 gap-1">
                      {layout.widgets.slice(0, 4).map((w: string, i: number) => (
                        <div
                          key={i}
                    ...
                    <div className="flex items-center gap-3 mt-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><Users className="w-3 h-3" />{layout.ownerName}</span>
                      <span className="flex items-center gap-1"><Eye className="w-3 h-3" />{layout.viewCount}</span>
                      <span className="flex items-center gap-1"><GitFork className="w-3 h-3" />{layout.forkCount}</span>
                    </div>
                  ...
                  <CardFooter className="pt-2 gap-2">
                    <Button
                      ...
                      className="flex-1"
                      onClick={() => {
                        navigator.clipboard.writeText(`${window.location.origin}/shared-layout/${layout.shareToken}`);
                    ...
                    {layout.permission !== "view-only" && (
                      <Button
                        size="sm"
                        className="flex-1"
                        onClick={() => forkMutation.mutate({ shareToken: layout.shareToken, userId: "me", userName: "You" })}
                      >
        ...
        {/* Import dialog */}
        <Dialog open={importDialog.open} onOpenChange={(o) => setImportDialog({ open: o, token: "" })}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Enter the share token (e.g., tok_abc123) or the full share URL from a team member.
              </p>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setImportDialog({ open: false, token: "" })}>Cancel</Button>
              <Button
                onClick={() => {
                  const token = shareTokenInput.includes("/") ? shareTokenInput.split("/").pop()! : shareTokenInput;
