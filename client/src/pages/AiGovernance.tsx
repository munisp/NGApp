  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">AI Governance Registry</h1>
          <p className="text-gray-400 text-sm mt-1">Register and audit AI systems operated by regulated organizations</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[{ label: "Total Systems", value: stats.total, icon: Bot, color: "text-blue-400" }, { label: "High Risk", value: stats.high, icon: AlertTriangle, color: "text-red-400" }, { label: "Processes PII", value: stats.pii, icon: AlertTriangle, color: "text-orange-400" }, { label: "Under Review", value: stats.pending, icon: Clock, color: "text-yellow-400" }].map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="bg-gray-800 rounded-xl border border-gray-700 p-4">
            <div className="flex items-center gap-2 mb-2"><Icon className={`w-5 h-5 ${color}`} /><span className="text-gray-400 text-sm">{label}</span></div>
      ...
      <div className="flex gap-3">
        <Select value={riskFilter} onValueChange={setRiskFilter}>
          <SelectTrigger className="w-48 bg-gray-800 border-gray-700 text-white"><SelectValue placeholder="Filter by risk..." /></SelectTrigger>
          <SelectContent>
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-700 bg-gray-900/50">
            <tr>{["System Name", "Vendor", "Purpose", "Risk Level", "PII", "Status", "Actions"].map(h => <th key={h} className="text-left px-4 py-3 text-gray-400 font-medium">{h}</th>)}</tr>
          </thead>
          <tbody>
            {(systems as any[]).length === 0 ? (
              ...
              <tr key={s.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3 text-gray-400">{s.vendor || "—"}</td>
                <td className="px-4 py-3 text-gray-400 max-w-xs truncate">{s.purpose || "—"}</td>
                <td className="px-4 py-3"><Badge className={`text-xs border ${RISK_COLORS[s.riskLevel] || "bg-gray-700 text-gray-300"}`}>{s.riskLevel}</Badge></td>
                ...
                <td className="px-4 py-3 flex gap-2">
                  <Button size="sm" variant="outline" className="text-xs border-gray-600" onClick={() => { setShowUpdate(s); setUpdateForm({ status: s.status, riskLevel: s.riskLevel, auditNotes: s.auditNotes || "" }); }}>Review</Button>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="bg-gray-900 border-gray-700 text-white">
          ...
          <div className="space-y-3">
            ...
            <div><Label>System Name</Label><Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="bg-gray-800 border-gray-700 mt-1" /></div>
            <div><Label>Vendor</Label><Input value={form.vendor} onChange={e => setForm(p => ({ ...p, vendor: e.target.value }))} className="bg-gray-800 border-gray-700 mt-1" /></div>
            <div><Label>Purpose</Label><Input value={form.purpose} onChange={e => setForm(p => ({ ...p, purpose: e.target.value }))} className="bg-gray-800 border-gray-700 mt-1" /></div>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="bg-gray-900 border-gray-700 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-400">This will permanently remove the AI system from the registry. This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={!!showUpdate} onOpenChange={() => setShowUpdate(null)}>
        <DialogContent className="bg-gray-900 border-gray-700 text-white">
          ...
          <div className="space-y-3">
            ...
            <div><Label>Audit Notes</Label><Textarea value={updateForm.auditNotes} onChange={e => setUpdateForm(p => ({ ...p, auditNotes: e.target.value }))} className="bg-gray-800 border-gray-700 mt-1" rows={3} /></div>
          </div>
          <DialogFooter>
