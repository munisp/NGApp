...
export function GdprConsentBanner({ agentId, forceShow = false }: GdprConsentBannerProps) {
  ...
  const erasureMutation = trpc.gdpr.requestErasure.useMutation({
    onSuccess: () => {
      toast.success("Erasure request submitted. You will be notified when complete.");
  ...
  const handleRequestErasure = () => {
    if (!confirm("⚠️ This will permanently anonymise your personal data. Continue?")) return;
    erasureMutation.mutate({ reason: "User-requested via consent banner", confirmPhrase: "DELETE MY DATA" });
  };
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: "linear-gradient(to top, oklch(0.10 0.015 230), transparent)" }}
      >
        <div
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-bold text-white mb-2" style={{ fontFamily: DISP }}>
                Your Privacy Matters — NDPR/GDPR Compliance
              </h3>
              <p className="text-sm text-gray-400 mb-4" style={{ fontFamily: DISP }}>
                We process your personal data (name, phone, BVN, transaction history) to provide financial services
                in compliance with the <strong>Nigeria Data Protection Regulation (NDPR)</strong> and <strong>GDPR</strong>.
                By continuing, you consent to this processing. You can export or request erasure of your data at any time.
              </p>
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  ...
                  style={{ background: "oklch(0.25 0.015 230)", color: "white", fontFamily: DISP }}
                >
                  Manage My Data
                </button>
                <a
                  ...
                  className="px-6 py-2.5 rounded-lg font-semibold text-sm transition-all"
                  style={{ background: "transparent", color: BLUE, border: `1px solid ${BLUE}`, fontFamily: DISP }}
                >
                  Privacy Policy
                </a>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
          ...
          <div className="max-w-2xl w-full rounded-2xl p-8 shadow-2xl"
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white" style={{ fontFamily: DISP }}>
                Manage My Data
              </h2>
              <button
            ...
            <div className="flex flex-col gap-4">
              {/* Export data */}
              <div className="rounded-xl p-6" style={{ background: "oklch(0.15 0.015 230)", border: `1px solid ${BORDER}` }}>
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center"
                    style={{ background: "oklch(0.60 0.22 260 / 0.2)" }}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-400 mb-4" style={{ fontFamily: DISP }}>
                      Download a JSON file containing all your personal data: profile, transactions, audit log, loyalty history, and KYC sessions.
                    </p>
                    <button
                      onClick={handleExportData}
                      disabled={exportMutation.isLoading || exportMutation.isFetching}
                      ...
                      style={{ background: BLUE, color: "white", fontFamily: DISP }}
                    >
                      {exportMutation.isLoading || exportMutation.isFetching ? "Exporting..." : "Download My Data"}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl p-6" style={{ background: "oklch(0.15 0.015 230)", border: `1px solid ${BORDER}` }}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-400 mb-4" style={{ fontFamily: DISP }}>
                      Request permanent anonymisation of your personal data. Your transaction history will be preserved
                      for audit compliance (CBN requirement), but all PII (name, phone, email, BVN, NIN) will be erased.
                    </p>
                    <button
                      onClick={handleRequestErasure}
                      disabled={isErasurePending}
                      className="px-6 py-2.5 rounded-lg font-semibold text-sm transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                      style={{ background: RED, color: "white", fontFamily: DISP }}
                    >
              ...
              <p className="text-xs text-gray-500 mt-4" style={{ fontFamily: MONO }}>
                All requests are logged in the audit trail. Erasure requests are processed within 30 days as required by NDPR Article 2.1(1)(d).
              </p>
            </div>
          </div>
        </div>
