...
export default function FloatTopUpTab() {
  ...
  const [rejectModal, setRejectModal] = useState<{ id: number; agentCode: string } | null>(null);
  const [rejectReason, setRejectReason] = useState("");
  const [approveModal, setApproveModal] = useState<{ id: number; agentCode: string; amount: number } | null>(null);
  const [approveNotes, setApproveNotes] = useState("");

  const { data: requests, refetch, isLoading } = trpc.agentMgmt.listTopUpRequests.useQuery({ status: filter });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Pending Requests", value: (requests ?? []).filter((r) => r.status === "pending").length, color: GOLD },
          { label: "Approved Today", value: (requests ?? []).filter((r) => r.status === "approved").length, color: GREEN },
          { label: "Rejected Today", value: (requests ?? []).filter((r) => r.status === "rejected").length, color: RED },
        ].map((c) => (
      ...
      <div className="overflow-x-auto rounded-xl" style={{ border: `1px solid ${BORDER}` }}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-32 gap-2">
            ...
            <div className="text-sm text-gray-500" style={{ fontFamily: DISP }}>No {filter} requests</div>
          </div>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr style={{ background: CARD, borderBottom: `1px solid ${BORDER}` }}>
                {["Agent", "Name", "Current Float", "Requested Amount", "Status", "Submitted", "Notes", "Actions"].map((h) => (
            ...
            <tbody>
              {(requests ?? []).map((req, i) => (
                <tr key={req.id}
                  ...
                  <td className="px-3 py-3 text-white" style={{ fontFamily: DISP }}>
                    {req.agentName ?? "—"}
                  </td>
                  <td className="px-3 py-3" style={{ color: GREEN, fontFamily: MONO }}>
                    {fmt(req.agentFloat ?? 0)}
                  </td>
                  <td className="px-3 py-3 font-black text-base" style={{ color: GOLD, fontFamily: MONO }}>
                  ...
                  <td className="px-3 py-3">
                    <span className="px-2 py-0.5 rounded-full text-xs font-semibold capitalize"
                      style={{
                        background:
                          req.status === "approved" ? "oklch(0.65 0.18 160 / 0.15)"
                          : req.status === "rejected" ? "oklch(0.60 0.22 25 / 0.15)"
                  ...
                  <td className="px-3 py-3 text-gray-500" style={{ fontFamily: MONO }}>
                    {new Date(req.createdAt).toLocaleString("en-NG", {
                      day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit",
                    })}
                  </td>
                  <td className="px-3 py-3 text-gray-500 max-w-32 truncate" style={{ fontFamily: DISP }}>
                    {req.notes ?? "—"}
                  </td>
                  <td className="px-3 py-3">
                    {req.status === "pending" && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => setApproveModal({ id: req.id, agentCode: req.agentCode ?? "", amount: req.requestedAmount })}
                          className="px-3 py-1 rounded-lg text-xs font-bold text-white"
                          style={{ background: GREEN, fontFamily: DISP }}>
                          Approve
                        </button>
                        <button
                          onClick={() => setRejectModal({ id: req.id, agentCode: req.agentCode ?? "" })}
                          className="px-3 py-1 rounded-lg text-xs font-bold text-white"
      ...
      {approveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="rounded-2xl p-6 w-96 flex flex-col gap-4" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
            <div className="text-base font-black text-white" style={{ fontFamily: DISP }}>
              Approve Float Top-Up
            </div>
            <div className="rounded-xl p-4 flex flex-col gap-2" style={{ background: BG }}>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500" style={{ fontFamily: DISP }}>Agent</span>
                <span className="font-bold" style={{ color: BLUE, fontFamily: MONO }}>{approveModal.agentCode}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500" style={{ fontFamily: DISP }}>Amount to Credit</span>
                <span className="font-black text-lg" style={{ color: GREEN, fontFamily: MONO }}>{fmt(approveModal.amount)}</span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => approveMut.mutate({ requestId: approveModal.id, notes: approveNotes || undefined })}
                disabled={approveMut.isPending}
                className="flex-1 py-2 rounded-xl text-sm font-bold text-white"
                style={{ background: GREEN, fontFamily: DISP, opacity: approveMut.isPending ? 0.5 : 1 }}>
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="rounded-2xl p-6 w-96 flex flex-col gap-4" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500" style={{ fontFamily: DISP }}>Reason for rejection *</label>
              <textarea
                value={rejectReason}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  rejectMut.mutate({ requestId: rejectModal.id, reason: rejectReason });
                }}
                disabled={rejectMut.isPending}
                className="flex-1 py-2 rounded-xl text-sm font-bold text-white"
                style={{ background: RED, fontFamily: DISP, opacity: rejectMut.isPending ? 0.5 : 1 }}>
