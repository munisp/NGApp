...
function SimSlotBadge({ slot, isActive }: { slot: string; isActive: boolean }) {
  return (
    <div className="flex flex-col items-center px-3 py-2 rounded-xl text-xs font-bold"
      style={{
        background: isActive ? "oklch(0.60 0.22 260 / 0.25)" : "oklch(0.14 0.02 240)",
...
// ─── Agent probe card ─────────────────────────────────────────────────────────
function AgentProbeCard({ agentCode, probes }: { agentCode: string; probes: ProbeRow[] }) {
  ...
  const chartData = useMemo(() => {
    const times = Array.from(new Set(probes.map(p => p.probedAt.toString()))).sort();
    return times.map(ts => {
      const row: Record<string, number | string> = {
        time: new Date(ts).toLocaleTimeString("en-NG", { hour: "2-digit", minute: "2-digit" }),
      };
      probes.filter(p => p.probedAt.toString() === ts).forEach(p => {
    ...
    <div className="rounded-2xl p-5 flex flex-col gap-4"
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="text-sm font-black text-white" style={{ fontFamily: DISP }}>
            Agent <span style={{ color: BLUE, fontFamily: MONO }}>{agentCode}</span>
            {latest?.terminalId && (
              <span className="ml-2 text-xs text-gray-500" style={{ fontFamily: MONO }}>
                ({latest.terminalId})
              </span>
            )}
          </div>
          {latest && (
            <div className="text-xs text-gray-500 mt-0.5" style={{ fontFamily: MONO }}>
              ...
              {latest.fwVersion && (
                <span className="ml-2 text-gray-600">fw {latest.fwVersion}</span>
              )}
            </div>
          )}
        </div>
        <div className="flex gap-2 flex-wrap">
          {Object.keys(SLOT_LABELS).map(slot => (
            <SimSlotBadge key={slot} slot={slot} isActive={slot === activeSlot} />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {Object.entries(bySlot).map(([slot, slotProbes]) => {
          ...
          return (
            <div key={slot} className="rounded-xl p-3 flex flex-col gap-1"
              style={{
                background: BG,
                border: `1px solid ${p.selected ? BLUE : BORDER}`,
              }}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold" style={{ color: carrierColor(p.carrier), fontFamily: DISP }}>
                  {p.carrier}
                </span>
                <span className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold"
                  style={{ background: `${sig.color}22`, color: sig.color, fontFamily: DISP }}>
                  {sig.label}
                </span>
              </div>
              <div className="text-xs text-gray-500 mt-0.5" style={{ fontFamily: MONO }}>
              ...
              <div className="text-lg font-black" style={{ color: sig.color, fontFamily: MONO }}>
                {p.rssi} dBm
              </div>
              <div className="text-xs text-gray-500 flex gap-2" style={{ fontFamily: MONO }}>
                <span>{p.latencyMs}ms</span>
                <span>{packetLoss}% loss</span>
              </div>
              <div className="text-xs mt-1" style={{ fontFamily: DISP }}>
                Score: <span className="font-bold" style={{ color: GOLD }}>{p.score}</span>
              </div>
            </div>
          );
        })}
      ...
      {chartData.length > 1 && (
        <div>
          <div className="text-xs font-semibold text-gray-400 mb-2" style={{ fontFamily: DISP }}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={chartData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="time"
                tick={{ fill: "oklch(0.55 0.015 230)", fontSize: 10, fontFamily: MONO }} />
              <YAxis domain={[-110, -50]}
                tick={{ fill: "oklch(0.55 0.015 230)", fontSize: 10, fontFamily: MONO }} />
              <Tooltip
                contentStyle={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 8, fontFamily: MONO, fontSize: 11 }}
...
// ─── Config editor ────────────────────────────────────────────────────────────
function OrchestratorConfigPanel({ terminalId, onClose }: { terminalId: string; onClose: () => void }) {
  const { data: cfg, isLoading } = trpc.simOrchestrator.listConfigs.useQuery(undefined, {
    select: (configs) => configs.find(c => c.terminalId === terminalId),
  });
  const upsert = trpc.simOrchestrator.upsertConfig.useMutation({
    onSuccess: () => { toast.success("Config saved"); onClose(); },
    onError: (e: any) => toast.error(e.message),
  });

  const [apiKey, setApiKey]         = useState("54link-sim-orchestrator-default-key");
  const [intervalMs, setIntervalMs] = useState(30000);
  const [relayEndpoint, setRelayEndpoint] = useState("https://api.54link.io/api/trpc/simOrchestrator.ingestProbe");
  ...
  useMemo(() => {
    if (cfg && !populated) {
      ...
      setRelayEndpoint(cfg.relayEndpoint ?? "https://api.54link.io/api/trpc/simOrchestrator.ingestProbe");
  ...
  if (isLoading) return (
    <div className="text-center py-8 text-gray-500" style={{ fontFamily: DISP }}>Loading config...</div>
  );
  ...
  return (
    <div className="rounded-2xl p-5 flex flex-col gap-4"
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm font-black text-white" style={{ fontFamily: DISP }}>
          Config — <span style={{ color: BLUE, fontFamily: MONO }}>{terminalId}</span>
        </div>
        <button onClick={onClose} className="text-xs text-gray-500 hover:text-white px-2 py-1 rounded"
          style={{ fontFamily: DISP }}>✕ Close</button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-gray-400" style={{ fontFamily: DISP }}>API Key</label>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-gray-400" style={{ fontFamily: DISP }}>Probe Interval (ms)</label>
          <input type="number" min={5000} max={300000} step={1000} value={intervalMs}
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs text-gray-400" style={{ fontFamily: DISP }}>Relay Endpoint</label>
        <input value={relayEndpoint} onChange={e => setRelayEndpoint(e.target.value)}
          className="px-3 py-2 rounded-lg text-xs text-white outline-none"
          style={{ background: BG, border: `1px solid ${BORDER}`, fontFamily: MONO }} />
      </div>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          style={{
            background: enabled ? "oklch(0.65 0.18 160 / 0.2)" : "oklch(0.60 0.22 25 / 0.2)",
        ...
        <button
          onClick={() => upsert.mutate({ terminalId, apiKey, probeIntervalMs: intervalMs, relayEndpoint, enabled })}
          disabled={upsert.isPending}
          className="flex-1 py-2 rounded-xl text-xs font-bold transition-all"
          style={{ background: BLUE, color: "white", fontFamily: DISP, opacity: upsert.isPending ? 0.6 : 1 }}>
...
export function SimOrchestratorTab() {
  const [activeTab, setActiveTab] = useState<"probes" | "map" | "failovers">("probes");
  ...
  const { data: configs } = trpc.simOrchestrator.listConfigs.useQuery(undefined, {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <div className="text-xs text-gray-500 mt-0.5" style={{ fontFamily: DISP }}>
            Intelligent multi-SIM network selection — {terminalIds.length} terminal{terminalIds.length !== 1 ? "s" : ""} configured
          </div>
        </div>
        <div className="flex items-center gap-2">
          {/* Tab switcher */}
          <div className="flex items-center gap-1 rounded-xl p-1 mr-2" style={{ background: BG, border: `1px solid ${BORDER}` }}>
            {(["probes", "map", "failovers"] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
                style={{
                  background: activeTab === tab ? "oklch(0.60 0.22 260 / 0.3)" : "transparent",
                  color: activeTab === tab ? "white" : "oklch(0.55 0.015 230)",
                  fontFamily: DISP,
                }}>
                {tab === "probes" ? "📶 Probes" : tab === "map" ? "🗺️ Coverage Map" : "⚠️ Failovers"}
              </button>
            ))}
          </div>
          {activeTab === "probes" && <label className="text-xs text-gray-400" style={{ fontFamily: DISP }}>History:</label>}
          {activeTab === "probes" && ([1, 6, 24, 72] as const).map(h => (
        ...
        <>
          ...
          {terminalIds.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs text-gray-500" style={{ fontFamily: DISP }}>Terminals:</span>
              {terminalIds.map(tid => (
                <button key={tid}
                  onClick={() => setConfigTerminal(configTerminal === tid ? null : tid)}
                  className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
                  style={{
                    background: configTerminal === tid ? "oklch(0.78 0.18 80 / 0.2)" : BG,
                    color: configTerminal === tid ? GOLD : "oklch(0.55 0.015 230)",
          ...
          {/* Loading */}
          {isLoading && (
            <div className="text-center py-12 text-gray-500" style={{ fontFamily: DISP }}>
              Loading probe data...
            </div>
          )}
          ...
          {!isLoading && agentIds.length === 0 && (
            <div className="rounded-2xl p-10 text-center flex flex-col items-center gap-3"
              ...
              <div className="text-sm font-bold text-white" style={{ fontFamily: DISP }}>No probe data yet</div>
              <div className="text-xs text-gray-500 max-w-sm" style={{ fontFamily: DISP }}>
                Deploy the Rust SIM Orchestrator daemon on your POS terminals. It will POST probe data to:
              </div>
              <div className="mt-1 px-4 py-2 rounded-xl text-xs font-semibold"
                style={{ background: "oklch(0.60 0.22 260 / 0.15)", color: BLUE, border: `1px solid ${BLUE}`, fontFamily: MONO }}>
