...
export function FailoverHistoryTab() {
  ...
  const { data, isLoading, refetch } = trpc.simOrchestrator.getFailoverHistory.useQuery(
    {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button
          ...
          style={{ background: "oklch(0.60 0.22 260 / 0.15)", color: BLUE, border: `1px solid ${BLUE}44`, fontFamily: DISP }}>
          ↻ Refresh
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Avg Latency", value: avgLatency > 0 ? `${avgLatency}ms` : "—", color: GREEN },
        ].map(({ label, value, color }) => (
          <div key={label} className="rounded-2xl p-4 flex flex-col gap-1"
            style={{ background: CARD, border: `1px solid ${BORDER}` }}>
            <div className="text-xs text-gray-500" style={{ fontFamily: DISP }}>{label}</div>
            <div className="text-xl font-black" style={{ color, fontFamily: DISP }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      {allTerminals.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-gray-500" style={{ fontFamily: DISP }}>Filter:</span>
          <button
          ...
          {allTerminals.map(tid => (
            <button key={tid}
              onClick={() => setTerminalFilter(terminalFilter === tid ? "" : tid)}
              className="px-3 py-1 rounded-lg text-xs font-semibold transition-all"
              style={{
                background: terminalFilter === tid ? "oklch(0.60 0.22 260 / 0.25)" : BG,
                color: terminalFilter === tid ? "white" : "oklch(0.55 0.015 230)",
      ...
      {!isLoading && rows.length === 0 && (
        <div className="rounded-2xl p-10 text-center flex flex-col items-center gap-3"
          ...
          <div className="text-sm font-bold text-white" style={{ fontFamily: DISP }}>
            No failovers recorded
          </div>
          <div className="text-xs text-gray-500 max-w-sm" style={{ fontFamily: DISP }}>
            The watchdog daemon will POST here when it performs an emergency SIM switch.
            All terminals are currently operating within thresholds.
          </div>
        </div>
      )}
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${BORDER}` }}>
          <div className="overflow-x-auto">
            <table className="w-full text-xs" style={{ fontFamily: MONO }}>
              <thead>
                <tr style={{ background: "oklch(0.12 0.02 240)", borderBottom: `1px solid ${BORDER}` }}>
                  {["Terminal", "Agent", "From", "→ To", "Reason", "Latency", "Loss", "TX Ref", "Switched At"].map(h => (
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={row.id}
                    ...
                    <td className="px-3 py-2 font-bold" style={{ color: BLUE }}>{row.terminalId}</td>
                    <td className="px-3 py-2" style={{ color: "oklch(0.75 0.015 230)" }}>{row.agentCode}</td>
                    <td className="px-3 py-2"><SlotBadge name={row.fromSlotName} /></td>
                    <td className="px-3 py-2"><SlotBadge name={row.toSlotName} /></td>
                    <td className="px-3 py-2"><ReasonBadge reason={row.reason} /></td>
                    <td className="px-3 py-2">
                      <span style={{ color: row.latencyMs > 3000 ? RED : GREEN }}>
                        {row.latencyMs.toLocaleString()}ms
                      </span>
                    </td>
                    <td className="px-3 py-2">
                      <span style={{ color: row.lossPercent > 20 ? RED : GREEN }}>
                        {row.lossPercent.toFixed(1)}%
                      </span>
                    </td>
                    <td className="px-3 py-2" style={{ color: "oklch(0.50 0.015 230)" }}>
                      {row.txRef ?? "—"}
                    </td>
                    <td className="px-3 py-2" style={{ color: "oklch(0.50 0.015 230)" }}>
