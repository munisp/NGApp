...
import { X, ChevronDown, ChevronUp, MessageSquare, Send, Trash2, Loader2 } from "lucide-react";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";

// ─── Types ───────────────────────────────────────────────────────────────────
type AnnouncementType = "info" | "warning" | "critical" | "maintenance" | "feature";
...
const TYPE_STYLES: Record<AnnouncementType, { bg: string; border: string; icon: string }> = {
  ...
  maintenance: { bg: "bg-purple-500/10", border: "border-purple-500/30", icon: "🔧" },
  feature:     { bg: "bg-emerald-500/10",border: "border-emerald-500/30",icon: "✨" },
};
...
function AnnouncementBar({ ann, onDismiss }: { ann: MappedAnnouncement; onDismiss: () => void }) {
  ...
  const { data: reactionsData, isLoading: reactionsLoading } = trpc.announcementReactions.getReactions.useQuery(
  ...
  const reactMutation = trpc.announcementReactions.react.useMutation({
    onMutate: async ({ emoji }) => {
      await utils.announcementReactions.getReactions.cancel({ announcementId: ann.id });
      const prev = utils.announcementReactions.getReactions.getData({ announcementId: ann.id });
      if (prev) {
        const reactionEntry = prev.reactions[emoji];
        const userReacted = reactionEntry?.users?.includes(CURRENT_USER_ID) ?? false;
        const updatedReactions = { ...prev.reactions };
        updatedReactions[emoji] = {
          count: userReacted ? (reactionEntry?.count ?? 1) - 1 : (reactionEntry?.count ?? 0) + 1,
          users: userReacted
            ? (reactionEntry?.users ?? []).filter((u: string) => u !== CURRENT_USER_ID)
    ...
    onError: (_err, _vars, ctx) => {
      if (ctx?.prev) {
        utils.announcementReactions.getReactions.setData({ announcementId: ann.id }, ctx.prev);
      }
    },
    onSettled: () => {
      utils.announcementReactions.getReactions.invalidate({ announcementId: ann.id });
    },
  });
  ...
  const deleteCommentMutation = trpc.announcementReactions.deleteComment.useMutation({
    onSuccess: () => {
      utils.announcementReactions.getReactions.invalidate({ announcementId: ann.id });
    },
  });
  ...
  return (
    <div className={cn("border-b px-4 py-2 transition-all", style.bg, style.border)}>
      {/* Main bar */}
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {!expanded && (
            <span className="text-xs text-muted-foreground ml-2 truncate">{(ann.message || "").slice(0, 80)}…</span>
          )}
        </div>
  ...
  return (
    <div className="w-full z-50">
      {visibleAnnouncements.map((ann: any) => (
        <AnnouncementBar key={ann.id} ann={ann} onDismiss={() => handleDismiss(ann.id)} />
      ))}
    </div>
  );
}
