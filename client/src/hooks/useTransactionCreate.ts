...
function buildFallbackUssd(txType: string, amount: number): string {
  ...
  switch (txType) {
    case "Transfer":      return `*966*2*${amt}#`;
    case "Cash Out":      return `*966*3*${amt}#`;
    case "Airtime":       return `*966*5*${amt}#`;
    case "Bill Payment":  return `*966*4*${amt}#`;
    default:              return `*966*${amt}#`;
  }
}
...
export function useTransactionCreate() {
  ...
  const submit = async (input: CreateTxInput): Promise<TxResult | null> => {
    ...
    if (!isOnline) {
      ...
      toast.warning("Offline — transaction queued for sync when connection is restored.");
      ...
      (async () => {
        try {
          ...
          const ussdString = (ussdResult as any).ussd_string ?? buildFallbackUssd(input.type, input.amount);
          const instructions = (ussdResult as any).instructions ?? `Dial ${ussdString} to pay via USSD.`;
          ...
          toast.info("USSD receipt queued for printing when printer reconnects");
        }
      })();

      setIsProcessing(false);
      const offlineRef = `OFL-${Date.now().toString(36).toUpperCase()}`;
      return { ref: offlineRef, commission: 0, pointsEarned: 0, floatBalance: 0 };
    }
