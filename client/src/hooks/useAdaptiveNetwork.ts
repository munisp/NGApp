...
const FEATURE_MATRIX: Record<NetworkTier, AdaptiveFeatures> = {
  "5g_wifi": {
    ...
    maxListPageSize: 100, textOnlyMode: false, enableOfflineQueue: true,
    compressionHint: "none", syncIntervalMs: 5000, requestTimeoutMs: 10000,
    maxRetries: 3, useSmssFallback: false, useUssdFallback: false,
  },
  "4g_lte": {
    useWebSocket: true, usePolling: false, pollingIntervalMs: 0,
    loadImages: true, loadCharts: true, enableAnimations: true,
    maxListPageSize: 50, textOnlyMode: false, enableOfflineQueue: true,
    compressionHint: "gzip", syncIntervalMs: 10000, requestTimeoutMs: 15000,
    maxRetries: 3, useSmssFallback: false, useUssdFallback: false,
  },
  "3g": {
    useWebSocket: false, usePolling: true, pollingIntervalMs: 30000,
    loadImages: false, loadCharts: false, enableAnimations: false,
    maxListPageSize: 25, textOnlyMode: false, enableOfflineQueue: true,
    compressionHint: "gzip", syncIntervalMs: 30000, requestTimeoutMs: 30000,
    maxRetries: 5, useSmssFallback: false, useUssdFallback: false,
  },
  "2g_edge": {
    useWebSocket: false, usePolling: true, pollingIntervalMs: 60000,
    loadImages: false, loadCharts: false, enableAnimations: false,
    maxListPageSize: 10, textOnlyMode: true, enableOfflineQueue: true,
    compressionHint: "deflate", syncIntervalMs: 60000, requestTimeoutMs: 60000,
    maxRetries: 10, useSmssFallback: true, useUssdFallback: false,
  },
  "2g_gprs": {
    useWebSocket: false, usePolling: true, pollingIntervalMs: 120000,
    loadImages: false, loadCharts: false, enableAnimations: false,
    maxListPageSize: 5, textOnlyMode: true, enableOfflineQueue: true,
    compressionHint: "deflate", syncIntervalMs: 120000, requestTimeoutMs: 120000,
    maxRetries: 15, useSmssFallback: true, useUssdFallback: true,
  },
  "offline": {
    useWebSocket: false, usePolling: false, pollingIntervalMs: 0,
    loadImages: false, loadCharts: false, enableAnimations: false,
    maxListPageSize: 5, textOnlyMode: true, enableOfflineQueue: true,
    compressionHint: "none", syncIntervalMs: 0, requestTimeoutMs: 0,
    maxRetries: 0, useSmssFallback: true, useUssdFallback: true,
  },
};
...
function signalBars(tier: NetworkTier): number {
  const bars: Record<NetworkTier, number> = {
    "5g_wifi": 4, "4g_lte": 3, "3g": 2, "2g_edge": 1, "2g_gprs": 1, "offline": 0,
  };
  return bars[tier];
}
...
export function useAdaptiveNetwork(probeIntervalMs = 15000) {
  ...
  const [features, setFeatures] = useState<AdaptiveFeatures>(FEATURE_MATRIX["4g_lte"]);
  const lastTierChange = useRef(Date.now());
  const probeHistory = useRef<number[]>([]);

  // Probe server latency
  const probeLatency = useCallback(async (): Promise<{ latencyMs: number; success: boolean }> => {
    try {
  ...
  const updateStatus = useCallback(async () => {
    ...
    const avg = probeHistory.current.reduce((a, b) => a + b, 0) / probeHistory.current.length;
    const jitterMs = Math.round(
      Math.sqrt(
        probeHistory.current.reduce((sum, v) => sum + (v - avg) ** 2, 0) / probeHistory.current.length
      )
    );
    ...
    const tier = detectTier(effectiveType, downlinkMbps, actualRtt, packetLossPct, isOnline);

    const state: ConnectionState = !isOnline || tier === "offline"
      ? "offline"
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      setStatus(prev => ({ ...prev, isOnline: false, tier: "offline", state: "offline", signalStrength: 0 }));
