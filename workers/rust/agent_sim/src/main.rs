...
fn simulate_agents(req: &ABMRequest) -> ABMResponse {
    ...
    let trajectories: Vec<AgentTrajectory> = req.agents.par_iter().map(|agent| {
        ...
        for month in 1..=req.duration_months {
            ...
            let improvement = gap * 0.02 * (budget_effect + staff_effect + tech_effect)
                * risk_factor * (1.0 + penalty_pressure);

            // Peer pressure from same-sector agents (network effect)
            let peer_effect = if network {
                let sector_avg: f64 = req.agents.iter()
                    .filter(|a| a.sector == agent.sector && a.id != agent.id)
                    .map(|a| a.compliance_score)
                    .sum::<f64>()
                    / req.agents.iter().filter(|a| a.sector == agent.sector && a.id != agent.id).count().max(1) as f64;
            ...
            current = (current + improvement + peer_effect + shock + breach_shock).clamp(0.0, 100.0);
    ...
    let avg_initial = trajectories.iter().map(|t| t.initial_compliance).sum::<f64>() / total;
    let avg_final = trajectories.iter().map(|t| t.final_compliance).sum::<f64>() / total;
    let total_breaches = trajectories.iter().filter(|t| t.breach_probability > 10.0).count() as i32;
    let total_penalties = trajectories.iter()
        .map(|t| if t.breach_probability > 10.0 { t.breach_probability * 50000.0 } else { 0.0 })
        .sum::<f64>();

    // Sector summary
    let mut sectors: Vec<String> = trajectories.iter().map(|t| format!("{}_{}", t.jurisdiction, t.sector)).collect();
    sectors.sort();
    sectors.dedup();
    let sector_summary: Vec<SectorSummary> = sectors.iter().map(|key| {
        let parts: Vec<&str> = key.splitn(2, '_').collect();
        let (j, s) = (parts[0], parts[1]);
        let agents: Vec<&AgentTrajectory> = trajectories.iter()
            .filter(|t| t.jurisdiction == j && t.sector == s).collect();
        let count = agents.len();
        SectorSummary {
            sector: s.to_string(),
            jurisdiction: j.to_string(),
            agent_count: count,
            avg_initial: (agents.iter().map(|a| a.initial_compliance).sum::<f64>() / count as f64 * 100.0).round() / 100.0,
            avg_final: (agents.iter().map(|a| a.final_compliance).sum::<f64>() / count as f64 * 100.0).round() / 100.0,
            delta: ((agents.iter().map(|a| a.compliance_delta).sum::<f64>() / count as f64) * 100.0).round() / 100.0,
            breaches: agents.iter().filter(|a| a.breach_probability > 10.0).count() as i32,
        }
    }).collect();
    ...
    let interactions = vec![
        InteractionEvent {
            month: 3, agent_a: "Banking-1".into(), agent_b: "Banking-2".into(),
            interaction_type: "peer_pressure".into(),
            effect: "Banking-2 increased compliance investment after seeing Banking-1 audit results".into(),
        },
        InteractionEvent {
            month: 6, agent_a: "Telecom-1".into(), agent_b: "Banking-1".into(),
