...
const FIRMWARE_VERSIONS = [
  "1.0.0", "1.1.0", "1.2.0", "1.2.1", "1.3.0", "2.0.0"
];
...
function randomFirmwareVersion() {
  return FIRMWARE_VERSIONS[Math.floor(Math.random() * FIRMWARE_VERSIONS.length)];
}
      ...
      const dlOk = check(dlRes, {
        ...
        "download: has downloadUrl":   r => {
          try {
            ...
            return typeof b.downloadUrl === "string" && b.downloadUrl.startsWith("http");
          } catch { return false; }
        },
...
export function setup() {
  ...
  const res = http.get(`${OTA_SERVICE_URL}/api/v1/ota/health`, { timeout: "5s" });
  if (res.status !== 200) {
    console.warn(`OTA service health check failed (status ${res.status}). Tests may fail.`);
  }
  console.log(`OTA Load Test: ${DEVICE_COUNT} simulated devices, base URL: ${OTA_SERVICE_URL}`);
