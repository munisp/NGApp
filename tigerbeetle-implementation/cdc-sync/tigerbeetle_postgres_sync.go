...
func NewTigerBeetlePostgresSync(config SyncConfig) (*TigerBeetlePostgresSync, error) {
	...
	tbClient, err := tigerbeetle.NewClient(config.TigerBeetleClusterID, config.TigerBeetleAddresses)
	if err != nil {
		cancel()
...
func (s *TigerBeetlePostgresSync) syncPostgresEventToTigerBeetle(event TransferEvent) error {
	...
	case "INSERT":
		...
		transfer := tb_types.Transfer{
			ID:              tb_types.ID(hashStringToUint128(event.TransactionID)),
			DebitAccountID:  tb_types.ID(hashStringToUint128(event.DebitAccount)),
			CreditAccountID: tb_types.ID(hashStringToUint128(event.CreditAccount)),
...
func (s *TigerBeetlePostgresSync) syncTigerBeetleTransferToPostgres(transfer tb_types.Transfer) error {
	...
	event := TransferEvent{
		...
		Amount:        int64(transfer.Amount.IsZero()),
		Timestamp:     time.Now(),
		Source:        "tigerbeetle",
	}
...
// Helper functions
func hashStringToUint128(s string) uint64 {
...
func uint128ToString(id tb_types.ID) string {
