...
func (a *Activities) SendCompensationNotificationActivity(ctx context.Context, req CompensationNotificationRequest) error {
	...
	err := a.NotificationService.SendCompensationNotification(ctx, notificationReq, req.Reason)
	if err != nil {
...
// Additional methods for PolicyRepository interface to support compensations

// PolicyRepository interface extension for compensating activities
type PolicyRepositoryExtended interface {
	PolicyRepository
	Delete(ctx context.Context, policyID string) error
}

// DocumentService interface extension for compensating activities
type DocumentServiceExtended interface {
	DocumentServiceClient
	DeletePolicyDocument(ctx context.Context, policyID string) error
}

// NotificationService interface extension for compensating activities
type NotificationServiceExtended interface {
	NotificationServiceClient
	SendCompensationNotification(ctx context.Context, req models.NotificationRequest, reason string) error
}
