...
func TestTierDetermination(t *testing.T) {
	ts := NewTierDeterminationService()
	ts.SetCurrentTier(1, TierStarter)

	// Update metrics that qualify for Growth
	ts.UpdateMetrics(ParticipantMetrics{
		ParticipantID:       1,
		MonthlyVolumeNGN:    2_000_000_000, // ₦2B
		AvgMonthlyVolume3Mo: 1_500_000_000, // ₦1.5B avg
		MonthsOnPlatform:    6,
		SanctionsBlocks90d:  0,
		TransferSuccessRate: 0.98,
		PrefundConsistency:  0.90,
	})

	// Should propose upgrade to Growth
	request := ts.EvaluateTier(1)
	if request == nil {
		t.Fatal("expected tier change request")
	}
	if request.ProposedTier != TierGrowth {
		t.Errorf("expected growth, got %s", request.ProposedTier)
	}
	if !request.AutoGenerated {
		t.Error("should be auto-generated")
	}

	// Approve
	approved := ts.ApproveTierChange(request.ID, "admin-001")
	if !approved {
		t.Error("approval should succeed")
	}
	if ts.GetParticipantTier(1) != TierGrowth {
		t.Errorf("tier should be growth after approval, got %s", ts.GetParticipantTier(1))
	}
}

func TestTierDeterminationNoChange(t *testing.T) {
	ts := NewTierDeterminationService()
	ts.SetCurrentTier(1, TierStarter)

	// Metrics insufficient for Growth
	ts.UpdateMetrics(ParticipantMetrics{
		ParticipantID:       1,
		MonthlyVolumeNGN:    100_000_000, // ₦100M — below ₦1B threshold
		AvgMonthlyVolume3Mo: 80_000_000,
		MonthsOnPlatform:    1,
		TransferSuccessRate: 0.90,
		PrefundConsistency:  0.50,
	})

	request := ts.EvaluateTier(1)
	if request != nil {
		t.Error("no tier change should be proposed for insufficient metrics")
	}
}

func TestCorridorAssignment(t *testing.T) {
	ts := NewTierDeterminationService()
	ts.SetCurrentTier(1, TierGrowth)

	cs := NewCorridorAssignmentService(ts)

	// Starter corridors should be available to Growth tier
	available := cs.GetAvailableCorridors(1)
	if len(available) == 0 {
		t.Fatal("expected available corridors for Growth tier")
	}

	// Growth tier can access NG-GH (minTier: starter)
	_, err := cs.AssignCorridor(1, "NG-GH", "admin-001", 500_000_000, 5_000_000_000)
	if err != nil {
		t.Fatalf("should be able to assign NG-GH: %v", err)
	}

	// Growth tier can access NG-GB (minTier: growth)
	_, err = cs.AssignCorridor(1, "NG-GB", "admin-001", 800_000_000, 8_000_000_000)
	if err != nil {
		t.Fatalf("should be able to assign NG-GB: %v", err)
	}

	// Verify assignments
	assigned := cs.GetAssignedCorridors(1)
	if len(assigned) != 2 {
		t.Errorf("expected 2 assigned corridors, got %d", len(assigned))
	}

	// Check access
	if !cs.IsCorridorAllowed(1, "NG-GH") {
		t.Error("NG-GH should be allowed")
	}
	if cs.IsCorridorAllowed(1, "NG-CN") {
		t.Error("NG-CN should not be allowed (not assigned)")
	}
}

func TestCorridorTierRestriction(t *testing.T) {
	ts := NewTierDeterminationService()
	ts.SetCurrentTier(1, TierStarter) // Starter only

	cs := NewCorridorAssignmentService(ts)

	// NG-CN requires Enterprise tier
	_, err := cs.AssignCorridor(1, "NG-CN", "admin-001", 100_000_000, 1_000_000_000)
	if err == nil {
		t.Error("Starter should not be able to access Enterprise-tier corridor NG-CN")
	}
}

func TestCorridorSuspension(t *testing.T) {
	ts := NewTierDeterminationService()
	ts.SetCurrentTier(1, TierGrowth)
	cs := NewCorridorAssignmentService(ts)

	cs.AssignCorridor(1, "NG-GH", "admin-001", 500_000_000, 5_000_000_000)
	if !cs.IsCorridorAllowed(1, "NG-GH") {
		t.Fatal("should be allowed before suspension")
	}

	cs.SuspendCorridor(1, "NG-GH", "Compliance review")
	if cs.IsCorridorAllowed(1, "NG-GH") {
		t.Error("should not be allowed after suspension")
	}
}

func TestFXIntegrationService(t *testing.T) {
	fx := NewFXIntegrationService(BloombergConfig{Enabled: false})

	// Update rate
	fx.UpdateRate("NGN/GHS", 0.00190, 0.00210, SourceBloomberg)

	rate, err := fx.GetRate("NGN/GHS")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rate.Source != SourceBloomberg {
		t.Errorf("expected bloomberg source, got %s", rate.Source)
	}
	if rate.Mid != 0.002 {
		t.Errorf("expected mid 0.002, got %f", rate.Mid)
	}
}

func TestFXSpreadOverride(t *testing.T) {
	fx := NewFXIntegrationService(BloombergConfig{Enabled: false})

	// Default spread for NG-GH is 60 bps
	spread := fx.GetEffectiveSpread("NG-GH")
	if spread != 60 {
		t.Errorf("expected default spread 60, got %d", spread)
	}

	// Apply override
	_, err := fx.ApplySpreadOverride("NG-GH", 40, "Promotional rate for Q2", "admin-001", nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	spread = fx.GetEffectiveSpread("NG-GH")
	if spread != 40 {
		t.Errorf("expected overridden spread 40, got %d", spread)
	}
}

func TestFXSpreadCBNCap(t *testing.T) {
	fx := NewFXIntegrationService(BloombergConfig{Enabled: false})

	// Try to set spread above CBN cap (80 bps for NG-GH)
	_, err := fx.ApplySpreadOverride("NG-GH", 100, "Test", "admin-001", nil)
	if err == nil {
		t.Error("should reject spread above CBN cap")
	}
}

func TestFXRateFreeze(t *testing.T) {
	fx := NewFXIntegrationService(BloombergConfig{Enabled: false})
	fx.UpdateRate("NGN/GHS", 0.00190, 0.00210, SourceBloomberg)
	...
	fx.FreezeAllRates("Market disruption", "admin-001")
	if !fx.IsRateFrozen() {
		t.Error("should be frozen after freeze")
	}

	// Rate should still be accessible when frozen
	rate, err := fx.GetRate("NGN/GHS")
	if err != nil {
		t.Fatalf("frozen rate should still be accessible: %v", err)
	}
	if rate.IsStale {
		t.Error("frozen rate should not be marked stale")
	}

	fx.UnfreezeRates("admin-001")
	if fx.IsRateFrozen() {
		t.Error("should not be frozen after unfreeze")
	}
}

func TestFXAuditLog(t *testing.T) {
	fx := NewFXIntegrationService(BloombergConfig{Enabled: false})

	fx.UpdateRate("NGN/GHS", 0.00190, 0.00210, SourceBloomberg)
	fx.ApplySpreadOverride("NG-GH", 50, "Test", "admin-001", nil)
	fx.FreezeAllRates("Test", "admin-001")

	log := fx.GetAuditLog(time.Now().Add(-1*time.Hour), 100)
	if len(log) < 3 {
