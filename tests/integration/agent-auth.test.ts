...
import { getAgentByCode, updateAgentLastLogin, writeAuditLog } from "../../server/db";
import bcrypt from "bcryptjs";

const mockAgent = {
  id: 1, agentCode: "AGT001", name: "Emeka Obi", phone: "08012345678",
  email: "emeka@test.com", pinHash: "$2b$10$hash", role: "agent" as const,
  tier: "Gold" as const, floatBalance: 500000, commissionBalance: 12000,
  loyaltyPoints: 850, isActive: true, location: "Lagos", createdAt: new Date(),
...
describe("Agent Authentication Flow", () => {
  ...
  describe("agent.login", () => {
    it("returns success and agent profile on valid credentials", async () => {
      ...
      const result = await caller.agent.login({ agentCode: "AGT001", pin: "123456" });
    ...
    it("rejects inactive agent", async () => {
      vi.mocked(getAgentByCode).mockResolvedValue({ ...mockAgent, isActive: false });
      const caller = appRouter.createCaller(makePublicCtx());
      await expect(
  ...
  describe("agent.me", () => {
    it("returns the authenticated agent profile when cookie is valid", async () => {
      ...
      expect(result === null || (result && result.agentCode === "AGT001")).toBe(true);
    });
