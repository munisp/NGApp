...
async function trpcQuery(proc, input = {}) {
  ...
  return {
    status: res.status,
    data: data?.[0]?.result?.data?.json ?? data?.[0]?.result?.data ?? data,
    error: data?.[0]?.error,
  };
}
...
await test("Health includes db/version/uptime", async () => {
  const { data } = await fetchJSON("/api/health");
  if (!data.status || !data.version) throw new Error("Missing fields");
});
...
await test("X-Frame-Options present", async () => {
  const { headers } = await fetchJSON("/api/health");
  if (!headers.get("x-frame-options")) throw new Error("Missing");
});
await test("Content-Security-Policy present", async () => {
  const res = await fetch(`${BASE}/`);
  if (!res.headers.get("content-security-policy")) throw new Error("Missing");
});
await test("X-Request-Id present", async () => {
  const { headers } = await fetchJSON("/api/health");
  if (!headers.get("x-request-id")) throw new Error("Missing");
});

// ── 3. Authentication ──────────────────────────────────────────────────
console.log("\n── 3. Authentication ──");
await test("auth.me returns for unauthenticated", async () => {
  await trpcQuery("auth.me");
});
await test("Agent login rejects invalid credentials", async () => {
  const { error } = await trpcMutation("agent.login", {
    agentCode: "INVALID",
    pin: "0000",
  });
...
const routerTests = [
  ...
  {
    name: "businessRules.getTransactionLimits",
    proc: "businessRules.getTransactionLimits",
  },
  { name: "businessRules.getKycTiers", proc: "businessRules.getKycTiers" },
  {
    name: "businessRules.getCommissionRates",
    proc: "businessRules.getCommissionRates",
  },
  {
    name: "businessRules.getLoyaltyRules",
    proc: "businessRules.getLoyaltyRules",
  },
  {
    name: "businessRules.getFraudScoringWeights",
    proc: "businessRules.getFraudScoringWeights",
  },
  { name: "businessRules.cbnLimits", proc: "businessRules.cbnLimits" },
];

for (const rt of routerTests) {
  await test(`tRPC ${rt.name} responds`, async () => {
    const { status } = await trpcQuery(rt.proc, rt.input || {});
...
const protectedEndpoints = [
  ...
  "fraud.rules",
  "analytics.overview",
  "analytics.agentLeaderboard",
  "mdm.listDevices",
  "mdm.listPolicies",
  "mdm.listOtaReleases",
  "geofencing.listZones",
  "geofencing.stats",
  "loyalty.dashboard",
  "loyalty.history",
  "settlement.list",
...
await test("tRPC returns error for unknown procedure", async () => {
  const res = await fetch(
    `${BASE}/api/trpc/nonexistent.procedure?batch=1&input=${encodeURIComponent(JSON.stringify({ 0: { json: {} } }))}`
  );
  const data = await res.json();
...
await test("Rejects XSS in agent login", async () => {
  const { error } = await trpcMutation("agent.login", {
    agentCode: '<script>alert("xss")</script>',
    pin: "1234",
  });
