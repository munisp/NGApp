// postgres-adapter-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/postgres-adapter/update", handleUpdate)
	http.HandleFunc("/v1/postgres-adapter/process", handleProcess)
	http.HandleFunc("/v1/postgres-adapter/audit", handleAudit)
	http.HandleFunc("/v1/postgres-adapter/stats", handleStats)
	http.HandleFunc("/v1/postgres-adapter/score", postgres_adapterScoreHandler)
	http.HandleFunc("/v1/postgres-adapter/validate", postgres_adapterValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"postgres-adapter-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"postgres-adapter-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/postgres-adapter/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/postgres-adapter/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/postgres-adapter/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/postgres-adapter/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/postgres-adapter/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/postgres-adapter/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/postgres-adapter/score", authMiddleware(postgres_adapterScoreHandler))
	mux.HandleFunc("/v1/postgres-adapter/validate", authMiddleware(postgres_adapterValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("postgres-adapter-go listening on :%s", port)))
