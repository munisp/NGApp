// whatsapp-business-gateway-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/whatsapp-business-gateway/list", handleList)
	http.HandleFunc("/v1/whatsapp-business-gateway/create", handleCreate)
	http.HandleFunc("/v1/whatsapp-business-gateway/update", handleUpdate)
	http.HandleFunc("/v1/whatsapp-business-gateway/process", handleProcess)
	http.HandleFunc("/v1/whatsapp-business-gateway/audit", handleAudit)
	http.HandleFunc("/v1/whatsapp-business-gateway/stats", handleStats)
	http.HandleFunc("/v1/whatsapp-business-gateway/health-score", whatsapp_business_gatewayHealthScoreHandler)
	http.HandleFunc("/v1/whatsapp-business-gateway/circuit-state", whatsapp_business_gatewayCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"whatsapp-business-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"whatsapp-business-gateway-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Messaging/Channels"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/whatsapp-business-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/whatsapp-business-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/whatsapp-business-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/whatsapp-business-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/whatsapp-business-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/whatsapp-business-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/whatsapp-business-gateway/health-score", authMiddleware(whatsapp_business_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/whatsapp-business-gateway/circuit-state", authMiddleware(whatsapp_business_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("whatsapp-business-gateway-go listening on :%s", port)))
