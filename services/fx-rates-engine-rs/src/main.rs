// fx-rates-engine-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8118".into()).parse().unwrap_or(8118);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "fx-rates-engine-rs".into(),
        table_name: "fx_rates_engine".into(),
    });
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/fx-rates-engine/list", web::get().to(list))
            .route("/v1/fx-rates-engine/stats", web::get().to(stats))
            .route("/v1/fx-rates-engine/{id}", web::get().to(get_by_id))
            .route("/v1/fx-rates-engine", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
