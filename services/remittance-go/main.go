// remittance-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"remittance-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"remittance-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/remittance/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/remittance/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/remittance/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/remittance/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/remittance/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/remittance/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/remittance/compute-fee", authMiddleware(remittanceFeeHandler))
	mux.HandleFunc("/v1/remittance/route", authMiddleware(remittanceRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("remittance-go listening on :%s", port)))
