// savings-products-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/savings-products/list", handleList)
	http.HandleFunc("/v1/savings-products/create", handleCreate)
	http.HandleFunc("/v1/savings-products/update", handleUpdate)
	http.HandleFunc("/v1/savings-products/process", handleProcess)
	http.HandleFunc("/v1/savings-products/audit", handleAudit)
	http.HandleFunc("/v1/savings-products/stats", handleStats)
	http.HandleFunc("/v1/savings-products/score", savings_productsScoreHandler)
	http.HandleFunc("/v1/savings-products/validate", savings_productsValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"savings-products-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"savings-products-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/savings-products/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/savings-products/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/savings-products/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/savings-products/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/savings-products/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/savings-products/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/savings-products/score", authMiddleware(savings_productsScoreHandler))
	mux.HandleFunc("/v1/savings-products/validate", authMiddleware(savings_productsValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("savings-products-go listening on :%s", port)))
