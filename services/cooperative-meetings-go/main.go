// cooperative-meetings-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"cooperative-meetings-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cooperative-meetings-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cooperative-meetings/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cooperative-meetings/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cooperative-meetings/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cooperative-meetings/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cooperative-meetings/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cooperative-meetings/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cooperative-meetings/score", authMiddleware(cooperative_meetingsScoreHandler))
	mux.HandleFunc("/v1/cooperative-meetings/validate", authMiddleware(cooperative_meetingsValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cooperative-meetings-go listening on :%s", port)))
