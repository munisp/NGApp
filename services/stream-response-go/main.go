// stream-response-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/stream-response/list", handleList)
	http.HandleFunc("/v1/stream-response/create", handleCreate)
	http.HandleFunc("/v1/stream-response/update", handleUpdate)
	http.HandleFunc("/v1/stream-response/process", handleProcess)
	http.HandleFunc("/v1/stream-response/audit", handleAudit)
	http.HandleFunc("/v1/stream-response/stats", handleStats)
	http.HandleFunc("/v1/stream-response/throughput", stream_responseThroughputHandler)
	http.HandleFunc("/v1/stream-response/partition", stream_responsePartitionHandler)
...
fmt.Fprintf(w, "requests_total{service=\"stream-response-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"stream-response-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/stream-response/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/stream-response/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/stream-response/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/stream-response/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/stream-response/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/stream-response/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/stream-response/throughput", authMiddleware(stream_responseThroughputHandler))
	mux.HandleFunc("/v1/stream-response/partition", authMiddleware(stream_responsePartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("stream-response-go listening on :%s", port)))
