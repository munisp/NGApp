// custom-domain-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"custom-domain-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"custom-domain-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/custom-domain/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/custom-domain/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/custom-domain/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/custom-domain/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/custom-domain/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/custom-domain/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/custom-domain/score", authMiddleware(custom_domainScoreHandler))
	mux.HandleFunc("/v1/custom-domain/validate", authMiddleware(custom_domainValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("custom-domain-go listening on :%s", port)))
