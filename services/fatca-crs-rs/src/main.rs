...
async fn healthz() -> HttpResponse {
    ...
    HttpResponse::Ok().json(serde_json::json!({
        "service": "fatca-crs-rs", "status": "healthy", "version": "1.0.0",
        "middleware": {
            "kafka": { "broker": c.kafka_broker, "topics": ["fatca.filings","crs.reports","compliance.notifications"] },
            "redis": { "url": c.redis_url }, "postgres": { "url": c.postgres_url, "tables": ["fatca_reports","crs_reports","reportable_accounts"] },
            "opensearch": { "url": c.opensearch_url }, "keycloak": { "url": c.keycloak_url, "realm": "54bank" },
            "permify": { "url": c.permify_url }, "dapr": { "url": c.dapr_url, "app_id": "fatca-crs-rs" },
