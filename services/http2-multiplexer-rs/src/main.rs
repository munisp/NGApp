...
// ─── Http2 Multiplexer — Platform/Infra ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "HTT-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Platform/Infra", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "HTT-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Platform/Infra", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "HTT-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Platform/Infra", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("HTT-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Http2 Multiplexer — Platform/Infra",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9502".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Http2 Multiplexer v2.0 (Platform/Infra) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/http2-multiplexer/process", web::post().to(process_record))
            .route("/v1/http2-multiplexer/audit", web::get().to(get_audit))
