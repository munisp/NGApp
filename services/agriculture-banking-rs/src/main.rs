use tokio_postgres;
// agriculture-banking-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM agriculture_banking";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM agriculture_banking ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "agriculture-banking-rs".into(),
        table_name: "agriculture_banking".into(),
    });
    
    println!("[agriculture-banking-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/agriculture-banking/list", web::get().to(list))
            .route("/v1/agriculture-banking/stats", web::get().to(stats))
            .route("/v1/agriculture-banking/{id}", web::get().to(get_by_id))
            .route("/v1/agriculture-banking", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
