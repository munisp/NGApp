// temporal-memoizer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/temporal-memoizer/list", handleList)
	http.HandleFunc("/v1/temporal-memoizer/create", handleCreate)
	http.HandleFunc("/v1/temporal-memoizer/update", handleUpdate)
	http.HandleFunc("/v1/temporal-memoizer/process", handleProcess)
	http.HandleFunc("/v1/temporal-memoizer/audit", handleAudit)
	http.HandleFunc("/v1/temporal-memoizer/stats", handleStats)
	http.HandleFunc("/v1/temporal-memoizer/score", temporal_memoizerScoreHandler)
	http.HandleFunc("/v1/temporal-memoizer/validate", temporal_memoizerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"temporal-memoizer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"temporal-memoizer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/temporal-memoizer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/temporal-memoizer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/temporal-memoizer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/temporal-memoizer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/temporal-memoizer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/temporal-memoizer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/temporal-memoizer/score", authMiddleware(temporal_memoizerScoreHandler))
	mux.HandleFunc("/v1/temporal-memoizer/validate", authMiddleware(temporal_memoizerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("temporal-memoizer-go listening on :%s", port)))
