// aml-training-tracker-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"aml-training-tracker-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"aml-training-tracker-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/aml-training-tracker/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/aml-training-tracker/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/aml-training-tracker/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/aml-training-tracker/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/aml-training-tracker/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/aml-training-tracker/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/aml-training-tracker/screen", authMiddleware(aml_training_trackerScreenHandler))
	mux.HandleFunc("/v1/aml-training-tracker/risk-score", authMiddleware(aml_training_trackerRiskScoreHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("aml-training-tracker-go listening on :%s", port)))
