// risk-scoring-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn main() -> std::io::Result<()> {
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "risk-scoring-rs".into(),
        table_name: "risk_scoring".into(),
    });
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/risk-scoring/list", web::get().to(list))
            .route("/v1/risk-scoring/stats", web::get().to(stats))
            .route("/v1/risk-scoring/{id}", web::get().to(get_by_id))
            .route("/v1/risk-scoring", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
