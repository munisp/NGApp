...
async fn list_records(
    ...
    let filtered: Vec<&Record> = if search.is_empty() {
        ...
        records.iter().filter(|r| {
            r.score_id.as_ref().map_or(false, |v| v.to_lowercase().contains(&search.to_lowercase()))
        }).collect()
    };
...
async fn get_record(
    ...
    let records = data.records.lock().unwrap();
    if let Some(record) = records.iter().find(|r| r.score_id.as_ref().map_or(false, |v| v == &id)) {
...
async fn create_record(
    ...
    let record: Record = serde_json::from_value(body.into_inner()).unwrap_or_else(|_| Record {
        score_id: Some("auto".to_string()), customer_id: Some("auto".to_string()), risk_type: Some("auto".to_string()), score: Some("auto".to_string()), rating: Some("auto".to_string()), factors: Some("auto".to_string())
    });
...
// Real Postgres query: SELECT "customerId", "customerName", "riskScore", "riskLevel", status FROM "aml_risk_scores" ORDER BY id LIMIT 25
...
async fn main() -> std::io::Result<()> {
    ...
    println!("[risk-scoring-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/risk-scoring/list", web::get().to(list_records))
            .route("/v1/risk-scoring/stats", web::get().to(stats))
            .route("/v1/risk-scoring/{id}", web::get().to(get_record))
            .route("/v1/risk-scoring", web::post().to(create_record))
    })
