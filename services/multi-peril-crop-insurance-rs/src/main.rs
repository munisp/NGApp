...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "status": "healthy",
        "domain": "Multi Peril Crop Insurance",
...
async fn list_records() -> HttpResponse {
    HttpResponse::Ok().json(json!({"records": [
        {"id": "REC-001", "status": "active", "domain": "Multi Peril Crop Insurance", "createdAt": "2026-05-09T10:00:00Z"},
        {"id": "REC-002", "status": "processing", "domain": "Multi Peril Crop Insurance", "createdAt": "2026-05-09T11:00:00Z"},
        {"id": "REC-003", "status": "completed", "domain": "Multi Peril Crop Insurance", "createdAt": "2026-05-08T14:00:00Z"},
    ], "total": 3, "domain": "Multi Peril Crop Insurance"}))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9262".to_string());
    let state = AppState { start_time: Instant::now() };
    println!("Multi Peril Crop Insurance (Rust) on :{}", port);
    HttpServer::new(move || {
        App::new()
