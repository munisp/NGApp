// notification-router-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/notification-router/list", handleList)
	http.HandleFunc("/v1/notification-router/create", handleCreate)
	http.HandleFunc("/v1/notification-router/update", handleUpdate)
	http.HandleFunc("/v1/notification-router/process", handleProcess)
	http.HandleFunc("/v1/notification-router/audit", handleAudit)
	http.HandleFunc("/v1/notification-router/stats", handleStats)
	http.HandleFunc("/v1/notification-router/score", notification_routerScoreHandler)
	http.HandleFunc("/v1/notification-router/validate", notification_routerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"notification-router-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"notification-router-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/notification-router/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/notification-router/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/notification-router/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/notification-router/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/notification-router/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/notification-router/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/notification-router/score", authMiddleware(notification_routerScoreHandler))
	mux.HandleFunc("/v1/notification-router/validate", authMiddleware(notification_routerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("notification-router-go listening on :%s", port)))
