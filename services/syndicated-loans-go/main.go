// syndicated-loans-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/syndicated-loans/list", handleList)
	http.HandleFunc("/v1/syndicated-loans/create", handleCreate)
	http.HandleFunc("/v1/syndicated-loans/update", handleUpdate)
	http.HandleFunc("/v1/syndicated-loans/process", handleProcess)
	http.HandleFunc("/v1/syndicated-loans/audit", handleAudit)
	http.HandleFunc("/v1/syndicated-loans/stats", handleStats)
	http.HandleFunc("/v1/syndicated-loans/compute-emi", syndicated_loansEMIHandler)
	http.HandleFunc("/v1/syndicated-loans/credit-check", syndicated_loansCreditCheckHandler)
...
fmt.Fprintf(w, "requests_total{service=\"syndicated-loans-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"syndicated-loans-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/syndicated-loans/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/syndicated-loans/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/syndicated-loans/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/syndicated-loans/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/syndicated-loans/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/syndicated-loans/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/syndicated-loans/compute-emi", authMiddleware(syndicated_loansEMIHandler))
	mux.HandleFunc("/v1/syndicated-loans/credit-check", authMiddleware(syndicated_loansCreditCheckHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("syndicated-loans-go listening on :%s", port)))
