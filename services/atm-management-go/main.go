// atm-management-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/atm-management/update", handleUpdate)
	http.HandleFunc("/v1/atm-management/process", handleProcess)
	http.HandleFunc("/v1/atm-management/audit", handleAudit)
	http.HandleFunc("/v1/atm-management/stats", handleStats)
	http.HandleFunc("/v1/atm-management/score", atm_managementScoreHandler)
	http.HandleFunc("/v1/atm-management/validate", atm_managementValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"atm-management-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"atm-management-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/atm-management/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/atm-management/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/atm-management/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/atm-management/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/atm-management/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/atm-management/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/atm-management/score", authMiddleware(atm_managementScoreHandler))
	mux.HandleFunc("/v1/atm-management/validate", authMiddleware(atm_managementValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("atm-management-go listening on :%s", port)))
