// apisix-gateway-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/apisix-gateway/list", handleList)
	http.HandleFunc("/v1/apisix-gateway/create", handleCreate)
	http.HandleFunc("/v1/apisix-gateway/update", handleUpdate)
	http.HandleFunc("/v1/apisix-gateway/process", handleProcess)
	http.HandleFunc("/v1/apisix-gateway/audit", handleAudit)
	http.HandleFunc("/v1/apisix-gateway/stats", handleStats)
	http.HandleFunc("/v1/apisix-gateway/health-score", apisix_gatewayHealthScoreHandler)
	http.HandleFunc("/v1/apisix-gateway/circuit-state", apisix_gatewayCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"apisix-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"apisix-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/apisix-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/apisix-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/apisix-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/apisix-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/apisix-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/apisix-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/apisix-gateway/health-score", authMiddleware(apisix_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/apisix-gateway/circuit-state", authMiddleware(apisix_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("apisix-gateway-go listening on :%s", port)))
