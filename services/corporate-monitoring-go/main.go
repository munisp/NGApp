// corporate-monitoring-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/corporate-monitoring/list", handleList)
	http.HandleFunc("/v1/corporate-monitoring/create", handleCreate)
	http.HandleFunc("/v1/corporate-monitoring/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"corporate-monitoring-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"corporate-monitoring-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/corporate-monitoring/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/corporate-monitoring/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/corporate-monitoring/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/corporate-monitoring/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/corporate-monitoring/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/corporate-monitoring/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/corporate-monitoring/score", authMiddleware(corporate_monitoringScoreHandler))
	mux.HandleFunc("/v1/corporate-monitoring/validate", authMiddleware(corporate_monitoringValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("corporate-monitoring-go listening on :%s", port)))
