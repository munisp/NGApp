// optimistic-ui-engine-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/optimistic-ui-engine/list", handleList)
	http.HandleFunc("/v1/optimistic-ui-engine/create", handleCreate)
	http.HandleFunc("/v1/optimistic-ui-engine/update", handleUpdate)
	http.HandleFunc("/v1/optimistic-ui-engine/process", handleProcess)
	http.HandleFunc("/v1/optimistic-ui-engine/audit", handleAudit)
	http.HandleFunc("/v1/optimistic-ui-engine/stats", handleStats)
	http.HandleFunc("/v1/optimistic-ui-engine/score", optimistic_ui_engineScoreHandler)
	http.HandleFunc("/v1/optimistic-ui-engine/validate", optimistic_ui_engineValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"optimistic-ui-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"optimistic-ui-engine-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Platform/Infra"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/optimistic-ui-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/optimistic-ui-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/optimistic-ui-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/optimistic-ui-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/optimistic-ui-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/optimistic-ui-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/optimistic-ui-engine/score", authMiddleware(optimistic_ui_engineScoreHandler))
	mux.HandleFunc("/v1/optimistic-ui-engine/validate", authMiddleware(optimistic_ui_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("optimistic-ui-engine-go listening on :%s", port)))
