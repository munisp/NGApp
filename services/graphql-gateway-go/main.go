// graphql-gateway-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/graphql-gateway/list", handleList)
	http.HandleFunc("/v1/graphql-gateway/create", handleCreate)
	http.HandleFunc("/v1/graphql-gateway/update", handleUpdate)
	http.HandleFunc("/v1/graphql-gateway/process", handleProcess)
	http.HandleFunc("/v1/graphql-gateway/audit", handleAudit)
	http.HandleFunc("/v1/graphql-gateway/stats", handleStats)
	http.HandleFunc("/v1/graphql-gateway/health-score", graphql_gatewayHealthScoreHandler)
	http.HandleFunc("/v1/graphql-gateway/circuit-state", graphql_gatewayCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"graphql-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"graphql-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/graphql-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/graphql-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/graphql-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/graphql-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/graphql-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/graphql-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/graphql-gateway/health-score", authMiddleware(graphql_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/graphql-gateway/circuit-state", authMiddleware(graphql_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("graphql-gateway-go listening on :%s", port)))
