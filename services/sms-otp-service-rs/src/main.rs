use tokio_postgres;
// sms-otp-service-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM sms_otp_service";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM sms_otp_service ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8652".into()).parse().unwrap_or(8652);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "sms-otp-service-rs".into(),
        table_name: "sms_otp_service".into(),
    });
    
    println!("[sms-otp-service-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/sms-otp-service/list", web::get().to(list))
            .route("/v1/sms-otp-service/stats", web::get().to(stats))
            .route("/v1/sms-otp-service/{id}", web::get().to(get_by_id))
            .route("/v1/sms-otp-service", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
