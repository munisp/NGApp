// white-label-engine-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/white-label-engine/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"white-label-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"white-label-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/white-label-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/white-label-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/white-label-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/white-label-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/white-label-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/white-label-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/white-label-engine/score", authMiddleware(white_label_engineScoreHandler))
	mux.HandleFunc("/v1/white-label-engine/validate", authMiddleware(white_label_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("white-label-engine-go listening on :%s", port)))
