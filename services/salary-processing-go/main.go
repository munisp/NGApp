// salary-processing-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/salary-processing/list", handleList)
	http.HandleFunc("/v1/salary-processing/create", handleCreate)
	http.HandleFunc("/v1/salary-processing/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"salary-processing-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"salary-processing-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/salary-processing/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/salary-processing/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/salary-processing/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/salary-processing/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/salary-processing/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/salary-processing/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/salary-processing/score", authMiddleware(salary_processingScoreHandler))
	mux.HandleFunc("/v1/salary-processing/validate", authMiddleware(salary_processingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("salary-processing-go listening on :%s", port)))
