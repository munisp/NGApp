...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "WAF-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Security", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "WAF-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Security", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "WAF-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Security", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("WAF-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Waf Rules Engine — Security",
        "middleware": {
            "kafka": "waf-rules-engine.events, waf-rules-engine.audit",
            "postgres": "waf_rules_engine_records",
            "redis": "waf-rules-engine_cache",
            "temporal": "WafRulesEngineWorkflow",
            "opensearch": "waf-rules-engine-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9582".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Waf Rules Engine v2.0 (Security) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/waf-rules-engine/list", web::get().to(list_records))
            .route("/v1/waf-rules-engine/create", web::post().to(create_record))
            .route("/v1/waf-rules-engine/process", web::post().to(process_record))
            .route("/v1/waf-rules-engine/audit", web::get().to(get_audit))
            .route("/v1/waf-rules-engine/stats", web::get().to(get_stats))
