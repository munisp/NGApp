// api-marketplace-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/api-marketplace/list", handleList)
	http.HandleFunc("/v1/api-marketplace/create", handleCreate)
	http.HandleFunc("/v1/api-marketplace/update", handleUpdate)
	http.HandleFunc("/v1/api-marketplace/process", handleProcess)
	http.HandleFunc("/v1/api-marketplace/audit", handleAudit)
	http.HandleFunc("/v1/api-marketplace/stats", handleStats)
	http.HandleFunc("/v1/api-marketplace/fx-convert", api_marketplaceFXHandler)
	http.HandleFunc("/v1/api-marketplace/risk-calc", api_marketplaceRiskHandler)
...
fmt.Fprintf(w, "requests_total{service=\"api-marketplace-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"api-marketplace-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/api-marketplace/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/api-marketplace/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/api-marketplace/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/api-marketplace/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/api-marketplace/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/api-marketplace/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/api-marketplace/fx-convert", authMiddleware(api_marketplaceFXHandler))
	mux.HandleFunc("/v1/api-marketplace/risk-calc", authMiddleware(api_marketplaceRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("api-marketplace-go listening on :%s", port)))
