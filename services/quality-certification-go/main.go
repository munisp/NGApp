// quality-certification-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"quality-certification-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"quality-certification-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/quality-certification/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/quality-certification/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/quality-certification/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/quality-certification/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/quality-certification/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/quality-certification/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/quality-certification/score", authMiddleware(quality_certificationScoreHandler))
	mux.HandleFunc("/v1/quality-certification/validate", authMiddleware(quality_certificationValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("quality-certification-go listening on :%s", port)))
