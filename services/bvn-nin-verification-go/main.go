// bvn-nin-verification-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"bvn-nin-verification-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"bvn-nin-verification-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/bvn-nin-verification/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/bvn-nin-verification/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/bvn-nin-verification/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/bvn-nin-verification/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/bvn-nin-verification/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/bvn-nin-verification/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/bvn-nin-verification/score", authMiddleware(bvn_nin_verificationScoreHandler))
	mux.HandleFunc("/v1/bvn-nin-verification/validate", authMiddleware(bvn_nin_verificationValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("bvn-nin-verification-go listening on :%s", port)))
