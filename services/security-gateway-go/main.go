// security-gateway-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"security-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"security-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/security-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/security-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/security-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/security-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/security-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/security-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/security-gateway/health-score", authMiddleware(security_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/security-gateway/circuit-state", authMiddleware(security_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("security-gateway-go listening on :%s", port)))
