// keycloak-enforcer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/keycloak-enforcer/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"keycloak-enforcer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"keycloak-enforcer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/keycloak-enforcer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/keycloak-enforcer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/keycloak-enforcer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/keycloak-enforcer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/keycloak-enforcer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/keycloak-enforcer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/keycloak-enforcer/score", authMiddleware(keycloak_enforcerScoreHandler))
	mux.HandleFunc("/v1/keycloak-enforcer/validate", authMiddleware(keycloak_enforcerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("keycloak-enforcer-go listening on :%s", port)))
