// event-streaming-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"event-streaming-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"event-streaming-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/event-streaming/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/event-streaming/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/event-streaming/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/event-streaming/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/event-streaming/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/event-streaming/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/event-streaming/throughput", authMiddleware(event_streamingThroughputHandler))
	mux.HandleFunc("/v1/event-streaming/partition", authMiddleware(event_streamingPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("event-streaming-go listening on :%s", port)))
