package main

import (
	"encoding/json"
	"fmt"
	"net/http"
	"os"
	"sync"
	"time"
)

var (
	mu    sync.RWMutex
	items []map[string]interface{}
)

func init() {
	if err := json.Unmarshal([]byte(seedJSON), &items); err != nil {
		panic("failed to parse seed data: " + err.Error())
	}
}

var seedJSON = `[
  {
    "id": "CSP-001",
    "domain": "app.54bank.app",
    "directives": {
      "script-src": "'nonce-{dynamic}'",
      "style-src": "'nonce-{dynamic}'",
      "img-src": "'self' data: https:",
      "connect-src": "'self' https://api.54bank.app",
      "frame-ancestors": "'none'"
    },
    "reportUri": "/api/csp-report",
    "violations24h": 234,
    "uniqueSources": 12,
    "status": "enforce"
  },
  {
    "id": "CSP-002",
    "domain": "admin.54bank.app",
    "directives": {
      "script-src": "'nonce-{dynamic}'",
      "style-src": "'nonce-{dynamic}'",
      "img-src": "'self'",
      "connect-src": "'self' https://api.54bank.app",
      "frame-ancestors": "'none'"
    },
    "reportUri": "/api/csp-report",
    "violations24h": 12,
    "uniqueSources": 3,
    "status": "enforce"
  },
  {
    "id": "CSP-003",
    "domain": "agent.54bank.app",
    "directives": {
      "script-src": "'nonce-{dynamic}'",
      "style-src": "'nonce-{dynamic}'",
      "img-src": "'self' data:",
      "connect-src": "'self' https://api.54bank.app",
      "frame-ancestors": "'self'"
    },
    "reportUri": "/api/csp-report",
    "violations24h": 56,
    "uniqueSources": 8,
    "status": "report-only"
  }
]`

func writeJSON(w http.ResponseWriter, code int, v interface{}) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(code)
	json.NewEncoder(w).Encode(v)
}

func handleHealthz(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, 200, map[string]interface{}{
		"service": "csp-nonce-engine-go",
		"status":  "healthy",
		"version": "1.0.0",
		"uptime":  time.Now().Format(time.RFC3339),
		"description": "Per-request CSP nonce generation, script/style nonce injection, hash-based CSP, violation reporting",
		"middleware": json.RawMessage(`{
    "kafka": {"broker": "kafka:9092", "topics": ["security.csp.nonce.engine.go"], "consumer_group": "csp-nonce-engine-go-cg"},
    "redis": {"url": "redis://redis:6379/0", "usage": "Session cache, rate limiting, blacklists"},
    "postgres": {"url": "postgresql://postgres:54bank@postgres:5432/banking", "tables": ["csp_policies"]},
    "opensearch": {"url": "https://opensearch:9200", "indices": ["security-csp-nonce-engine-go"]},
    "keycloak": {"issuer": "https://auth.54bank.app/realms/54bank", "realm": "54bank"},
    "permify": {"endpoint": "permify:3476", "schema": "security"},
    "dapr": {"appId": "csp-nonce-engine-go", "pubsub": "54bank-pubsub"},
    "fluvio": {"endpoint": "fluvio:9003", "topics": ["security.csp.nonce.engine.go"]},
    "temporal": {"namespace": "54bank-security", "taskQueue": "csp-nonce-engine-go-queue"},
    "mojaloop": {"hub": "mojaloop:4000", "usage": "Payment security correlation"},
    "tigerbeetle": {"cluster": "tigerbeetle:3000", "ledger": 27},
    "lakehouse": {"endpoint": "lakehouse:8080", "tables": ["security.csp_policies"]},
    "apisix": {"admin": "apisix:9180", "routes": ["/v1/policies/*"]},
    "openappsec": {"endpoint": "openappsec:8090", "policy": "security-hardening"}
  }`),
	})
}

func handleList(w http.ResponseWriter, r *http.Request) {
	mu.RLock()
	copy := make([]map[string]interface{}, len(items))
	for i, item := range items {
		copy[i] = item
	}
	mu.RUnlock()

	if r.Method == "POST" {
		var body map[string]interface{}
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
			writeJSON(w, 400, map[string]string{"error": "invalid JSON"})
			return
		}
		body["createdAt"] = time.Now().Format(time.RFC3339)
		mu.Lock()
		items = append(items, body)
		mu.Unlock()
		writeJSON(w, 201, body)
		return
	}

	writeJSON(w, 200, map[string]interface{}{"total": len(copy), "csp_policies": copy})
}

func handleStats(w http.ResponseWriter, r *http.Request) {
	mu.RLock()
	defer mu.RUnlock()
	statusMap := make(map[string]int)
	for _, item := range items {
		if s, ok := item["status"].(string); ok {
			statusMap[s]++
		}
	}
	writeJSON(w, 200, map[string]interface{}{"total": len(items), "byStatus": statusMap, "generatedAt": time.Now().Format(time.RFC3339)})
}

func main() {
	mux := http.NewServeMux()
	mux.HandleFunc("/healthz", handleHealthz)
	mux.HandleFunc("/v1/policies/list", handleList)
	mux.HandleFunc("/v1/policies/stats", handleStats)

	port := os.Getenv("PORT")
	if port == "" {
		port = "8531"
	}
	fmt.Printf("CSP Nonce Engine listening on :%s\n", port)
	if err := http.ListenAndServe(":"+port, mux); err != nil {
		fmt.Fprintf(os.Stderr, "server error: %v\n", err)
		os.Exit(1)
	}
}
