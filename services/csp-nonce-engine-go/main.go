// csp-nonce-engine-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/csp-nonce-engine/list", handleList)
	http.HandleFunc("/v1/csp-nonce-engine/create", handleCreate)
	http.HandleFunc("/v1/csp-nonce-engine/update", handleUpdate)
	http.HandleFunc("/v1/csp-nonce-engine/process", handleProcess)
	http.HandleFunc("/v1/csp-nonce-engine/audit", handleAudit)
	http.HandleFunc("/v1/csp-nonce-engine/stats", handleStats)
	http.HandleFunc("/v1/csp-nonce-engine/score", csp_nonce_engineScoreHandler)
	http.HandleFunc("/v1/csp-nonce-engine/validate", csp_nonce_engineValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"csp-nonce-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"csp-nonce-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/csp-nonce-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/csp-nonce-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/csp-nonce-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/csp-nonce-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/csp-nonce-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/csp-nonce-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/csp-nonce-engine/score", authMiddleware(csp_nonce_engineScoreHandler))
	mux.HandleFunc("/v1/csp-nonce-engine/validate", authMiddleware(csp_nonce_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("csp-nonce-engine-go listening on :%s", port)))
