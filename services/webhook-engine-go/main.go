// webhook-engine-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/webhook-engine/list", handleList)
	http.HandleFunc("/v1/webhook-engine/create", handleCreate)
	http.HandleFunc("/v1/webhook-engine/update", handleUpdate)
	http.HandleFunc("/v1/webhook-engine/process", handleProcess)
	http.HandleFunc("/v1/webhook-engine/audit", handleAudit)
	http.HandleFunc("/v1/webhook-engine/stats", handleStats)
	http.HandleFunc("/v1/webhook-engine/score", webhook_engineScoreHandler)
	http.HandleFunc("/v1/webhook-engine/validate", webhook_engineValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"webhook-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"webhook-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/webhook-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/webhook-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/webhook-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/webhook-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/webhook-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/webhook-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/webhook-engine/score", authMiddleware(webhook_engineScoreHandler))
	mux.HandleFunc("/v1/webhook-engine/validate", authMiddleware(webhook_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("webhook-engine-go listening on :%s", port)))
