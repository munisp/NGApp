// post-harvest-loss-tracker-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/post-harvest-loss-tracker/update", handleUpdate)
	http.HandleFunc("/v1/post-harvest-loss-tracker/process", handleProcess)
	http.HandleFunc("/v1/post-harvest-loss-tracker/audit", handleAudit)
	http.HandleFunc("/v1/post-harvest-loss-tracker/stats", handleStats)
	http.HandleFunc("/v1/post-harvest-loss-tracker/score", post_harvest_loss_trackerScoreHandler)
	http.HandleFunc("/v1/post-harvest-loss-tracker/validate", post_harvest_loss_trackerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"post-harvest-loss-tracker-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"post-harvest-loss-tracker-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/post-harvest-loss-tracker/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/score", authMiddleware(post_harvest_loss_trackerScoreHandler))
	mux.HandleFunc("/v1/post-harvest-loss-tracker/validate", authMiddleware(post_harvest_loss_trackerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("post-harvest-loss-tracker-go listening on :%s", port)))
