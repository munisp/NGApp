// image-scanner-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"image-scanner-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"image-scanner-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/image-scanner/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/image-scanner/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/image-scanner/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/image-scanner/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/image-scanner/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/image-scanner/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/image-scanner/score", authMiddleware(image_scannerScoreHandler))
	mux.HandleFunc("/v1/image-scanner/validate", authMiddleware(image_scannerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("image-scanner-go listening on :%s", port)))
