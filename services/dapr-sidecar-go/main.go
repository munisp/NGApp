// dapr-sidecar-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"dapr-sidecar-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"dapr-sidecar-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/dapr-sidecar/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/dapr-sidecar/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/dapr-sidecar/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/dapr-sidecar/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/dapr-sidecar/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/dapr-sidecar/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/dapr-sidecar/score", authMiddleware(dapr_sidecarScoreHandler))
	mux.HandleFunc("/v1/dapr-sidecar/validate", authMiddleware(dapr_sidecarValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("dapr-sidecar-go listening on :%s", port)))
