// permify-authz-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/permify-authz/list", handleList)
	http.HandleFunc("/v1/permify-authz/create", handleCreate)
	http.HandleFunc("/v1/permify-authz/update", handleUpdate)
	http.HandleFunc("/v1/permify-authz/process", handleProcess)
	http.HandleFunc("/v1/permify-authz/audit", handleAudit)
	http.HandleFunc("/v1/permify-authz/stats", handleStats)
	http.HandleFunc("/v1/permify-authz/validate", permify_authzValidateHandler)
	http.HandleFunc("/v1/permify-authz/rate-limit", permify_authzRateLimitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"permify-authz-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"permify-authz-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/permify-authz/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/permify-authz/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/permify-authz/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/permify-authz/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/permify-authz/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/permify-authz/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/permify-authz/validate", authMiddleware(permify_authzValidateHandler))
	mux.HandleFunc("/v1/permify-authz/rate-limit", authMiddleware(permify_authzRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("permify-authz-go listening on :%s", port)))
