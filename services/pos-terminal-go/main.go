// pos-terminal-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/pos-terminal/update", handleUpdate)
	http.HandleFunc("/v1/pos-terminal/process", handleProcess)
	http.HandleFunc("/v1/pos-terminal/audit", handleAudit)
	http.HandleFunc("/v1/pos-terminal/stats", handleStats)
	http.HandleFunc("/v1/pos-terminal/compute-fee", pos_terminalFeeHandler)
	http.HandleFunc("/v1/pos-terminal/route", pos_terminalRouteHandler)
...
fmt.Fprintf(w, "requests_total{service=\"pos-terminal-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"pos-terminal-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/pos-terminal/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/pos-terminal/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/pos-terminal/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/pos-terminal/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/pos-terminal/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/pos-terminal/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/pos-terminal/compute-fee", authMiddleware(pos_terminalFeeHandler))
	mux.HandleFunc("/v1/pos-terminal/route", authMiddleware(pos_terminalRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("pos-terminal-go listening on :%s", port)))
