// pos-terminal-go — Production microservice with Postgres integration (stdlib-only)
package main
...
func listHandler(w http.ResponseWriter, r *http.Request) {
	...
	if db != nil {
		var total int
		db.QueryRow(`SELECT count(*) FROM "pos_terminals"`).Scan(&total)

		rows, err := db.Query(fmt.Sprintf(`SELECT row_to_json(t)::text FROM (SELECT * FROM "pos_terminals" ORDER BY 1 LIMIT %d OFFSET %d) t`, limit, offset))
		if err == nil {
			defer rows.Close()
...
func main() {
	...
	log.Printf("[pos-terminal-go] Starting on :%s", port)
