// virtual-accounts-go — Production microservice with Postgres integration (stdlib-only)
package main
...
func initDB() {
	dbURL := os.Getenv("DATABASE_URL")
	if dbURL == "" {
		log.Println("[virtual-accounts-go] DATABASE_URL not set, running without DB")
		return
	}
	var err error
	db, err = sql.Open("postgres", dbURL)
	if err != nil {
		log.Printf("[virtual-accounts-go] DB connection error: %v", err)
		return
	}
	...
	log.Println("[virtual-accounts-go] Connected to Postgres")
}
...
func dataHandler(w http.ResponseWriter, r *http.Request) {
	...
	var total int
	db.QueryRow(`SELECT count(*) FROM "virtualAccounts"`).Scan(&total)

	rows, err := db.Query(fmt.Sprintf(`SELECT "tenantId", "accountName", "accountType", balance, status FROM "virtualAccounts" ORDER BY id LIMIT %d OFFSET %d`, limit, offset))
	if err != nil {
...
func main() {
	...
	log.Printf("[virtual-accounts-go] Starting on :%s", port)
