// branch-operations-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"branch-operations-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"branch-operations-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/branch-operations/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/branch-operations/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/branch-operations/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/branch-operations/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/branch-operations/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/branch-operations/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/branch-operations/score", authMiddleware(branch_operationsScoreHandler))
	mux.HandleFunc("/v1/branch-operations/validate", authMiddleware(branch_operationsValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("branch-operations-go listening on :%s", port)))
