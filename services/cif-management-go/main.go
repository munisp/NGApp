// cif-management-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/cif-management/list", handleList)
	http.HandleFunc("/v1/cif-management/create", handleCreate)
	http.HandleFunc("/v1/cif-management/update", handleUpdate)
	http.HandleFunc("/v1/cif-management/process", handleProcess)
	http.HandleFunc("/v1/cif-management/audit", handleAudit)
	http.HandleFunc("/v1/cif-management/stats", handleStats)
	http.HandleFunc("/v1/cif-management/score", cif_managementScoreHandler)
	http.HandleFunc("/v1/cif-management/validate", cif_managementValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"cif-management-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cif-management-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cif-management/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cif-management/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cif-management/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cif-management/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cif-management/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cif-management/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cif-management/score", authMiddleware(cif_managementScoreHandler))
	mux.HandleFunc("/v1/cif-management/validate", authMiddleware(cif_managementValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cif-management-go listening on :%s", port)))
