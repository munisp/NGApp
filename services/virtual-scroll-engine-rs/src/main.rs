...
// ─── Virtual Scroll Engine — Platform/Infra ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "VIR-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Platform/Infra", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "VIR-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Platform/Infra", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "VIR-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Platform/Infra", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("VIR-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Virtual Scroll Engine — Platform/Infra",
        "middleware": {
            "kafka": "virtual-scroll-engine.events, virtual-scroll-engine.audit",
            "postgres": "virtual_scroll_engine_records",
            "redis": "virtual-scroll-engine_cache",
            "temporal": "VirtualScrollEngineWorkflow",
            "opensearch": "virtual-scroll-engine-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9579".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Virtual Scroll Engine v2.0 (Platform/Infra) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/virtual-scroll-engine/list", web::get().to(list_records))
            .route("/v1/virtual-scroll-engine/create", web::post().to(create_record))
            .route("/v1/virtual-scroll-engine/process", web::post().to(process_record))
            .route("/v1/virtual-scroll-engine/audit", web::get().to(get_audit))
            .route("/v1/virtual-scroll-engine/stats", web::get().to(get_stats))
