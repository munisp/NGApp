// keycloak-admin-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/keycloak-admin/score", keycloak_adminScoreHandler)
	http.HandleFunc("/v1/keycloak-admin/validate", keycloak_adminValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"keycloak-admin-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"keycloak-admin-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/keycloak-admin/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/keycloak-admin/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/keycloak-admin/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/keycloak-admin/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/keycloak-admin/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/keycloak-admin/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/keycloak-admin/score", authMiddleware(keycloak_adminScoreHandler))
	mux.HandleFunc("/v1/keycloak-admin/validate", authMiddleware(keycloak_adminValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("keycloak-admin-go listening on :%s", port)))
