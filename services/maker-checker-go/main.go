// maker-checker-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/maker-checker/list", handleList)
	http.HandleFunc("/v1/maker-checker/create", handleCreate)
	http.HandleFunc("/v1/maker-checker/update", handleUpdate)
	http.HandleFunc("/v1/maker-checker/process", handleProcess)
	http.HandleFunc("/v1/maker-checker/audit", handleAudit)
	http.HandleFunc("/v1/maker-checker/stats", handleStats)
	http.HandleFunc("/v1/maker-checker/score", maker_checkerScoreHandler)
	http.HandleFunc("/v1/maker-checker/validate", maker_checkerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"maker-checker-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"maker-checker-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/maker-checker/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/maker-checker/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/maker-checker/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/maker-checker/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/maker-checker/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/maker-checker/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/maker-checker/score", authMiddleware(maker_checkerScoreHandler))
	mux.HandleFunc("/v1/maker-checker/validate", authMiddleware(maker_checkerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("maker-checker-go listening on :%s", port)))
