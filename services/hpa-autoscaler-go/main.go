// hpa-autoscaler-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"hpa-autoscaler-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"hpa-autoscaler-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/hpa-autoscaler/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/hpa-autoscaler/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/hpa-autoscaler/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/hpa-autoscaler/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/hpa-autoscaler/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/hpa-autoscaler/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/hpa-autoscaler/score", authMiddleware(hpa_autoscalerScoreHandler))
	mux.HandleFunc("/v1/hpa-autoscaler/validate", authMiddleware(hpa_autoscalerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("hpa-autoscaler-go listening on :%s", port)))
