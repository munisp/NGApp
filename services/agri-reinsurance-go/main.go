// agri-reinsurance-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/agri-reinsurance/list", handleList)
	http.HandleFunc("/v1/agri-reinsurance/create", handleCreate)
	http.HandleFunc("/v1/agri-reinsurance/update", handleUpdate)
	http.HandleFunc("/v1/agri-reinsurance/process", handleProcess)
	http.HandleFunc("/v1/agri-reinsurance/audit", handleAudit)
	http.HandleFunc("/v1/agri-reinsurance/stats", handleStats)
	http.HandleFunc("/v1/agri-reinsurance/yield-score", agri_reinsuranceYieldHandler)
	http.HandleFunc("/v1/agri-reinsurance/risk-assess", agri_reinsuranceRiskHandler)
...
fmt.Fprintf(w, "requests_total{service=\"agri-reinsurance-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"agri-reinsurance-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/agri-reinsurance/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/agri-reinsurance/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/agri-reinsurance/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/agri-reinsurance/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/agri-reinsurance/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/agri-reinsurance/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/agri-reinsurance/yield-score", authMiddleware(agri_reinsuranceYieldHandler))
	mux.HandleFunc("/v1/agri-reinsurance/risk-assess", authMiddleware(agri_reinsuranceRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("agri-reinsurance-go listening on :%s", port)))
