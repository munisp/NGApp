use tokio_postgres;
// farm-boundary-mapping-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM farm_boundary_mapping";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM farm_boundary_mapping ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8603".into()).parse().unwrap_or(8603);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "farm-boundary-mapping-rs".into(),
        table_name: "farm_boundary_mapping".into(),
    });
    
    println!("[farm-boundary-mapping-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/farm-boundary-mapping/list", web::get().to(list))
            .route("/v1/farm-boundary-mapping/stats", web::get().to(stats))
            .route("/v1/farm-boundary-mapping/{id}", web::get().to(get_by_id))
            .route("/v1/farm-boundary-mapping", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
