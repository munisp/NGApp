// qr-payments-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/qr-payments/list", handleList)
	http.HandleFunc("/v1/qr-payments/create", handleCreate)
	http.HandleFunc("/v1/qr-payments/update", handleUpdate)
	http.HandleFunc("/v1/qr-payments/process", handleProcess)
	http.HandleFunc("/v1/qr-payments/audit", handleAudit)
	http.HandleFunc("/v1/qr-payments/stats", handleStats)
	http.HandleFunc("/v1/qr-payments/compute-fee", qr_paymentsFeeHandler)
	http.HandleFunc("/v1/qr-payments/route", qr_paymentsRouteHandler)
...
fmt.Fprintf(w, "requests_total{service=\"qr-payments-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"qr-payments-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/qr-payments/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/qr-payments/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/qr-payments/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/qr-payments/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/qr-payments/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/qr-payments/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/qr-payments/compute-fee", authMiddleware(qr_paymentsFeeHandler))
	mux.HandleFunc("/v1/qr-payments/route", authMiddleware(qr_paymentsRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("qr-payments-go listening on :%s", port)))
