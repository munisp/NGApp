// tenant-metering-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/tenant-metering/list", handleList)
	http.HandleFunc("/v1/tenant-metering/create", handleCreate)
	http.HandleFunc("/v1/tenant-metering/update", handleUpdate)
	http.HandleFunc("/v1/tenant-metering/process", handleProcess)
	http.HandleFunc("/v1/tenant-metering/audit", handleAudit)
	http.HandleFunc("/v1/tenant-metering/stats", handleStats)
	http.HandleFunc("/v1/tenant-metering/score", tenant_meteringScoreHandler)
	http.HandleFunc("/v1/tenant-metering/validate", tenant_meteringValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"tenant-metering-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"tenant-metering-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/tenant-metering/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/tenant-metering/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/tenant-metering/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/tenant-metering/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/tenant-metering/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/tenant-metering/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/tenant-metering/score", authMiddleware(tenant_meteringScoreHandler))
	mux.HandleFunc("/v1/tenant-metering/validate", authMiddleware(tenant_meteringValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("tenant-metering-go listening on :%s", port)))
