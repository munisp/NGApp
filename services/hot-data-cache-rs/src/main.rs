...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "HOT-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "KYC/Identity", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "HOT-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "KYC/Identity", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "HOT-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "KYC/Identity", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("HOT-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Hot Data Cache — KYC/Identity",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9500".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Hot Data Cache v2.0 (KYC/Identity) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/hot-data-cache/process", web::post().to(process_record))
            .route("/v1/hot-data-cache/audit", web::get().to(get_audit))
