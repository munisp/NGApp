// acgsf-guarantee-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"acgsf-guarantee-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"acgsf-guarantee-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/acgsf-guarantee/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/acgsf-guarantee/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/acgsf-guarantee/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/acgsf-guarantee/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/acgsf-guarantee/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/acgsf-guarantee/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/acgsf-guarantee/yield-score", authMiddleware(acgsf_guaranteeYieldHandler))
	mux.HandleFunc("/v1/acgsf-guarantee/risk-assess", authMiddleware(acgsf_guaranteeRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("acgsf-guarantee-go listening on :%s", port)))
