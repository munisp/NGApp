// apisix-plugin-optimizer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/apisix-plugin-optimizer/update", handleUpdate)
	http.HandleFunc("/v1/apisix-plugin-optimizer/process", handleProcess)
	http.HandleFunc("/v1/apisix-plugin-optimizer/audit", handleAudit)
	http.HandleFunc("/v1/apisix-plugin-optimizer/stats", handleStats)
	http.HandleFunc("/v1/apisix-plugin-optimizer/health-score", apisix_plugin_optimizerHealthScoreHandler)
	http.HandleFunc("/v1/apisix-plugin-optimizer/circuit-state", apisix_plugin_optimizerCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"apisix-plugin-optimizer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"apisix-plugin-optimizer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/apisix-plugin-optimizer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/health-score", authMiddleware(apisix_plugin_optimizerHealthScoreHandler))
	mux.HandleFunc("/v1/apisix-plugin-optimizer/circuit-state", authMiddleware(apisix_plugin_optimizerCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("apisix-plugin-optimizer-go listening on :%s", port)))
