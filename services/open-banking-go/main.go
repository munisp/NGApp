// open-banking-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"open-banking-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"open-banking-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/open-banking/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/open-banking/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/open-banking/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/open-banking/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/open-banking/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/open-banking/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/open-banking/score", authMiddleware(open_bankingScoreHandler))
	mux.HandleFunc("/v1/open-banking/validate", authMiddleware(open_bankingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("open-banking-go listening on :%s", port)))
