// escrow-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/escrow/list", handleList)
	http.HandleFunc("/v1/escrow/create", handleCreate)
	http.HandleFunc("/v1/escrow/update", handleUpdate)
	http.HandleFunc("/v1/escrow/process", handleProcess)
	http.HandleFunc("/v1/escrow/audit", handleAudit)
	http.HandleFunc("/v1/escrow/stats", handleStats)
	http.HandleFunc("/v1/escrow/score", escrowScoreHandler)
	http.HandleFunc("/v1/escrow/validate", escrowValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"escrow-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"escrow-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/escrow/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/escrow/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/escrow/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/escrow/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/escrow/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/escrow/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/escrow/score", authMiddleware(escrowScoreHandler))
	mux.HandleFunc("/v1/escrow/validate", authMiddleware(escrowValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("escrow-go listening on :%s", port)))
