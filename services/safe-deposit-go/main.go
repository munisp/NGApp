// safe-deposit-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/safe-deposit/list", handleList)
	http.HandleFunc("/v1/safe-deposit/create", handleCreate)
	http.HandleFunc("/v1/safe-deposit/update", handleUpdate)
	http.HandleFunc("/v1/safe-deposit/process", handleProcess)
	http.HandleFunc("/v1/safe-deposit/audit", handleAudit)
	http.HandleFunc("/v1/safe-deposit/stats", handleStats)
	http.HandleFunc("/v1/safe-deposit/score", safe_depositScoreHandler)
	http.HandleFunc("/v1/safe-deposit/validate", safe_depositValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"safe-deposit-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"safe-deposit-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/safe-deposit/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/safe-deposit/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/safe-deposit/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/safe-deposit/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/safe-deposit/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/safe-deposit/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/safe-deposit/score", authMiddleware(safe_depositScoreHandler))
	mux.HandleFunc("/v1/safe-deposit/validate", authMiddleware(safe_depositValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("safe-deposit-go listening on :%s", port)))
