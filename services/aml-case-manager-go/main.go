// aml-case-manager-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"aml-case-manager-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"aml-case-manager-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/aml-case-manager/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/aml-case-manager/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/aml-case-manager/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/aml-case-manager/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/aml-case-manager/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/aml-case-manager/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/aml-case-manager/screen", authMiddleware(aml_case_managerScreenHandler))
	mux.HandleFunc("/v1/aml-case-manager/risk-score", authMiddleware(aml_case_managerRiskScoreHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("aml-case-manager-go listening on :%s", port)))
