...
// ─── Mojaloop Tb Bridge — Cross-Border ────────────────────────────────────────────────
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Mojaloop Tb Bridge — Cross-Border",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9519".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Mojaloop Tb Bridge v2.0 (Cross-Border) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/mojaloop-tb-bridge/process", web::post().to(process_record))
            .route("/v1/mojaloop-tb-bridge/audit", web::get().to(get_audit))
