// ddos-protection-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"ddos-protection-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"ddos-protection-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/ddos-protection/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/ddos-protection/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/ddos-protection/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/ddos-protection/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/ddos-protection/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/ddos-protection/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/ddos-protection/score", authMiddleware(ddos_protectionScoreHandler))
	mux.HandleFunc("/v1/ddos-protection/validate", authMiddleware(ddos_protectionValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("ddos-protection-go listening on :%s", port)))
