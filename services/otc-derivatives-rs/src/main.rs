...
async fn healthz() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({
        "status": "ok", "service": "otc-derivatives",
        "middleware": {
            "kafka": { "broker": env_or("KAFKA_BROKER", "localhost:9092"), "topics": ["deriv.trade.booked", "deriv.trade.settled", "deriv.margin.call", "deriv.valuation.completed"] },
            "redis": { "url": env_or("REDIS_URL", "redis://localhost:6379"), "cache_keys": ["deriv:curves", "deriv:volatility_surface", "deriv:counterparty_exposure"] },
            "postgres": { "url": env_or("DATABASE_URL", "postgresql://ndsep_user:ndsep_secure_2026@localhost:5432/ndsep_db"), "tables": ["otc_trades", "otc_valuations", "otc_collateral", "otc_cashflows"] },
            "opensearch": { "url": env_or("OPENSEARCH_URL", "http://localhost:9200"), "indices": ["otc-trades", "otc-valuations", "otc-audit"] },
            "keycloak": { "url": env_or("KEYCLOAK_URL", "http://localhost:8080"), "realm": "54bank", "client": "otc-derivatives-service" },
            "permify": { "url": env_or("PERMIFY_URL", "http://localhost:3476"), "resources": ["otc_trade", "otc_valuation", "otc_margin"] },
            "dapr": { "url": env_or("DAPR_URL", "http://localhost:3500"), "app_id": "otc-derivatives", "pubsub": "deriv-events" },
            "fluvio": { "url": env_or("FLUVIO_URL", "localhost:9003"), "topics": ["deriv-market-data-feed", "deriv-position-updates"] },
            "temporal": { "url": env_or("TEMPORAL_URL", "localhost:7233"), "workflows": ["DerivativeValuationWorkflow", "MarginCallWorkflow", "CashflowSettlementWorkflow", "HedgeEffectivenessWorkflow"] },
            "mojaloop": { "url": env_or("MOJALOOP_URL", "http://localhost:3002"), "usage": "collateral-settlement" },
            "tigerbeetle": { "url": env_or("TIGERBEETLE_URL", "localhost:3000"), "ledgers": ["deriv_mtm", "deriv_collateral", "deriv_cashflows", "deriv_pnl"] },
            "lakehouse": { "url": env_or("LAKEHOUSE_URL", "http://localhost:8181"), "tables": ["otc_trade_history", "otc_valuation_snapshots", "otc_risk_metrics"] },
            "apisix": { "url": env_or("APISIX_URL", "http://localhost:9080"), "routes": ["/api/derivatives/*"] },
            "openappsec": { "url": env_or("OPENAPPSEC_URL", "http://localhost:4000"), "policy": "deriv-waf-rules" }
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    ...
    println!("OTC Derivatives service on :8161");
    HttpServer::new(move || {
        App::new()
