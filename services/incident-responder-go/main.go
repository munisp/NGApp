// incident-responder-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"incident-responder-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"incident-responder-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Testing/Observability"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/incident-responder/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/incident-responder/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/incident-responder/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/incident-responder/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/incident-responder/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/incident-responder/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/incident-responder/score", authMiddleware(incident_responderScoreHandler))
	mux.HandleFunc("/v1/incident-responder/validate", authMiddleware(incident_responderValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("incident-responder-go listening on :%s", port)))
