// notification-service-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"notification-service-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"notification-service-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/notification-service/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/notification-service/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/notification-service/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/notification-service/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/notification-service/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/notification-service/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/notification-service/score", authMiddleware(notification_serviceScoreHandler))
	mux.HandleFunc("/v1/notification-service/validate", authMiddleware(notification_serviceValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("notification-service-go listening on :%s", port)))
