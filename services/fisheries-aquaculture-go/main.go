// fisheries-aquaculture-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"fisheries-aquaculture-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"fisheries-aquaculture-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/fisheries-aquaculture/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/fisheries-aquaculture/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/fisheries-aquaculture/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/fisheries-aquaculture/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/fisheries-aquaculture/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/fisheries-aquaculture/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/fisheries-aquaculture/score", authMiddleware(fisheries_aquacultureScoreHandler))
	mux.HandleFunc("/v1/fisheries-aquaculture/validate", authMiddleware(fisheries_aquacultureValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("fisheries-aquaculture-go listening on :%s", port)))
