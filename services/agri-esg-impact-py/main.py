#!/usr/bin/env python3
"""agri-esg-impact-py — Production Python microservice with Postgres, Kafka, Redis integration."""
...
logging.basicConfig(level=logging.INFO, format='[agri-esg-impact-py] %(levelname)s %(message)s')
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path in ('/healthz', '/health'):
            self._health()
        elif path == '/v1/agri-esg-impact/list':
            self._list(params)
        elif path == '/v1/agri-esg-impact/stats':
            self._stats()
        elif path.startswith('/v1/agri-esg-impact/'):
    ...
    def _list(self, params):
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # Count total
                cur.execute(f'SELECT count(*) as cnt FROM "agri_esg_impact"')
                total = cur.fetchone()['cnt']
                
                # Fetch rows
                cur.execute(f'SELECT * FROM "agri_esg_impact" ORDER BY id LIMIT %s OFFSET %s', (limit, offset))
    ...
    def _stats(self):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(f'SELECT count(*) FROM "agri_esg_impact"')
    ...
    def _get_by_id(self, item_id):
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(f'SELECT * FROM "agri_esg_impact" WHERE id = %s', (item_id,))
                row = cur.fetchone()
                if row:
                    item = dict(row)
                    for k, v in item.items():
    ...
    def _json(self, code, data):
        ...
        self.send_header("X-Service", "agri-esg-impact-py")
...
@app.route("/data")
def data_endpoint():
    """Query cooperative_management from Postgres."""
    ...
    try:
        ...
        offset = (page - 1) * limit
        cur.execute('SELECT count(*) FROM "cooperative_management"')
        total = cur.fetchone()["count"]
        cur.execute('SELECT "tenantId", name, category, status FROM "cooperative_management" ORDER BY id LIMIT %s OFFSET %s', (limit, offset))
        items = cur.fetchall()
        conn.close()
