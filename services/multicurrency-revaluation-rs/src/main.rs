...
// ─── Multicurrency Revaluation — Treasury/Markets ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "MUL-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Treasury/Markets", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "MUL-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Treasury/Markets", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "MUL-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Treasury/Markets", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Multicurrency Revaluation — Treasury/Markets",
        "middleware": {
            "kafka": "multicurrency-revaluation.events, multicurrency-revaluation.audit",
            "postgres": "multicurrency_revaluation_records",
            "redis": "multicurrency-revaluation_cache",
            "temporal": "MulticurrencyRevaluationWorkflow",
            "opensearch": "multicurrency-revaluation-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9523".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Multicurrency Revaluation v2.0 (Treasury/Markets) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/multicurrency-revaluation/list", web::get().to(list_records))
            .route("/v1/multicurrency-revaluation/create", web::post().to(create_record))
            .route("/v1/multicurrency-revaluation/process", web::post().to(process_record))
            .route("/v1/multicurrency-revaluation/audit", web::get().to(get_audit))
            .route("/v1/multicurrency-revaluation/stats", web::get().to(get_stats))
