...
async fn healthz() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({
        ...
        "middleware": {
            "kafka": { "broker": env_or("KAFKA_BROKER", "localhost:9092"), "topics": ["basel.rwa.calculated", "basel.capital.report", "basel.breach.alert"] },
            "redis": { "url": env_or("REDIS_URL", "redis://localhost:6379"), "cache_keys": ["basel:rwa_cache", "basel:risk_weights", "basel:capital_ratios"] },
            "postgres": { "url": env_or("DATABASE_URL", "postgresql://ndsep_user:ndsep_secure_2026@localhost:5432/ndsep_db"), "tables": ["rwa_exposures", "capital_ratios", "pillar3_disclosures"] },
            "opensearch": { "url": env_or("OPENSEARCH_URL", "http://localhost:9200"), "indices": ["basel-rwa", "basel-capital", "basel-audit"] },
            "keycloak": { "url": env_or("KEYCLOAK_URL", "http://localhost:8080"), "realm": "54bank", "client": "basel-engine-service" },
            "permify": { "url": env_or("PERMIFY_URL", "http://localhost:3476"), "resources": ["rwa_exposure", "capital_report", "pillar3_disclosure"] },
            "dapr": { "url": env_or("DAPR_URL", "http://localhost:3500"), "app_id": "basel-engine", "pubsub": "basel-events" },
            "fluvio": { "url": env_or("FLUVIO_URL", "localhost:9003"), "topics": ["basel-exposure-feed", "basel-market-data"] },
            "temporal": { "url": env_or("TEMPORAL_URL", "localhost:7233"), "workflows": ["RWACalculationWorkflow", "CapitalAdequacyWorkflow", "StressTestWorkflow"] },
            "mojaloop": { "url": env_or("MOJALOOP_URL", "http://localhost:3002"), "usage": "interbank-exposure-calculation" },
            "tigerbeetle": { "url": env_or("TIGERBEETLE_URL", "localhost:3000"), "ledgers": ["basel_rwa", "basel_capital", "basel_provisions"] },
            "lakehouse": { "url": env_or("LAKEHOUSE_URL", "http://localhost:8181"), "tables": ["rwa_history", "capital_ratio_history", "stress_test_results"] },
            "apisix": { "url": env_or("APISIX_URL", "http://localhost:9080"), "routes": ["/api/basel/*"] },
            "openappsec": { "url": env_or("OPENAPPSEC_URL", "http://localhost:4000"), "policy": "basel-waf-rules" }
        }
    }))
}
