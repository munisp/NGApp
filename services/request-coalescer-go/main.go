// request-coalescer-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"request-coalescer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"request-coalescer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/request-coalescer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/request-coalescer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/request-coalescer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/request-coalescer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/request-coalescer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/request-coalescer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/request-coalescer/score", authMiddleware(request_coalescerScoreHandler))
	mux.HandleFunc("/v1/request-coalescer/validate", authMiddleware(request_coalescerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("request-coalescer-go listening on :%s", port)))
