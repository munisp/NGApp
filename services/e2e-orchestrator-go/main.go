// e2e-orchestrator-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"e2e-orchestrator-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"e2e-orchestrator-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/e2e-orchestrator/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/e2e-orchestrator/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/e2e-orchestrator/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/e2e-orchestrator/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/e2e-orchestrator/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/e2e-orchestrator/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/e2e-orchestrator/score", authMiddleware(e2e_orchestratorScoreHandler))
	mux.HandleFunc("/v1/e2e-orchestrator/validate", authMiddleware(e2e_orchestratorValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("e2e-orchestrator-go listening on :%s", port)))
