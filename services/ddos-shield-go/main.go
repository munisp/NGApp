// ddos-shield-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/ddos-shield/update", handleUpdate)
	http.HandleFunc("/v1/ddos-shield/process", handleProcess)
	http.HandleFunc("/v1/ddos-shield/audit", handleAudit)
	http.HandleFunc("/v1/ddos-shield/stats", handleStats)
	http.HandleFunc("/v1/ddos-shield/score", ddos_shieldScoreHandler)
	http.HandleFunc("/v1/ddos-shield/validate", ddos_shieldValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"ddos-shield-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"ddos-shield-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/ddos-shield/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/ddos-shield/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/ddos-shield/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/ddos-shield/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/ddos-shield/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/ddos-shield/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/ddos-shield/score", authMiddleware(ddos_shieldScoreHandler))
	mux.HandleFunc("/v1/ddos-shield/validate", authMiddleware(ddos_shieldValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("ddos-shield-go listening on :%s", port)))
