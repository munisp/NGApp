// body-limit-enforcer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/body-limit-enforcer/list", handleList)
	http.HandleFunc("/v1/body-limit-enforcer/create", handleCreate)
	http.HandleFunc("/v1/body-limit-enforcer/update", handleUpdate)
	http.HandleFunc("/v1/body-limit-enforcer/process", handleProcess)
	http.HandleFunc("/v1/body-limit-enforcer/audit", handleAudit)
	http.HandleFunc("/v1/body-limit-enforcer/stats", handleStats)
	http.HandleFunc("/v1/body-limit-enforcer/score", body_limit_enforcerScoreHandler)
	http.HandleFunc("/v1/body-limit-enforcer/validate", body_limit_enforcerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"body-limit-enforcer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"body-limit-enforcer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/body-limit-enforcer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/body-limit-enforcer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/body-limit-enforcer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/body-limit-enforcer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/body-limit-enforcer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/body-limit-enforcer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/body-limit-enforcer/score", authMiddleware(body_limit_enforcerScoreHandler))
	mux.HandleFunc("/v1/body-limit-enforcer/validate", authMiddleware(body_limit_enforcerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("body-limit-enforcer-go listening on :%s", port)))
