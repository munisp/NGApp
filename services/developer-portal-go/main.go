// developer-portal-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/developer-portal/list", handleList)
	http.HandleFunc("/v1/developer-portal/create", handleCreate)
	http.HandleFunc("/v1/developer-portal/update", handleUpdate)
	http.HandleFunc("/v1/developer-portal/process", handleProcess)
	http.HandleFunc("/v1/developer-portal/audit", handleAudit)
	http.HandleFunc("/v1/developer-portal/stats", handleStats)
	http.HandleFunc("/v1/developer-portal/score", developer_portalScoreHandler)
	http.HandleFunc("/v1/developer-portal/validate", developer_portalValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"developer-portal-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"developer-portal-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/developer-portal/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/developer-portal/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/developer-portal/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/developer-portal/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/developer-portal/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/developer-portal/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/developer-portal/score", authMiddleware(developer_portalScoreHandler))
	mux.HandleFunc("/v1/developer-portal/validate", authMiddleware(developer_portalValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("developer-portal-go listening on :%s", port)))
