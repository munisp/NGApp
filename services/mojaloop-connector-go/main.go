// mojaloop-connector-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/mojaloop-connector/list", handleList)
	http.HandleFunc("/v1/mojaloop-connector/create", handleCreate)
	http.HandleFunc("/v1/mojaloop-connector/update", handleUpdate)
	http.HandleFunc("/v1/mojaloop-connector/process", handleProcess)
	http.HandleFunc("/v1/mojaloop-connector/audit", handleAudit)
	http.HandleFunc("/v1/mojaloop-connector/stats", handleStats)
	http.HandleFunc("/v1/mojaloop-connector/score", mojaloop_connectorScoreHandler)
	http.HandleFunc("/v1/mojaloop-connector/validate", mojaloop_connectorValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"mojaloop-connector-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"mojaloop-connector-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Cross-Border"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/mojaloop-connector/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/mojaloop-connector/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/mojaloop-connector/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/mojaloop-connector/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/mojaloop-connector/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/mojaloop-connector/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/mojaloop-connector/score", authMiddleware(mojaloop_connectorScoreHandler))
	mux.HandleFunc("/v1/mojaloop-connector/validate", authMiddleware(mojaloop_connectorValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("mojaloop-connector-go listening on :%s", port)))
