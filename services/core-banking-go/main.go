// core-banking-go — Production microservice with Postgres integration (stdlib-only)
package main
...
func createHandler(w http.ResponseWriter, r *http.Request) {
	...
	if idempKey != "" {
		log.Printf("[core-banking-go] Idempotency key: %s", idempKey)
	}
...
func main() {
	...
	mux.HandleFunc("/v1/core-banking/list", listHandler)
	mux.HandleFunc("/v1/core-banking/stats", statsHandler)
	mux.HandleFunc("/v1/core-banking/", getByIdHandler)
	mux.HandleFunc("/v1/core-banking", createHandler)

	log.Printf("[core-banking-go] Starting on :%s", port)
