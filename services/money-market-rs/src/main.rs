...
fn seed_deals() -> Vec<Deal> {
    let deals = vec![
        ...
        ("MM-007", "cd", "54Bank Treasury", "54B-TSY", 3_000_000_000.0, 11.50, 365, "2026-01-15", "2027-01-15", "ACT/365", "active", "NGN-SETTLE-007", 0, None, None),
...
async fn healthz() -> HttpResponse {
    ...
    HttpResponse::Ok().json(serde_json::json!({
        "status": "ok", "service": "money-market",
        "middleware": {
            "kafka": { "broker": mw.kafka_broker, "topics": ["mm.deal.created", "mm.deal.matured", "mm.deal.rolled"] },
            "redis": { "url": mw.redis_url, "cache_keys": ["mm:rates", "mm:positions", "mm:counterparty_limits"] },
            "postgres": { "url": mw.postgres_url, "tables": ["mm_deals", "mm_counterparties", "mm_interest_accruals"] },
            "opensearch": { "url": mw.opensearch_url, "indices": ["mm-deals", "mm-audit-trail"] },
            "keycloak": { "url": mw.keycloak_url, "realm": "54bank", "client": "money-market-service" },
            "permify": { "url": mw.permify_url, "resources": ["money_market_deal", "mm_settlement"] },
            "dapr": { "url": mw.dapr_url, "app_id": "money-market", "pubsub": "mm-events" },
            "fluvio": { "url": mw.fluvio_url, "topics": ["mm-rate-feeds", "mm-position-updates"] },
            "temporal": { "url": mw.temporal_url, "workflows": ["DealMaturityWorkflow", "RolloverWorkflow", "SettlementWorkflow"] },
            "mojaloop": { "url": mw.mojaloop_url, "usage": "interbank-settlement-clearing" },
            "tigerbeetle": { "url": mw.tigerbeetle_url, "ledgers": ["mm_placements", "mm_borrowings", "mm_repos"] },
            "lakehouse": { "url": mw.lakehouse_url, "tables": ["mm_deals_history", "mm_rate_snapshots"] },
            "apisix": { "url": mw.apisix_url, "routes": ["/api/money-market/*"] },
            "openappsec": { "url": mw.openappsec_url, "policy": "mm-waf-rules" }
        }
    }))
}
