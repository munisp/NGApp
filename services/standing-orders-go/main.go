// standing-orders-go — Production microservice with Postgres integration (stdlib-only)
package main
...
func listHandler(w http.ResponseWriter, r *http.Request) {
	...
	if db != nil {
		...
		rows, err := db.Query(fmt.Sprintf(`SELECT row_to_json(t)::text FROM (SELECT * FROM "standing_orders" ORDER BY 1 LIMIT %d OFFSET %d) t`, limit, offset))
		if err == nil {
			defer rows.Close()
...
func main() {
	...
	log.Printf("[standing-orders-go] Starting on :%s", port)
