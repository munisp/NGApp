// sms-banking-gateway-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/sms-banking-gateway/list", handleList)
	http.HandleFunc("/v1/sms-banking-gateway/create", handleCreate)
	http.HandleFunc("/v1/sms-banking-gateway/update", handleUpdate)
	http.HandleFunc("/v1/sms-banking-gateway/process", handleProcess)
	http.HandleFunc("/v1/sms-banking-gateway/audit", handleAudit)
	http.HandleFunc("/v1/sms-banking-gateway/stats", handleStats)
	http.HandleFunc("/v1/sms-banking-gateway/health-score", sms_banking_gatewayHealthScoreHandler)
	http.HandleFunc("/v1/sms-banking-gateway/circuit-state", sms_banking_gatewayCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"sms-banking-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"sms-banking-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/sms-banking-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/sms-banking-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/sms-banking-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/sms-banking-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/sms-banking-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/sms-banking-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/sms-banking-gateway/health-score", authMiddleware(sms_banking_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/sms-banking-gateway/circuit-state", authMiddleware(sms_banking_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("sms-banking-gateway-go listening on :%s", port)))
