// bank-guarantees-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/bank-guarantees/update", handleUpdate)
	http.HandleFunc("/v1/bank-guarantees/process", handleProcess)
	http.HandleFunc("/v1/bank-guarantees/audit", handleAudit)
	http.HandleFunc("/v1/bank-guarantees/stats", handleStats)
	http.HandleFunc("/v1/bank-guarantees/score", bank_guaranteesScoreHandler)
	http.HandleFunc("/v1/bank-guarantees/validate", bank_guaranteesValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"bank-guarantees-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"bank-guarantees-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/bank-guarantees/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/bank-guarantees/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/bank-guarantees/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/bank-guarantees/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/bank-guarantees/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/bank-guarantees/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/bank-guarantees/score", authMiddleware(bank_guaranteesScoreHandler))
	mux.HandleFunc("/v1/bank-guarantees/validate", authMiddleware(bank_guaranteesValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("bank-guarantees-go listening on :%s", port)))
