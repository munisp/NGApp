// telegram-mini-app-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"telegram-mini-app-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"telegram-mini-app-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/telegram-mini-app/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/telegram-mini-app/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/telegram-mini-app/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/telegram-mini-app/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/telegram-mini-app/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/telegram-mini-app/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/telegram-mini-app/score", authMiddleware(telegram_mini_appScoreHandler))
	mux.HandleFunc("/v1/telegram-mini-app/validate", authMiddleware(telegram_mini_appValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("telegram-mini-app-go listening on :%s", port)))
