...
async fn main() -> Result<()> {
    ...
    info!("Starting Iroh P2P Fintech Service...");

    let p2p_payments = p2p_payments::P2PPaymentService::new().await?;
    let wallet_sync = wallet_sync::WalletSyncService::new().await?;
    let agent_banking = agent_banking::AgentBankingService::new().await?;
    let fraud_detection = fraud_detection::FraudDetectionService::new().await?;
    let kyc_transfer = kyc_transfer::KycTransferService::new().await?;
    let merchant_mesh = merchant_mesh::MerchantMeshService::new().await?;
...
async fn agent_submit_transaction(
    ...
    state
        ...
        .map(Json)
        .map_err(|e| {
            warn!("Agent transaction failed: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })
}
...
async fn broadcast_fraud_alert(
    ...
    state
        ...
        .map(Json)
        .map_err(|e| {
            warn!("Fraud alert broadcast failed: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })
}
...
async fn subscribe_fraud_topic(
    ...
    state
        ...
        .map(Json)
        .map_err(|e| {
            warn!("Fraud topic subscribe failed: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })
}
...
async fn register_merchant(
    ...
    state
        ...
        .map(Json)
        .map_err(|e| {
            warn!("Merchant registration failed: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })
}

async fn discover_merchants(
    State(state): State<Arc<AppState>>,
) -> Json<Vec<MerchantInfo>> {
...
async fn merchant_transact(
    ...
    state
        ...
        .map(Json)
        .map_err(|e| {
            warn!("Merchant transaction failed: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })
}
...
async fn get_metrics(
    ...
    let p2p_stats = state.p2p_payments.get_stats().await;
    let sync_stats = state.wallet_sync.get_stats().await;
    let agent_stats = state.agent_banking.get_stats().await;
    let fraud_stats = state.fraud_detection.get_stats().await;
    let kyc_stats = state.kyc_transfer.get_stats().await;
    let merchant_stats = state.merchant_mesh.get_stats().await;

    Json(ServiceMetrics {
        p2p_transfers: p2p_stats,
        wallet_syncs: sync_stats,
        agent_transactions: agent_stats,
        fraud_alerts: fraud_stats,
        kyc_transfers: kyc_stats,
        merchant_transactions: merchant_stats,
    })
}
