// credit-bureau-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("22100".into()).parse().unwrap_or(22100);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "credit-bureau-rs".into(),
        table_name: "credit_bureau".into(),
    });
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/credit-bureau/list", web::get().to(list))
            .route("/v1/credit-bureau/stats", web::get().to(stats))
            .route("/v1/credit-bureau/{id}", web::get().to(get_by_id))
            .route("/v1/credit-bureau", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
