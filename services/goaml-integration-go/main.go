// goaml-integration-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/goaml-integration/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"goaml-integration-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"goaml-integration-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/goaml-integration/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/goaml-integration/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/goaml-integration/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/goaml-integration/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/goaml-integration/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/goaml-integration/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/goaml-integration/screen", authMiddleware(goaml_integrationScreenHandler))
	mux.HandleFunc("/v1/goaml-integration/risk-score", authMiddleware(goaml_integrationRiskScoreHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("goaml-integration-go listening on :%s", port)))
