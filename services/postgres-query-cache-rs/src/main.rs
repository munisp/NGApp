...
// postgres-query-cache-rs — Production-hardened service
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().json(json!({"status": "healthy", "service": "postgres-query-cache-rs"}))
}

async fn cached_query(body: web::Json<serde_json::Value>) -> HttpResponse {
...
async fn list_records(req: HttpRequest, state: web::Data<AppState>) -> HttpResponse {
    ...
    let limit: i64 = 50;
    match db_query(&state, "postgres_query_cache_rs", page, limit).await {
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(1);
