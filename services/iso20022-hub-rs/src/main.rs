...
async fn parse_message(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
...
async fn generate_message(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
...
async fn validate_message(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
