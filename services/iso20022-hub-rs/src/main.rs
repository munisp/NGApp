...
fn seed() -> (Vec<Iso20022Message>, Vec<ValidationRule>) {
    let messages = vec![
        Iso20022Message {
            ...
            sender_bic: "54BANKLAGOS".into(), receiver_bic: "ABORNGLA".into(),
        ...
        Iso20022Message {
            ...
            sender_bic: "ZENITHNGLA".into(), receiver_bic: "54BANKLAGOS".into(),
        ...
        Iso20022Message {
            ...
            sender_bic: "54BANKLAGOS".into(), receiver_bic: "CLEARING".into(),
        ...
        Iso20022Message {
            ...
            sender_bic: "54BANKLAGOS".into(), receiver_bic: "CBNNGLA".into(),
        ...
        Iso20022Message {
            ...
            sender_bic: "CBNNGLA".into(), receiver_bic: "54BANKLAGOS".into(),
        ...
        Iso20022Message {
            ...
            sender_bic: "54BANKLAGOS".into(), receiver_bic: "DGLNGLA".into(),
...
async fn healthz() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({
        "status": "ok", "service": "iso20022-hub",
        "middleware": {
            "kafka": { "broker": env_or("KAFKA_BROKER", "localhost:9092"), "topics": ["iso.message.received", "iso.message.validated", "iso.message.routed", "iso.message.settled"] },
            "redis": { "url": env_or("REDIS_URL", "redis://localhost:6379"), "cache_keys": ["iso:bic_directory", "iso:clearing_rules", "iso:message_cache"] },
            "postgres": { "url": env_or("DATABASE_URL", "postgresql://ndsep_user:ndsep_secure_2026@localhost:5432/ndsep_db"), "tables": ["iso_messages", "iso_validation_rules", "iso_routing_rules"] },
            "opensearch": { "url": env_or("OPENSEARCH_URL", "http://localhost:9200"), "indices": ["iso-messages", "iso-audit-trail"] },
            "keycloak": { "url": env_or("KEYCLOAK_URL", "http://localhost:8080"), "realm": "54bank", "client": "iso20022-hub-service" },
            "permify": { "url": env_or("PERMIFY_URL", "http://localhost:3476"), "resources": ["iso_message", "iso_validation_rule"] },
            "dapr": { "url": env_or("DAPR_URL", "http://localhost:3500"), "app_id": "iso20022-hub", "pubsub": "iso-events" },
            "fluvio": { "url": env_or("FLUVIO_URL", "localhost:9003"), "topics": ["iso-nip-stream", "iso-rtgs-stream", "iso-swift-stream"] },
            "temporal": { "url": env_or("TEMPORAL_URL", "localhost:7233"), "workflows": ["MessageValidationWorkflow", "MessageRoutingWorkflow", "SettlementWorkflow"] },
            "mojaloop": { "url": env_or("MOJALOOP_URL", "http://localhost:3002"), "usage": "cross-border-iso20022-settlement" },
            "tigerbeetle": { "url": env_or("TIGERBEETLE_URL", "localhost:3000"), "ledgers": ["iso_settlements", "iso_clearing"] },
            "lakehouse": { "url": env_or("LAKEHOUSE_URL", "http://localhost:8181"), "tables": ["iso_message_archive", "iso_statistics"] },
            "apisix": { "url": env_or("APISIX_URL", "http://localhost:9080"), "routes": ["/api/iso20022/*"] },
            "openappsec": { "url": env_or("OPENAPPSEC_URL", "http://localhost:4000"), "policy": "iso-waf-rules" }
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    ...
    println!("ISO 20022 Hub service on :8162");
    HttpServer::new(move || {
        App::new()
