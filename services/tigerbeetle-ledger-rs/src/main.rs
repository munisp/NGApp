...
// ─── Tigerbeetle Ledger — Infrastructure/Data ────────────────────────────────────────────────
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Tigerbeetle Ledger — Infrastructure/Data",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9569".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Tigerbeetle Ledger v2.0 (Infrastructure/Data) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/tigerbeetle-ledger/process", web::post().to(process_record))
            .route("/v1/tigerbeetle-ledger/audit", web::get().to(get_audit))
