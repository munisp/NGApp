// pgbouncer-manager-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"pgbouncer-manager-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"pgbouncer-manager-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/pgbouncer-manager/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/pgbouncer-manager/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/pgbouncer-manager/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/pgbouncer-manager/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/pgbouncer-manager/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/pgbouncer-manager/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/pgbouncer-manager/score", authMiddleware(pgbouncer_managerScoreHandler))
	mux.HandleFunc("/v1/pgbouncer-manager/validate", authMiddleware(pgbouncer_managerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("pgbouncer-manager-go listening on :%s", port)))
