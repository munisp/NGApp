// materialized-view-engine-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"materialized-view-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"materialized-view-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/materialized-view-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/materialized-view-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/materialized-view-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/materialized-view-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/materialized-view-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/materialized-view-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/materialized-view-engine/score", authMiddleware(materialized_view_engineScoreHandler))
	mux.HandleFunc("/v1/materialized-view-engine/validate", authMiddleware(materialized_view_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("materialized-view-engine-go listening on :%s", port)))
