// mojaloop-admin-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/mojaloop-admin/list", handleList)
	http.HandleFunc("/v1/mojaloop-admin/create", handleCreate)
	http.HandleFunc("/v1/mojaloop-admin/update", handleUpdate)
	http.HandleFunc("/v1/mojaloop-admin/process", handleProcess)
	http.HandleFunc("/v1/mojaloop-admin/audit", handleAudit)
	http.HandleFunc("/v1/mojaloop-admin/stats", handleStats)
	http.HandleFunc("/v1/mojaloop-admin/score", mojaloop_adminScoreHandler)
	http.HandleFunc("/v1/mojaloop-admin/validate", mojaloop_adminValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"mojaloop-admin-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"mojaloop-admin-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/mojaloop-admin/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/mojaloop-admin/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/mojaloop-admin/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/mojaloop-admin/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/mojaloop-admin/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/mojaloop-admin/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/mojaloop-admin/score", authMiddleware(mojaloop_adminScoreHandler))
	mux.HandleFunc("/v1/mojaloop-admin/validate", authMiddleware(mojaloop_adminValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("mojaloop-admin-go listening on :%s", port)))
