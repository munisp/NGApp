// debt-collection-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/debt-collection/update", handleUpdate)
	http.HandleFunc("/v1/debt-collection/process", handleProcess)
	http.HandleFunc("/v1/debt-collection/audit", handleAudit)
	http.HandleFunc("/v1/debt-collection/stats", handleStats)
	http.HandleFunc("/v1/debt-collection/score", debt_collectionScoreHandler)
	http.HandleFunc("/v1/debt-collection/validate", debt_collectionValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"debt-collection-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"debt-collection-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/debt-collection/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/debt-collection/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/debt-collection/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/debt-collection/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/debt-collection/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/debt-collection/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/debt-collection/score", authMiddleware(debt_collectionScoreHandler))
	mux.HandleFunc("/v1/debt-collection/validate", authMiddleware(debt_collectionValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("debt-collection-go listening on :%s", port)))
