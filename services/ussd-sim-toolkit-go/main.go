// ussd-sim-toolkit-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/ussd-sim-toolkit/update", handleUpdate)
	http.HandleFunc("/v1/ussd-sim-toolkit/process", handleProcess)
	http.HandleFunc("/v1/ussd-sim-toolkit/audit", handleAudit)
	http.HandleFunc("/v1/ussd-sim-toolkit/stats", handleStats)
	http.HandleFunc("/v1/ussd-sim-toolkit/score", ussd_sim_toolkitScoreHandler)
	http.HandleFunc("/v1/ussd-sim-toolkit/validate", ussd_sim_toolkitValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"ussd-sim-toolkit-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"ussd-sim-toolkit-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/ussd-sim-toolkit/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/ussd-sim-toolkit/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/ussd-sim-toolkit/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/ussd-sim-toolkit/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/ussd-sim-toolkit/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/ussd-sim-toolkit/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/ussd-sim-toolkit/score", authMiddleware(ussd_sim_toolkitScoreHandler))
	mux.HandleFunc("/v1/ussd-sim-toolkit/validate", authMiddleware(ussd_sim_toolkitValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("ussd-sim-toolkit-go listening on :%s", port)))
