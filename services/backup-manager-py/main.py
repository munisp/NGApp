...
class Handler(BaseHTTPRequestHandler):
    ...
    def respond(self, code, data):
        ...
        self.send_header("X-Service", "backup-manager-py")
        self.end_headers()
    ...
    def do_GET(self):
        ...
        if path == "/healthz":
            self.respond(200, {
                "service": "backup-manager-py", "status": "healthy", "version": "2.0.0",
                "uptime_secs": int(time.time() - START_TIME),
                "domain": "Backup Manager — Infrastructure/Ops",
                "middleware": {
                    "kafka": "backup-manager.events, backup-manager.audit",
                    "postgres": "backup_manager_records",
                    "redis": "backup-manager_cache",
                    "temporal": "BackupManagerWorkflow",
                    "permify": "backup-manager:manage, backup-manager:view",
                    "opensearch": "backup-manager-2026",
                },
            })
        elif path == "/v1/backup-manager/list":
        ...
        elif path == "/v1/backup-manager/audit":
            self.respond(200, {"audit_log": audit_log, "total": len(audit_log)})
        elif path == "/v1/backup-manager/stats":
    ...
    def do_POST(self):
        ...
        if path == "/v1/backup-manager/create":
            rec = {
...
if __name__ == "__main__":
    ...
    print(f"Backup Manager v2.0 (Infrastructure/Ops) on :{port}")
