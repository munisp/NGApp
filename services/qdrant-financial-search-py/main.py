...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/v1/search/semantic":
            ...
            cache_set("semantic_search_last", json.dumps(body))
            ...
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            result = db_insert(SERVICE_NAME, body)
            cache_set("index_document_last", json.dumps(body))
            gl_url = os.environ.get("GL_ENGINE_URL", "http://gl-engine-go:8080")
            ...
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            result = db_insert(SERVICE_NAME, body)
            cache_set("find_similar_last", json.dumps(body))
            gl_url = os.environ.get("GL_ENGINE_URL", "http://gl-engine-go:8080")
            ...
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            result = db_insert(SERVICE_NAME, body)
            cache_set("last_post", json.dumps(body))
