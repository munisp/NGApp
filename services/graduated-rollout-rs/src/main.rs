use tokio_postgres;
// graduated-rollout-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM graduated_rollout";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM graduated_rollout ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8235".into()).parse().unwrap_or(8235);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "graduated-rollout-rs".into(),
        table_name: "graduated_rollout".into(),
    });
    
    println!("[graduated-rollout-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/graduated-rollout/list", web::get().to(list))
            .route("/v1/graduated-rollout/stats", web::get().to(stats))
            .route("/v1/graduated-rollout/{id}", web::get().to(get_by_id))
            .route("/v1/graduated-rollout", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
