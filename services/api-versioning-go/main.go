// api-versioning-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/api-versioning/list", handleList)
	http.HandleFunc("/v1/api-versioning/create", handleCreate)
	http.HandleFunc("/v1/api-versioning/update", handleUpdate)
	http.HandleFunc("/v1/api-versioning/process", handleProcess)
	http.HandleFunc("/v1/api-versioning/audit", handleAudit)
	http.HandleFunc("/v1/api-versioning/stats", handleStats)
	http.HandleFunc("/v1/api-versioning/score", api_versioningScoreHandler)
	http.HandleFunc("/v1/api-versioning/validate", api_versioningValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"api-versioning-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"api-versioning-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/api-versioning/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/api-versioning/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/api-versioning/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/api-versioning/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/api-versioning/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/api-versioning/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/api-versioning/score", authMiddleware(api_versioningScoreHandler))
	mux.HandleFunc("/v1/api-versioning/validate", authMiddleware(api_versioningValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("api-versioning-go listening on :%s", port)))
