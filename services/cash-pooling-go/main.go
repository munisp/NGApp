// cash-pooling-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/cash-pooling/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"cash-pooling-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cash-pooling-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Treasury/Markets"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cash-pooling/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cash-pooling/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cash-pooling/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cash-pooling/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cash-pooling/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cash-pooling/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cash-pooling/score", authMiddleware(cash_poolingScoreHandler))
	mux.HandleFunc("/v1/cash-pooling/validate", authMiddleware(cash_poolingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cash-pooling-go listening on :%s", port)))
