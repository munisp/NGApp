// kafka-broker-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"kafka-broker-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"kafka-broker-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/kafka-broker/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/kafka-broker/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/kafka-broker/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/kafka-broker/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/kafka-broker/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/kafka-broker/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/kafka-broker/throughput", authMiddleware(kafka_brokerThroughputHandler))
	mux.HandleFunc("/v1/kafka-broker/partition", authMiddleware(kafka_brokerPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("kafka-broker-go listening on :%s", port)))
