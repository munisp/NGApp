// expense-mgmt-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"expense-mgmt-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"expense-mgmt-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/expense-mgmt/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/expense-mgmt/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/expense-mgmt/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/expense-mgmt/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/expense-mgmt/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/expense-mgmt/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/expense-mgmt/score", authMiddleware(expense_mgmtScoreHandler))
	mux.HandleFunc("/v1/expense-mgmt/validate", authMiddleware(expense_mgmtValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("expense-mgmt-go listening on :%s", port)))
