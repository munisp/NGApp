// browser-fingerprint-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"browser-fingerprint-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"browser-fingerprint-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/browser-fingerprint/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/browser-fingerprint/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/browser-fingerprint/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/browser-fingerprint/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/browser-fingerprint/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/browser-fingerprint/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/browser-fingerprint/score", authMiddleware(browser_fingerprintScoreHandler))
	mux.HandleFunc("/v1/browser-fingerprint/validate", authMiddleware(browser_fingerprintValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("browser-fingerprint-go listening on :%s", port)))
