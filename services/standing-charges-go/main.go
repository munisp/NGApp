// standing-charges-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/standing-charges/list", handleList)
	http.HandleFunc("/v1/standing-charges/create", handleCreate)
	http.HandleFunc("/v1/standing-charges/update", handleUpdate)
	http.HandleFunc("/v1/standing-charges/process", handleProcess)
	http.HandleFunc("/v1/standing-charges/audit", handleAudit)
	http.HandleFunc("/v1/standing-charges/stats", handleStats)
	http.HandleFunc("/v1/standing-charges/score", standing_chargesScoreHandler)
	http.HandleFunc("/v1/standing-charges/validate", standing_chargesValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"standing-charges-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"standing-charges-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Billing"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/standing-charges/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/standing-charges/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/standing-charges/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/standing-charges/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/standing-charges/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/standing-charges/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/standing-charges/score", authMiddleware(standing_chargesScoreHandler))
	mux.HandleFunc("/v1/standing-charges/validate", authMiddleware(standing_chargesValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("standing-charges-go listening on :%s", port)))
