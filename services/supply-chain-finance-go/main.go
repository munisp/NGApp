// supply-chain-finance-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/supply-chain-finance/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"supply-chain-finance-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"supply-chain-finance-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/supply-chain-finance/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/supply-chain-finance/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/supply-chain-finance/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/supply-chain-finance/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/supply-chain-finance/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/supply-chain-finance/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/supply-chain-finance/score", authMiddleware(supply_chain_financeScoreHandler))
	mux.HandleFunc("/v1/supply-chain-finance/validate", authMiddleware(supply_chain_financeValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("supply-chain-finance-go listening on :%s", port)))
