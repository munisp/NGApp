...
// ─── Relationship Pricing — Banking Ops ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "REL-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Banking Ops", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "REL-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Banking Ops", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "REL-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Banking Ops", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("REL-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Relationship Pricing — Banking Ops",
        "middleware": {
            "kafka": "relationship-pricing.events, relationship-pricing.audit",
            "postgres": "relationship_pricing_records",
            "redis": "relationship-pricing_cache",
            "temporal": "RelationshipPricingWorkflow",
            "opensearch": "relationship-pricing-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9547".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Relationship Pricing v2.0 (Banking Ops) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/relationship-pricing/list", web::get().to(list_records))
            .route("/v1/relationship-pricing/create", web::post().to(create_record))
            .route("/v1/relationship-pricing/process", web::post().to(process_record))
            .route("/v1/relationship-pricing/audit", web::get().to(get_audit))
            .route("/v1/relationship-pricing/stats", web::get().to(get_stats))
