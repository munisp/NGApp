use tokio_postgres;
// portfolio-mgmt-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM portfolio_mgmt";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM portfolio_mgmt ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("2023".into()).parse().unwrap_or(2023);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "portfolio-mgmt-rs".into(),
        table_name: "portfolio_mgmt".into(),
    });
    
    println!("[portfolio-mgmt-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/portfolio-mgmt/list", web::get().to(list))
            .route("/v1/portfolio-mgmt/stats", web::get().to(stats))
            .route("/v1/portfolio-mgmt/{id}", web::get().to(get_by_id))
            .route("/v1/portfolio-mgmt", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
