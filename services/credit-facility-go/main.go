// credit-facility-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"credit-facility-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"credit-facility-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/credit-facility/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/credit-facility/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/credit-facility/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/credit-facility/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/credit-facility/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/credit-facility/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/credit-facility/compute-emi", authMiddleware(credit_facilityEMIHandler))
	mux.HandleFunc("/v1/credit-facility/credit-check", authMiddleware(credit_facilityCreditCheckHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("credit-facility-go listening on :%s", port)))
