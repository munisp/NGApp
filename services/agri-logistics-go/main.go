// agri-logistics-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"agri-logistics-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"agri-logistics-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/agri-logistics/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/agri-logistics/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/agri-logistics/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/agri-logistics/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/agri-logistics/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/agri-logistics/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/agri-logistics/yield-score", authMiddleware(agri_logisticsYieldHandler))
	mux.HandleFunc("/v1/agri-logistics/risk-assess", authMiddleware(agri_logisticsRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("agri-logistics-go listening on :%s", port)))
