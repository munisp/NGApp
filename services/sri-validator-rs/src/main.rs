...
// sri-validator-rs — Production-hardened service
...
async fn validate_sri(body: web::Json<serde_json::Value>) -> HttpResponse {
    let input = body.into_inner();
    let hash_s = input.get("hash").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let hash = hash_s.as_str();
    let result = sri_format_valid(hash);
...
async fn list_records(req: HttpRequest, state: web::Data<AppState>) -> HttpResponse {
    ...
    let limit: i64 = 50;
    match db_query(&state, "sri_validator_rs", page, limit).await {
