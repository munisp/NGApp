// agri-input-marketplace-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/agri-input-marketplace/list", handleList)
	http.HandleFunc("/v1/agri-input-marketplace/create", handleCreate)
	http.HandleFunc("/v1/agri-input-marketplace/update", handleUpdate)
	http.HandleFunc("/v1/agri-input-marketplace/process", handleProcess)
	http.HandleFunc("/v1/agri-input-marketplace/audit", handleAudit)
	http.HandleFunc("/v1/agri-input-marketplace/stats", handleStats)
	http.HandleFunc("/v1/agri-input-marketplace/yield-score", agri_input_marketplaceYieldHandler)
	http.HandleFunc("/v1/agri-input-marketplace/risk-assess", agri_input_marketplaceRiskHandler)
...
fmt.Fprintf(w, "requests_total{service=\"agri-input-marketplace-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"agri-input-marketplace-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/agri-input-marketplace/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/agri-input-marketplace/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/agri-input-marketplace/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/agri-input-marketplace/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/agri-input-marketplace/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/agri-input-marketplace/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/agri-input-marketplace/yield-score", authMiddleware(agri_input_marketplaceYieldHandler))
	mux.HandleFunc("/v1/agri-input-marketplace/risk-assess", authMiddleware(agri_input_marketplaceRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("agri-input-marketplace-go listening on :%s", port)))
