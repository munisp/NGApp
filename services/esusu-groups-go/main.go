// esusu-groups-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"esusu-groups-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"esusu-groups-go\"} %d\n", errs)
}
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("esusu-groups-go listening on :%s", port)))
