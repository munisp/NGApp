// cbn-agsmeis-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"cbn-agsmeis-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cbn-agsmeis-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cbn-agsmeis/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cbn-agsmeis/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cbn-agsmeis/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cbn-agsmeis/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cbn-agsmeis/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cbn-agsmeis/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cbn-agsmeis/generate", authMiddleware(cbn_agsmeisGenerateHandler))
	mux.HandleFunc("/v1/cbn-agsmeis/validate-period", authMiddleware(cbn_agsmeisValidateHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cbn-agsmeis-go listening on :%s", port)))
