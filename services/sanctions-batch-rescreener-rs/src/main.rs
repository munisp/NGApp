use tokio_postgres;
// sanctions-batch-rescreener-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM sanctions_batch_rescreener";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM sanctions_batch_rescreener ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "sanctions-batch-rescreener-rs".into(),
        table_name: "sanctions_batch_rescreener".into(),
    });
    
    println!("[sanctions-batch-rescreener-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/sanctions-batch-rescreener/{id}", web::get().to(get_by_id))
            .route("/v1/sanctions-batch-rescreener", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
