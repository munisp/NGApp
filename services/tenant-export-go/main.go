// tenant-export-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/tenant-export/update", handleUpdate)
	http.HandleFunc("/v1/tenant-export/process", handleProcess)
	http.HandleFunc("/v1/tenant-export/audit", handleAudit)
	http.HandleFunc("/v1/tenant-export/stats", handleStats)
	http.HandleFunc("/v1/tenant-export/score", tenant_exportScoreHandler)
	http.HandleFunc("/v1/tenant-export/validate", tenant_exportValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"tenant-export-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"tenant-export-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/tenant-export/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/tenant-export/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/tenant-export/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/tenant-export/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/tenant-export/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/tenant-export/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/tenant-export/score", authMiddleware(tenant_exportScoreHandler))
	mux.HandleFunc("/v1/tenant-export/validate", authMiddleware(tenant_exportValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("tenant-export-go listening on :%s", port)))
