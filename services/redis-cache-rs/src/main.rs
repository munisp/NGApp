...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8202".into()).parse().unwrap_or(8202);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "redis-cache-rs".into(),
        table_name: "redis_cache".into(),
    });
    
    println!("[redis-cache-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/redis-cache/{id}", web::get().to(get_by_id))
            .route("/v1/redis-cache", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
