...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Redis Cache — KYC/Identity",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9546".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Redis Cache v2.0 (KYC/Identity) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/redis-cache/process", web::post().to(process_record))
            .route("/v1/redis-cache/audit", web::get().to(get_audit))
