// gl-engine-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8251".into()).parse().unwrap_or(8251);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "gl-engine-rs".into(),
        table_name: "gl_engine".into(),
    });
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/gl-engine/list", web::get().to(list))
            .route("/v1/gl-engine/stats", web::get().to(stats))
            .route("/v1/gl-engine/{id}", web::get().to(get_by_id))
            .route("/v1/gl-engine", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
