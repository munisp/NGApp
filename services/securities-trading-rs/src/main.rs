...
async fn place_order(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
...
async fn match_orders(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
...
async fn portfolio_value(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
