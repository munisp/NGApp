...
// ─── Securities Trading — Treasury/Markets ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "SEC-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Treasury/Markets", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "SEC-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Treasury/Markets", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "SEC-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Treasury/Markets", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Securities Trading — Treasury/Markets",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9555".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Securities Trading v2.0 (Treasury/Markets) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/securities-trading/process", web::post().to(process_record))
            .route("/v1/securities-trading/audit", web::get().to(get_audit))
