use tokio_postgres;
// read-replica-router-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM read_replica_router";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM read_replica_router ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "read-replica-router-rs".into(),
        table_name: "read_replica_router".into(),
    });
    
    println!("[read-replica-router-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/read-replica-router/list", web::get().to(list))
            .route("/v1/read-replica-router/stats", web::get().to(stats))
            .route("/v1/read-replica-router/{id}", web::get().to(get_by_id))
            .route("/v1/read-replica-router", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
