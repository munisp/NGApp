use tokio_postgres;
// adaptive-rate-limiter-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM adaptive_rate_limiter";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM adaptive_rate_limiter ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8493".into()).parse().unwrap_or(8493);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "adaptive-rate-limiter-rs".into(),
        table_name: "adaptive_rate_limiter".into(),
    });
    
    println!("[adaptive-rate-limiter-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/adaptive-rate-limiter/list", web::get().to(list))
            .route("/v1/adaptive-rate-limiter/stats", web::get().to(stats))
            .route("/v1/adaptive-rate-limiter/{id}", web::get().to(get_by_id))
            .route("/v1/adaptive-rate-limiter", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
