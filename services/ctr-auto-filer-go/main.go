// ctr-auto-filer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/ctr-auto-filer/update", handleUpdate)
	http.HandleFunc("/v1/ctr-auto-filer/process", handleProcess)
	http.HandleFunc("/v1/ctr-auto-filer/audit", handleAudit)
	http.HandleFunc("/v1/ctr-auto-filer/stats", handleStats)
	http.HandleFunc("/v1/ctr-auto-filer/score", ctr_auto_filerScoreHandler)
	http.HandleFunc("/v1/ctr-auto-filer/validate", ctr_auto_filerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"ctr-auto-filer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"ctr-auto-filer-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "AML/Compliance"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/ctr-auto-filer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/ctr-auto-filer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/ctr-auto-filer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/ctr-auto-filer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/ctr-auto-filer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/ctr-auto-filer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/ctr-auto-filer/score", authMiddleware(ctr_auto_filerScoreHandler))
	mux.HandleFunc("/v1/ctr-auto-filer/validate", authMiddleware(ctr_auto_filerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("ctr-auto-filer-go listening on :%s", port)))
