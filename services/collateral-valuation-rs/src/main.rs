use tokio_postgres;
// collateral-valuation-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM collateral_valuation";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM collateral_valuation ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "collateral-valuation-rs".into(),
        table_name: "collateral_valuation".into(),
    });
    
    println!("[collateral-valuation-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/collateral-valuation/list", web::get().to(list))
            .route("/v1/collateral-valuation/stats", web::get().to(stats))
            .route("/v1/collateral-valuation/{id}", web::get().to(get_by_id))
            .route("/v1/collateral-valuation", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
