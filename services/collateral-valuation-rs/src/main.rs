...
fn seed_valuations() -> Vec<Valuation> {
    vec![
        ...
        Valuation { id: "VAL-006".into(), collateral_id: "COL-006".into(), collateral_type: "cash_deposit".into(), description: "Lien on fixed deposit — 12-month tenor".into(), owner: "Dangote Cement PLC".into(), market_value: 2_000_000_000.0, forced_sale_value: 2_000_000_000.0, haircut_pct: 0.0, net_realizable_value: 2_000_000_000.0, currency: "NGN".into(), valuer: "54Bank Internal".into(), valuation_date: "2026-05-01".into(), expiry_date: "2027-05-01".into(), insurance_value: 0.0, insurance_expiry: "N/A".into(), lien_status: "perfected".into(), status: "current".into() },
    ]
}
...
async fn main() -> std::io::Result<()> {
    let addr = std::env::var("ADDR").unwrap_or_else(|_| "0.0.0.0:8154".to_string());
    let state = web::Data::new(AppState {
        valuations: Mutex::new(seed_valuations()),
    });
    println!("collateral-valuation listening on {addr}");
    HttpServer::new(move || {
        App::new()
