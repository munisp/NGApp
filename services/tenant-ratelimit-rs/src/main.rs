...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "TEN-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Platform", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "TEN-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Platform", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "TEN-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Platform", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("TEN-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Tenant Ratelimit — Platform",
        "middleware": {
            "kafka": "tenant-ratelimit.events, tenant-ratelimit.audit",
            "postgres": "tenant_ratelimit_records",
            "redis": "tenant-ratelimit_cache",
            "temporal": "TenantRatelimitWorkflow",
            "opensearch": "tenant-ratelimit-2026",
        }
    }))
}
...
async fn list_records(state: web::Data<AppState>) -> HttpResponse {
    ...
    HttpResponse::Ok().json(json!({"records": *records, "total": records.len(), "domain": "Platform"}))
}
...
async fn get_stats(state: web::Data<AppState>) -> HttpResponse {
    ...
    HttpResponse::Ok().json(json!({
        ...
        "avgScore": avg_score, "domain": "Platform",
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9566".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Tenant Ratelimit v2.0 (Platform) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/tenant-ratelimit/list", web::get().to(list_records))
            .route("/v1/tenant-ratelimit/create", web::post().to(create_record))
            .route("/v1/tenant-ratelimit/process", web::post().to(process_record))
            .route("/v1/tenant-ratelimit/audit", web::get().to(get_audit))
            .route("/v1/tenant-ratelimit/stats", web::get().to(get_stats))
