// sorted-set-ranking-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"sorted-set-ranking-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"sorted-set-ranking-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/sorted-set-ranking/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/sorted-set-ranking/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/sorted-set-ranking/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/sorted-set-ranking/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/sorted-set-ranking/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/sorted-set-ranking/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/sorted-set-ranking/score", authMiddleware(sorted_set_rankingScoreHandler))
	mux.HandleFunc("/v1/sorted-set-ranking/validate", authMiddleware(sorted_set_rankingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("sorted-set-ranking-go listening on :%s", port)))
