...
// ─── Whatsapp Banking Flows — Messaging/Channels ────────────────────────────────────────────────
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Whatsapp Banking Flows — Messaging/Channels",
        "middleware": {
            "kafka": "whatsapp-banking-flows.events, whatsapp-banking-flows.audit",
            "postgres": "whatsapp_banking_flows_records",
            "redis": "whatsapp-banking-flows_cache",
            "temporal": "WhatsappBankingFlowsWorkflow",
            "opensearch": "whatsapp-banking-flows-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9584".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Whatsapp Banking Flows v2.0 (Messaging/Channels) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/whatsapp-banking-flows/list", web::get().to(list_records))
            .route("/v1/whatsapp-banking-flows/create", web::post().to(create_record))
            .route("/v1/whatsapp-banking-flows/process", web::post().to(process_record))
            .route("/v1/whatsapp-banking-flows/audit", web::get().to(get_audit))
            .route("/v1/whatsapp-banking-flows/stats", web::get().to(get_stats))
