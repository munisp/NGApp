// nirsal-agro-geocoop-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/nirsal-agro-geocoop/list", handleList)
	http.HandleFunc("/v1/nirsal-agro-geocoop/create", handleCreate)
	http.HandleFunc("/v1/nirsal-agro-geocoop/update", handleUpdate)
	http.HandleFunc("/v1/nirsal-agro-geocoop/process", handleProcess)
	http.HandleFunc("/v1/nirsal-agro-geocoop/audit", handleAudit)
	http.HandleFunc("/v1/nirsal-agro-geocoop/stats", handleStats)
	http.HandleFunc("/v1/nirsal-agro-geocoop/score", nirsal_agro_geocoopScoreHandler)
	http.HandleFunc("/v1/nirsal-agro-geocoop/validate", nirsal_agro_geocoopValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"nirsal-agro-geocoop-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"nirsal-agro-geocoop-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/nirsal-agro-geocoop/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/score", authMiddleware(nirsal_agro_geocoopScoreHandler))
	mux.HandleFunc("/v1/nirsal-agro-geocoop/validate", authMiddleware(nirsal_agro_geocoopValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("nirsal-agro-geocoop-go listening on :%s", port)))
