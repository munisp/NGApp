// leasing-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/leasing/list", handleList)
	http.HandleFunc("/v1/leasing/create", handleCreate)
	http.HandleFunc("/v1/leasing/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"leasing-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"leasing-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/leasing/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/leasing/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/leasing/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/leasing/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/leasing/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/leasing/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/leasing/score", authMiddleware(leasingScoreHandler))
	mux.HandleFunc("/v1/leasing/validate", authMiddleware(leasingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("leasing-go listening on :%s", port)))
