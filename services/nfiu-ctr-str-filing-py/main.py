#!/usr/bin/env python3
"""nfiu-ctr-str-filing-py — Production Python microservice with Postgres, Kafka, Redis integration."""
...
logging.basicConfig(level=logging.INFO, format='[nfiu-ctr-str-filing-py] %(levelname)s %(message)s')
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path in ('/healthz', '/health'):
            ...
            self._stats()
        elif path.startswith('/v1/nfiu-ctr-str-filing/'):
    ...
    def _list(self, params):
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # Count total
                cur.execute(f'SELECT count(*) as cnt FROM "nfiu_ctr_str_filing"')
                total = cur.fetchone()['cnt']
                
                # Fetch rows
                cur.execute(f'SELECT * FROM "nfiu_ctr_str_filing" ORDER BY id LIMIT %s OFFSET %s', (limit, offset))
    ...
    def _stats(self):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(f'SELECT count(*) FROM "nfiu_ctr_str_filing"')
    ...
    def _get_by_id(self, item_id):
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(f'SELECT * FROM "nfiu_ctr_str_filing" WHERE id = %s', (item_id,))
                row = cur.fetchone()
                if row:
                    item = dict(row)
                    for k, v in item.items():
