// fraud-detection-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn main() -> std::io::Result<()> {
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "fraud-detection-rs".into(),
        table_name: "fraud_detection".into(),
    });
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/fraud-detection/list", web::get().to(list))
            .route("/v1/fraud-detection/stats", web::get().to(stats))
            .route("/v1/fraud-detection/{id}", web::get().to(get_by_id))
            .route("/v1/fraud-detection", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
