// db-migration-manager-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/db-migration-manager/update", handleUpdate)
	http.HandleFunc("/v1/db-migration-manager/process", handleProcess)
	http.HandleFunc("/v1/db-migration-manager/audit", handleAudit)
	http.HandleFunc("/v1/db-migration-manager/stats", handleStats)
	http.HandleFunc("/v1/db-migration-manager/score", db_migration_managerScoreHandler)
	http.HandleFunc("/v1/db-migration-manager/validate", db_migration_managerValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"db-migration-manager-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"db-migration-manager-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/db-migration-manager/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/db-migration-manager/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/db-migration-manager/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/db-migration-manager/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/db-migration-manager/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/db-migration-manager/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/db-migration-manager/score", authMiddleware(db_migration_managerScoreHandler))
	mux.HandleFunc("/v1/db-migration-manager/validate", authMiddleware(db_migration_managerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("db-migration-manager-go listening on :%s", port)))
