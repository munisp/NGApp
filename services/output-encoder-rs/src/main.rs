use tokio_postgres;
// output-encoder-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM output_encoder";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM output_encoder ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8518".into()).parse().unwrap_or(8518);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "output-encoder-rs".into(),
        table_name: "output_encoder".into(),
    });
    
    println!("[output-encoder-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/output-encoder/list", web::get().to(list))
            .route("/v1/output-encoder/stats", web::get().to(stats))
            .route("/v1/output-encoder/{id}", web::get().to(get_by_id))
            .route("/v1/output-encoder", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
