...
@app.route("/data")
def data_endpoint():
    """Query regulatoryReports from Postgres."""
    db_url = os.environ.get("DATABASE_URL", "")
    if not db_url:
    ...
    try:
        ...
        offset = (page - 1) * limit
        cur.execute('SELECT count(*) FROM "regulatoryReports"')
        total = cur.fetchone()["count"]
        cur.execute('SELECT "tenantId", status FROM "regulatoryReports" ORDER BY id LIMIT %s OFFSET %s', (limit, offset))
        items = cur.fetchall()
        conn.close()
