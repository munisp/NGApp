...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Sanctions Screening — AML/Compliance",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9552".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Sanctions Screening v2.0 (AML/Compliance) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/sanctions-screening/process", web::post().to(process_record))
            .route("/v1/sanctions-screening/audit", web::get().to(get_audit))
