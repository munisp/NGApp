use tokio_postgres;
// sanctions-screening-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM sanctions_screening";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM sanctions_screening ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("8283".into()).parse().unwrap_or(8283);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "sanctions-screening-rs".into(),
        table_name: "sanctions_screening".into(),
    });
    
    println!("[sanctions-screening-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/sanctions-screening/list", web::get().to(list))
            .route("/v1/sanctions-screening/stats", web::get().to(stats))
            .route("/v1/sanctions-screening/{id}", web::get().to(get_by_id))
            .route("/v1/sanctions-screening", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
