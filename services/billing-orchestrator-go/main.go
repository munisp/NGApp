// billing-orchestrator-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/billing-orchestrator/list", handleList)
	http.HandleFunc("/v1/billing-orchestrator/create", handleCreate)
	http.HandleFunc("/v1/billing-orchestrator/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"billing-orchestrator-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"billing-orchestrator-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/billing-orchestrator/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/billing-orchestrator/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/billing-orchestrator/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/billing-orchestrator/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/billing-orchestrator/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/billing-orchestrator/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/billing-orchestrator/score", authMiddleware(billing_orchestratorScoreHandler))
	mux.HandleFunc("/v1/billing-orchestrator/validate", authMiddleware(billing_orchestratorValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("billing-orchestrator-go listening on :%s", port)))
