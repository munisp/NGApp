// voice-ivr-menu-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"voice-ivr-menu-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"voice-ivr-menu-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/voice-ivr-menu/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/voice-ivr-menu/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/voice-ivr-menu/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/voice-ivr-menu/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/voice-ivr-menu/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/voice-ivr-menu/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/voice-ivr-menu/score", authMiddleware(voice_ivr_menuScoreHandler))
	mux.HandleFunc("/v1/voice-ivr-menu/validate", authMiddleware(voice_ivr_menuValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("voice-ivr-menu-go listening on :%s", port)))
