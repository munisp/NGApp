...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Livestock Management — Agriculture",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9517".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Livestock Management v2.0 (Agriculture) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/livestock-management/process", web::post().to(process_record))
            .route("/v1/livestock-management/audit", web::get().to(get_audit))
