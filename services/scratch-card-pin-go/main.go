// scratch-card-pin-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"scratch-card-pin-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"scratch-card-pin-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/scratch-card-pin/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/scratch-card-pin/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/scratch-card-pin/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/scratch-card-pin/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/scratch-card-pin/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/scratch-card-pin/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/scratch-card-pin/score", authMiddleware(scratch_card_pinScoreHandler))
	mux.HandleFunc("/v1/scratch-card-pin/validate", authMiddleware(scratch_card_pinValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("scratch-card-pin-go listening on :%s", port)))
