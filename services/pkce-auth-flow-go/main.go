// pkce-auth-flow-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"pkce-auth-flow-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"pkce-auth-flow-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/pkce-auth-flow/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/pkce-auth-flow/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/pkce-auth-flow/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/pkce-auth-flow/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/pkce-auth-flow/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/pkce-auth-flow/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/pkce-auth-flow/validate", authMiddleware(pkce_auth_flowValidateHandler))
	mux.HandleFunc("/v1/pkce-auth-flow/rate-limit", authMiddleware(pkce_auth_flowRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("pkce-auth-flow-go listening on :%s", port)))
