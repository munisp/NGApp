// cac-realtime-api-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"cac-realtime-api-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cac-realtime-api-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cac-realtime-api/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cac-realtime-api/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cac-realtime-api/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cac-realtime-api/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cac-realtime-api/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cac-realtime-api/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cac-realtime-api/score", authMiddleware(cac_realtime_apiScoreHandler))
	mux.HandleFunc("/v1/cac-realtime-api/validate", authMiddleware(cac_realtime_apiValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cac-realtime-api-go listening on :%s", port)))
