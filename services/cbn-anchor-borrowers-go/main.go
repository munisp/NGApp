// cbn-anchor-borrowers-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"cbn-anchor-borrowers-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cbn-anchor-borrowers-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cbn-anchor-borrowers/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/yield-score", authMiddleware(cbn_anchor_borrowersYieldHandler))
	mux.HandleFunc("/v1/cbn-anchor-borrowers/risk-assess", authMiddleware(cbn_anchor_borrowersRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cbn-anchor-borrowers-go listening on :%s", port)))
