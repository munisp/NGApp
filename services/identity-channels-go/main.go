// identity-channels-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"identity-channels-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"identity-channels-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/identity-channels/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/identity-channels/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/identity-channels/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/identity-channels/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/identity-channels/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/identity-channels/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/identity-channels/score", authMiddleware(identity_channelsScoreHandler))
	mux.HandleFunc("/v1/identity-channels/validate", authMiddleware(identity_channelsValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("identity-channels-go listening on :%s", port)))
