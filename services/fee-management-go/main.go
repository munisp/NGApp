// fee-management-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/fee-management/list", handleList)
	http.HandleFunc("/v1/fee-management/create", handleCreate)
	http.HandleFunc("/v1/fee-management/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"fee-management-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"fee-management-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/fee-management/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/fee-management/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/fee-management/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/fee-management/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/fee-management/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/fee-management/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/fee-management/score", authMiddleware(fee_managementScoreHandler))
	mux.HandleFunc("/v1/fee-management/validate", authMiddleware(fee_managementValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("fee-management-go listening on :%s", port)))
