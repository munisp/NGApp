// security-hardening-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"security-hardening-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"security-hardening-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/security-hardening/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/security-hardening/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/security-hardening/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/security-hardening/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/security-hardening/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/security-hardening/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/security-hardening/fx-convert", authMiddleware(security_hardeningFXHandler))
	mux.HandleFunc("/v1/security-hardening/risk-calc", authMiddleware(security_hardeningRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("security-hardening-go listening on :%s", port)))
