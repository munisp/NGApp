...
async fn healthz() -> HttpResponse {
    ...
    HttpResponse::Ok().json(serde_json::json!({
        "service": "interbank-lending-rs", "status": "healthy", "version": "1.0.0",
        "middleware": {
            "kafka": { "broker": c.kafka_broker, "topics": ["interbank.deals", "interbank.settlements", "interbank.rates"], "usage": "deal events and rate feeds" },
            "redis": { "url": c.redis_url, "cache_keys": ["interbank:rates", "interbank:limits", "interbank:positions"] },
            "postgres": { "url": c.postgres_url, "tables": ["interbank_deals", "interbank_rates", "counterparty_limits"] },
            "opensearch": { "url": c.opensearch_url, "indices": ["interbank-deals", "interbank-audit"] },
            "keycloak": { "url": c.keycloak_url, "realm": "54bank", "client": "interbank-lending" },
            "permify": { "url": c.permify_url, "resources": ["interbank_deal", "counterparty_limit"] },
            "dapr": { "url": c.dapr_url, "app_id": "interbank-lending", "pubsub": "interbank-pubsub" },
            "fluvio": { "url": c.fluvio_url, "topics": ["interbank-rate-stream", "deal-settlement-stream"] },
            "temporal": { "url": c.temporal_url, "workflows": ["DealSettlementWorkflow", "RateUpdateWorkflow"] },
            "mojaloop": { "url": c.mojaloop_url, "usage": "interbank settlement via NIBSS" },
            "tigerbeetle": { "url": c.tigerbeetle_url, "ledgers": ["interbank_nostro", "interbank_vostro"] },
            "lakehouse": { "url": c.lakehouse_url, "tables": ["interbank_deals_history", "rate_analytics"] },
            "apisix": { "url": c.apisix_url, "routes": ["/v1/interbank/*"] },
            "openappsec": { "url": c.openappsec_url, "policy": "interbank-waf" }
        }
    }))
}
