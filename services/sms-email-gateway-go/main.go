// sms-email-gateway-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"sms-email-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"sms-email-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/sms-email-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/sms-email-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/sms-email-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/sms-email-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/sms-email-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/sms-email-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/sms-email-gateway/health-score", authMiddleware(sms_email_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/sms-email-gateway/circuit-state", authMiddleware(sms_email_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("sms-email-gateway-go listening on :%s", port)))
