// redis-session-store-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/redis-session-store/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"redis-session-store-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"redis-session-store-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/redis-session-store/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/redis-session-store/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/redis-session-store/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/redis-session-store/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/redis-session-store/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/redis-session-store/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/redis-session-store/validate", authMiddleware(redis_session_storeValidateHandler))
	mux.HandleFunc("/v1/redis-session-store/rate-limit", authMiddleware(redis_session_storeRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("redis-session-store-go listening on :%s", port)))
