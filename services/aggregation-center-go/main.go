// aggregation-center-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"aggregation-center-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"aggregation-center-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/aggregation-center/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/aggregation-center/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/aggregation-center/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/aggregation-center/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/aggregation-center/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/aggregation-center/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/aggregation-center/yield-score", authMiddleware(aggregation_centerYieldHandler))
	mux.HandleFunc("/v1/aggregation-center/risk-assess", authMiddleware(aggregation_centerRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("aggregation-center-go listening on :%s", port)))
