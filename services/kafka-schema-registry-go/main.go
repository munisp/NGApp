// kafka-schema-registry-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/kafka-schema-registry/list", handleList)
	http.HandleFunc("/v1/kafka-schema-registry/create", handleCreate)
	http.HandleFunc("/v1/kafka-schema-registry/update", handleUpdate)
	http.HandleFunc("/v1/kafka-schema-registry/process", handleProcess)
	http.HandleFunc("/v1/kafka-schema-registry/audit", handleAudit)
	http.HandleFunc("/v1/kafka-schema-registry/stats", handleStats)
	http.HandleFunc("/v1/kafka-schema-registry/throughput", kafka_schema_registryThroughputHandler)
	http.HandleFunc("/v1/kafka-schema-registry/partition", kafka_schema_registryPartitionHandler)
...
fmt.Fprintf(w, "requests_total{service=\"kafka-schema-registry-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"kafka-schema-registry-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/kafka-schema-registry/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/kafka-schema-registry/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/kafka-schema-registry/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/kafka-schema-registry/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/kafka-schema-registry/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/kafka-schema-registry/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/kafka-schema-registry/throughput", authMiddleware(kafka_schema_registryThroughputHandler))
	mux.HandleFunc("/v1/kafka-schema-registry/partition", authMiddleware(kafka_schema_registryPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("kafka-schema-registry-go listening on :%s", port)))
