// project-finance-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/project-finance/list", handleList)
	http.HandleFunc("/v1/project-finance/create", handleCreate)
	http.HandleFunc("/v1/project-finance/update", handleUpdate)
	http.HandleFunc("/v1/project-finance/process", handleProcess)
	http.HandleFunc("/v1/project-finance/audit", handleAudit)
	http.HandleFunc("/v1/project-finance/stats", handleStats)
	http.HandleFunc("/v1/project-finance/score", project_financeScoreHandler)
	http.HandleFunc("/v1/project-finance/validate", project_financeValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"project-finance-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"project-finance-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/project-finance/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/project-finance/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/project-finance/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/project-finance/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/project-finance/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/project-finance/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/project-finance/score", authMiddleware(project_financeScoreHandler))
	mux.HandleFunc("/v1/project-finance/validate", authMiddleware(project_financeValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("project-finance-go listening on :%s", port)))
