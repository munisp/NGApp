// regulatory-reporting-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"regulatory-reporting-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"regulatory-reporting-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/regulatory-reporting/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/regulatory-reporting/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/regulatory-reporting/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/regulatory-reporting/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/regulatory-reporting/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/regulatory-reporting/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/regulatory-reporting/generate", authMiddleware(regulatory_reportingGenerateHandler))
	mux.HandleFunc("/v1/regulatory-reporting/validate-period", authMiddleware(regulatory_reportingValidateHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("regulatory-reporting-go listening on :%s", port)))
