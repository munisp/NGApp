// helm-validator-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/helm-validator/update", handleUpdate)
	http.HandleFunc("/v1/helm-validator/process", handleProcess)
	http.HandleFunc("/v1/helm-validator/audit", handleAudit)
	http.HandleFunc("/v1/helm-validator/stats", handleStats)
	http.HandleFunc("/v1/helm-validator/score", helm_validatorScoreHandler)
	http.HandleFunc("/v1/helm-validator/validate", helm_validatorValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"helm-validator-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"helm-validator-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/helm-validator/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/helm-validator/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/helm-validator/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/helm-validator/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/helm-validator/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/helm-validator/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/helm-validator/score", authMiddleware(helm_validatorScoreHandler))
	mux.HandleFunc("/v1/helm-validator/validate", authMiddleware(helm_validatorValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("helm-validator-go listening on :%s", port)))
