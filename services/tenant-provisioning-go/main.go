// tenant-provisioning-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/tenant-provisioning/list", handleList)
	http.HandleFunc("/v1/tenant-provisioning/create", handleCreate)
	http.HandleFunc("/v1/tenant-provisioning/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"tenant-provisioning-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"tenant-provisioning-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/tenant-provisioning/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/tenant-provisioning/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/tenant-provisioning/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/tenant-provisioning/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/tenant-provisioning/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/tenant-provisioning/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/tenant-provisioning/score", authMiddleware(tenant_provisioningScoreHandler))
	mux.HandleFunc("/v1/tenant-provisioning/validate", authMiddleware(tenant_provisioningValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("tenant-provisioning-go listening on :%s", port)))
