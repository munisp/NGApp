// grid-token-card-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"grid-token-card-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"grid-token-card-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/grid-token-card/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/grid-token-card/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/grid-token-card/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/grid-token-card/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/grid-token-card/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/grid-token-card/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/grid-token-card/validate", authMiddleware(grid_token_cardValidateHandler))
	mux.HandleFunc("/v1/grid-token-card/rate-limit", authMiddleware(grid_token_cardRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("grid-token-card-go listening on :%s", port)))
