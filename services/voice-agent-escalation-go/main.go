// voice-agent-escalation-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"voice-agent-escalation-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"voice-agent-escalation-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/voice-agent-escalation/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/voice-agent-escalation/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/voice-agent-escalation/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/voice-agent-escalation/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/voice-agent-escalation/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/voice-agent-escalation/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/voice-agent-escalation/score", authMiddleware(voice_agent_escalationScoreHandler))
	mux.HandleFunc("/v1/voice-agent-escalation/validate", authMiddleware(voice_agent_escalationValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("voice-agent-escalation-go listening on :%s", port)))
