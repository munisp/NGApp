// whatsapp-payment-integration-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"whatsapp-payment-integration-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"whatsapp-payment-integration-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/whatsapp-payment-integration/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/whatsapp-payment-integration/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/whatsapp-payment-integration/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/whatsapp-payment-integration/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/whatsapp-payment-integration/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/whatsapp-payment-integration/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/whatsapp-payment-integration/compute-fee", authMiddleware(whatsapp_payment_integrationFeeHandler))
	mux.HandleFunc("/v1/whatsapp-payment-integration/route", authMiddleware(whatsapp_payment_integrationRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("whatsapp-payment-integration-go listening on :%s", port)))
