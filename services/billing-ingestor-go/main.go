// billing-ingestor-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"billing-ingestor-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"billing-ingestor-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/billing-ingestor/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/billing-ingestor/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/billing-ingestor/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/billing-ingestor/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/billing-ingestor/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/billing-ingestor/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/billing-ingestor/score", authMiddleware(billing_ingestorScoreHandler))
	mux.HandleFunc("/v1/billing-ingestor/validate", authMiddleware(billing_ingestorValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("billing-ingestor-go listening on :%s", port)))
