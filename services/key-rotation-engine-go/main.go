// key-rotation-engine-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/key-rotation-engine/update", handleUpdate)
	http.HandleFunc("/v1/key-rotation-engine/process", handleProcess)
	http.HandleFunc("/v1/key-rotation-engine/audit", handleAudit)
	http.HandleFunc("/v1/key-rotation-engine/stats", handleStats)
	http.HandleFunc("/v1/key-rotation-engine/score", key_rotation_engineScoreHandler)
	http.HandleFunc("/v1/key-rotation-engine/validate", key_rotation_engineValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"key-rotation-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"key-rotation-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/key-rotation-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/key-rotation-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/key-rotation-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/key-rotation-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/key-rotation-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/key-rotation-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/key-rotation-engine/score", authMiddleware(key_rotation_engineScoreHandler))
	mux.HandleFunc("/v1/key-rotation-engine/validate", authMiddleware(key_rotation_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("key-rotation-engine-go listening on :%s", port)))
