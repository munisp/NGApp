use tokio_postgres;
// reconciliation-engine-rs — Production Rust microservice with Postgres, Kafka, Redis
...
async fn list(state: web::Data<AppState>, query: web::Query<ListParams>) -> HttpResponse {
    ...
    if !db_url.is_empty() {
        if let Ok((client, connection)) = tokio_postgres::connect(db_url, tokio_postgres::NoTls).await {
            ...
            let count_sql = "SELECT COUNT(*) FROM reconciliation_engine";
            let total: i64 = client.query_one(count_sql, &[]).await
                .map(|r| r.get::<_, i64>(0)).unwrap_or(0);
            let row_sql = format!(
                    "SELECT row_to_json(t)::text as doc FROM (SELECT * FROM reconciliation_engine ORDER BY 1 LIMIT {} OFFSET {}) t",
                    limit, (page - 1) * limit
                );
...
async fn main() -> std::io::Result<()> {
    let port: u16 = std::env::var("PORT").unwrap_or("32000".into()).parse().unwrap_or(32000);
    ...
    let state = web::Data::new(AppState {
        ...
        db_url,
        service_name: "reconciliation-engine-rs".into(),
        table_name: "reconciliation_engine".into(),
    });
    
    println!("[reconciliation-engine-rs] Starting on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/reconciliation-engine/list", web::get().to(list))
            .route("/v1/reconciliation-engine/stats", web::get().to(stats))
            .route("/v1/reconciliation-engine/{id}", web::get().to(get_by_id))
            .route("/v1/reconciliation-engine", web::post().to(create))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
