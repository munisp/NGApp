...
// ─── Reconciliation Engine — Banking Ops ────────────────────────────────────────────────
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Reconciliation Engine — Banking Ops",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9544".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Reconciliation Engine v2.0 (Banking Ops) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/reconciliation-engine/process", web::post().to(process_record))
            .route("/v1/reconciliation-engine/audit", web::get().to(get_audit))
