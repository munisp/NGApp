...
// ─── Response Compressor — Platform/Infra ────────────────────────────────────────────────
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Response Compressor — Platform/Infra",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9549".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Response Compressor v2.0 (Platform/Infra) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/response-compressor/process", web::post().to(process_record))
            .route("/v1/response-compressor/audit", web::get().to(get_audit))
