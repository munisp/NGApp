// tigerbeetle-sync-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"tigerbeetle-sync-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"tigerbeetle-sync-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/tigerbeetle-sync/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/tigerbeetle-sync/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/tigerbeetle-sync/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/tigerbeetle-sync/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/tigerbeetle-sync/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/tigerbeetle-sync/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/tigerbeetle-sync/throughput", authMiddleware(tigerbeetle_syncThroughputHandler))
	mux.HandleFunc("/v1/tigerbeetle-sync/partition", authMiddleware(tigerbeetle_syncPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("tigerbeetle-sync-go listening on :%s", port)))
