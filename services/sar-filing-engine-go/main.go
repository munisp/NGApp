// sar-filing-engine-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"sar-filing-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"sar-filing-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/sar-filing-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/sar-filing-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/sar-filing-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/sar-filing-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/sar-filing-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/sar-filing-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/sar-filing-engine/screen", authMiddleware(sar_filing_engineScreenHandler))
	mux.HandleFunc("/v1/sar-filing-engine/risk-score", authMiddleware(sar_filing_engineRiskScoreHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("sar-filing-engine-go listening on :%s", port)))
