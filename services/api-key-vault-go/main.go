// api-key-vault-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/api-key-vault/list", handleList)
	http.HandleFunc("/v1/api-key-vault/create", handleCreate)
	http.HandleFunc("/v1/api-key-vault/update", handleUpdate)
	http.HandleFunc("/v1/api-key-vault/process", handleProcess)
	http.HandleFunc("/v1/api-key-vault/audit", handleAudit)
	http.HandleFunc("/v1/api-key-vault/stats", handleStats)
	http.HandleFunc("/v1/api-key-vault/validate", api_key_vaultValidateHandler)
	http.HandleFunc("/v1/api-key-vault/rate-limit", api_key_vaultRateLimitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"api-key-vault-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"api-key-vault-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/api-key-vault/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/api-key-vault/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/api-key-vault/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/api-key-vault/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/api-key-vault/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/api-key-vault/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/api-key-vault/validate", authMiddleware(api_key_vaultValidateHandler))
	mux.HandleFunc("/v1/api-key-vault/rate-limit", authMiddleware(api_key_vaultRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("api-key-vault-go listening on :%s", port)))
