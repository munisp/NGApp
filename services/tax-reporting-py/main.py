...
domain_stats = {
    ...
    "processed_today": 12, "domain": "Banking Ops",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def respond(self, code, data):
        ...
        self.send_header("X-Service", "tax-reporting-py")
        self.end_headers()
    ...
    def do_GET(self):
        ...
        if path == "/healthz":
            self.respond(200, {
                "service": "tax-reporting-py", "status": "healthy", "version": "2.0.0",
                "uptime_secs": int(time.time() - START_TIME),
                "domain": "Tax Reporting — Banking Ops",
                "middleware": {
                    "kafka": "tax-reporting.events, tax-reporting.audit",
                    "postgres": "tax_reporting_records",
                    "redis": "tax-reporting_cache",
                    "temporal": "TaxReportingWorkflow",
                    "permify": "tax-reporting:manage, tax-reporting:view",
                    "opensearch": "tax-reporting-2026",
                },
            })
        elif path == "/v1/tax-reporting/list":
            params = parse_qs(urlparse(self.path).query)
            status_filter = params.get("status", [None])[0]
            filtered = [r for r in records if not status_filter or r["status"] == status_filter]
            self.respond(200, {"records": filtered, "total": len(filtered), "domain": "Banking Ops"})
        elif path == "/v1/tax-reporting/audit":
            self.respond(200, {"audit_log": audit_log, "total": len(audit_log)})
        elif path == "/v1/tax-reporting/stats":
    ...
    def do_POST(self):
        ...
        if path == "/v1/tax-reporting/create":
            rec = {
                "id": gen_id(), "type": body.get("type", "primary"),
                "status": "pending", "domain": "Banking Ops", "data": body,
...
if __name__ == "__main__":
    ...
    print(f"Tax Reporting v2.0 (Banking Ops) on :{port}")
