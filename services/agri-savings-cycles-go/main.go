// agri-savings-cycles-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"agri-savings-cycles-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"agri-savings-cycles-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/agri-savings-cycles/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/agri-savings-cycles/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/agri-savings-cycles/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/agri-savings-cycles/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/agri-savings-cycles/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/agri-savings-cycles/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/agri-savings-cycles/yield-score", authMiddleware(agri_savings_cyclesYieldHandler))
	mux.HandleFunc("/v1/agri-savings-cycles/risk-assess", authMiddleware(agri_savings_cyclesRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("agri-savings-cycles-go listening on :%s", port)))
