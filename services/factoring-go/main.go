// factoring-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"factoring-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"factoring-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/factoring/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/factoring/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/factoring/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/factoring/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/factoring/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/factoring/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/factoring/score", authMiddleware(factoringScoreHandler))
	mux.HandleFunc("/v1/factoring/validate", authMiddleware(factoringValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("factoring-go listening on :%s", port)))
