// regulatory-sandbox-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/regulatory-sandbox/list", handleList)
	http.HandleFunc("/v1/regulatory-sandbox/create", handleCreate)
	http.HandleFunc("/v1/regulatory-sandbox/update", handleUpdate)
	http.HandleFunc("/v1/regulatory-sandbox/process", handleProcess)
	http.HandleFunc("/v1/regulatory-sandbox/audit", handleAudit)
	http.HandleFunc("/v1/regulatory-sandbox/stats", handleStats)
	http.HandleFunc("/v1/regulatory-sandbox/generate", regulatory_sandboxGenerateHandler)
	http.HandleFunc("/v1/regulatory-sandbox/validate-period", regulatory_sandboxValidateHandler)
...
fmt.Fprintf(w, "requests_total{service=\"regulatory-sandbox-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"regulatory-sandbox-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Regulatory"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/regulatory-sandbox/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/regulatory-sandbox/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/regulatory-sandbox/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/regulatory-sandbox/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/regulatory-sandbox/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/regulatory-sandbox/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/regulatory-sandbox/generate", authMiddleware(regulatory_sandboxGenerateHandler))
	mux.HandleFunc("/v1/regulatory-sandbox/validate-period", authMiddleware(regulatory_sandboxValidateHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("regulatory-sandbox-go listening on :%s", port)))
