// equipment-leasing-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"equipment-leasing-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"equipment-leasing-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/equipment-leasing/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/equipment-leasing/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/equipment-leasing/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/equipment-leasing/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/equipment-leasing/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/equipment-leasing/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/equipment-leasing/score", authMiddleware(equipment_leasingScoreHandler))
	mux.HandleFunc("/v1/equipment-leasing/validate", authMiddleware(equipment_leasingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("equipment-leasing-go listening on :%s", port)))
