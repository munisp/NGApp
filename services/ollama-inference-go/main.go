// ollama-inference-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/ollama-inference/list", handleList)
	http.HandleFunc("/v1/ollama-inference/create", handleCreate)
	http.HandleFunc("/v1/ollama-inference/update", handleUpdate)
	http.HandleFunc("/v1/ollama-inference/process", handleProcess)
	http.HandleFunc("/v1/ollama-inference/audit", handleAudit)
	http.HandleFunc("/v1/ollama-inference/stats", handleStats)
	http.HandleFunc("/v1/ollama-inference/score", ollama_inferenceScoreHandler)
	http.HandleFunc("/v1/ollama-inference/validate", ollama_inferenceValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"ollama-inference-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"ollama-inference-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/ollama-inference/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/ollama-inference/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/ollama-inference/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/ollama-inference/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/ollama-inference/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/ollama-inference/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/ollama-inference/score", authMiddleware(ollama_inferenceScoreHandler))
	mux.HandleFunc("/v1/ollama-inference/validate", authMiddleware(ollama_inferenceValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("ollama-inference-go listening on :%s", port)))
