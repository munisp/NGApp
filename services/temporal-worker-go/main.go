// temporal-worker-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"temporal-worker-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"temporal-worker-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/temporal-worker/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/temporal-worker/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/temporal-worker/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/temporal-worker/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/temporal-worker/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/temporal-worker/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/temporal-worker/score", authMiddleware(temporal_workerScoreHandler))
	mux.HandleFunc("/v1/temporal-worker/validate", authMiddleware(temporal_workerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("temporal-worker-go listening on :%s", port)))
