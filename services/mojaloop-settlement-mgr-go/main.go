// mojaloop-settlement-mgr-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/mojaloop-settlement-mgr/list", handleList)
	http.HandleFunc("/v1/mojaloop-settlement-mgr/create", handleCreate)
	http.HandleFunc("/v1/mojaloop-settlement-mgr/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"mojaloop-settlement-mgr-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"mojaloop-settlement-mgr-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/compute-fee", authMiddleware(mojaloop_settlement_mgrFeeHandler))
	mux.HandleFunc("/v1/mojaloop-settlement-mgr/route", authMiddleware(mojaloop_settlement_mgrRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("mojaloop-settlement-mgr-go listening on :%s", port)))
