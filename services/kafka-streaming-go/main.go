// kafka-streaming-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/kafka-streaming/list", handleList)
	http.HandleFunc("/v1/kafka-streaming/create", handleCreate)
	http.HandleFunc("/v1/kafka-streaming/update", handleUpdate)
	http.HandleFunc("/v1/kafka-streaming/process", handleProcess)
	http.HandleFunc("/v1/kafka-streaming/audit", handleAudit)
	http.HandleFunc("/v1/kafka-streaming/stats", handleStats)
	http.HandleFunc("/v1/kafka-streaming/throughput", kafka_streamingThroughputHandler)
	http.HandleFunc("/v1/kafka-streaming/partition", kafka_streamingPartitionHandler)
...
fmt.Fprintf(w, "requests_total{service=\"kafka-streaming-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"kafka-streaming-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/kafka-streaming/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/kafka-streaming/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/kafka-streaming/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/kafka-streaming/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/kafka-streaming/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/kafka-streaming/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/kafka-streaming/throughput", authMiddleware(kafka_streamingThroughputHandler))
	mux.HandleFunc("/v1/kafka-streaming/partition", authMiddleware(kafka_streamingPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("kafka-streaming-go listening on :%s", port)))
