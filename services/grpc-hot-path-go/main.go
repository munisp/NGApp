// grpc-hot-path-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"grpc-hot-path-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"grpc-hot-path-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/grpc-hot-path/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/grpc-hot-path/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/grpc-hot-path/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/grpc-hot-path/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/grpc-hot-path/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/grpc-hot-path/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/grpc-hot-path/score", authMiddleware(grpc_hot_pathScoreHandler))
	mux.HandleFunc("/v1/grpc-hot-path/validate", authMiddleware(grpc_hot_pathValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("grpc-hot-path-go listening on :%s", port)))
