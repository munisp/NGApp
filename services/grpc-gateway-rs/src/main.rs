...
// ─── Grpc Gateway — Infrastructure/Data ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "GRP-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Infrastructure/Data", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "GRP-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Infrastructure/Data", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "GRP-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Infrastructure/Data", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}

fn rand_id() -> String {
    let t = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap();
    format!("GRP-{:08X}", (t.subsec_nanos() ^ (t.as_secs() as u32)) & 0xFFFFFFFF)
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Grpc Gateway — Infrastructure/Data",
        "middleware": {
            "kafka": "grpc-gateway.events, grpc-gateway.audit",
            "postgres": "grpc_gateway_records",
            "redis": "grpc-gateway_cache",
            "temporal": "GrpcGatewayWorkflow",
            "opensearch": "grpc-gateway-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9499".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Grpc Gateway v2.0 (Infrastructure/Data) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/grpc-gateway/list", web::get().to(list_records))
            .route("/v1/grpc-gateway/create", web::post().to(create_record))
            .route("/v1/grpc-gateway/process", web::post().to(process_record))
            .route("/v1/grpc-gateway/audit", web::get().to(get_audit))
            .route("/v1/grpc-gateway/stats", web::get().to(get_stats))
