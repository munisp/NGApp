// event-bus-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/event-bus/update", handleUpdate)
	http.HandleFunc("/v1/event-bus/process", handleProcess)
	http.HandleFunc("/v1/event-bus/audit", handleAudit)
	http.HandleFunc("/v1/event-bus/stats", handleStats)
	http.HandleFunc("/v1/event-bus/throughput", event_busThroughputHandler)
	http.HandleFunc("/v1/event-bus/partition", event_busPartitionHandler)
...
fmt.Fprintf(w, "requests_total{service=\"event-bus-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"event-bus-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/event-bus/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/event-bus/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/event-bus/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/event-bus/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/event-bus/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/event-bus/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/event-bus/throughput", authMiddleware(event_busThroughputHandler))
	mux.HandleFunc("/v1/event-bus/partition", authMiddleware(event_busPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("event-bus-go listening on :%s", port)))
