// prepared-stmt-cache-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"prepared-stmt-cache-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"prepared-stmt-cache-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/prepared-stmt-cache/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/prepared-stmt-cache/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/prepared-stmt-cache/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/prepared-stmt-cache/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/prepared-stmt-cache/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/prepared-stmt-cache/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/prepared-stmt-cache/throughput", authMiddleware(prepared_stmt_cacheThroughputHandler))
	mux.HandleFunc("/v1/prepared-stmt-cache/partition", authMiddleware(prepared_stmt_cachePartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("prepared-stmt-cache-go listening on :%s", port)))
