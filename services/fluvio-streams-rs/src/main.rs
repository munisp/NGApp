...
// ─── Fluvio Streams — Infrastructure/Data ────────────────────────────────────────────────
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Fluvio Streams — Infrastructure/Data",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9495".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Fluvio Streams v2.0 (Infrastructure/Data) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/fluvio-streams/process", web::post().to(process_record))
            .route("/v1/fluvio-streams/audit", web::get().to(get_audit))
