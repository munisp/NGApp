// nirsal-credit-guarantee-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"nirsal-credit-guarantee-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"nirsal-credit-guarantee-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/nirsal-credit-guarantee/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/compute-emi", authMiddleware(nirsal_credit_guaranteeEMIHandler))
	mux.HandleFunc("/v1/nirsal-credit-guarantee/credit-check", authMiddleware(nirsal_credit_guaranteeCreditCheckHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("nirsal-credit-guarantee-go listening on :%s", port)))
