// telegram-bot-gateway-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/telegram-bot-gateway/update", handleUpdate)
	http.HandleFunc("/v1/telegram-bot-gateway/process", handleProcess)
	http.HandleFunc("/v1/telegram-bot-gateway/audit", handleAudit)
	http.HandleFunc("/v1/telegram-bot-gateway/stats", handleStats)
	http.HandleFunc("/v1/telegram-bot-gateway/health-score", telegram_bot_gatewayHealthScoreHandler)
	http.HandleFunc("/v1/telegram-bot-gateway/circuit-state", telegram_bot_gatewayCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"telegram-bot-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"telegram-bot-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/telegram-bot-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/telegram-bot-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/telegram-bot-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/telegram-bot-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/telegram-bot-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/telegram-bot-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/telegram-bot-gateway/health-score", authMiddleware(telegram_bot_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/telegram-bot-gateway/circuit-state", authMiddleware(telegram_bot_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("telegram-bot-gateway-go listening on :%s", port)))
