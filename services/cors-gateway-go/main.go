// cors-gateway-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"cors-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cors-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cors-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cors-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cors-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cors-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cors-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cors-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cors-gateway/health-score", authMiddleware(cors_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/cors-gateway/circuit-state", authMiddleware(cors_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cors-gateway-go listening on :%s", port)))
