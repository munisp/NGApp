// approval-workflow-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/approval-workflow/list", handleList)
	http.HandleFunc("/v1/approval-workflow/create", handleCreate)
	http.HandleFunc("/v1/approval-workflow/update", handleUpdate)
	http.HandleFunc("/v1/approval-workflow/process", handleProcess)
	http.HandleFunc("/v1/approval-workflow/audit", handleAudit)
	http.HandleFunc("/v1/approval-workflow/stats", handleStats)
	http.HandleFunc("/v1/approval-workflow/score", approval_workflowScoreHandler)
	http.HandleFunc("/v1/approval-workflow/validate", approval_workflowValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"approval-workflow-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"approval-workflow-go\"} %d\n", errs)
}
...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, 200, map[string]interface{}{"records": filtered, "total": len(filtered), "domain": "Platform"})
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/approval-workflow/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/approval-workflow/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/approval-workflow/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/approval-workflow/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/approval-workflow/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/approval-workflow/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/approval-workflow/score", authMiddleware(approval_workflowScoreHandler))
	mux.HandleFunc("/v1/approval-workflow/validate", authMiddleware(approval_workflowValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("approval-workflow-go listening on :%s", port)))
