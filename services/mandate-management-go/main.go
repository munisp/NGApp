// mandate-management-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"mandate-management-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"mandate-management-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/mandate-management/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/mandate-management/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/mandate-management/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/mandate-management/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/mandate-management/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/mandate-management/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/mandate-management/score", authMiddleware(mandate_managementScoreHandler))
	mux.HandleFunc("/v1/mandate-management/validate", authMiddleware(mandate_managementValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("mandate-management-go listening on :%s", port)))
