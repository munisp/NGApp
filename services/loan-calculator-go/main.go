// loan-calculator-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"loan-calculator-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"loan-calculator-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/loan-calculator/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/loan-calculator/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/loan-calculator/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/loan-calculator/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/loan-calculator/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/loan-calculator/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/loan-calculator/compute-emi", authMiddleware(loan_calculatorEMIHandler))
	mux.HandleFunc("/v1/loan-calculator/credit-check", authMiddleware(loan_calculatorCreditCheckHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("loan-calculator-go listening on :%s", port)))
