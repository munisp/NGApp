// kafka-consumer-optimizer-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"kafka-consumer-optimizer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"kafka-consumer-optimizer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/kafka-consumer-optimizer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/throughput", authMiddleware(kafka_consumer_optimizerThroughputHandler))
	mux.HandleFunc("/v1/kafka-consumer-optimizer/partition", authMiddleware(kafka_consumer_optimizerPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("kafka-consumer-optimizer-go listening on :%s", port)))
