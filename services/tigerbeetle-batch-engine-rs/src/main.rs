...
// ─── Tigerbeetle Batch Engine — Payments ────────────────────────────────────────────────
...
fn seed_records() -> Vec<Record> {
    vec![
        Record { id: "TIG-001".into(), record_type: "primary".into(), status: "active".into(), data: json!({"domain": "Payments", "priority": "high"}), score: 0.95, version: 1, created_at: "2026-05-09T10:00:00Z".into(), updated_at: "2026-05-09T10:00:00Z".into() },
        Record { id: "TIG-002".into(), record_type: "secondary".into(), status: "processing".into(), data: json!({"domain": "Payments", "priority": "medium"}), score: 0.82, version: 2, created_at: "2026-05-09T11:00:00Z".into(), updated_at: "2026-05-09T11:30:00Z".into() },
        Record { id: "TIG-003".into(), record_type: "primary".into(), status: "completed".into(), data: json!({"domain": "Payments", "priority": "low"}), score: 0.91, version: 1, created_at: "2026-05-08T14:00:00Z".into(), updated_at: "2026-05-09T08:00:00Z".into() },
    ]
}
...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Tigerbeetle Batch Engine — Payments",
        "middleware": {
            "kafka": "tigerbeetle-batch-engine.events, tigerbeetle-batch-engine.audit",
            "postgres": "tigerbeetle_batch_engine_records",
            "redis": "tigerbeetle-batch-engine_cache",
            "temporal": "TigerbeetleBatchEngineWorkflow",
            "opensearch": "tigerbeetle-batch-engine-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9568".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Tigerbeetle Batch Engine v2.0 (Payments) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/tigerbeetle-batch-engine/list", web::get().to(list_records))
            .route("/v1/tigerbeetle-batch-engine/create", web::post().to(create_record))
            .route("/v1/tigerbeetle-batch-engine/process", web::post().to(process_record))
            .route("/v1/tigerbeetle-batch-engine/audit", web::get().to(get_audit))
            .route("/v1/tigerbeetle-batch-engine/stats", web::get().to(get_stats))
