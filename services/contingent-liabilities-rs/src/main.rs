...
async fn assess_probability(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
...
async fn monitor_triggers(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
...
async fn disclosure_report(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
    let classification_s = input.get("classification").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let classification = classification_s.as_str();
