...
// dormancy-management-rs — Production-hardened service
...
async fn unclaimed_funds(body: web::Json<serde_json::Value>) -> HttpResponse {
    let input = body.into_inner();
...
async fn check_dormancy(body: web::Json<serde_json::Value>) -> HttpResponse {
    let input = body.into_inner();
    let last_txn_days = input.get("last_txn_days").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
    let result = dormancy_status(last_txn_days);
    HttpResponse::Ok().json(json!({
...
async fn activate_dormant(body: web::Json<serde_json::Value>) -> HttpResponse {
    let input = body.into_inner();
    let status_s = input.get("status").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let status = status_s.as_str();
    let result = restriction_level(status);
...
async fn list_records(req: HttpRequest, state: web::Data<AppState>) -> HttpResponse {
    ...
    let limit: i64 = 50;
    match db_query(&state, "dormancy_management_rs", page, limit).await {
