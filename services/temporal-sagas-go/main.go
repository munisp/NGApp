// temporal-sagas-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"temporal-sagas-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"temporal-sagas-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/temporal-sagas/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/temporal-sagas/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/temporal-sagas/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/temporal-sagas/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/temporal-sagas/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/temporal-sagas/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/temporal-sagas/score", authMiddleware(temporal_sagasScoreHandler))
	mux.HandleFunc("/v1/temporal-sagas/validate", authMiddleware(temporal_sagasValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("temporal-sagas-go listening on :%s", port)))
