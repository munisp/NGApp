// route-schema-enforcer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/route-schema-enforcer/update", handleUpdate)
	http.HandleFunc("/v1/route-schema-enforcer/process", handleProcess)
	http.HandleFunc("/v1/route-schema-enforcer/audit", handleAudit)
	http.HandleFunc("/v1/route-schema-enforcer/stats", handleStats)
	http.HandleFunc("/v1/route-schema-enforcer/throughput", route_schema_enforcerThroughputHandler)
	http.HandleFunc("/v1/route-schema-enforcer/partition", route_schema_enforcerPartitionHandler)
...
fmt.Fprintf(w, "requests_total{service=\"route-schema-enforcer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"route-schema-enforcer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/route-schema-enforcer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/route-schema-enforcer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/route-schema-enforcer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/route-schema-enforcer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/route-schema-enforcer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/route-schema-enforcer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/route-schema-enforcer/throughput", authMiddleware(route_schema_enforcerThroughputHandler))
	mux.HandleFunc("/v1/route-schema-enforcer/partition", authMiddleware(route_schema_enforcerPartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("route-schema-enforcer-go listening on :%s", port)))
