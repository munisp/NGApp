// payment-investigation-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/payment-investigation/list", handleList)
	http.HandleFunc("/v1/payment-investigation/create", handleCreate)
	http.HandleFunc("/v1/payment-investigation/update", handleUpdate)
	http.HandleFunc("/v1/payment-investigation/process", handleProcess)
	http.HandleFunc("/v1/payment-investigation/audit", handleAudit)
	http.HandleFunc("/v1/payment-investigation/stats", handleStats)
	http.HandleFunc("/v1/payment-investigation/compute-fee", payment_investigationFeeHandler)
	http.HandleFunc("/v1/payment-investigation/route", payment_investigationRouteHandler)
...
fmt.Fprintf(w, "requests_total{service=\"payment-investigation-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"payment-investigation-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/payment-investigation/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/payment-investigation/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/payment-investigation/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/payment-investigation/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/payment-investigation/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/payment-investigation/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/payment-investigation/compute-fee", authMiddleware(payment_investigationFeeHandler))
	mux.HandleFunc("/v1/payment-investigation/route", authMiddleware(payment_investigationRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("payment-investigation-go listening on :%s", port)))
