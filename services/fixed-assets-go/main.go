// fixed-assets-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"fixed-assets-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"fixed-assets-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/fixed-assets/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/fixed-assets/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/fixed-assets/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/fixed-assets/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/fixed-assets/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/fixed-assets/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/fixed-assets/fx-convert", authMiddleware(fixed_assetsFXHandler))
	mux.HandleFunc("/v1/fixed-assets/risk-calc", authMiddleware(fixed_assetsRiskHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("fixed-assets-go listening on :%s", port)))
