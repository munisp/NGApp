...
async fn list_records() -> HttpResponse {
    HttpResponse::Ok().json(json!({"records": [
        {"id": "REC-001", "status": "active", "domain": "Biometric Auth", "createdAt": "2026-05-09T10:00:00Z"},
        {"id": "REC-002", "status": "processing", "domain": "Biometric Auth", "createdAt": "2026-05-09T11:00:00Z"},
        {"id": "REC-003", "status": "completed", "domain": "Biometric Auth", "createdAt": "2026-05-08T14:00:00Z"},
    ], "total": 3, "domain": "Biometric Auth"}))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9206".to_string());
    let state = AppState { start_time: Instant::now() };
    println!("Biometric Auth (Rust) on :{}", port);
    HttpServer::new(move || {
        App::new()
