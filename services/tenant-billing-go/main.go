// tenant-billing-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/tenant-billing/list", handleList)
	http.HandleFunc("/v1/tenant-billing/create", handleCreate)
	http.HandleFunc("/v1/tenant-billing/update", handleUpdate)
	http.HandleFunc("/v1/tenant-billing/process", handleProcess)
	http.HandleFunc("/v1/tenant-billing/audit", handleAudit)
	http.HandleFunc("/v1/tenant-billing/stats", handleStats)
	http.HandleFunc("/v1/tenant-billing/score", tenant_billingScoreHandler)
	http.HandleFunc("/v1/tenant-billing/validate", tenant_billingValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"tenant-billing-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"tenant-billing-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/tenant-billing/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/tenant-billing/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/tenant-billing/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/tenant-billing/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/tenant-billing/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/tenant-billing/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/tenant-billing/score", authMiddleware(tenant_billingScoreHandler))
	mux.HandleFunc("/v1/tenant-billing/validate", authMiddleware(tenant_billingValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("tenant-billing-go listening on :%s", port)))
