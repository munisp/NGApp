// cdn-edge-cache-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"cdn-edge-cache-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"cdn-edge-cache-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/cdn-edge-cache/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/cdn-edge-cache/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/cdn-edge-cache/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/cdn-edge-cache/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/cdn-edge-cache/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/cdn-edge-cache/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/cdn-edge-cache/throughput", authMiddleware(cdn_edge_cacheThroughputHandler))
	mux.HandleFunc("/v1/cdn-edge-cache/partition", authMiddleware(cdn_edge_cachePartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("cdn-edge-cache-go listening on :%s", port)))
