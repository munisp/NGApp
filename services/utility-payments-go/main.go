// utility-payments-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/utility-payments/list", handleList)
	http.HandleFunc("/v1/utility-payments/create", handleCreate)
	http.HandleFunc("/v1/utility-payments/update", handleUpdate)
...
fmt.Fprintf(w, "requests_total{service=\"utility-payments-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"utility-payments-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/utility-payments/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/utility-payments/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/utility-payments/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/utility-payments/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/utility-payments/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/utility-payments/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/utility-payments/compute-fee", authMiddleware(utility_paymentsFeeHandler))
	mux.HandleFunc("/v1/utility-payments/route", authMiddleware(utility_paymentsRouteHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("utility-payments-go listening on :%s", port)))
