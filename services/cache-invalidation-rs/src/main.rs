...
// cache-invalidation-rs — Production-hardened service
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().json(json!({"status": "healthy", "service": "cache-invalidation-rs"}))
}
...
async fn list_records(req: HttpRequest, state: web::Data<AppState>) -> HttpResponse {
    ...
    let limit: i64 = 50;
    match db_query(&state, "cache_invalidation_rs", page, limit).await {
