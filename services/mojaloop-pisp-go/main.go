// mojaloop-pisp-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"mojaloop-pisp-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"mojaloop-pisp-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/mojaloop-pisp/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/mojaloop-pisp/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/mojaloop-pisp/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/mojaloop-pisp/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/mojaloop-pisp/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/mojaloop-pisp/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/mojaloop-pisp/score", authMiddleware(mojaloop_pispScoreHandler))
	mux.HandleFunc("/v1/mojaloop-pisp/validate", authMiddleware(mojaloop_pispValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("mojaloop-pisp-go listening on :%s", port)))
