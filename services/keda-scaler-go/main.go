// keda-scaler-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"keda-scaler-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"keda-scaler-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/keda-scaler/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/keda-scaler/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/keda-scaler/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/keda-scaler/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/keda-scaler/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/keda-scaler/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/keda-scaler/score", authMiddleware(keda_scalerScoreHandler))
	mux.HandleFunc("/v1/keda-scaler/validate", authMiddleware(keda_scalerValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("keda-scaler-go listening on :%s", port)))
