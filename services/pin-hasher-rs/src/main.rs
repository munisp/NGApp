...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Pin Hasher — Payments",
        "middleware": {
            "kafka": "pin-hasher.events, pin-hasher.audit",
            "postgres": "pin_hasher_records",
            "redis": "pin-hasher_cache",
            "temporal": "PinHasherWorkflow",
            "opensearch": "pin-hasher-2026",
        }
    }))
}
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9534".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Pin Hasher v2.0 (Payments) on :{}", port);
    HttpServer::new(move || {
        App::new()
            .app_data(state.clone())
            .route("/healthz", web::get().to(healthz))
            .route("/v1/pin-hasher/list", web::get().to(list_records))
            .route("/v1/pin-hasher/create", web::post().to(create_record))
            .route("/v1/pin-hasher/process", web::post().to(process_record))
            .route("/v1/pin-hasher/audit", web::get().to(get_audit))
