// api-key-enforcer-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/api-key-enforcer/list", handleList)
	http.HandleFunc("/v1/api-key-enforcer/create", handleCreate)
	http.HandleFunc("/v1/api-key-enforcer/update", handleUpdate)
	http.HandleFunc("/v1/api-key-enforcer/process", handleProcess)
	http.HandleFunc("/v1/api-key-enforcer/audit", handleAudit)
	http.HandleFunc("/v1/api-key-enforcer/stats", handleStats)
	http.HandleFunc("/v1/api-key-enforcer/validate", api_key_enforcerValidateHandler)
	http.HandleFunc("/v1/api-key-enforcer/rate-limit", api_key_enforcerRateLimitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"api-key-enforcer-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"api-key-enforcer-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/api-key-enforcer/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/api-key-enforcer/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/api-key-enforcer/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/api-key-enforcer/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/api-key-enforcer/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/api-key-enforcer/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/api-key-enforcer/validate", authMiddleware(api_key_enforcerValidateHandler))
	mux.HandleFunc("/v1/api-key-enforcer/rate-limit", authMiddleware(api_key_enforcerRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("api-key-enforcer-go listening on :%s", port)))
