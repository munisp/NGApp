...
// table-partitioner-rs — Production-hardened service
...
async fn partition(body: web::Json<serde_json::Value>) -> HttpResponse {
...
async fn list_records(req: HttpRequest, state: web::Data<AppState>) -> HttpResponse {
    ...
    let limit: i64 = 50;
    match db_query(&state, "table_partitioner_rs", page, limit).await {
