// secrets-vault-go — Production-hardened service
package main
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/secrets-vault/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/secrets-vault/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/secrets-vault/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/secrets-vault/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/secrets-vault/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/secrets-vault/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/secrets-vault/validate", authMiddleware(secrets_vaultValidateHandler))
	mux.HandleFunc("/v1/secrets-vault/rate-limit", authMiddleware(secrets_vaultRateLimitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("secrets-vault-go listening on :%s", port)))
