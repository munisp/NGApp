// tenant-isolation-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/tenant-isolation/update", handleUpdate)
	http.HandleFunc("/v1/tenant-isolation/process", handleProcess)
	http.HandleFunc("/v1/tenant-isolation/audit", handleAudit)
	http.HandleFunc("/v1/tenant-isolation/stats", handleStats)
	http.HandleFunc("/v1/tenant-isolation/score", tenant_isolationScoreHandler)
	http.HandleFunc("/v1/tenant-isolation/validate", tenant_isolationValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"tenant-isolation-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"tenant-isolation-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/tenant-isolation/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/tenant-isolation/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/tenant-isolation/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/tenant-isolation/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/tenant-isolation/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/tenant-isolation/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/tenant-isolation/score", authMiddleware(tenant_isolationScoreHandler))
	mux.HandleFunc("/v1/tenant-isolation/validate", authMiddleware(tenant_isolationValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("tenant-isolation-go listening on :%s", port)))
