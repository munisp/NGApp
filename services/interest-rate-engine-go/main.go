// interest-rate-engine-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/interest-rate-engine/list", handleList)
	http.HandleFunc("/v1/interest-rate-engine/create", handleCreate)
	http.HandleFunc("/v1/interest-rate-engine/update", handleUpdate)
	http.HandleFunc("/v1/interest-rate-engine/process", handleProcess)
	http.HandleFunc("/v1/interest-rate-engine/audit", handleAudit)
	http.HandleFunc("/v1/interest-rate-engine/stats", handleStats)
	http.HandleFunc("/v1/interest-rate-engine/score", interest_rate_engineScoreHandler)
	http.HandleFunc("/v1/interest-rate-engine/validate", interest_rate_engineValidateRequestHandler)
...
fmt.Fprintf(w, "requests_total{service=\"interest-rate-engine-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"interest-rate-engine-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/interest-rate-engine/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/interest-rate-engine/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/interest-rate-engine/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/interest-rate-engine/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/interest-rate-engine/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/interest-rate-engine/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/interest-rate-engine/score", authMiddleware(interest_rate_engineScoreHandler))
	mux.HandleFunc("/v1/interest-rate-engine/validate", authMiddleware(interest_rate_engineValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("interest-rate-engine-go listening on :%s", port)))
