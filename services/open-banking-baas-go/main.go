// open-banking-baas-go — Production-hardened service
package main
...
fmt.Fprintf(w, "requests_total{service=\"open-banking-baas-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"open-banking-baas-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/open-banking/apis", authMiddleware(openBankingAPIs))
	mux.HandleFunc("/v1/ai/credit-scoring", authMiddleware(aiCreditScoring))
	mux.HandleFunc("/v1/embedded-finance", authMiddleware(embeddedFinance))
	mux.HandleFunc("/v1/open-banking-baas/score", authMiddleware(open_banking_baasScoreHandler))
	mux.HandleFunc("/v1/open-banking-baas/validate", authMiddleware(open_banking_baasValidateRequestHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("open-banking-baas-go listening on :%s", port)))
