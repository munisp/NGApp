...
// contract-test-rs — Production-hardened service
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().json(json!({"status": "healthy", "service": "contract-test-rs"}))
}
...
async fn list_records(req: HttpRequest, state: web::Data<AppState>) -> HttpResponse {
    ...
    let limit: i64 = 50;
    match db_query(&state, "contract_test_rs", page, limit).await {
