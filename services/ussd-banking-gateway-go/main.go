// ussd-banking-gateway-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/ussd-banking-gateway/list", handleList)
	http.HandleFunc("/v1/ussd-banking-gateway/create", handleCreate)
	http.HandleFunc("/v1/ussd-banking-gateway/update", handleUpdate)
	http.HandleFunc("/v1/ussd-banking-gateway/process", handleProcess)
	http.HandleFunc("/v1/ussd-banking-gateway/audit", handleAudit)
	http.HandleFunc("/v1/ussd-banking-gateway/stats", handleStats)
	http.HandleFunc("/v1/ussd-banking-gateway/health-score", ussd_banking_gatewayHealthScoreHandler)
	http.HandleFunc("/v1/ussd-banking-gateway/circuit-state", ussd_banking_gatewayCircuitHandler)
...
fmt.Fprintf(w, "requests_total{service=\"ussd-banking-gateway-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"ussd-banking-gateway-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/ussd-banking-gateway/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/ussd-banking-gateway/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/ussd-banking-gateway/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/ussd-banking-gateway/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/ussd-banking-gateway/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/ussd-banking-gateway/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/ussd-banking-gateway/health-score", authMiddleware(ussd_banking_gatewayHealthScoreHandler))
	mux.HandleFunc("/v1/ussd-banking-gateway/circuit-state", authMiddleware(ussd_banking_gatewayCircuitHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("ussd-banking-gateway-go listening on :%s", port)))
