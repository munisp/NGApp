// sw-api-cache-go — Production-hardened service
package main
...
func main() {
	...
	http.HandleFunc("/v1/sw-api-cache/update", handleUpdate)
	http.HandleFunc("/v1/sw-api-cache/process", handleProcess)
	http.HandleFunc("/v1/sw-api-cache/audit", handleAudit)
	http.HandleFunc("/v1/sw-api-cache/stats", handleStats)
	http.HandleFunc("/v1/sw-api-cache/throughput", sw_api_cacheThroughputHandler)
	http.HandleFunc("/v1/sw-api-cache/partition", sw_api_cachePartitionHandler)
...
fmt.Fprintf(w, "requests_total{service=\"sw-api-cache-go\"} %d\n", reqs)
fmt.Fprintf(w, "# HELP errors_total Total errors\n")
fmt.Fprintf(w, "# TYPE errors_total counter\n")
fmt.Fprintf(w, "errors_total{service=\"sw-api-cache-go\"} %d\n", errs)
}
...
mux.HandleFunc("/v1/create", authMiddleware(createHandler))
	...
	mux.HandleFunc("/v1/sw-api-cache/list", authMiddleware(handleList))
	mux.HandleFunc("/v1/sw-api-cache/create", authMiddleware(handleCreate))
	mux.HandleFunc("/v1/sw-api-cache/update", authMiddleware(handleUpdate))
	mux.HandleFunc("/v1/sw-api-cache/process", authMiddleware(handleProcess))
	mux.HandleFunc("/v1/sw-api-cache/audit", authMiddleware(handleAudit))
	mux.HandleFunc("/v1/sw-api-cache/stats", authMiddleware(handleStats))
	mux.HandleFunc("/v1/sw-api-cache/throughput", authMiddleware(sw_api_cacheThroughputHandler))
	mux.HandleFunc("/v1/sw-api-cache/partition", authMiddleware(sw_api_cachePartitionHandler))
...
go func() {
    log.Println(jsonLog("INFO", fmt.Sprintf("sw-api-cache-go listening on :%s", port)))
