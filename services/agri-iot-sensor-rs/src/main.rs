...
async fn healthz(state: web::Data<AppState>) -> HttpResponse {
    HttpResponse::Ok().json(json!({
        ...
        "domain": "Agri Iot Sensor — Agriculture",
        "middleware": {
...
async fn main() -> std::io::Result<()> {
    let port = std::env::var("PORT").unwrap_or_else(|_| "9465".to_string());
    let state = web::Data::new(AppState {
        start_time: Instant::now(),
        records: Mutex::new(seed_records()),
        audit_log: Mutex::new(vec![]),
    });
    println!("Agri Iot Sensor v2.0 (Agriculture) on :{}", port);
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/agri-iot-sensor/process", web::post().to(process_record))
            .route("/v1/agri-iot-sensor/audit", web::get().to(get_audit))
