...
class _SecurityHardeningScreenState extends State<SecurityHardeningScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'category', 'severity', 'enforced'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'category', 'severity', 'enforced'];
    final labels = ['ID', 'Policy', 'Category', 'Severity', 'Enforced'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Security Hardening'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Security Hardening')),
