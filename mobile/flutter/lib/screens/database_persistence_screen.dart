...
class _DatabasePersistenceScreenState extends State<DatabasePersistenceScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['name', 'columns', 'rls', 'tenantScoped'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['name', 'columns', 'rls', 'tenantScoped'];
    final labels = ['Table', 'Cols', 'RLS', 'Tenant'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Database Persistence'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Database Persistence')),
