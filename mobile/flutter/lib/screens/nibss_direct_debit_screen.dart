...
class _NibssDirectDebitScreenState extends State<NibssDirectDebitScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'payer', 'payee', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'payer', 'payee', 'amount', 'status'];
    final labels = ['ID', 'Payer', 'Payee', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'NIBSS Direct Debit'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('NIBSS Direct Debit')),
