...
class _MakerCheckerScreenState extends State<MakerCheckerScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'type', 'action', 'level', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'type', 'action', 'level', 'status'];
    final labels = ['ID', 'Type', 'Action', 'Level', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Maker-Checker'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Maker-Checker')),
