...
class _CustomDomainScreenState extends State<CustomDomainScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'domain', 'tenant', 'ssl', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'domain', 'tenant', 'ssl', 'status'];
    final labels = ['ID', 'Domain', 'Tenant', 'SSL', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Custom Domain'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Custom Domain')),
