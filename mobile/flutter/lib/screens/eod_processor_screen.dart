...
class _EodProcessorScreenState extends State<EodProcessorScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'step', 'duration', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'step', 'duration', 'status'];
    final labels = ['ID', 'Step', 'Duration', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'EOD Processor'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('EOD Processor')),
