...
class _KycTriggersScreenState extends State<KycTriggersScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'trigger', 'action', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'trigger', 'action', 'status'];
    final labels = ['ID', 'Trigger', 'Action', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'KYC Triggers'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('KYC Triggers')),
