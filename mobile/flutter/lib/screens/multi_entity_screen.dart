...
class _MultiEntityScreenState extends State<MultiEntityScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'entity', 'type', 'country', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'entity', 'type', 'country', 'status'];
    final labels = ['ID', 'Entity', 'Type', 'Country', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Multi-Entity'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Multi-Entity')),
