...
class _CreditRiskScreenState extends State<CreditRiskScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['customer', 'score', 'grade', 'pd', 'lgd'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['customer', 'score', 'grade', 'pd', 'lgd'];
    final labels = ['Customer', 'Score', 'Grade', 'PD', 'LGD'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Credit Risk'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Credit Risk')),
