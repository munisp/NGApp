...
class _RegulatoryCalendarScreenState extends State<RegulatoryCalendarScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'event', 'regulator', 'deadline', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'event', 'regulator', 'deadline', 'status'];
    final labels = ['ID', 'Event', 'Regulator', 'Deadline', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Regulatory Calendar'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Regulatory Calendar')),
