...
class _CollateralValuationScreenState extends State<CollateralValuationScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'asset', 'lastVal', 'currentVal', 'method'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'asset', 'lastVal', 'currentVal', 'method'];
    final labels = ['ID', 'Asset', 'Last Value', 'Current', 'Method'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Collateral Valuation'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Collateral Valuation')),
