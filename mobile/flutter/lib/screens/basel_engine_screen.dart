...
class _BaselEngineScreenState extends State<BaselEngineScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['metric', 'value', 'requirement', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['metric', 'value', 'requirement', 'status'];
    final labels = ['Metric', 'Value', 'Requirement', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Basel Engine'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Basel Engine')),
