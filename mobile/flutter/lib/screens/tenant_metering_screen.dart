...
class _TenantMeteringScreenState extends State<TenantMeteringScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'tenant', 'apiCalls', 'storage', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'tenant', 'apiCalls', 'storage', 'status'];
    final labels = ['ID', 'Tenant', 'API Calls', 'Storage', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Tenant Metering'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tenant Metering')),
