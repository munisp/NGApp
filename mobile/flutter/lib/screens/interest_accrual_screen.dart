...
class _InterestAccrualScreenState extends State<InterestAccrualScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['account', 'product', 'principal', 'rate', 'accrued'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['account', 'product', 'principal', 'rate', 'accrued'];
    final labels = ['Account', 'Product', 'Principal', 'Rate', 'Accrued'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Interest Accrual'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Interest Accrual')),
