...
class _CustomerTransfersScreenState extends State<CustomerTransfersScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'to', 'amount', 'date', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'to', 'amount', 'date', 'status'];
    final labels = ['ID', 'To', 'Amount', 'Date', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Customer Transfers'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Customer Transfers')),
