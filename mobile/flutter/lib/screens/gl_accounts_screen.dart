...
class _GlAccountsScreenState extends State<GlAccountsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['code', 'name', 'debit', 'credit', 'balance'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['code', 'name', 'debit', 'credit', 'balance'];
    final labels = ['Code', 'Name', 'Debit', 'Credit', 'Balance'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'GL Accounts'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('GL Accounts')),
