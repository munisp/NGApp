...
class _OpensearchScreenState extends State<OpensearchScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'cluster', 'nodes', 'indices', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'cluster', 'nodes', 'indices', 'status'];
    final labels = ['ID', 'Cluster', 'Nodes', 'Indices', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'OpenSearch'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('OpenSearch')),
