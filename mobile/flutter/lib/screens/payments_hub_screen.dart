...
class _PaymentsHubScreenState extends State<PaymentsHubScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'sender', 'receiver', 'amount', 'channel', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'sender', 'receiver', 'amount', 'channel', 'status'];
    final labels = ['ID', 'Sender', 'Receiver', 'Amount', 'Channel', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Payments Hub'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Payments Hub')),
