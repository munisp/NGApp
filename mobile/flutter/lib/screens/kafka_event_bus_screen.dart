...
class _KafkaEventBusScreenState extends State<KafkaEventBusScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['name', 'partitions', 'replication', 'consumers'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['name', 'partitions', 'replication', 'consumers'];
    final labels = ['Topic', 'Partitions', 'Replication', 'Consumers'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Kafka Event Bus'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Kafka Event Bus')),
