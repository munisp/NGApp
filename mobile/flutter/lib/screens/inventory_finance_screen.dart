...
class _InventoryFinanceScreenState extends State<InventoryFinanceScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'borrower', 'commodity', 'value', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'borrower', 'commodity', 'value', 'status'];
    final labels = ['ID', 'Borrower', 'Commodity', 'Value', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Inventory Finance'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Inventory Finance')),
