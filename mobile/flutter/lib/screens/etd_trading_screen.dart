...
class _EtdTradingScreenState extends State<EtdTradingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'contract', 'type', 'qty', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'contract', 'type', 'qty', 'status'];
    final labels = ['ID', 'Contract', 'Type', 'Qty', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'ETD Trading'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('ETD Trading')),
