...
class _VirtualAccountsScreenState extends State<VirtualAccountsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['van', 'customer', 'purpose', 'collections', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['van', 'customer', 'purpose', 'collections', 'status'];
    final labels = ['VAN', 'Customer', 'Purpose', 'Collections', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Virtual Accounts'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Virtual Accounts')),
