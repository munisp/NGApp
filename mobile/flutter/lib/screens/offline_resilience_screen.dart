...
class _OfflineResilienceScreenState extends State<OfflineResilienceScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'profile', 'bandwidth', 'strategy', 'score'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'profile', 'bandwidth', 'strategy', 'score'];
    final labels = ['ID', 'Profile', 'Bandwidth', 'Strategy', 'Score'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Offline Resilience'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Offline Resilience')),
