...
class _KeycloakScreenState extends State<KeycloakScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'realm', 'users', 'clients', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'realm', 'users', 'clients', 'status'];
    final labels = ['ID', 'Realm', 'Users', 'Clients', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Keycloak'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Keycloak')),
