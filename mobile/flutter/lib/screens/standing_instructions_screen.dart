...
class _StandingInstructionsScreenState extends State<StandingInstructionsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'type', 'trigger', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'type', 'trigger', 'status'];
    final labels = ['ID', 'Customer', 'Type', 'Trigger', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Standing Instructions'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Standing Instructions')),
