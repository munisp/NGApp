...
class _GraduatedRolloutScreenState extends State<GraduatedRolloutScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'feature', 'stage', 'percentage', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'feature', 'stage', 'percentage', 'status'];
    final labels = ['ID', 'Feature', 'Stage', 'Rollout %', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Graduated Rollout'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Graduated Rollout')),
