...
class _MurabahaCalculatorScreenState extends State<MurabahaCalculatorScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['product', 'cost', 'markup', 'total', 'tenor'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['product', 'cost', 'markup', 'total', 'tenor'];
    final labels = ['Product', 'Cost Price', 'Markup', 'Total', 'Tenor'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Murabaha Calculator'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Murabaha Calculator')),
