...
class _CustomerQrScreenState extends State<CustomerQrScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'merchant', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'merchant', 'amount', 'status'];
    final labels = ['ID', 'Merchant', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Customer QR'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Customer QR')),
