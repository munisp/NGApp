...
class _TrustEstateScreenState extends State<TrustEstateScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'type', 'value', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'type', 'value', 'status'];
    final labels = ['ID', 'Trust', 'Type', 'Value', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Trust & Estate'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Trust & Estate')),
