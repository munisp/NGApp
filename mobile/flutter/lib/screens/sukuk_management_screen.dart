...
class _SukukManagementScreenState extends State<SukukManagementScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'type', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'type', 'amount', 'status'];
    final labels = ['ID', 'Sukuk', 'Type', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Sukuk Management'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Sukuk Management')),
