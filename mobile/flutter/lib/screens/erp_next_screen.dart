...
class _ErpNextScreenState extends State<ErpNextScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'module', 'direction', 'lastSync', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'module', 'direction', 'lastSync', 'status'];
    final labels = ['ID', 'Module', 'Direction', 'Last Sync', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'ERP Integration'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('ERP Integration')),
