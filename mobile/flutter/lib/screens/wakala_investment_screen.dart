...
class _WakalaInvestmentScreenState extends State<WakalaInvestmentScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'investor', 'amount', 'return', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'investor', 'amount', 'return', 'status'];
    final labels = ['ID', 'Investor', 'Amount', 'Return', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Wakala Investment'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Wakala Investment')),
