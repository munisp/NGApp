...
class _TigerbeetleLedgerScreenState extends State<TigerbeetleLedgerScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'description', 'ledger', 'balance'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'description', 'ledger', 'balance'];
    final labels = ['ID', 'Description', 'Ledger', 'Balance'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'TigerBeetle Ledger'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('TigerBeetle Ledger')),
