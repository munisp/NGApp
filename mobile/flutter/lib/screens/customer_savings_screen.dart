...
class _CustomerSavingsScreenState extends State<CustomerSavingsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'product', 'balance', 'rate'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'product', 'balance', 'rate'];
    final labels = ['ID', 'Product', 'Balance', 'Rate'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Customer Savings'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Customer Savings')),
