...
class _TellerScreenState extends State<TellerScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'teller', 'branch', 'txns', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'teller', 'branch', 'txns', 'status'];
    final labels = ['ID', 'Teller', 'Branch', 'Txns Today', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Teller Operations'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Teller Operations')),
