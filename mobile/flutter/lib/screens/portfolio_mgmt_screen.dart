...
class _PortfolioMgmtScreenState extends State<PortfolioMgmtScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['asset', 'alloc', 'value', 'perf'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['asset', 'alloc', 'value', 'perf'];
    final labels = ['Asset Class', 'Allocation', 'Value', 'Performance'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Portfolio Mgmt'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Portfolio Mgmt')),
