...
class _ExpenseMgmtScreenState extends State<ExpenseMgmtScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'employee', 'amount', 'category', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'employee', 'amount', 'category', 'status'];
    final labels = ['ID', 'Employee', 'Amount', 'Category', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Expense Mgmt'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Expense Mgmt')),
