...
class _SupplyChainFinanceScreenState extends State<SupplyChainFinanceScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'anchor', 'suppliers', 'volume', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'anchor', 'suppliers', 'volume', 'status'];
    final labels = ['ID', 'Anchor', 'Suppliers', 'Volume', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Supply Chain Finance'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Supply Chain Finance')),
