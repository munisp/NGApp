...
class _TenantProvisioningScreenState extends State<TenantProvisioningScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'tenant', 'package', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'tenant', 'package', 'status'];
    final labels = ['ID', 'Tenant', 'Package', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Tenant Provisioning'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tenant Provisioning')),
