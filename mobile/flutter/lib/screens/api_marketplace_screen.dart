...
class _ApiMarketplaceScreenState extends State<ApiMarketplaceScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'version', 'calls', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'version', 'calls', 'status'];
    final labels = ['ID', 'API', 'Version', 'Calls/Day', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'API Marketplace'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('API Marketplace')),
