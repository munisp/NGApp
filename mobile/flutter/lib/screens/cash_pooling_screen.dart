...
class _CashPoolingScreenState extends State<CashPoolingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'group', 'accounts', 'balance', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'group', 'accounts', 'balance', 'status'];
    final labels = ['ID', 'Group', 'Accounts', 'Total', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Cash Pooling'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Cash Pooling')),
