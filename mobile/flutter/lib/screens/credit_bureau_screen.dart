...
class _CreditBureauScreenState extends State<CreditBureauScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['bvn', 'name', 'score', 'facilities', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['bvn', 'name', 'score', 'facilities', 'status'];
    final labels = ['BVN', 'Name', 'Score', 'Facilities', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Credit Bureau'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Credit Bureau')),
