...
class _TreasuryLiquidityScreenState extends State<TreasuryLiquidityScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['metric', 'value', 'threshold', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['metric', 'value', 'threshold', 'status'];
    final labels = ['Metric', 'Value', 'Threshold', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Liquidity Mgmt'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Liquidity Mgmt')),
