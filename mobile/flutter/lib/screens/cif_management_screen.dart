...
class _CifManagementScreenState extends State<CifManagementScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['cifNo', 'name', 'address', 'state'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['cifNo', 'name', 'address', 'state'];
    final labels = ['CIF', 'Name', 'Address', 'State'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'CIF / Address Mgmt'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('CIF / Address Mgmt')),
