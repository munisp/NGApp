...
class _DdosProtectionScreenState extends State<DdosProtectionScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'type', 'action', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'type', 'action', 'status'];
    final labels = ['ID', 'Rule', 'Type', 'Action', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'DDoS Protection'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('DDoS Protection')),
