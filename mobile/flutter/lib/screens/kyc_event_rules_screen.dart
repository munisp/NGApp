...
class _KycEventRulesScreenState extends State<KycEventRulesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'event', 'action', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'event', 'action', 'status'];
    final labels = ['ID', 'Event', 'Action', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'KYC Event Rules'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('KYC Event Rules')),
