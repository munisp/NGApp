...
class _CardTokensScreenState extends State<CardTokensScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['token', 'pan', 'merchant', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['token', 'pan', 'merchant', 'status'];
    final labels = ['Token', 'Card', 'Merchant', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Card Tokenization'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Card Tokenization')),
