...
class _WebhookDeliveriesScreenState extends State<WebhookDeliveriesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'webhook', 'response', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'webhook', 'response', 'status'];
    final labels = ['ID', 'Webhook', 'Response', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Webhook Deliveries'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Webhook Deliveries')),
