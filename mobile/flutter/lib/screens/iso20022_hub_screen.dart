...
class _Iso20022HubScreenState extends State<Iso20022HubScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'msgType', 'direction', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'msgType', 'direction', 'status'];
    final labels = ['ID', 'Type', 'Direction', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'ISO 20022 Hub'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('ISO 20022 Hub')),
