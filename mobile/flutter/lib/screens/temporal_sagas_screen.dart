...
class _TemporalSagasScreenState extends State<TemporalSagasScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'saga', 'steps', 'current', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'saga', 'steps', 'current', 'status'];
    final labels = ['ID', 'Saga', 'Steps', 'Current', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Temporal Sagas'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Temporal Sagas')),
