...
class _FxRevaluationScreenState extends State<FxRevaluationScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['currency', 'position', 'rate', 'pnl'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['currency', 'position', 'rate', 'pnl'];
    final labels = ['Currency', 'Position', 'Rate', 'P&L'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'FX Revaluation'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('FX Revaluation')),
