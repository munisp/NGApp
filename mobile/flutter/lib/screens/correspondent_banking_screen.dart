...
class _CorrespondentBankingScreenState extends State<CorrespondentBankingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['bic', 'name', 'country', 'currency', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['bic', 'name', 'country', 'currency', 'status'];
    final labels = ['BIC', 'Bank', 'Country', 'Ccy', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Correspondent Banking'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Correspondent Banking')),
