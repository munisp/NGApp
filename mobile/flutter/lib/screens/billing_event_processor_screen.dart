...
class _BillingEventProcessorScreenState extends State<BillingEventProcessorScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'event', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'event', 'amount', 'status'];
    final labels = ['ID', 'Event', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Billing Events'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Billing Events')),
