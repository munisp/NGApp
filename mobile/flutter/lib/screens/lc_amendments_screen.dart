...
class _LcAmendmentsScreenState extends State<LcAmendmentsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'lcRef', 'type', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'lcRef', 'type', 'status'];
    final labels = ['ID', 'LC Ref', 'Amendment', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'LC Amendments'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('LC Amendments')),
