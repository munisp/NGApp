...
class _RateLimitingScreenState extends State<RateLimitingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['name', 'reqPerMin', 'burst', 'tenants'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['name', 'reqPerMin', 'burst', 'tenants'];
    final labels = ['Tier', 'Req/Min', 'Burst', 'Tenants'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'API Rate Limiting'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('API Rate Limiting')),
