...
class _ChequeImagingScreenState extends State<ChequeImagingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'chequeNo', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'chequeNo', 'amount', 'status'];
    final labels = ['ID', 'Cheque No', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Cheque Imaging'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Cheque Imaging')),
