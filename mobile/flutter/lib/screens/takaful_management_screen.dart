...
class _TakafulManagementScreenState extends State<TakafulManagementScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'type', 'contribution', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'type', 'contribution', 'status'];
    final labels = ['ID', 'Plan', 'Type', 'Contribution', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Takaful'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Takaful')),
