...
class _MicrofinanceEngineScreenState extends State<MicrofinanceEngineScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'group', 'savings', 'loans', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'group', 'savings', 'loans', 'status'];
    final labels = ['ID', 'Group', 'Savings', 'Loans Active', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Microfinance Engine'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Microfinance Engine')),
