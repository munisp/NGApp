...
class _EnairaCbdcScreenState extends State<EnairaCbdcScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'wallet', 'tier', 'balance', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'wallet', 'tier', 'balance', 'status'];
    final labels = ['ID', 'Wallet', 'Tier', 'Balance', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'eNaira CBDC'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('eNaira CBDC')),
