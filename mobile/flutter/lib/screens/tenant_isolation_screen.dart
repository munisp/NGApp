...
class _TenantIsolationScreenState extends State<TenantIsolationScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'tenant', 'dataPartition', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'tenant', 'dataPartition', 'status'];
    final labels = ['ID', 'Tenant', 'Partition', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Tenant Isolation'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tenant Isolation')),
