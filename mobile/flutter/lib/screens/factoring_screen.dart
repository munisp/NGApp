...
class _FactoringScreenState extends State<FactoringScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'seller', 'buyer', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'seller', 'buyer', 'amount', 'status'];
    final labels = ['ID', 'Seller', 'Buyer', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Factoring'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Factoring')),
