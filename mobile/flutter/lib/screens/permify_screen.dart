...
class _PermifyScreenState extends State<PermifyScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'schema', 'relations', 'checks', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'schema', 'relations', 'checks', 'status'];
    final labels = ['ID', 'Schema', 'Relations', 'Checks/s', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Permify'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Permify')),
