...
class _KafkaStreamingScreenState extends State<KafkaStreamingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'pipeline', 'throughput', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'pipeline', 'throughput', 'status'];
    final labels = ['ID', 'Pipeline', 'Throughput', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Kafka Streaming'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Kafka Streaming')),
