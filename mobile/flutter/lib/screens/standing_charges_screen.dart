...
class _StandingChargesScreenState extends State<StandingChargesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'amount', 'frequency'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'amount', 'frequency'];
    final labels = ['ID', 'Charge', 'Amount', 'Frequency'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Standing Charges'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Standing Charges')),
