...
class _OtcDerivativesScreenState extends State<OtcDerivativesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'type', 'counterparty', 'notional', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'type', 'counterparty', 'notional', 'status'];
    final labels = ['ID', 'Type', 'Counterparty', 'Notional', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'OTC Derivatives'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('OTC Derivatives')),
