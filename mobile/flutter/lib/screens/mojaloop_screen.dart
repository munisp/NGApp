...
class _MojaloopScreenState extends State<MojaloopScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'participant', 'type', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'participant', 'type', 'amount', 'status'];
    final labels = ['ID', 'Participant', 'Type', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Mojaloop Hub'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Mojaloop Hub')),
