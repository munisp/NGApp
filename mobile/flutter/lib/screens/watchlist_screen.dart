...
class _WatchlistScreenState extends State<WatchlistScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'list', 'match', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'list', 'match', 'status'];
    final labels = ['ID', 'Name', 'List', 'Match', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Watchlist / Sanctions'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Watchlist / Sanctions')),
