...
class _IslamicBankingScreenState extends State<IslamicBankingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'type', 'rate', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'type', 'rate', 'status'];
    final labels = ['ID', 'Product', 'Type', 'Profit Rate', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Islamic Banking'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Islamic Banking')),
