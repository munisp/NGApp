...
class _InfraRedisScreenState extends State<InfraRedisScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'instance', 'memory', 'keys', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'instance', 'memory', 'keys', 'status'];
    final labels = ['ID', 'Instance', 'Memory', 'Keys', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Infra: Redis'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Infra: Redis')),
