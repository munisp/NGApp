...
class _PbacEngineScreenState extends State<PbacEngineScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'effect', 'scope', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'effect', 'scope', 'status'];
    final labels = ['ID', 'Policy', 'Effect', 'Scope', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'PBAC Engine'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('PBAC Engine')),
