...
class _EtlPipelinesScreenState extends State<EtlPipelinesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'source', 'target', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'source', 'target', 'status'];
    final labels = ['ID', 'Pipeline', 'Source', 'Target', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'ETL Pipelines'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('ETL Pipelines')),
