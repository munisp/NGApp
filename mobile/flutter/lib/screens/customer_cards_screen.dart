...
class _CustomerCardsScreenState extends State<CustomerCardsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['pan', 'type', 'scheme', 'expiry', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['pan', 'type', 'scheme', 'expiry', 'status'];
    final labels = ['Card', 'Type', 'Scheme', 'Expiry', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Customer Cards'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Customer Cards')),
