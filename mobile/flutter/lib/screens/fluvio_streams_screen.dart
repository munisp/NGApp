...
class _FluvioStreamsScreenState extends State<FluvioStreamsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'topic', 'partitions', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'topic', 'partitions', 'status'];
    final labels = ['ID', 'Topic', 'Partitions', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Fluvio Streams'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Fluvio Streams')),
