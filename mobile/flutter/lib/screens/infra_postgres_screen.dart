...
class _InfraPostgresScreenState extends State<InfraPostgresScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'database', 'size', 'connections'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'database', 'size', 'connections'];
    final labels = ['ID', 'Database', 'Size', 'Connections'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Infra: Postgres'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Infra: Postgres')),
