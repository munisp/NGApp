...
class _BrandedCommsScreenState extends State<BrandedCommsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'channel', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'channel', 'status'];
    final labels = ['ID', 'Template', 'Channel', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Branded Comms'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Branded Comms')),
