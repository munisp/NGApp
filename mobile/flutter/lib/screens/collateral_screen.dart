...
class _CollateralScreenState extends State<CollateralScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'type', 'description', 'value', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'type', 'description', 'value', 'status'];
    final labels = ['ID', 'Type', 'Description', 'Value', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Collateral'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Collateral')),
