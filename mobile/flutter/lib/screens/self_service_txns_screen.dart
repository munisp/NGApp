...
class _SelfServiceTxnsScreenState extends State<SelfServiceTxnsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'type', 'amount', 'channel'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'type', 'amount', 'channel'];
    final labels = ['ID', 'Customer', 'Type', 'Amount', 'Channel'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Self-Service Transactions'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Self-Service Transactions')),
