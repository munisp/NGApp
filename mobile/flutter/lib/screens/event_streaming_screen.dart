...
class _EventStreamingScreenState extends State<EventStreamingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'stream', 'throughput', 'lag', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'stream', 'throughput', 'lag', 'status'];
    final labels = ['ID', 'Stream', 'Throughput', 'Lag', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Event Streaming'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Event Streaming')),
