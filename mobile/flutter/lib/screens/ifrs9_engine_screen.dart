...
class _Ifrs9EngineScreenState extends State<Ifrs9EngineScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['portfolio', 'stage', 'balance', 'ecl', 'coverage'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['portfolio', 'stage', 'balance', 'ecl', 'coverage'];
    final labels = ['Portfolio', 'Stage', 'Balance', 'ECL', 'Coverage'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'IFRS 9 Engine'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('IFRS 9 Engine')),
