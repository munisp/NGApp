...
class _MoneyMarketScreenState extends State<MoneyMarketScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'instrument', 'counterparty', 'amount', 'rate'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'instrument', 'counterparty', 'amount', 'rate'];
    final labels = ['ID', 'Instrument', 'Counterparty', 'Amount', 'Rate'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Money Market'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Money Market')),
