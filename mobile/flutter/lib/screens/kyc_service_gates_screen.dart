...
class _KycServiceGatesScreenState extends State<KycServiceGatesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'gate', 'threshold', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'gate', 'threshold', 'status'];
    final labels = ['ID', 'Gate', 'Threshold', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'KYC Service Gates'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('KYC Service Gates')),
