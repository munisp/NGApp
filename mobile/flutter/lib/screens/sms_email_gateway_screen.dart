...
class _SmsEmailGatewayScreenState extends State<SmsEmailGatewayScreen> {
  ...
  final List<Map<String, String>> _items = [
    {'id': 'GW-001', 'provider': 'Africa's Talking', 'type': 'SMS', 'status': 'Active'},
  ];
  ...
  void _showCreateDialog() {
    final keys = ['id', 'provider', 'type', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'provider', 'type', 'status'];
    final labels = ['ID', 'Provider', 'Type', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'SMS/Email Gateway'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('SMS/Email Gateway')),
