...
class _LoanAccountsScreenState extends State<LoanAccountsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'outstanding', 'nextPmt', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'outstanding', 'nextPmt', 'status'];
    final labels = ['ID', 'Customer', 'Outstanding', 'Next Payment', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Loan Accounts'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Loan Accounts')),
