...
class _SeedRegistryScreenState extends State<SeedRegistryScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'entity', 'records', 'lastSeed'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'entity', 'records', 'lastSeed'];
    final labels = ['ID', 'Entity', 'Records', 'Last Seed'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Seed Registry'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Seed Registry')),
