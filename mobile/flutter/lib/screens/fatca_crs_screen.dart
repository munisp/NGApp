...
class _FatcaCrsScreenState extends State<FatcaCrsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'jurisdiction', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'jurisdiction', 'status'];
    final labels = ['ID', 'Customer', 'Jurisdiction', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'FATCA / CRS'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('FATCA / CRS')),
