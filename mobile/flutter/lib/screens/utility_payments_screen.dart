...
class _UtilityPaymentsScreenState extends State<UtilityPaymentsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'biller', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'biller', 'amount', 'status'];
    final labels = ['ID', 'Customer', 'Biller', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Utility Payments'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Utility Payments')),
