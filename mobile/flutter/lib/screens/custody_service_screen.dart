...
class _CustodyServiceScreenState extends State<CustodyServiceScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['security', 'qty', 'value', 'custodian'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['security', 'qty', 'value', 'custodian'];
    final labels = ['Security', 'Qty', 'Value', 'Custodian'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Custody Services'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Custody Services')),
