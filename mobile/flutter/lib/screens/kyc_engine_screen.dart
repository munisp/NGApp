...
class _KycEngineScreenState extends State<KycEngineScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'idType', 'idNo', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'idType', 'idNo', 'status'];
    final labels = ['ID', 'Customer', 'ID Type', 'ID Number', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'KYC Engine'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('KYC Engine')),
