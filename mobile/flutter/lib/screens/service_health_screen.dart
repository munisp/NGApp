...
class _ServiceHealthScreenState extends State<ServiceHealthScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['name', 'status', 'uptime', 'port'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['name', 'status', 'uptime', 'port'];
    final labels = ['Service', 'Status', 'Uptime', 'Port'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Service Health'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Service Health')),
