...
class _StatementGeneratorScreenState extends State<StatementGeneratorScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'account', 'period', 'format', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'account', 'period', 'format', 'status'];
    final labels = ['ID', 'Account', 'Period', 'Format', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Statement Generator'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Statement Generator')),
