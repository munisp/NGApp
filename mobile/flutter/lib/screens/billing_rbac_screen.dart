...
class _BillingRbacScreenState extends State<BillingRbacScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'role', 'permissions', 'users'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'role', 'permissions', 'users'];
    final labels = ['ID', 'Role', 'Permissions', 'Users'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Billing RBAC'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Billing RBAC')),
