...
class _KycOverridesScreenState extends State<KycOverridesScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'field', 'reason', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'field', 'reason', 'status'];
    final labels = ['ID', 'Customer', 'Field', 'Reason', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'KYC Overrides'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('KYC Overrides')),
