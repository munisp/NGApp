...
class _InterbankLendingScreenState extends State<InterbankLendingScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'counterparty', 'amount', 'rate', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'counterparty', 'amount', 'rate', 'status'];
    final labels = ['ID', 'Counterparty', 'Amount', 'Rate', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Interbank Lending'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Interbank Lending')),
