...
class _WebhookEngineScreenState extends State<WebhookEngineScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'endpoints', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'endpoints', 'status'];
    final labels = ['ID', 'Engine', 'Endpoints', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Webhook Engine'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Webhook Engine')),
