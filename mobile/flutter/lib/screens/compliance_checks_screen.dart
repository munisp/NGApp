...
class _ComplianceChecksScreenState extends State<ComplianceChecksScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'regulation', 'area', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'regulation', 'area', 'status'];
    final labels = ['ID', 'Regulation', 'Area', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Compliance Checks'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Compliance Checks')),
