...
class _CustomerSegmentsScreenState extends State<CustomerSegmentsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'name', 'count', 'avgBalance'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'name', 'count', 'avgBalance'];
    final labels = ['ID', 'Segment', 'Customers', 'Avg Balance'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Customer Segments'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Customer Segments')),
