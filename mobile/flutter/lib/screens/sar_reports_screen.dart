...
class _SarReportsScreenState extends State<SarReportsScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'subject', 'type', 'amount', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'subject', 'type', 'amount', 'status'];
    final labels = ['ID', 'Subject', 'Type', 'Amount', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'SAR Reports'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('SAR Reports')),
