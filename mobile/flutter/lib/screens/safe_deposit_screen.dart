...
class _SafeDepositScreenState extends State<SafeDepositScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'customer', 'boxSize', 'branch', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'customer', 'boxSize', 'branch', 'status'];
    final labels = ['ID', 'Customer', 'Size', 'Branch', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Safe Deposit'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Safe Deposit')),
