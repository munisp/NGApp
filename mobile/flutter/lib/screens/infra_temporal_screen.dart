...
class _InfraTemporalScreenState extends State<InfraTemporalScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'workflow', 'running', 'completed'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'workflow', 'running', 'completed'];
    final labels = ['ID', 'Workflow', 'Running', 'Completed'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Infra: Temporal'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Infra: Temporal')),
