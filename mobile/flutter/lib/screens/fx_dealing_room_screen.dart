...
class _FxDealingRoomScreenState extends State<FxDealingRoomScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'pair', 'amount', 'rate', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'pair', 'amount', 'rate', 'status'];
    final labels = ['ID', 'Pair', 'Amount', 'Rate', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'FX Dealing Room'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('FX Dealing Room')),
