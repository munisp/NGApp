...
class _LockerScreenState extends State<LockerScreen> {
  ...
  void _showCreateDialog() {
    final keys = ['id', 'branch', 'size', 'status'];
  ...
  void _showDetail(Map<String, String> item) {
    final keys = ['id', 'branch', 'size', 'status'];
    final labels = ['ID', 'Branch', 'Size', 'Status'];
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(item[keys[0]] ?? 'Locker Management'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Locker Management')),
