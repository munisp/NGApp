...
class RateAlertsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('RateAlerts'.replaceAll(RegExp(r'(?<=[a-z])(?=[A-Z])'), ' '))),
      body: Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.construction, size: 64, color: Theme.of(context).primaryColor),
          const SizedBox(height: 16),
          Text('RateAlerts'.replaceAll(RegExp(r'(?<=[a-z])(?=[A-Z])'), ' '), style: Theme.of(context).textTheme.headlineSmall),
